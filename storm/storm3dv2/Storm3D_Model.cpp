...
namespace {
    bool hasInfiniteCylinderCollision(const VC3 &rayOrigin,
                                      const VC3 &rayEnd,
                                      const VC3 &cylinderPoint1,
                                      const VC3 &cylinderPoint2,
...
Model_BoneAnimation::Model_BoneAnimation(Storm3D_BoneAnimation *transition_,
                                         Storm3D_BoneAnimation *animation_,
                                         bool                   looping,
                                         int                    blend_time_,
                                         int                    animation_time_)
...
bool Storm3D_Model::LoadS3D(const char *filename)
{
    ...
    for (int i = strlen(path); (i >= 0) && (filename[i] != '\\') && (filename[i] != '/'); i--) {
        path[i] = 0;
    }
    ...
    for (int i = 0; i < header.num_objects; i++) {
        ...
        if (s3d_version >= 6) {
            ...
            if (hard_rotation != 0) {
                ...
                obj.position[0] = obj.position[0]
                                  * cosf( (float)hard_rotation * 3.1415927f / 180 ) + obj.position[2] * sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
                obj.position[2] = obj.position[2] * cosf( (float)hard_rotation * 3.1415927f / 180 ) - origp0 *sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
            }
            if (mirrored) {
                if (mirroredX)
        ...
        for (int i2 = 0; i2 < originalVertexAmount; i2++) {
            ...
            if (hard_rotation != 0) {
                ...
                vertex.position[0] = vertex.position[0]
                                     * cosf( (float)hard_rotation * 3.1415927f / 180 ) + vertex.position[2] * sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
                vertex.position[2] = vertex.position[2]
                                     * cosf( (float)hard_rotation * 3.1415927f / 180 ) - origp0 *sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
                vertex.normal[0] = vertex.normal[0]
                                   * cosf( (float)hard_rotation * 3.1415927f / 180 ) + vertex.normal[2] * sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
                vertex.normal[2] = vertex.normal[2] * cosf( (float)hard_rotation * 3.1415927f / 180 ) - orign0 *sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
            }
            if (mirrored) {
                if (mirroredX) {
            ...
            tempverts[i2] =
                Storm3D_Vertex( VC3(vertex.position), VC3(vertex.normal), VC2(vertex.texturecoords),
                                VC2(vertex.texturecoords2) );
        ...
        if (blackEdge || fatboy) {
            ...
            for (int i2 = 0; i2 < originalVertexAmount; i2++) {
                ...
                if (seamAmount >= 2) {
                    ...
                    if (blackEdge)
                        tempverts[i2
                                  + originalVertexAmount].position = tempverts[i2].position + seamnorm * blackEdgeSize;
                } else {
                    if (fatboy)
                        tempverts[i2].position += tempverts[i2].normal * fatboySize;
                    if (blackEdge)
                        tempverts[i2
                                  + originalVertexAmount].position = tempverts[i2].position + tempverts[i2].normal
                                                                     * blackEdgeSize;
                }
            }
            if (fatboy)
        ...
        for (int i2 = 0; i2 < originalFaceAmount; i2++) {
            ...
            if (blackEdge) {
                ...
                tempfaces[i2
                          + originalFaceAmount].vertex_index[0] = tempfaces[i2].vertex_index[2] + originalVertexAmount;
                tempfaces[i2
                          + originalFaceAmount].vertex_index[1] = tempfaces[i2].vertex_index[1] + originalVertexAmount;
                tempfaces[i2
                          + originalFaceAmount].vertex_index[2] = tempfaces[i2].vertex_index[0] + originalVertexAmount;
            }
        }
            ...
            for (int vn = 0; vn < tmesh->vertex_amount; vn++) {
                ...
                if (hard_rotation != 0) {
                    ...
                    vertex.normal[0] = vertex.normal[0]
                         ...
                         (float)hard_rotation * 3.1415927f / 180 );
                    vertex.normal[2] = vertex.normal[2]
                                       * cosf( (float)hard_rotation * 3.1415927f / 180 ) - orign0 *sinf(
                         (float)hard_rotation * 3.1415927f / 180 );
                }
                if (mirrored) {
                    if (mirroredX) {
    ...
    for (int i = 0; i < header.num_helpers; i++) {
        ...
        if (s3d_version >= 6) {
            ...
            if (hard_rotation != 0) {
                ...
                help.position[0] = help.position[0]
                                   * cosf( (float)hard_rotation * 3.1415927f / 180 ) + help.position[2] * sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
                help.position[2] = help.position[2] * cosf( (float)hard_rotation * 3.1415927f / 180 ) - origp0 *sinf(
                     (float)hard_rotation * 3.1415927f / 180 );
            }
            if (mirrored) {
                if (mirroredX)
        ...
        if (thelp->GetHelperType() != IStorm3D_Helper::HTYPE_POINT)
            for (int i2 = 0; i2 < help.o1key_amount; i2++) { // Other1
                ...
                if (thelp->GetHelperType() == IStorm3D_Helper::HTYPE_VECTOR)
                    ( (IStorm3D_Helper_Vector *)thelp )->Animation_AddNewDirectionKeyFrame( temp_kf.keytime,
                                                                                            VC3(temp_kf.x, temp_kf.y,
                                                                                                temp_kf.z) );
                else if (thelp->GetHelperType() == IStorm3D_Helper::HTYPE_BOX)
                    ( (IStorm3D_Helper_Box *)thelp )->Animation_AddNewSizeKeyFrame( temp_kf.keytime,
                                                                                    VC3(temp_kf.x, temp_kf.y, temp_kf.z) );
                else if (thelp->GetHelperType() == IStorm3D_Helper::HTYPE_CAMERA)
                    ( (IStorm3D_Helper_Camera *)thelp )->Animation_AddNewDirectionKeyFrame( temp_kf.keytime,
                                                                                            VC3(temp_kf.x, temp_kf.y,
                                                                                                temp_kf.z) );
        ...
        if (thelp->GetHelperType() == IStorm3D_Helper::HTYPE_CAMERA)
            for (int i2 = 0; i2 < help.o2key_amount; i2++) { // Other2
                ...
                // Add it to helper
                ( (IStorm3D_Helper_Camera *)thelp )->Animation_AddNewUpVectorKeyFrame( temp_kf.keytime,
                                                                                       VC3(temp_kf.x, temp_kf.y,
                                                                                           temp_kf.z) );
            }
    ...
    if (cloneHelpers) {
        ...
        for (; it != helpers.end(); ++it) {
            ...
            if (type == IStorm3D_Helper::HTYPE_POINT) {
                ...
                Storm3D_Helper_Point *point =
                    static_cast<Storm3D_Helper_Point *>( clone->Helper_Point_New(helper->GetName(), helper->position) );
                base = point;
                newHelper = point;
            } else if (type == IStorm3D_Helper::HTYPE_VECTOR) {
                Storm3D_Helper_Vector *helper = static_cast<Storm3D_Helper_Vector *>(originalHelper);
                Storm3D_Helper_Vector *vector =
                    static_cast<Storm3D_Helper_Vector *>( clone->Helper_Vector_New(helper->GetName(), helper->position,
                                                                                   helper->direction) );
                base = vector;
                newHelper = vector;
            } else if (type == IStorm3D_Helper::HTYPE_CAMERA) {
                Storm3D_Helper_Camera *helper = static_cast<Storm3D_Helper_Camera *>(originalHelper);
                Storm3D_Helper_Camera *camera =
                    static_cast<Storm3D_Helper_Camera *>( clone->Helper_Camera_New(helper->name, helper->position,
                                                                                   helper->direction, helper->upvec) );
                base = camera;
                newHelper = camera;
            }
 ...
 */
void Storm3D_Model::RayTrace(const VC3             &position,
                             const VC3             &direction_normalized,
                             float                  ray_length,
                             ...
                             bool                   accurate)
{
    ...
    if (!bones.empty() && !terrain_object && bone_collision) {
        ...
        for (set<Storm3D_Model_Object *>::iterator io = collision_objects.begin(); io != collision_objects.end();
             ++io) {
...
bool Storm3D_Model::BlendToAnimation(IStorm3D_BoneAnimation *transition_,
                                     IStorm3D_BoneAnimation *animation_,
                                     int                     blend_time,
                                     bool                    loop)
{
...
bool Storm3D_Model::BlendWithAnimationIn(IStorm3D_BoneAnimation *transition_,
                                         IStorm3D_BoneAnimation *animation_,
                                         int                     blend_in_time,
                                         bool                    loop,
                                         float                   blend_factor)
...
bool Storm3D_Model::BlendWithAnimationOut(IStorm3D_BoneAnimation *transition_,
