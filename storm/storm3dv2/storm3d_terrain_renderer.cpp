...
struct Storm3D_TerrainRendererData {
    ...
    Storm3D_TerrainRendererData(Storm3D                      &storm_,
                                IStorm3D_TerrainRendererBase &rendererBase,
                                Storm3D_TerrainHeightmap     &heightMap_,
                                Storm3D_TerrainGroup         &groups_,
                                Storm3D_TerrainModels        &models_,
                                Storm3D_TerrainDecalSystem   &decalSystem_)
        :   storm(storm_),
        ...
        if (pass == Solid) {
            if (renderHeightmap) {
                ...
                if (forcedDirectionalLightEnabled) {
                    ...
                    float sunStrength =
                        (forcedDirectionalLightColor.r + forcedDirectionalLightColor.g
                    + forcedDirectionalLightColor.b) / 3.f;
                    sunDir *= sunStrength;
...
//! Constructor
Storm3D_TerrainRenderer::Storm3D_TerrainRenderer(Storm3D                    &storm,
                                                 Storm3D_TerrainHeightmap   &heightMap,
                                                 Storm3D_TerrainGroup       &groups,
                                                 Storm3D_TerrainModels      &models,
                                                 Storm3D_TerrainDecalSystem &decalSystem)
{
    boost::scoped_ptr<Storm3D_TerrainRendererData> tempData( new Storm3D_TerrainRendererData(storm,
                                                                                             *this,
                                                                                             heightMap,
                                                                                             groups,
                                                                                             models,
...
/*!
    ...
    \param color
 */
void Storm3D_TerrainRenderer::renderLightTexture(const VC2        &start,
                                                 const VC2        &end,
...
void Storm3D_TerrainRenderer::renderTargets(Storm3D_Scene &scene)
{
    ...
    data->models.setForcedDirectional(data->forcedDirectionalLightEnabled,
                                      data->forcedDirectionalLightDirection,
                                      data->forcedDirectionalLightColor);
    ...
    {
        SDL_Rect frameRect;
        memset( &frameRect, '\0', sizeof(SDL_Rect) );
...
void Storm3D_TerrainRenderer::renderBase(Storm3D_Scene &scene)
{
    ...
    glScissor(data->scissorRectSizes[0].x,
              data->scissorRectSizes[0].y,
              data->scissorRectSizes[0].w,
              data->scissorRectSizes[0].h);
    glEnable(GL_SCISSOR_TEST);
 ...
 */
    // Render map
    if (data->renderMode == IStorm3D_TerrainRenderer::Normal || data->renderMode ==
        IStorm3D_TerrainRenderer::LightOnly) {
 ...
 */
void Storm3D_TerrainRenderer::render(IStorm3D_TerrainRendererBase::RenderMode mode,
                                     Storm3D_Scene                           &scene,
    ...
    if (mode == SpotBuffer && spot) {
        ...
        if (mode == SpotProjectionSolid) {
            ...
            if (data->renderHeightmap) {
                ...
                data->terrainTexture->bind();
                for (int i = 0; i < 4; i++) {
                    glProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 18 + i, data->terrainTextureProjection.raw
                                                 + (4 * i) );
                }

                data->heightMap.renderDepth(scene, &spotCamera, Storm3D_TerrainHeightmap::Projection,
                                            spot->getType(), spot);
            }
