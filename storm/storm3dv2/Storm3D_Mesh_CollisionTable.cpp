...
struct CollisionFace {
    ...
    void RayTrace(const VC3             &position,
...
void CollisionFace::RayTrace(const VC3             &position,
                             const VC3             &direction,
                             float                  ray_length,
                             Storm3D_CollisionInfo &rti,
                             bool                   accurate)
{
    ...
    VC3 pvec(direction.y * e02.z - direction.z * e02.y,
             direction.z * e02.x - direction.x * e02.z,
             direction.x * e02.y - direction.y * e02.x);
...
void Storm3D_Mesh_CollisionTable::ReBuild(Storm3D_Mesh *mesh)
{
    ...
    for (int fi = 0; fi < face_amount; ++fi) {
        ...
        // Create edges
        faces[fi].e01 = owner->vertexes[owner->faces[0][fi].vertex_index[1]].position
                        - owner->vertexes[owner->faces[0][fi].vertex_index[0]].position;
        faces[fi].e02 = owner->vertexes[owner->faces[0][fi].vertex_index[2]].position
                        - owner->vertexes[owner->faces[0][fi].vertex_index[0]].position;
...
//------------------------------------------------------------------
bool Storm3D_Mesh_CollisionTable::RayTrace(const VC3             &position,
                                           const VC3             &direction,
                                           float                  ray_length,
                                           Storm3D_CollisionInfo &rti,
                                           bool                   accurate)
...
//------------------------------------------------------------------
bool Storm3D_Mesh_CollisionTable::SphereCollision(const VC3             &position,
                                                  float                  radius,
                                                  Storm3D_CollisionInfo &colinfo,
                                                  bool                   accurate)
{
    if (!tree)
        return false;
