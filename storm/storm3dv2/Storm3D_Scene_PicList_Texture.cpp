...
//! Constructor
Storm3D_Scene_PicList_Picture::Storm3D_Scene_PicList_Picture(Storm3D          *s2,
                                                             Storm3D_Scene    *scene,
                                                             Storm3D_Material *_mat,
                                                             VC2               _position,
                                                             VC2               _size,
                                                             float             alpha_,
                                                             float             rotation_,
                                                             ...
                                                             float             y2_,
                                                             bool              wrap_)
    :   Storm3D_Scene_PicList(s2, scene, _position,
                              _size), material(_mat), alpha(alpha_), rotation(rotation_), x1(x1_), y1(y1_), x2(x2_),
    y2(y2_), wrap(wrap_),
    customShape(NULL)
{
}
...
void Storm3D_Scene_PicList_Picture3D::Render()
{
    ...
    if ( scene->camera.GetTransformedToScreen(position, scpos, w, rz) )
        if (rz < scene->camera.vis_range) {
            ...
            uint32_t col =
                COLOR_RGBA( (int)(c.r * 255.0f), (int)(c.g * 255.0f), (int)(c.b * 255.0f), (int)(newAlpha * 255.0f) );
