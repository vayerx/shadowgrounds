 ...
 */
void Storm3D_ShaderManager::setSpot(const COL &color,
                                    const VC3 &position,
                                    const VC3 &direction,
                                    float      range,
                                    float      fadeFactor)
...
void Storm3D_ShaderManager::SetShader(Storm3D_Model_Object *object)
{
    ...
    if (!projected_shaders && !fake_shadow_shaders && !fake_depth_shaders) {
        ...
        // Set transparency?
        float ambient_temp[] =
        { sun_properties.w * ambient_color.x, sun_properties.w * ambient_color.y, sun_properties.w * ambient_color.z,
         sun_properties.w * ambient_color.w };
        float ambient[] =
        { ambient_temp[0] + model_ambient_color.x + ambient_force_color.x, ambient_temp[1] + model_ambient_color.y
         + ambient_force_color.y,
         ambient_temp[2] + model_ambient_color.z + ambient_force_color.z, ambient_temp[3]
         + model_ambient_color.w + ambient_force_color.w };
...
void Storm3D_ShaderManager::SetWorldTransform(const D3DXMATRIX &tm, bool forceTextureTm, bool terrain)
{
    ...
    if (lighting_shaders) {
        ...
        if (LIGHT_MAX_AMOUNT > 1) {
            float light_position2_temp[] =
            { light_position[1].x, light_position[1].y, light_position[1].z, textureOffset.y };
...
{
    float v[] =
    { ambient_force_color.x + ambient_color.x, ambient_force_color.y + ambient_color.y, ambient_force_color.z
     + ambient_color.z, ambient_force_color.w + ambient_color.w };
