...
void Storm3D_Helper_Animation::Apply(Storm3D_Scene *scene)
{
    ...
    if (first_poskey) {
        ...
        if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
            ...
            ( (IStorm3D_Helper_Point *)owner )->SetPosition( (kfp->position
    ...
    if ( (htype == IStorm3D_Helper::HTYPE_VECTOR) ||
        ...
        if (first_dirkey) {
            ...
            if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
                ...
                ( (IStorm3D_Helper_Vector *)owner )->SetDirection( (kfp->position
                                                                    * ipol) + ( old_kfp->position * (1.0f - ipol) ) );
            }
            /*else if ((old_kfp)&&(old_kfp!=first_poskey)) // Interpolate between last and first
               {
    ...
    if (htype == IStorm3D_Helper::HTYPE_CAMERA)
        if (first_upveckey) {
            ...
            if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
                ...
                // Interpolate position+
                // This is a camera...
                ( (IStorm3D_Helper_Camera *)owner )->SetUpVector( (kfp->position
                                                                   * ipol) + ( old_kfp->position * (1.0f - ipol) ) );
            }
            /*else if ((old_kfp)&&(old_kfp!=first_poskey)) // Interpolate between last and first
               {
