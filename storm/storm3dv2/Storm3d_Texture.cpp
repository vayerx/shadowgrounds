...
//! Constructor
Storm3D_Texture::Storm3D_Texture(Storm3D *s2, const char *_filename, Uint32 _texloadflags,
                                 Uint32 _tex_identity, const void *data, size_t data_size) :
    decentAlpha(false),
    Storm3D2(s2),
    texhandle(0),
    ...
    auto_release(true)
{
    ...
    if (img->format->BytesPerPixel == 1) {
        ...
        size_t bufSize = dw * dh * 4 + 4;
        imageData.reset(new unsigned char[bufSize]);
        unsigned char *target = imageData.get();
        memset(target, '\0', bufSize);
        ...
        for (int y = 0; y < dh /*img->h*/; y++) {
            for (int x = 0; x < dw /*img->w*/; x++) {
                /*
                   ...
                   SDL_GetRGBA(*((Uint32 *) (src + (y * img->pitch + x * img->format->BytesPerPixel))), img->format, target, target + 1, target + 2, target + 3);
                 */
                int sx = int(sxd * x), sy = int(syd * y);
                unsigned char A =
                    *( src + (sy * img->pitch + sx * img->format->BytesPerPixel + img->format->Ashift / 8) ) | aorr;
                if (map) A = 0x00;
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, internalfmt, dw, dh, fmt, GL_UNSIGNED_BYTE,
                      imageData.get() ? imageData.get() : img->pixels);
    texfmt = internalfmt;
...
//! Load DDS texture
static const char *loadDDS_(bool        compressed,
                            int         mipmaps,
                            int         w,
                            int         h,
                            GLenum     &fmt,
...
//    if (height>ss.height) height=ss.height;
    ...
    if (textype != TEXTYPE_RAM && textype != TEXTYPE_DYNAMIC && textype != TEXTYPE_DYNAMIC_LOCKABLE && textype !=
    ...
    glTexParameteri(glTextype, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glErrors();
    boost::scoped_array<char> emptyBuffer(new char[width * height * 4]);
    ...
    if (glTextype == GL_TEXTURE_CUBE_MAP) {
        ...
        glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, emptyBuffer.get() );
        texfmt = GL_RGBA;
    }
    glErrors();
 ...
 */
bool Storm3D_Texture::IsIdenticalWith(const char *_filename, Uint32 _texloadflags, Uint32 _tex_identity) const
{
...
/*!
    \return identity
 */
Uint32 Storm3D_Texture::GetTexIdentity()
{
    return tex_identity;
}
 ...
 */
void Storm3D_Texture::Copy32BitSysMembufferToTexture(Uint32 *sysbuffer)
{
    glBindTexture(GL_TEXTURE_2D, texhandle);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, texfmt, GL_UNSIGNED_BYTE, sysbuffer);
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
/*!
    \param sysbuffer source memory buffer
 */
void Storm3D_Texture::CopyTextureTo32BitSysMembuffer(Uint32 *sysbuffer)
{
    glBindTexture(GL_TEXTURE_2D, texhandle);
...
void Storm3D_Texture::Apply(int stage)
{
    ...
    if ( (textype == TEXTYPE_BASIC) || (textype == TEXTYPE_BASIC_RENDER) || (textype == TEXTYPE_BASIC_RPOOL) ||
         (textype == TEXTYPE_DYNAMIC) || (textype == TEXTYPE_DYNAMIC_LOCKABLE) ||
         (textype == TEXTYPE_DYNAMIC_LOCKABLE_32) ) {
        glDisable(GL_TEXTURE_CUBE_MAP);
        glDisable(GL_TEXTURE_3D);
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, texhandle);
