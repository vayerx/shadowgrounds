...
void D3DXMatrixAffineTransformation(D3DXMATRIX &out,
                                    float       scale,
                                    const VC3  &rotCenter,
                                    const QUAT &rot,
                                    const VC3  &trans) {
...
void D3DXVec3Transform(VC4 &out, const VC3 &in, const D3DXMATRIX &mat) {
    ...
    // the one below is probably right
    VC4 temp;
...
void D3DXVec3TransformCoord(VC3 &out, const VC3 &in, const D3DXMATRIX &mat) {
    // FIXME: should use code from above
    // and just do the divide here
    VC4 temp;
    temp.x = mat._11 * in.x + mat._21 * in.y + mat._31 * in.z + mat._41 * 1.0f;
    temp.y = mat._12 * in.x + mat._22 * in.y + mat._32 * in.z + mat._42 * 1.0f;
    temp.z = mat._13 * in.x + mat._23 * in.y + mat._33 * in.z + mat._43 * 1.0f;
    temp.w = mat._14 * in.x + mat._24 * in.y + mat._34 * in.z + mat._44 * 1.0f;
    out.x = temp.x / temp.w;
    out.y = temp.y / temp.w;
    out.z = temp.z / temp.w;
}
...
void Framebuffer::setRenderTarget(boost::shared_ptr<glTexWrapper> tex,
                                  boost::shared_ptr<glTexWrapper> depthstencil,
                                  GLenum                          textarget) {
    glErrors();
    ...
    if ( tempW != depthstencil->getWidth() || tempH != depthstencil->getHeight() ) {
        igiosWarning("Framebuffer::setRenderTarget: depthstencil size has changed! Should be (%dx%d), is (%dx%d)\n",
                     depthstencil->getWidth(),
                     depthstencil->getHeight(),
                     tempW,
                     tempH);
        igios_backtrace();
    }
