    ...
    if (first_meshkey) {
        ...
        if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
            ...
            for (int vx = 0; vx < vam; vx++) {
                ...
                vbuf[vx].position =
                    (kfp->vertexes[vx].position
                     - old_kfp->vertexes[vx].position) * ipol + old_kfp->vertexes[vx].position;
                vbuf[vx].normal =
                    (kfp->vertexes[vx].normal - old_kfp->vertexes[vx].normal) * ipol + old_kfp->vertexes[vx].normal;
            }
        }
