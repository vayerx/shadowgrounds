...
struct VXFORMAT_DECAL {
    ...
    // Constructor
    VXFORMAT_DECAL(const VC3 &position_,
                   const VC3 &normal_,
                   uint32_t   color_,
                   const VC2 &texcoords_,
                   const VC2 &texcoords2_)
        :   position(position_),
        normal(normal_),
        color(color_),
...
struct DecalMaterial {
    ...
    void applyProjection(const COL &spotColor)
    {
        ...
        float factor = 1.0f;
        float diffuse[4] =
        { diffuseColor.r *factor *spotColor.r, diffuseColor.g *factor *spotColor.g, diffuseColor.b *factor *spotColor.b,
         1.0f };
