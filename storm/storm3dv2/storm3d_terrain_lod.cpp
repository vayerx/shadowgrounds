...
private:
    ...
    void generateLink(unsigned short *buffer,
                      int             resolution,
                      bool            leftLod,
                      bool            rightLod,
                      bool            upLod,
                      ...
                      unsigned char  *clipBuffer)
    {
        ...
        if (!leftLod) {
            ...
            createFace(buffer,
                       -1,
                       0,
                       lodFactor * resolution,
                       lodFactor * resolution + lodFactor,
                       resolution,
                       clipBuffer);
            createFace(buffer, -1, (resolution - 1 - lodFactor) * resolution, (resolution - 1) * resolution,
                       (resolution - 1 - lodFactor) * resolution + lodFactor, resolution, clipBuffer);
        } else {
        ...
        if (!rightLod) {
            ...
            createFace(buffer,
                       -1,
                       resolution - 1,
                       resolution * lodFactor + resolution - 1 - lodFactor,
                       resolution * lodFactor + resolution - 1,
                       resolution,
                       clipBuffer);
            createFace(buffer,
                       -1,
                       (resolution - 1 - lodFactor) * resolution + resolution - 1 - lodFactor,
                       (resolution - 1) * resolution + resolution - 1,
                       (resolution - 1 - lodFactor) * resolution + resolution - 1,
                       resolution,
                       clipBuffer);
        } else {
        ...
        if (!upLod) {
            ...
            createFace(buffer,
                       ...
                       resolution - 1,
                       endVertex,
                       lodFactor * resolution + endVertex,
                       resolution,
                       clipBuffer);
        } else {
        ...
        if (!downLod) {
            ...
            createFace(buffer, -1, (resolution - 1) * resolution, (resolution - 1) * resolution + lodFactor,
                       (resolution - 1 - lodFactor) * resolution + lodFactor, resolution, clipBuffer);
            createFace(buffer,
                       -1,
                       (resolution - 1) * resolution + resolution - 1 - lodFactor,
                       (resolution - 1) * resolution + resolution - 1,
                       (resolution - 1 - lodFactor) * resolution + resolution - 1 - lodFactor,
                       resolution,
                       clipBuffer);
    ...
    void generateTriangles(unsigned short *buffer,
                           int             resolution,
                           int             x1,
                           int             y1,
                           int             x2,
    ...
    void generateHorizontal(unsigned short *buffer,
                            int             resolution,
                            int             column,
                            int             direction,
                            unsigned char  *clipBuffer)
    ...
    void createFace(unsigned short *buffer,
                    int             direction,
                    int             f1,
                    int             f2,
                    int             f3,
            ...
            for (int r = 0; r < 2; ++r) {
                for (int u = 0; u < 2; ++u) {
                    for (int d = 0; d < 2; ++d) {
                        ...
                        fullBuffers[index].generate(LodIndexBuffer::FullBuffer,
                                                    resolution,
                                                    lod,
                                                    leftLod,
                                                    rightLod,
                                                    ...
                                                    downLod,
                                                    clipBuffer);
                        linkBuffers[index].generate(LodIndexBuffer::LinkBuffer,
                                                    resolution,
                                                    lod,
                                                    leftLod,
                                                    rightLod,
...
/*!
    ...
    \param rangeY2
 */
void Storm3D_TerrainLod::render(Storm3D_Scene &scene,
                                int            subMask,
                                float          range,
                                float          rangeX1,
                                float          rangeY1,
