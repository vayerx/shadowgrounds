...
struct Storm3D_SpotlightData {
    ...
    void createCone()
    {
        ...
        // Vertices
        {
            ...
            coneVertexBuffer.create(CONE_VERTICES,
                                    3 * sizeof(float) + 3 * sizeof(float) + sizeof(unsigned int) + 2 * sizeof(float),
                                    false);
...
bool Storm3D_Spotlight::setAsRenderTarget(const D3DXMATRIX &cameraView)
{
    ...
    if (data->shadowMap) {
        ...
        if ( data->fbo->validate() ) {
            ...
            Storm3D_ShaderManager::GetSingleton()->SetViewMatrix(data->properties.lightView[0]);
            //Storm3D_ShaderManager::GetSingleton()->SetViewProjectionMatrix(data->properties.lightProjection, data->properties.lightView);
            Storm3D_ShaderManager::GetSingleton()->setSpot(data->properties.color,
...
void Storm3D_Spotlight::renderCone(Storm3D_Camera &camera, float timeFactor, bool renderGlows)
{
    ...
    float colorData[4] =
    { data->properties.color.r * colorMul, data->properties.color.g * colorMul, data->properties.color.b * colorMul,
      1.f };
    if (renderGlows) {
        if (normalPass) {
            colorData[0] *= 0.2f;
...
/*!
    ...
    \param shadowQuality shadow quality
 */
void Storm3D_Spotlight::createShadowBuffers(Storm3D &storm, bool ps14, bool ps20, int shadowQuality)
{
