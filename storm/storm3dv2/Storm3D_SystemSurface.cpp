...
void Storm3D_SystemSurface::RenderFaceList(Storm3D_Scene  *scene,
                                           D3DMATRIX       worldmx,
                                           Storm3D_Face   *faces,
                                           ...
                                           int             num_vertexes)
{
    ...
    for (int fc = 0; fc < num_faces; fc++) {
        ...
        for (int fcc = 0; fcc < fcamt; fcc++) {
            // Sort vertexes
            int vhi, vmd, vlo;
            if (newfaces[fcc].vxs[1].y < newfaces[fcc].vxs[0].y) {
                vhi = 1; vlo = 0;
            } else { vhi = 0; vlo = 1; }
            if (newfaces[fcc].vxs[2].y < newfaces[fcc].vxs[vhi].y) {
                vmd = vhi; vhi = 2;
            } else {
                if (newfaces[fcc].vxs[vlo].y < newfaces[fcc].vxs[2].y) {
                    vmd = vlo; vlo = 2;
                } else { vmd = 2; }
            }
