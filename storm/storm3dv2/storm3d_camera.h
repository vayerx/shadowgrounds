...
public:
    ...
    void ForceOrthogonalProjection(bool  force,
                                   float minX = 0.0f,
                                   float maxX = 0.0f,
                                   float minY = 0.0f,
                                   float maxY = 0.0f);                                                                       // when force == false, disables forcing the matrix.
    ...
    // Get view matrix
    const D3DXMATRIX&GetVP() { return vp; }
    const D3DXMATRIX&GetV() { return mv; }
    ...
    Storm3D_Camera( Storm3D *Storm3D2, const VC3 &_position = VC3(1, 0, 0), const VC3 &_target = VC3(0,
                                                                                                     0,
                                                                                                     0),
                    float _vis_range = 1000, float _fov = PI / 3, const VC3 &_upvec = VC3(0, 1, 0) );
