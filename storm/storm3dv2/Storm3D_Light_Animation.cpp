    ...
    if (ltype != IStorm3D_Light::LTYPE_DIRECTIONAL)
        if (first_poskey) {
            ...
            if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
                ...
                ( (IStorm3D_Light_Point *)owner )->SetPosition( (kfp->position
                                                                 * ipol) + ( old_kfp->position * (1.0f - ipol) ) );
            }
    ...
    if ( (ltype == IStorm3D_Light::LTYPE_SPOT) ||
        ...
        if (first_dirkey) {
            ...
            if ( (old_kfp) && (kfp) ) { // Between 2 keyframes
                ...
                if (ltype ==
                    IStorm3D_Light::LTYPE_SPOT) ( (IStorm3D_Light_Spot *)owner )->SetDirection( (kfp->position
                                                                                                 * ipol)
                                                                                                + ( old_kfp->position
                                                                                                    * (1.0f - ipol) ) );
                else ( (IStorm3D_Light_Directional *)owner )->SetDirection( (kfp->position
                                                                             * ipol)
                                                                            + ( old_kfp->position * (1.0f - ipol) ) );
            }
            /*else if ((old_kfp)&&(old_kfp!=first_poskey)) // Interpolate between last and first
               {
                //Calculate time delta (and interpolation)
