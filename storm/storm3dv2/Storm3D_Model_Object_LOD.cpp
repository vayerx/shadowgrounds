...
Storm3D_Mesh_LOD::~Storm3D_Mesh_LOD()
{
    ...
    if (vxfacelists) {
        // Delete temp lists
        for (int vx = 0; vx < obj_vx_amount; vx++) {
            SAFE_DELETE(vxfacelists[vx]);
        }
        delete[] vxfacelists;
    }
...
//------------------------------------------------------------------
void Storm3D_Mesh_LOD::UpdateDOT3LightVectors(Storm3D_Mesh               *owner,
                                              Storm3D_Scene_LightHandler &active_lights,
                                              Storm3D_Model_Object       *object)
{
    ...
    for (int vx = 0; vx < lod_vertex_amount; vx++) {
        ...
        int pr = vix * 9;
        temp.x = ldir.x
                 * owner->DOT3_VertexMatrix[pr
                                            + 0] + ldir.y
                 * owner->DOT3_VertexMatrix[pr + 3] + ldir.z * owner->DOT3_VertexMatrix[pr + 6];
        temp.y = ldir.x
                 * owner->DOT3_VertexMatrix[pr
                                            + 1] + ldir.y
                 * owner->DOT3_VertexMatrix[pr + 4] + ldir.z * owner->DOT3_VertexMatrix[pr + 7];
        temp.z = ldir.x
                 * owner->DOT3_VertexMatrix[pr
                                            + 2] + ldir.y
                 * owner->DOT3_VertexMatrix[pr + 5] + ldir.z * owner->DOT3_VertexMatrix[pr + 8];
        temp.Normalize();

        // Save light direction to diffuse component
        vxp->lightvector =
            ( ( (int)(temp.x
                      * 127) + 127 ) << 16 ) + ( ( (int)(-temp.z * 127) + 127 ) << 8 ) + ( (int)(temp.y * 127) + 127 );
...
void Storm3D_Mesh_LOD::OptimizeAndBuildBuffers(Storm3D_Mesh *owner)
{
    ...
    for (int i = 0; i < owner->vertex_amount; i++) {
        ovx_rix[i] = -1;
    }
