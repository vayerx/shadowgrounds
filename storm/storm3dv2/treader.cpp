...
bool VideoBackgroundLoader::init(const char *filename, IStorm3D_StreamBuilder *builder)
{
	...
	if (mContext->audioindex != -1 && builder) {
		...
		if (!mContext->audiocodec) LOG_WARNING("Unable to find suitable audio codec.");
		else {
			if (avcodec_open(mContext->audiocodecctx, mContext->audiocodec) < 0) LOG_WARNING("Unable to open audio codec.");
			else {
				...
				if (!mContext->audiobuffer) LOG_WARNING("Unable to allocate audio buffer.");
				else {
