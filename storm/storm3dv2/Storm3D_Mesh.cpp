...
void Storm3D_Mesh::PrepareMaterialForRender(Storm3D_Scene *scene, Storm3D_Model_Object *object)
{
    ...
    if ( (fabsf(object->scale.x - 1.0f) >= 0.001) || (fabsf(object->scale.y - 1.0f) >= 0.001) ||
         (fabsf(object->scale.z - 1.0f) >= 0.001) )
    ...
    if (bone_weights) {
        for (unsigned int i = 0; i < bone_chunks[lod].size(); ++i) {
            ...
            glDrawRangeElements(GL_TRIANGLES,
                                0,
                                bone_chunks[lod][i].vertex_count,
                                bone_chunks[lod][i].index_count * 3,
                                GL_UNSIGNED_SHORT,
                                NULL);
        ...
        for (int lod = 0; lod < lodLevels; ++lod) {
            ...
            for (int i = 0; i < face_amount[lod]; ++i) {
                ...
                for (unsigned int j = 0; j < bone_chunks[lod].size(); ++j) {
                    ...
                    if (int(chunk.bone_indices.size() + weight_index - bones_found) <
                        Storm3D_ShaderManager::BONE_INDICES)
                ...
                if (index == -1) {
                    ...
                    for (int k = 0; k < weight_index; ++k) {
                        if ( find(insert_chunk.bone_indices.begin(), insert_chunk.bone_indices.end(),
                                  weights[k]) != insert_chunk.bone_indices.end() )
                            continue;
            ...
            for (int i = 0; i < bone_chunk_amount; ++i) {
                ...
                // Create index buffer
                {
                    ...
                    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                                 sizeof(GLushort) * chunk_face_list.size() * 3, NULL, GL_DYNAMIC_DRAW);

                    GLushort *ip =
                        reinterpret_cast<unsigned short *>( glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY) );
                    if (ip)
                ...
                // Create vertex buffer
                {
                    ...
                    for (unsigned int i = 0; i < vertex_list.size(); ++i) {
                        ...
                        p[i] =
                            VXFORMAT_BLEND(vertexes[vertex_index].position,
    ...
    if ( (update_vx) && (!bone_weights) ) {
        ...
        if ( temp < (int)( vertex_amount * sizeof(VXFORMAT_TC2) ) ) {
            ...
            igiosWarning( "size: %d vertex amount: %d %d\n", temp, vertex_amount,
                          int( vertex_amount * sizeof(VXFORMAT_TC2) ) );
        } else {
 ...
 */
bool Storm3D_Mesh::RayTrace(const VC3             &position,
...
#define VectorToRGBA(x, y, z) \
    ( ( ( (UINT)( (UINT)( (x) \
                          * 127 ) \
                  + 127 ) ) \
        << 16 ) | ( ( (UINT)( (UINT)( (y) * 127 ) + 127 ) ) << 8 ) | ( ( (UINT)( (UINT)( (z) * 127 ) + 127 ) ) ) )
