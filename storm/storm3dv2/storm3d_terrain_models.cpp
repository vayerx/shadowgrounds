...
namespace {
    struct ModelObserver : public IModelObserver {
        ...
        ModelObserver(Quadtree<Storm3D_Model> *tree_, Quadtree<Storm3D_Model>::Entity *entity_,
                      std::vector<Storm3D_Model *> *visibleModels_, DataBase *data_)
            :   tree(tree_),
            entity(entity_),
            data(data_)
        {
...
struct Storm3D_TerrainModelsData : public DataBase {
        ...
        if (frustumsEnabled)
            for (int i = 0; i < MAX_VISIBILITY_STRUCTURES; ++i) {
                ...
                for (; it != end; ++it) {
                    ...
                    bool hasAlpha = false;
                    if (alphaType != IStorm3D_Material::ATYPE_NONE && alphaType !=
                        IStorm3D_Material::ATYPE_USE_ALPHATEST)
                        hasAlpha = true;
    ...
    bool testVisibility(Storm3D_Model_Object &o, Storm3D_Mesh &mesh, Storm3D_Model &model,
            ...
            for (; it != end; ++it) {
                ...
                if (updateCulling) {
                    ...
                    for (; it != end; ++it) {
                        ...
                        bool needTesting = o->real_visibility_id > frame || frame - o->real_visibility_id >
                                           OBJECT_CULL_RATE;
    ...
    bool shouldRender(RenderType            renderType,
                      Storm3D_Model_Object *object,
                      const Frustum        *frustum,
                      const Frustum        *frustum2,
                      const IStorm3D_Model *skipModel,
                      RenderFlags           flags,
                      const VC2            &minplane,
                      const VC2            &maxplane,
                      int                   activeVisibility) const
    {
        ...
        if (flags == SkipTerrainObjects && object->parent_model->terrain_object)
            return false;
        if (flags == SkipTerrainLightmappedObjects && object->parent_model->terrain_object &&
            object->parent_model->terrain_lightmapped_object)
            return false;
    ...
    void applyGeneralMaterial(RenderType            renderType,
 ...
 */
            ...
            float glow = material.GetGlow() * material.GetGlowFactor();
            if (material.GetAlphaType() != IStorm3D_Material::ATYPE_NONE && material.GetAlphaType() !=
                IStorm3D_Material::ATYPE_ADD)
                glow *= 1.f - material.GetTransparency();
    ...
    void applySolidMaterial(RenderType            renderType,
                            Storm3D_Material     &material,
                            Storm3D_Model        &model,
                            Storm3D_Model_Object &object,
...
//#endif
    ...
    void applyAlphaMaterial(RenderType            renderType,
                            Storm3D_Material     &material,
                            Storm3D_Model        &model,
                            Storm3D_Model_Object &object,
                            Storm3D_Spotlight    *spot)
            ...
            if (alphaType == IStorm3D_Material::ATYPE_ADD) {
                ...
                if (spot)
                    ...
                    glBlendFunc(GL_ZERO, GL_SRC_COLOR);
                        ...
                        (renderType == SpotProjection && object.force_lighting_alpha_enable) ) {
                if (spot)
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE);
                else
    ...
    bool renderSolid(RenderType             renderType,
                     Storm3D_Scene         &scene,
                     Storm3D_Spotlight     *spot,
                     Storm3D_FakeSpotlight *fakeSpot,
                     const IStorm3D_Model  *skipModel,
                     RenderFlags            flags)
    {
...
//#endif
        ...
        for (; it != solidObjects[active_visibility][objectType].end(); ++it) {
            ...
            if ( !shouldRender(renderType, object, frustum, frustum2, skipModel, flags, minPlane, maxPlane,
                               active_visibility) )
                continue;
...
//#endif
    ...
    void renderAlpha(RenderType             renderType,
                     Storm3D_Scene         &scene,
                     Storm3D_Spotlight     *spot,
                     Storm3D_FakeSpotlight *fakeSpot,
                     const IStorm3D_Model  *skipModel,
 ...
 */
void Storm3D_TerrainModels::RayTrace(const VC3             &position,
                                     const VC3             &direction,
                                     float                  rayLength,
...
/*!
    ...
    \param accurate true to use accurate collision detection
 */
void Storm3D_TerrainModels::SphereCollision(const VC3             &position,
                                            float                  radius,
                                            Storm3D_CollisionInfo &info,
                                            bool                   accurate) const
{
...
/*!
    ...
    \param skipModel
 */
void Storm3D_TerrainModels::renderDepth(Storm3D_Scene        &scene,
