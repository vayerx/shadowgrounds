 ...
 */
void Storm3D_Scene::RenderSceneWithParams(bool              flip,
                                          bool              disable_hsr,
                                          bool              update_time,
                                          bool              render_mirrored,
                                          IStorm3D_Texture *target)
{
    storm3d_dip_calls = 0;
    ...
    if ( terrains.empty() )
        for (set<IStorm3D_Model *>::iterator mit = models.begin(); mit != models.end(); ++mit) {
            ...
            for (set<IStorm3D_Model_Object *>::iterator io = mod->objects.begin(); io != mod->objects.end(); ++io) {
                ...
                int lp = 0;
                for (; renderlist_points[lp] > points; lp++) {
                    ;                                   // OK!
    ...
    for (int i = 0; renderlist_obj[i]; i++) {
        ...
        if (m) {
            ...
            if (alphaType == IStorm3D_Material::ATYPE_NONE) {
                ...
                else if (alphaType == IStorm3D_Material::ATYPE_USE_TEXTRANSPARENCY || renderlist_obj[i]->force_alpha >
                         0.0001f)
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            }
        ...
        // Horrible ...
        {
            ...
            if (mod->type_flag == 0)
                Storm3D_ShaderManager::GetSingleton()->SetLight(0, VC3(-2.5f, 5.f, -10.f), COL(0.03f,
                                                                                               0.03f,
                                                                                               0.03f), 20.f);
            else
                Storm3D_ShaderManager::GetSingleton()->SetLight(0, VC3(2.5f, 5.f, -10.f), COL(0.03f,
...
/*!
    ...
    \param y2
 */
void Storm3D_Scene::Render2D_Picture(IStorm3D_Material *mat,
                                     VC2                position,
                                     VC2                size,
                                     float              alpha,
                                     float              rotation,
...
{
    // Create new
    Storm3D_Scene_PicList *pl = new Storm3D_Scene_PicList_Picture(Storm3D2,
                                                                  this,
                                                                  (Storm3D_Material *)mat,
                                                                  position,
                                                                  size,
                                                                  alpha,
                                                                  rotation,
...
{
    // Create new
    Storm3D_Scene_PicList_Picture *pl =
        new Storm3D_Scene_PicList_Picture(Storm3D2, this, (Storm3D_Material *)mat, VC2(0, 0), VC2(1,
                                                                                                  1), alpha, 0, 0, 0, 1,
                                          1, wrap);
...
{
    // Create new
    Storm3D_Scene_PicList *pl = new Storm3D_Scene_PicList_Picture3D(Storm3D2,
                                                                    this,
                                                                    (Storm3D_Material *)mat,
                                                                    position,
                                                                    size);
 ...
 */
void Storm3D_Scene::Render2D_Text(IStorm3D_Font *font,
                                  VC2            position,
                                  VC2            size,
                                  const char    *text,
                                  float          alpha,
                                  const COL     &colorFactor)
{
    // Create new
    Storm3D_Scene_PicList *pl = new Storm3D_Scene_PicList_Font(Storm3D2,
                                                               this,
                                                               (Storm3D_Font *)font,
                                                               position,
                                                               size,
...
//! Raytrace
void Storm3D_Scene::RayTrace(const VC3             &position,
...
{
    for (set<IStorm3D_Terrain *>::iterator itr = terrains.begin(); itr != terrains.end(); ++itr) {
        static_cast<Storm3D_TerrainRenderer &>( (*itr)->getRenderer() ).setFog(_fog_active,
                                                                               fog_start_range,
                                                                               fog_end_range,
                                                                               color);
    }
        ...
        if (target /*&& haxValue > 1*/) {
            ...
            if ( !render_target->IsCube() ) {
                ...
                if ( !terrains.empty() ) {
                    ...
                    shouldRenderParticleReflection = renderer.enableFeature(
                        IStorm3D_TerrainRenderer::ParticleReflection,
                        false);
                    renderer.enableFeature(IStorm3D_TerrainRenderer::ParticleReflection, shouldRenderParticleReflection);
                    renderParticles = renderer.enableFeature(IStorm3D_TerrainRenderer::Particles,
                                                             shouldRenderParticleReflection);
                    ...
                    renderDistortion = renderer.enableFeature(IStorm3D_TerrainRenderer::Distortion,
                                                              shouldRenderDistortion);
                    renderGlow = renderer.enableFeature(IStorm3D_TerrainRenderer::Glow, shouldRenderGlow);
                    renderGlowImproved = renderer.enableFeature(IStorm3D_TerrainRenderer::BetterGlowSampling,
                                                                shouldRenderGlowImproved);
                    renderFakes = renderer.enableFeature(IStorm3D_TerrainRenderer::FakeLights, shouldRenderFakes);
                    renderFakeShadows = renderer.enableFeature(IStorm3D_TerrainRenderer::FakeShadows,
                                                               shouldRenderFakeShadows);
                    renderSpotShadows = renderer.enableFeature(IStorm3D_TerrainRenderer::SpotShadows,
                                                               shouldRenderSpotShadows);
