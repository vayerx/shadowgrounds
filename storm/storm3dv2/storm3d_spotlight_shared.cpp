...
void Storm3D_SpotlightShared::updateMatricesOffCenter(const D3DXMATRIX &cameraView,
                                                      const VC2        &min,
                                                      const VC2        &max,
                                                      float             height,
...
namespace {
    void calculateLineToScissor(const VC3      &v1,
                                const VC3      &v2,
                                const Frustum  &frustum,
                                Storm3D_Camera &camera,
                                const VC2I     &screenSize,
                                int            &minX,
                                int            &minY,
                                int            &maxX,
                                int            &maxY)
    {
        ...
        for (i = 0; i < 5; ++i) {
            ...
            if (r1 < 0 || r2 < 0) {
                VC3 ab = v[1] - v[0];
                float t =
                    ( plane[i].range_to_origin
                      - plane[i].planenormal.GetDotWith(v[0]) ) / plane[i].planenormal.GetDotWith(
                        ab);
                if (t >= 0.f && t <= 1.f) {
                    VC3 clamp = v[0] + (ab * t);
