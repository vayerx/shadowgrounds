...
bool Storm3D::RenderSceneToArray(IStorm3D_Scene *stormScene, unsigned char *dest, int width, int height)
{
    ...
    if (target) {
        ...
        target->CopyTextureTo32BitSysMembuffer( (Uint32 *) dest );
        Remove(target);
        return true;
    } else {
...
bool Storm3D::SetDisplayMode(int width, int height)
{
    ...
    Storm3D_Spotlight::querySizes(*this, true, shadow_quality);
    ...
    Storm3D_Spotlight::createShadowBuffers(*this, true, true, shadow_quality);
...
/*!
    ...
    \param brightness
 */
void Storm3D::SetGammaRamp(float gamma,
                           float brightness,
                           float contrast,
                           float red,
                           float green,
...
/*!
    ...
    \param peakBlue
 */
void Storm3D::SetGammaPeak(bool  peakEnabled,
                           float peakPosition,
                           float peakLowShape,
                           float peakHighShape,
                           float peakRed,
...
{
    Storm3D_SurfaceInfo size = GetScreenSize();
    unsigned short w = size.width, h = size.height;
    unsigned char *imgdata = new unsigned char[w * h * 3];
    memset(imgdata, 0, w * h * 3);
    glReadBuffer(GL_BACK);
    glReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, imgdata);

    unsigned int empty = 0, colormode = 3;
    unsigned char type = 2, bits = 24;
    ...
    if ( ( file = fopen(file_name, "wb") ) != 0 ) {
        ...
        unsigned int s = w * h * colormode;
        for (unsigned int i = 0; i < s; i += colormode) {
            unsigned char tmp = imgdata[i];
            imgdata[i] = imgdata[i + 2];
            imgdata[i + 2] = tmp;
        }

        fwrite(imgdata, 1, s, file);
...
/*!
    ...
    \param texidentity
 */
IStorm3D_Texture *Storm3D::CreateNewTexture(const char *originalFilename,
                                            uint32_t    texloadcaps,
                                            uint32_t    texidentity,
                                            const void *data,
                                            size_t      data_size)
{
...
IStorm3D_VideoStreamer *Storm3D::CreateVideoStreamer(const char             *file_name,
                                                     IStorm3D_StreamBuilder *streamBuilder,
                                                     bool                    loop)
{
    Storm3D_VideoStreamer *streamer = new Storm3D_VideoStreamer(*this,
                                                                file_name,
                                                                streamBuilder,
                                                                loop,
                                                                downscale_videos,
                                                                highcolorrange_videos);
