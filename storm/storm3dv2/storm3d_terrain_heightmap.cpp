...
static void getPosition(VC3                  &result,
                        int                   xIndex,
                        int                   yIndex,
...
struct TerrainBlock {
    ...
    void fillBuffer(const VC2I                          &start,
                    const VC2I                          &end,
                    boost::scoped_array<unsigned short> &buffer,
                    int                                  blockX,
                    int                                  blockY,
                    const VC2I                          &resolution,
                    const VC3                           &scale,
...
struct Storm3D_TerrainHeightmapData {
    ...
    VC2I convertWorldToObstacleMap(const VC2 &position) const
    {
        ...
        return VC2I(
                   (int)( (position.x * mmult_world_map.x
                           * obstacle_map_multiplier) + ( (float)(resolution.x - 1) / (2.0f / obstacle_map_multiplier) ) ),
                   (int)( (position.y * mmult_world_map.z
                           * obstacle_map_multiplier) + ( (float)(resolution.y - 1) / (2.0f / obstacle_map_multiplier) ) ) );
    }
    ...
    VC2 convertObstacleMapToWorld(const VC2I &position) const
    {
        ...
        return VC2(
                    (float)( float(position.x) + 0.5f
                             - ( (resolution.x
                                  - 1)
                                 / (2.0f / obstacle_map_multiplier) ) ) * mmult_map_world.x / obstacle_map_multiplier,
                   (float)( float(position.y) + 0.5f
                            - ( (resolution.y
                                 - 1)
                                / (2.0f / obstacle_map_multiplier) ) ) * mmult_map_world.z / obstacle_map_multiplier );
    }
 ...
 */
void Storm3D_TerrainHeightmap::setHeightMap(const unsigned short *buffer,
                                            const VC2I           &resolution,
                                            const VC3            &size,
                                            int                   textureDetail,
                                            unsigned short       *forceMap,
 ...
 */
void Storm3D_TerrainHeightmap::setObstacleHeightmap(const unsigned short *obstacleHeightmap,
                                                    const util::AreaMap  *areaMap)
{
 ...
 */
void Storm3D_TerrainHeightmap::renderDepth(Storm3D_Scene           &scene,
                                           Storm3D_Camera          *camera,
                                           RenderMode               mode,
                                           IStorm3D_Spotlight::Type spot_type,
                                           Storm3D_Spotlight       *spot)
{
    std::vector<RenderBlock> &visibleBlocks = data->visibleBlocks;
    ...
    for (unsigned int i = 0; i < visibleBlocks.size(); ++i) {
        ...
        if (camera) {
            VC3 blockPosition(-data->size.x / 2 + data->positionDelta.x / 2,
                              0,
                              -data->size.z / 2 + data->positionDelta.y / 2);
...
/*!
    ...
    \param textureB
 */
void Storm3D_TerrainHeightmap::setPartialBlendMap(int              blockIndex,
                                                  int              subMask,
                                                  Storm3D_Texture &blend,
                                                  int              textureA,
                                                  int              textureB)
{
    assert( !data->blocks.empty() );
...
VC3 Storm3D_TerrainHeightmap::getFaceNormal(const VC2 &position) const
{
    ...
    VC3 scale( data->size.x / (data->collResolution.x - 1), data->size.y / 65535.f,
               data->size.z / (data->collResolution.y - 1) );
    ...
    getPosition(positionUR, pos.x + 1, pos.y - 1,
                data->collisionHeightMap.get(), data->size, data->collResolution, scale);
 ...
 */
void Storm3D_TerrainHeightmap::rayTrace(const VC3             &position,
                                        const VC3             &directionNormalized,
                                        float                  rayLength,
                                        Storm3D_CollisionInfo &rti,
                                        ObstacleCollisionInfo &oci,
                                        bool                   accurate,
                                        bool                   lineOfSight) const
{
    ...
    if (accurate) {
        ...
        VC3 mmult_world_map =
            VC3( (float)data->collResolution.x, 65535.0f, (float)data->collResolution.y ) / data->size;
        ...
        for (i = 0; i < dx; i++) {
            ...
            if (mapHeight > h) {
                ...
                if (data->obstacleHeightmap != NULL && data->areaMap != NULL) {
                    int obstacleHeight = mapHeight + 3
                                         * ( (int)(data->obstacleHeightmap[obstacleBlockIndex]
                                                   & OBSTACLE_MAP_MASK_HEIGHT) );
                    if (obstacleHeight > h)
                             ...
                             && !data->areaMap->isAreaValue(obstacleBlockIndex, skipObstacle2Mask, skipObstacle2Value) )
                        {
                            ...
                            VC3 hitnormal;
                            if (fabs( float(obstacleHeight / 3 - prevObstacleHeight / 3) ) * mmult_map_world.y <
                                0.2f) {
                                hitnormal = VC3(0, 1, 0);
                            } else {
