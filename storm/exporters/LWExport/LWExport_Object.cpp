...
#  pragma warning(disable: 4512) // Assignment operator could no be generated
        ...
        namespace {
            ...
            struct ScanStruct {
                ScanStruct(LWMeshInfo              *meshInfo_,
                           std::vector<LWVertex>   *vertices_,
                           std::vector<LWFace>     *faces_,
                           std::vector<LWMaterial> &materials_,
            ...
            for (unsigned int i = 0; i < bones.size(); ++i) {
                ...
                for (unsigned int j = 0; j < vertices.size(); ++j) {
                    ...
                    if ( meshInfo->pntVGet(meshInfo, vertices[j].id, &weight) )
                        ...
                        for (int k = 0; k < LWVertex::MAX_WEIGHTS; ++k) {
                            if ( vertices[j].boneIndices[k] == -1 || fabs(vertices[j].boneWeights[k]) <
                                 fabsf(weight) ) {
                ...
                for (int j = 0; j < 3; ++j) {
                    if (projection == TXPRJ_UVMAP) { // UV-map
                        ...
                        if (meshInfo->pntVSelect(meshInfo, map) != 2) {
                            Manager::getSingleton()->getExporter()->printWarning(
                                name + ": UV-map isn't 2-dimensional. Discarding.");
                            continue;
                        }
 ...
 */
                ...
                for (int j = 0; j < 3; ++j) {
                    if (projection == TXPRJ_UVMAP) { // UV-map
                        ...
                        if (meshInfo->pntVSelect(meshInfo, map) != 2) {
                            Manager::getSingleton()->getExporter()->printWarning(
                                name + ": No actual UV-map defined. Discarding.");
                            //materials[faces[i].materialId].removeLightmap();
                            continue;
                        }
...
// Callback for getting face (polygon) data
...
#endif
        {
            ...
            if (type != LWPOLTYPE_FACE || vertices != 3) {
                Manager::getSingleton()->getExporter()->printWarning(
                    scanInfo->fileName + ": Non-triangular polygon found. Discarding.");
                return 0;
            }
 ...
 */
            ...
            if ( onSameEdge(v1, v2, v3, 0.000001f) ) {
                Manager::getSingleton()->getExporter()->printWarning(
                    scanInfo->fileName + ": Degenerate polygon found. Discarding.");
                return false;
            }
