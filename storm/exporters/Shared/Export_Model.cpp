 ...
 */
        void Model::getMaterials(std::vector<int>                              &result,
        ...
        std::vector<std::string> Model::getTextures(const std::vector<std::string>                &objectNames,
        ...
        const
        {
            ...
            for (unsigned int j = 0; j < usedMaterials.size(); ++j) {
                ...
                const Material &material =
                    ( index < int( materials.size() ) ) ? materials[index] : lightMaterials[index - materials.size()];
