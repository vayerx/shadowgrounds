...
// Helper functions
        ...
        std::string saveFileDialog(HWND               windowHandle,
...
// Handle our mighty dialog
        BOOL CALLBACK ExportDialog(HWND windowHandle, UINT message, WPARAM wParameter, LPARAM)
        {
            ...
            if (message == WM_INITDIALOG) {
                ...
                SendDlgItemMessage( windowHandle, IDC_TRANSFORM_TYPE, CB_ADDSTRING, 0,
                                    reinterpret_cast<LPARAM>("Pivot as origin") );
                SendDlgItemMessage( windowHandle, IDC_TRANSFORM_TYPE, CB_ADDSTRING, 0,
                                    reinterpret_cast<LPARAM>("Pivot as transform") );
                ...
                if (wParameter == WM_DESTROY) {
                    ...
                    if (listState == BONES) {
                        ...
                        int objects = SendMessage( GetDlgItem(windowHandle,
                                                              IDC_LISTBOX), LB_GETSELITEMS, objectAmount,
                                                   reinterpret_cast<long>(&indices[0]) );
                    ...
                    if (fileName.size() > 0) {
                        ...
                        exporter->getModel().saveToFile(fileName, objectNames,
                                                        exporter->getBoneId(), copyTextures, transformType, chop,
                                                        vcache, hasBones);
                    ...
                    if (listState == OBJECTS) {
                        ...
                        int bones = SendMessage( GetDlgItem(windowHandle,
                                                            IDC_LISTBOX), LB_GETSELITEMS, boneAmount,
                                                 reinterpret_cast<long>(&indices[0]) );
        ...
        void createListItems(HWND windowHandle)
        {
            ...
            if (listState == OBJECTS) {
                ...
                for (unsigned int i = 0; i < objects.size(); ++i) {
                    ...
                    int index =
                        SendMessage( GetDlgItem(windowHandle, IDC_LISTBOX), LB_ADDSTRING, 0, reinterpret_cast<long>(str) );
