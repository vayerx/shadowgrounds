        ...
        namespace {
            ...
            class DX {
                ...
                void createDevice()
                {
                    ...
                    d3dHandle->CreateDevice(D3DADAPTER_DEFAULT,
                                            D3DDEVTYPE_HAL,
                                            windowHandle,
            ...
            public:
                ...
                bool generateMeshes(const std::vector<Vertex> &vertices, const std::vector<Face> &faces)
                {
                    ...
                    D3DXGeneratePMesh(cleanedMesh, adjacency.get(), &attributeWeights, 0,
                                      cleanedMesh->GetNumVertices() / 3, D3DXMESHSIMP_VERTEX, &lodMesh);
                    if (!lodMesh) {
                        ...
                        D3DXGeneratePMesh(cleanedMesh,
                                          adjacency.get(), &attributeWeights, 0,
                                          cleanedMesh->GetNumVertices() / 3, D3DXMESHSIMP_VERTEX, &lodMesh);
                ...
                void getFaces(std::vector<Face> &faces, int lodLevel, int lodDetail)
                {
                    ...
                    int vertexAmount =
                        (LOD_AMOUNT - lodLevel - 1) * (maxVertex - minVertex) / (LOD_AMOUNT - 1) + minVertex;
