...
public:
    ...
    virtual void RenderSceneWithParams(bool              flip,
                                       bool              disable_hsr,
                                       bool              update_time,
                                       bool              render_mirrored,
                                       IStorm3D_Texture *target) = 0;
    ...
    virtual void Render2D_Picture(IStorm3D_Material *mat,
                                  VC2                position,
                                  VC2                size,
                                  float              alpha = 1.f,
                                  float              rotation = 0.f,
    ...
    virtual void Render2D_Picture(IStorm3D_Material  *mat,
                                  ...
                                  float               alpha,
                                  bool                wrap = false) = 0;
    virtual void Render2D_Text( IStorm3D_Font *font, VC2 position, VC2 size, const char *text, float alpha = 1.f,
                                const COL &colorFactor = COL(1.f, 1.f,
                                                             1.f) ) = 0;
    virtual void Render2D_Text( IStorm3D_Font *font, VC2 position, VC2 size, const wchar_t *text, float alpha = 1.f,
                                const COL &colorFactor = COL(1.f, 1.f,
                                                             1.f) ) = 0;

    // Test collision (to each model in scene)
    virtual void RayTrace(const VC3             &position,
                          const VC3             &direction_normalized,
                          float                  ray_length,
                          Storm3D_CollisionInfo &cinf,
                          bool                   accurate = false) = 0;
    virtual void SphereCollision(const VC3 &position, float radius, Storm3D_CollisionInfo &cinf, bool accurate =
                                     true) = 0;
...
public:
    ...
    virtual void ForceOrthogonalProjection(bool  force,
                                           float minx = 0.0f,
                                           float maxx = 0.0f,
                                           float miny = 0.0f,
                                           float maxy = 0.0f) = 0;                                                                       // when force == false, disables forcing the matrix.
