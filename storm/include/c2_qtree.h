...
template <class T> class QuadtreeEntity;
template <class T> class Quadtree;
template <class T> class QuadtreeRaytraceCollision;
template <class T> class QuadtreeSphereCollision;
template <class T> class QuadtreeFrustumIterator;
...
template <class T> class QuadtreeEntity {
    ...
    void reinsert()
    {
        if ( contains(implementation->node->looseArea,
                      implementation->sphere) || implementation->instance->fits(implementation->node->looseArea) ) {
...
template <class T> Quadtree<T>::Quadtree(const VC2 &min_, const VC2 &max_)
    :   root(0)
{
...
template <class T> typename Quadtree<T>::Entity
* Quadtree<T>::insert(T * instance, const VC3 &position, float radius) {
    if (!instance)
        return 0;
...
template <class T> void Quadtree<T>::erase(typename Quadtree<T>::Entity *entity)
{
    entity->implementation->node->erase(entity->implementation);
    delete entity->implementation;
}

template <class T> void Quadtree<T>::RayTrace(Ray &ray, Storm3D_CollisionInfo &info, bool accurate)
{
    QuadtreeRaytraceCollision<T> rayTracer(*this, ray, info, accurate);
}

template <class T> void Quadtree<T>::SphereCollision(const Sphere &sphere, Storm3D_CollisionInfo &info, bool accurate)
{
