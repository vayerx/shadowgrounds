...
void Keyb3_UpdateDevices()
{
    ...
    // psdtest
    {
        bool window_now_active = (GetFocus() == MGHwnd) && (GetActiveWindow() == MGHwnd) &&
                                 (GetForegroundWindow() == MGHwnd);
    ...
    if (!RawInputDeviceHandler::keyboardInitialized && Device_Keyboard)
        if (UseKeyboard) {
            ...
            if (!keyboardFailure)
                for (i = 0; i < 256; i++) {
                    nappis.keysdown[i] = ( (diks[i] & 0x80) != 0 );
                }
        }
...
//                ZeroMemory(RawInputDeviceHandler::getMouseInfo (l), sizeof(RawInputDeviceHandler::MouseInfo));
        ...
        // hiiren rajat
        for (unsigned int l = 0; l < RawInputDeviceHandler::numMice; l++) {
            if (RawInputDeviceHandler::getMouseInfo(l)->X > hiiri[l].max_x) RawInputDeviceHandler::getMouseInfo(l)->X =
                    hiiri[l].max_x;
            if (RawInputDeviceHandler::getMouseInfo(l)->Y > hiiri[l].max_y) RawInputDeviceHandler::getMouseInfo(l)->Y =
                    hiiri[l].max_y;
 ...
 */
            ...
            for (i = 0; i < MAX_MOUSEBUTTONS; i++) {
                hiiri[0].nappi[i] = ( (dims.rgbButtons[i] & 0x80) != 0 );
            }
    ...
    for (int jn = 0; jn < MAX_JOYSTICKS; jn++) {
        ...
        if (ok) {
            ...
            joy[jn].x = dijs.lX;
            joy[jn].y = dijs.lY;
            for (i = 0; i < MAX_JOYBUTTONS; i++) {
                joy[jn].nappi[i] = ( (dijs.rgbButtons[i] & 0x80) != 0 );
            }
