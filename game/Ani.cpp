...
namespace game
{
            ...
            if (recordBufUsed + slen >= recordBufAlloced) {
                ...
                if (recordBufAlloced * 2 > ANI_MAX_RECORD_BUF_ALLOC) {
                    Logger::getInstance()->warning(
                        "AniImpl::writeToRecordBuf - Maximum allocated record buffer size limit reached.");
                    Logger::getInstance()->debug(this->name);
                    return;
                }
                // recreate the record buffer and make it bigger
                Logger::getInstance()->debug(
                    "AniImpl::writeToRecordBuf - Allocated record buffer full, reallocating a bigger buffer.");
                Logger::getInstance()->debug(this->name);
    ...
    Ani::Ani(GameScripting *gameScripting, Unit *unit)
    {
        if (unit == NULL)
            Logger::getInstance()->error(
                "Ani - Constructor called with null unit parameter (this will be a fatal error).");
        ...
        if (impl->recordPathName != NULL && impl->name != NULL) {
            if (strlen(impl->name) + strlen(impl->recordPathName) + 16 < 256) {
                ...
                if (f != NULL) {
                    if (!appendToOld) {
                        Logger::getInstance()->debug(
                            "Ani::startRecord - Note, a file with the record name exists, it will be overwritten.");
                        Logger::getInstance()->debug(
                            "Ani::startRecord - To prevent the file from being overwritten, use cancelRecord to cancel this recording.");
                    }
                    ...
                    if (f != NULL) {
                        ...
                        Logger::getInstance()->error(
                            "Ani::startRecord - Fix this problem before stopRecord or the recorded data may be lost.");
                    }
                }
#endif
            } else {
                Logger::getInstance()->error(
                    "Ani::startRecord - Note, file path too long, recorded data will be lost unless the problem is fixed.");
            }
        } else {
        ...
        if (impl->recordPathName != NULL && impl->name != NULL) {
            ...
            if (strlen(impl->name) + strlen(impl->recordPathName) + 16 < 256) {
                ...
                if (f != NULL) {
                    ...
                    Logger::getInstance()->error(
                        "Ani::stopRecord - Record save failed, file could not be opened for writing.");
                }
#endif
            } else {
    ...
    void Ani::skipToMark(const char *mark)
    {
            ...
            && mark[0] != '\0')
        {
            ...
            Logger::getInstance()->error(
                "Ani::skipToMark - Cannot skip to given mark, given mark name too long, empty or null.");
        }
    }
    ...
    void Ani::aniWarp(float x, float z)
    {
        ...
        float tmpX = pos.x;
        pos.x = pos.x * cosf( UNIT_ANGLE_TO_RAD(AniImpl::globalRotation) ) + pos.z *sinf( UNIT_ANGLE_TO_RAD(
                                                                                              AniImpl::globalRotation) );
        pos.z = pos.z * cosf( UNIT_ANGLE_TO_RAD(AniImpl::globalRotation) ) - tmpX *sinf( UNIT_ANGLE_TO_RAD(
                                                                                             AniImpl::globalRotation) );
        ...
        if (animset != NULL)
            if ( animset->isAnimationInSet(anim) ) {
                if (animset->getAnimationBlendNumber(anim) != 0)
                    ...
                    Logger::getInstance()->warning(
                        "Ani::aniEndAnim - Ending a base (non-blended) animation. Something probably went wrong?");
