...
namespace game
{
        ...
        if (muzzleflashEffect != NULL) {
            if ( unit->isMuzzleflashVisible() ) {
                Logger::getInstance()->debug(
                    "UnitActor::createMuzzleflash - Unit's muzzleflash already visible while trying to create another.");
                    ...
                    if (unitVisual) {
                        ...
                        if (unitModel) {
                            ...
                            if ( unit->getWeaponType(weap)->getCustomWeaponBarrelHelper() ) {
                                bool helperFound = util::getHelperPosition(unitModel, unit->getWeaponType(
                                                                               weap)->getCustomWeaponBarrelHelper(),
                                                                           weaponPosition);
                                if (!helperFound)
                                    Logger::getInstance()->warning(
                                        "UnitActor::actWeaponry - Custom weapon barrel helper not found.");
                            } else {
                                ...
                                bool helperFound = util::getHelperPosition(unitModel,
                                                                           modelHelperName.c_str(), weaponPosition);
                                if (!helperFound) {
                                    std::string boneHelperName = std::string("HELPER_BONE_") + barrelName;
                                    helperFound = util::getHelperPosition(unitModel,
                                                                          boneHelperName.c_str(), weaponPosition);
                                    if (!helperFound)
                                        Logger::getInstance()->warning(
                                            "UnitActor::actWeaponry - Weapon barrel helper not found.");
                                }
                            }
                        }
                    }
    ...
    void UnitActor::createEject(Unit *unit, int weaponNumber)
    {
        ...
        if (ejectEffect != NULL) {
            if ( unit->isEjectVisible() ) {
                Logger::getInstance()->debug(
                    "UnitActor::createEject - Unit's eject \"already visible\" (in progress) while trying to create another.");
                    ...
                    if (unitVisual) {
                        ...
                        if (unitModel) {
                            ...
                            if ( unit->getUnitType()->hasMechControls() ) {
                                ...
                                helperFound =
                                    util::getHelperPositionOffset(unitModel, boneHelperName.c_str(), VC3(offset,
                                                                                                         0,
                                                                                                         0),
                                                                  weaponPosition);
                            }
                            ...
                            if (!helperFound) {
                                ...
                                if (!helperFound) {
                                    ...
                                    if (!helperFound)
                                        Logger::getInstance()->warning(
                                            "UnitActor::createEject - Weapon eject helper not found.");
                                }
                            }
                        }
                    }
    ...
    void UnitActor::getAutoAimNear(Unit      *unit,
                                   const VC3 &target,
                                   Unit     **autoAimed,
                                   bool      *aimedVertical,
                                   bool      *aimedHorizontal)
    {
        ...
        while ( iter.iterateAvailable() ) {
                 ...
                 && game->isHostile( unit->getOwner(), other->getOwner() ) )
            {
                ...
                if (distSq < nearestDistSq) {
                    ...
                    float otherAngle = util::PositionDirectionCalculator::calculateDirection( unitPos,
                                                                                              other->getPosition() );
                    float angleRot = util::AngleRotationCalculator::getRotationForAngles(targAngle, otherAngle, aaAngle);
                    float angleRotVert = util::AngleRotationCalculator::getRotationForAngles(targAngle,
                                                                                             otherAngle,
                                                                                             aaAngleHoriz);
    ...
    void UnitActor::makeFireSound(Unit *unit, int weapon, const char *fireSound)
    {
        ...
        if (handle != -1) {
            ...
            handle = game->gameUI->parseSoundFromDefinitionString(fireSound,
                                                                  pos.x,
                                                                  pos.y,
                                                                  pos.z,
                                                                  &looped,
                                                                  ...
                                                                  &key,
                                                                  true,
                                                                  unit->getUnitType()->getSoundRange(),
                                                                  DEFAULT_SOUND_PRIORITY_NORMAL);
        } else {
...
//                && game->gameUI->isThirdPersonView(unit->getOwner()))
        ...
        if ( (unit->getUnitType()->isJumpTurnable()
              || ( unit->getJumpTotalTime() - unit->getJumpCounter() ) <
              unit->getUnitType()->getTurnTimeAtJumpStart() / GAME_TICK_MSEC)
...
//    if (unit->getMoveState() != Unit::UNIT_MOVE_STATE_UNCONSCIOUS)
    ...
    void UnitActor::handleRechargeableWeapons(Unit *unit, bool &primaryPressed, bool &secondaryPressed)
    {
            ...
            || rechargeablePrimaryWeapon)
        {
            if ( unit->isWeaponRecharging() ) {
                ...
                if ( (
                         ( ( (secondaryPressed &&
                              rechargeableSecondaryWeapon) || (primaryPressed && rechargeablePrimaryWeapon) )
                    ...
                    if (unitVisual) {
                        ...
                        if (unitModel) {
                            ...
                            if ( unit->getWeaponType(weap)->getCustomWeaponBarrelHelper() ) {
                                ...
                                if (!helperFound)
                                    Logger::getInstance()->warning(
                                        "UnitActor::handleRechargeableWeapons - Custom weapon barrel helper not found.");
                            } else {
                                std::string barrelName = "WeaponBarrel";
                                if (unit->getRotatedWeaponBarrel(weap) >= 2)
                                    barrelName += int2str( unit->getRotatedWeaponBarrel(weap) );
                                ...
                                if (!helperFound) {
                                    ...
                                    if (!helperFound)
                                        Logger::getInstance()->warning(
                                            "UnitActor::handleRechargeableWeapons - Weapon barrel helper not found.");
                                }
                            }
                        }
                    }
                                                          ...
                                                          float *rotationAngle)
    {
            ...
            && unit->getWeaponType( unit->getSelectedWeapon() ) != NULL)
        {
            if (secondaryPressed) {
                ...
                if ( unit->getWeaponType( unit->getSelectedWeapon() )->getPartTypeId() ==
                     PARTTYPE_ID_STRING_TO_INT("W_Shotg") )
                ...
                if ( unit->getWeaponType( unit->getSelectedWeapon() )->getPartTypeId() ==
                     PARTTYPE_ID_STRING_TO_INT("W_Rocket") )
            ...
            if ( (primaryPressed || secondaryPressed)
                 && ( (unit->getWeaponAmmoInClip( unit->getSelectedWeapon() ) == 0
                      ...
                      || (unit->getWeaponAmmoAmount( unit->getSelectedWeapon() ) == 0) ||
                      !unit->isWeaponOperable( unit->getSelectedWeapon() ) ) )
                if ( !unit->isClipEmptySoundDone() ) {
                    const char *soundfile = unit->getWeaponType( unit->getSelectedWeapon() )->getClipEmptySound();
                    if (soundfile != NULL) {
                        VC3 pos = unit->getPosition();
                        game->gameUI->playSoundEffect(soundfile,
                                                      pos.x,
                                                      pos.y,
                                                      pos.z,
                                                      false,
                    ...
                    if ( !unit->isWeaponOperable( unit->getSelectedWeapon() ) ) {
                        ...
                        std::string locale = "gui_weapon_" + std::string( w->getPartTypeIdString() )
                                             + "_inoperable_message";
                        std::transform(locale.begin(), locale.end(), locale.begin(), ( int (*)(int) )tolower);

                        const char *message = NULL;
                        if ( ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                                               locale.c_str(), &message) )
                            ...
                            bool *doForwardMove, bool *doBackMove, bool *doLeftMove, bool *doRightMove)
    {
        ...
        if (jc > 0) {
            ...
            if (unit->getJumpTotalTime() - jc == ROLL_JUMP_ALLOW_STOP_TIME / GAME_TICK_MSEC) {
                ...
                if (jsound != NULL)
                    game->gameUI->playSoundEffect(jsound,
                                                  pos.x,
                                                  pos.y,
                                                  pos.z,
                                                  false,
                            ...
                            bool doRotation, float rotationAngle, bool doMove, bool *rotated)
    {
        ...
        if (unit->getJumpCounter() > 0) {
            if ( ( unit->getJumpTotalTime() - unit->getJumpCounter() ) <
                 unit->getUnitType()->getTurnTimeAtJumpStart() / GAME_TICK_MSEC )
        ...
        if (doRotation) {
            ...
            if ( unitType->isVehicle() && !unitType->isFlying() ) {
                ...
                // less rotation speed
                float factSpeed = util::AngleRotationCalculator::getFactoredRotationForAngles(rotation.y,
                                                                                              rotationAngle,
                                                                                              rotacc);
                if (factSpeed != 0) {
                    if (factSpeed > 0) {
                ...
                rotSpeed = turnspeed * util::AngleRotationCalculator::getRotationForAngles(rotation.y,
                                                                                           rotationAngle,
                                                                                           rotacc);
            }
...
//                                && game->gameUI->isThirdPersonView(unit->getOwner()))
                    ...
                    if (unit->getSpeed() == Unit::UNIT_SPEED_FAST) {
                            ...
                            && unit->getJumpTotalTime() > 0)
                        {
                            ...
                            int timeAtString =
                                ( ( unit->getJumpTotalTime()
                                    - unit->getJumpCounter() ) * JUMPTIMING_STRING_LENGTH ) / unit->getJumpTotalTime();
...
//                    && game->gameMap->getCoverMap()->getDistanceToNearestCover(ox, oy) > 2
        ...
        if ( doMove && unit->getUnitType()->getWalkOnMaterialEnabled() &&
             game->gameUI->getFirstPerson(game->singlePlayerNumber) &&
             IsCloseEnough2D(game->gameUI->getFirstPerson(game->singlePlayerNumber)->getPosition(), position,
                             walkOnMaterialRange) ) {
            WalkOnMaterialData *material_data = NULL;

            bool enough_time_passed_for_effect =
                (game->gameTimer - movementParticleEffectCreationTime) * GAME_TICK_MSEC >
                unit->getUnitType()->getWalkOnMaterialParticleEffectTimeOut();
            bool enough_time_passed_for_sound = (game->gameTimer - movementSoundEffectCreationTime) * GAME_TICK_MSEC >
                                                unit->getUnitType()->getWalkOnMaterialSoundEffectTimeOut();
            ...
            if (enough_time_passed_for_sound || enough_time_passed_for_effect) {
                ...
                for (unsigned int i = 0; i < footStepTriggers.size(); i++) {
                    ...
                    int trigger_duration =
                        (int)( (trigger->time_max - trigger->time_min) / unit->getAnimationSpeedFactor() );
                ...
                if (material_data != NULL && material_data->particleEffectName.empty() == false) {
                    ...
                    if (effId == -1) {
                        Logger::getInstance()->error(
                            "UnitType::actWalk - No walk on material visual effect with given name found.");
                        // Logger::getInstance()->debug(objectEvents[i].effect.c_str());
                    } else {
                        // TODO: proper lifetime
                        int lifetime = GAME_TICKS_PER_SECOND / 2;
                        VisualEffect *effect = vman->createNewManagedVisualEffect(effId,
                                                                                  lifetime,
                                                                                  NULL,
                                                                                  NULL,
                                                                                  pos,
            ...
            if (stepped && enough_time_passed_for_sound) {
                ...
                if (material_data != NULL && material_data->soundEffects.empty() == false) {
                    ...
                    game->gameUI->playSoundEffect(
                        material_data->soundEffects[ sound_pos ].file.c_str(), position.x, position.y, position.z, loop,
                        volume, range, DEFAULT_SOUND_PRIORITY_NORMAL);
                }
            }
            // </sound effect hack>
    ...
    void UnitActor::actOutdoorBreathingSteam(Unit *unit)
    {
        ...
        if ( ut->getBreathingParticleEffect().empty() == false &&
             game->gameUI->getFirstPerson(game->singlePlayerNumber) &&
             IsCloseEnough2D(game->gameUI->getFirstPerson(game->singlePlayerNumber)->getPosition(), unit->getPosition(),
                             walkOnMaterialRange) &&
             (game->gameTimer - breathingParticleEffectCreationTime) * GAME_TICK_MSEC >
             ut->getBreathingParticleEffectTimeOut() )
        {
            ...
            if (game->gameMap->getAreaMap()->isAreaAnyValue(x, y, AREAMASK_INBUILDING) == 0) {
                ...
                if (unitModel) {
                    ...
                    if (!helperFound) {
                        Logger::getInstance()->warning(
                            "UnitActor::actOutdoorBreathingSteam - Could not find a helper to put the particle breath animation onto.");
                    } else {
                        ui::VisualEffectManager *vman = game->gameUI->getVisualEffectManager();
                        int effId = vman->getVisualEffectIdByName( ut->getBreathingParticleEffect().c_str() );
                        if (effId == -1) {
                            Logger::getInstance()->error(
                                "UnitType::actOutdoorBreathingSteam - No particle effect with given name found.");
                            // Logger::getInstance()->debug(objectEvents[i].effect.c_str());
                        } else {
                            // TODO: proper lifetime
                            int lifetime = GAME_TICKS_PER_SECOND / 2;
                ...
                if (hp < -30) {
                    ...
                    if (unit->getMoveState() == Unit::UNIT_MOVE_STATE_UNCONSCIOUS) {
                        ...
                        while ( iter.iterateAvailable() ) {
                                ...
                                && u->getMoveState() != Unit::UNIT_MOVE_STATE_UNCONSCIOUS)
                            {
                                if ( u->getOwner() == unit->getOwner() ) {
                                    ...
                                    if (posdiff.GetSquareLength() < HPGAIN_FRIENDS_NEAR_RANGE
                                        * HPGAIN_FRIENDS_NEAR_RANGE) {
                                        friendsNear = true;
                                    ...
                                    if ( game->isHostile( unit->getOwner(), u->getOwner() ) ) {
                                        VC3 posdiff = u->getPosition() - unit->getPosition();
                                        if (posdiff.GetSquareLength() < HPGAIN_HOSTILES_NEAR_RANGE
                                            * HPGAIN_HOSTILES_NEAR_RANGE) {
                                            hostilesNear = true;
                                            break;
                                        }
                                    }
                        ...
                        if ( unit->getHP() < unit->getUnitType()->getHPGainMax() ) {
                            if ( ( unit->getHP() + unit->getUnitType()->getHPGain() ) >
                                 unit->getUnitType()->getHPGainMax() )
...
// HACK: claw proto hack...
                ...
                unit->targeting.setLineOfFireToTarget(false,
                                                      foo_prone * unitType->getLineOfFireLoseTime() / GAME_TICK_MSEC,
                                                      game->gameTimer);
#else
                if (unit->getSpeed() == Unit::UNIT_SPEED_CRAWL)
                ...
                unit->targeting.setLineOfFireToTarget(false,
                                                      unitType->getLineOfFireLoseTime() / GAME_TICK_MSEC,
                                                      game->gameTimer);
#endif
                ...
                if ( posDiffSqLen < 2 * 2
                    ...
                    unit->targeting.setLineOfFireToTarget(true,
                                                          unitType->getLineOfFireLoseTime() / GAME_TICK_MSEC,
                                                          game->gameTimer);
                     ...
                     && posDiffSqLen < ( unit->getMaxWeaponRange() * unit->getMaxWeaponRange() ) )
                {
                    ...
                    while ( ownUnitsIter.iterateAvailable() ) {
                        ...
                        if (posdiff.GetSquareLength() < UNITACTOR_LOF_IGNORE_FRIENDLY_RANGE
                            * UNITACTOR_LOF_IGNORE_FRIENDLY_RANGE)
                    ...
                    if (cinfo.hit) {
                        if (cinfo.hitUnit) {
                            if ( ( game->isHostile( unit->getOwner(), cinfo.unit->getOwner() ) )
                                ...
                                unit->targeting.setLineOfFireToTarget(true,
                                                                      unitType->getLineOfFireLoseTime()
                                                                      / GAME_TICK_MSEC, game->gameTimer);
                        } else {
                            if (unit->targeting.getTargetUnit() == NULL) {
...
//                        && !cannotRotate)
                    ...
                    float fofRotate = 0;
                    fofRotate = util::AngleRotationCalculator::getRotationForAngles(rotation.y,
                                                                                    destAngle,
                                                                                    (float)fofAngle);
...
//            if (unit->isDirectControl() && game->gameUI->isThirdPersonView(unit->getOwner()))
            ...
            if ( unit->hasAnyWeaponReady() || unit->isClipReloading() ) {
                if ( unit->isDirectControl() ) {
                    ...
                    Weapon *w = unit->getSelectedWeapon() !=
                                -1 ? unit->getWeaponType( unit->getSelectedWeapon() ) : NULL;
                    if (w)
...

        {
                ...
                if ( w && w->usesLaunchSpeed() ) {
                    ...
                    if (unit->getAttachedWeapon(wnum) != -1) {
                        ...
                        if ( w && w->usesLaunchSpeed() )
                            ...
                            launchspeed_fire_released = !game->gameUI->isLocalPlayerDirectControlOn(
                                DIRECT_CTRL_FIRE_SECONDARY,
                                unit);
                        else
                            w = NULL;
                    } else {
            ...
            // primary is same as secondary
                ...
                launchspeed_fire_released =
                    !game->gameUI->isLocalPlayerDirectControlOn(DIRECT_CTRL_FIRE,
                                                                unit) && !game->gameUI->isLocalPlayerDirectControlOn(
                        DIRECT_CTRL_FIRE_GRENADE,
                        unit);
            }
            ...
            if ( launchspeed_fire_released && w && w->usesLaunchSpeed() )
                ...
                if (unit->getFireWaitDelay(wnum) <= 0) {
                    ...
                    if ( (unit->getWeaponAmmoAmount(wnum) == 0 &&
                          w->getAmmoUsage() != 0) || unit->getFireReloadDelay(wnum) > 0 ) {
                        // reset launch speed
            ...
            //if (fabs(unit->getLastBoneAimDirection() - aimAngle) > 0.1f)
            {
                ...
                if (aimspeed >= 0 && aimacc >= 0) {
                    ...
                    if ( handle != -1 && game->gameUI->getSoundMixer() &&
                         !game->gameUI->getSoundMixer()->isSoundPlaying(handle) ) {
                    ...
                    if (turndir != 0 && handle == -1) {
                        ...
                        if ( sound_name && game->gameUI->getSoundMixer() ) {
                            VC3 pos = unit->getPosition();
                            handle = game->gameUI->playSoundEffect(sound_name,
                                                                   pos.x,
                                                                   pos.y,
                                                                   pos.z,
                                                                   true,
                    ...
                    // already playing sound
                        ...
                        if ( turndir == 0 && timeDelta > timeReq && game->gameUI->getSoundMixer() ) {
                            ...
                            game->gameUI->getSoundMixer()->setSoundFrequency(handle,
                                                                             unit->getTurningSoundFrequency() * vol
                                                                             / 100);
                ...
                if ( handle != -1 && game->gameUI->getSoundMixer() ) {
                    ...
                    game->gameUI->getSoundMixer()->setSoundPosition(handle,
                                                                    pos.x,
                                                                    pos.y,
                                                                    pos.z,
                                                                    vel.x,
...
//nearest90IsNext = false;
        ...
        preferredRotate =
            util::AngleRotationCalculator::getRotationForAngles(rotation.y, preferredRotation,
                                                                (unit->getUnitType()->getTurning()
                                                                 / GAME_TICKS_PER_SECOND) / 1.8f);
        ...
        for (int weap = 0; weap < UNIT_MAX_WEAPONS; weap++) {
            ...
            if (doRealShot || doCopyShot) {
                    ...
                    && unit->getWeaponType(weap)->getRechargePeakTime() > 0)
                {
                    ...
                    {
                        stepNumber = 1
                                     + ( unit->getWeaponRechargeAmount()
                                         - unit->getWeaponRechargeMin() ) * unit->getWeaponRechargeSteps()
                                     / ( unit->getWeaponRechargePeak() - unit->getWeaponRechargeMin() );
                        if ( stepNumber > unit->getWeaponRechargeSteps() )
                            stepNumber = unit->getWeaponRechargeSteps();
                    } else {
                        Logger::getInstance()->error(
                            "UnitActor::actWeaponry - Erronous recharge min/peak/step values given for recharging weapon.");
                        assert(
                            !
                            "UnitActor::actWeaponry - Erronous recharge min/peak/step values given for recharging weapon.");
                    }
                    ...
                    if ( PARTTYPE_ID_STRING_VALID(tmp) ) {
                        ...
                        if (pt != NULL) {
                            if ( pt->isInherited( getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Bull") ) ) ) {
                                ...
                                Logger::getInstance()->error(
                                    "UnitActor::actWeaponry - Non-bullet type part-type id given for recharging weapon.");
                                assert(
                                    !
                                    "UnitActor::actWeaponry - Non-bullet type part-type id given for recharging weapon.");
                            }
                            ...
                            Logger::getInstance()->error(
                                "UnitActor::actWeaponry - Given part-type id has no numbered part-types for recharging weapon.");
                            assert(
                                !
                                "UnitActor::actWeaponry - Given part-type id has no numbered part-types for recharging weapon.");
                        }
            ...
            if (doRealShot) {
                ...
                if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_TOPDOWN_SHOOTER) )
                         ...
                         && ( game->gameUI->isThirdPersonView( unit->getOwner() ) || unit->isSideways() ) )
                    {
                        ...
                        SceneSelection *sceneSel = game->gameUI->getSceneSelection(
                             game->gameUI->getClientNumberForUnit(unit) );
                        if (sceneSel->hit) {
                            VC3 tpos;
                ...
                if (doDirectControlFire) {
                    ...
                    target.y +=
                        (float)( 0.1f - unit->getWeaponType(weap)->getRange() ) * sinf( UNIT_ANGLE_TO_RAD(wBetaAngle) );
                        ...
                        && shotNumber > 0)
                    {
                        ...
                        if (targUnit != NULL) {
                            ...
                            target +=
                                ( ( targUnit->getPosition()
                                    + VC3(0, targUnit->getUnitType()->getAimHeightStanding(), 0) ) * nonSweepWeight );
                        } else {
                        ...
                        if (targUnit != NULL) {
                            ...
                            if ( ( (targUnit->getMoveState() == Unit::UNIT_MOVE_STATE_UNCONSCIOUS
                                   ...
                                   && ra != 0 )
                                 || ra == 1 )
                                target = targUnit->getPosition() + VC3(0, targUnit->getUnitType()->getAimHeightCrawling(
                                                                           ), 0);
                            else
                                target = targUnit->getPosition() + VC3(0, targUnit->getUnitType()->getAimHeightStanding(
                                                                           ), 0);
                            ...
                            if ( targUnit->getUnitType()->doesUseAimpointHelper() )
                                ...
                                {
                                    VC3 tmp = target;
                                    bool helperFound = util::getHelperPosition(
                                        targUnit->getVisualObject()->getStormModel(), "HELPER_BONE_Aimpoint", tmp);
                                    if (helperFound)
                                        target = tmp;
                                    else
                                        LOG_ERROR_W_DEBUG(
                                             "UnitActor::actWeaponry - No aim point helper found in target unit model.",
                                            targUnit->getUnitType()->getName() );
                                }
                ...
                if ( w->getShootDirectionLimit() > 0.0f && w->getShootDirectionLimit() < (180.0f - 0.001f) ) {
                    ...
                    QUAT q;
                    q.MakeFromAngles(UNIT_ANGLE_TO_RAD( unit->getLastBoneAimBetaAngle() ),
                                     UNIT_ANGLE_TO_RAD( rot.y + unit->getLastBoneAimDirection() ), 0);
            ...
            for (int rep = 0; rep < repeatNow; rep++) {
                if (doCopyShot) {
                    ...
                    if (proj == NULL) {
                        Logger::getInstance()->error(
                            "UnitActor::actWeaponry - Null projectile to copy (possibly weapon repeat amount and delay too big).");
                    ...
                    spreadVec.x += spreadAmount
                                   * (float)( (game->gameRandom->nextInt() % 201) - 100 ) / (100.0f * 100.0f);
                    spreadVec.y += spreadAmount
                                   * (float)( (game->gameRandom->nextInt() % 201) - 100 ) / (100.0f * 100.0f);
                    spreadVec.z += spreadAmount
                                   * (float)( (game->gameRandom->nextInt() % 201) - 100 ) / (100.0f * 100.0f);
                    ...
                    for (int tx = -collRadius; tx <= collRadius; tx++) {
                                   ...
                                   && (ty == collRadius || ty == -collRadius) ) )
                        {
                            ...
                            if ( game->getGameScene()->isBlockedAtScaled( pos.x, pos.z,
                                                                          game->gameMap->getScaledHeightAt(pos.x,
                                                                                                           pos.y) ) ) {
                                if (tx < 0)
...
#  define UA_WALL_VEL_FACTOR 0.75f
                                      ...
                                      VC3 oldPosition)
    {
        ...
        if (unit->isDirectControl()
            && abs( game->gameMap->getHeightmapHeightAt(obstX / GAMEMAP_PATHFIND_ACCURACY, obstY
                                                        / GAMEMAP_PATHFIND_ACCURACY)
                    - game->gameMap->getHeightmapHeightAt(oldObstX / GAMEMAP_PATHFIND_ACCURACY, oldObstY
                                                          / GAMEMAP_PATHFIND_ACCURACY) ) > maxSlope)
            tooSlope = true;

        if ( !unitType->isFlying()
             && unit->doesCollisionCheck()
             && (game->getGameScene()->getConditionalBlockingCountForUnit(obstX, obstY, position.y
                                                                          + unit_obst_height + extraheight) >
                 unit_obst_amount
                 || tooSlope) )
        {
                ...
                && !tooSlope)
            {
                ...
                if (oldObstY == obstY + 1 && oldObstX == obstX - 1) {
                    ...
                    if (game->getGameScene()->getConditionalBlockingCountForUnit(oldObstX, obstY, position.y
                                                                                 + unit_obst_height_near
                                                                                 + extraheight) > unit_obst_amount_near)
                        intersectSouthWall = true;
                    else
                        intersectWestWall = true;
                }
                            ...
                            && obstY < game->gameMap->getObstacleSizeY() - 1)
                        {
                            ...
                            /*
                             ...
                             + unit_obst_height_near + extraheight) > unit_obst_amount_near
                                ...
                                be = true;
                             */
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(obstX, obstY - 1,
                                                                                          0) > unit_obst_amount_near
                                 && game->gameMap->isRoundedObstacle(obstX, obstY - 1) )
                                bn = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(obstX, obstY + 1,
                                                                                          0) > unit_obst_amount_near
                                 && game->gameMap->isRoundedObstacle(obstX, obstY + 1) )
                                bs = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(obstX - 1, obstY,
                                                                                          0) > unit_obst_amount_near
                                 && game->gameMap->isRoundedObstacle(obstX - 1, obstY) )
                                bw = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(obstX + 1, obstY,
                            ...
                            float blockStartX =
                                ( (float)obstX * game->gameMap->getScaledSizeX()
                                  / (float)game->gameMap->getObstacleSizeX() - game->gameMap->getScaledSizeX() * 0.5f );
                            float blockStartY =
                                ( (float)obstY * game->gameMap->getScaledSizeY()
                                  / (float)game->gameMap->getObstacleSizeY() - game->gameMap->getScaledSizeY() * 0.5f );
                            float blockSizeX =
                                ( (float)(obstX
                                          + 1) * game->gameMap->getScaledSizeX()
                                  / (float)game->gameMap->getObstacleSizeX()
                                  - game->gameMap->getScaledSizeX() * 0.5f ) - blockStartX;
                            float blockSizeY =
                                ( (float)(obstY
                                          + 1) * game->gameMap->getScaledSizeY()
                                  / (float)game->gameMap->getObstacleSizeY()
                                  - game->gameMap->getScaledSizeY() * 0.5f ) - blockStartY;
                            ...
                            if (diagNEtoSW2) {
                                ...
                                if (intersectNEtoSW2) {
                                    VC2 crossing = VC2(0.7071068f, 0.7071068f);
                                    float dprod =
                                        crossing.GetDotWith( VC2(blockSizeX - blockPosX, blockSizeY - blockPosY) );
                                    VC2 dprodVec = VC2(dprod * 0.7071068f, dprod * 0.7071068f);
                                    VC2 offset = dprodVec - VC2(0.5f * blockSizeX, 0.5f * blockSizeY);
                            ...
                            && oldObstY < game->gameMap->getObstacleSizeY() - 1)
                        {
                            ...
                            /*
                             ...
                             + unit_obst_height_near + extraheight) > unit_obst_amount_near
                                ...
                                be = true;
                             */
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(oldObstX, oldObstY - 1,
                                                                                          0) > unit_obst_amount_near
                                 && game->gameMap->isRoundedObstacle(oldObstX, oldObstY - 1) )
                                bn = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(oldObstX, oldObstY + 1,
                                 ...
                                 && game->gameMap->isRoundedObstacle(oldObstX, oldObstY + 1) )
                                bs = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(oldObstX - 1, oldObstY,
                                                                                          0) > unit_obst_amount_near
                                 && game->gameMap->isRoundedObstacle(oldObstX - 1, oldObstY) )
                                bw = true;
                            if ( game->getGameScene()->getConditionalBlockingCountForUnit(oldObstX + 1, oldObstY,
                            ...
                            float blockStartX =
                                ( (float)oldObstX * game->gameMap->getScaledSizeX()
                                  / (float)game->gameMap->getObstacleSizeX() - game->gameMap->getScaledSizeX() * 0.5f );
                            float blockStartY =
                                ( (float)oldObstY * game->gameMap->getScaledSizeY()
                                  / (float)game->gameMap->getObstacleSizeY() - game->gameMap->getScaledSizeY() * 0.5f );
                            float blockSizeX =
                                ( (float)(oldObstX
                                          + 1) * game->gameMap->getScaledSizeX()
        ...
        if (unit->getWalkDelay() > 0)
            ...
            if (unit->getAnimationSet() != NULL) {
                     ...
                     && unit->isWeaponVisible( unit->getSelectedWeapon() ) )
                {
                    ...
                    if ( unit->getAnimationSet()->isAnimationInSet( ANIM_SHOOT_TYPE0_STANDING
                                                                    + unit->getWeaponType( unit->getSelectedWeapon() )
                                                                    ->getWeaponAnimationType() )
                         && unit->isShootAnimStanding() )
                        return;
                    else
                        if (unit->getAnimationSet()->getAnimationBlendNumber( ANIM_SHOOT_TYPE0
                                                                              + unit->getWeaponType( unit->
                                                                                                     getSelectedWeapon() )
                                                                              ->getWeaponAnimationType() ) == 0)
                            return;
                } else {
...
// HACK: claw proto
                         ...
                         && unit->isWeaponVisible( unit->getSelectedWeapon() ) )
                    {
                        if (unit->getWeaponType( unit->getSelectedWeapon() ) != NULL) {
                            ...
                            if (unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType() >= 0
                                && unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType() <
                                ANIM_TYPES_AMOUNT)
                                anim = ANIM_STAND_TYPE0
                                       + unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType();
                            else
                        ...
                        if (animRequests.turnLeft)
                            if ( unit->getAnimationSet()->isAnimationInSet(ANIM_TURN_LEFT) ) {
                                     ...
                                     && unit->isWeaponVisible( unit->getSelectedWeapon() ) )
                                {
                                    if (unit->getWeaponType( unit->getSelectedWeapon() ) != NULL) {
                                        // NOTE: these anims must be incremental in numbers
                                        if (unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType()
                                            >= 0
                                            && unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType()
                                            < ANIM_TYPES_AMOUNT)
                                            anim = ANIM_TURN_LEFT_TYPE0
                                                   + unit->getWeaponType( unit->getSelectedWeapon() )->
                                                   getWeaponAnimationType();
                                        else
                                            assert(!"UnitActor::applyAnimations - Weapon animation type out or range.");
            ...
            if (animRequests.setMoveAnim) {
                    ...
                    anim == ANIM_SPRINT || anim == ANIM_JUMP)
                {
                    ...
                    float rotDirFactored = util::AngleRotationCalculator::getFactoredRotationForAngles(
                        rotation.y,
                        velDir,
                        45.0f
                        - strafeAnimBias);
...
// --- END VEL BASED ---
            ...
            if (animRequests.setSpecialAnim != NO_SPECIAL_ANIMATION_REQUEST) {
                if (animRequests.setSpecialAnim == SPECIAL_ANIMATION_ELECTRIFIED) {
                    ...
                    else if ( unit->getAnimationSet()->isAnimationInSet(ANIM_STAND) )
                             ...
                             && unit->isWeaponVisible( unit->getSelectedWeapon() ) )
                        {
                            if (unit->getWeaponType( unit->getSelectedWeapon() ) != NULL) {
                                if (unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType() >= 0
                                    && unit->getWeaponType( unit->getSelectedWeapon() )->getWeaponAnimationType() <
                                    ANIM_TYPES_AMOUNT)
                                    unit->getAnimationSet()->animate( unit, ANIM_AIM_TYPE0
                                                                      + unit->getWeaponType(
                                                                           unit->getSelectedWeapon() )->
                                                                      getWeaponAnimationType() );
                                else
                                    assert(0);
                            } else {
                    ...
                    if ( unit->isWeaponActive(i) ) {
                        ...
                        if (reloaded) {
                                 ...
                                 && unit->getAnimationSet()->isAnimationInSet(reload_anim) )
                            {
                                ...
                                if ( w->usesCustomTimeFactor() )
                                    fact = unit->getCustomTimeFactor();
                                Animator::setAnimationSpeedFactor(unit,
                                                                  unit->getAnimationSet()->getAnimationFileNumber(
                                                                      reload_anim), fact);
                            ...
                            if (reloadSound != NULL) {
                                ...
                                int key = 0;
                                VC3 pos = unit->getPosition();
                                int h = game->gameUI->parseSoundFromDefinitionString(reloadSound,
                                                                                     pos.x,
                                                                                     pos.y,
                                                                                     pos.z,
                                                                                     &looped,
                                ...
                                if ( h != -1 && w->usesCustomTimeFactor() && game->gameUI->getSoundMixer() ) {
                                    ...
                                    game->gameUI->getSoundMixer()->setSoundFrequency( h,
                                                                                      (int)( freq
                                                                                             * unit->
                                                                                             getCustomTimeFactor() ) );
                                }
    ...
    bool UnitActor::shoot(Unit *unit, const VC3 &target)
    {
        ...
        for (int i = 0; i < UNIT_MAX_WEAPONS; i++) {
            ...
            if (w != NULL) {
                ...
                //////////////////////////////////////////
                // launch speed
                if (unit->isWeaponActive(i) && w->usesLaunchSpeed() &&
                    (unit->getWeaponAmmoAmount(i) > 0 ||
                     w->getAmmoUsage() == 0) && unit->getFireWaitDelay(i) == 0 && unit->getFireReloadDelay(i) == 0) {
                    ...
                    if ( unit->useWeaponAmmo(i) ) {
                        ...
                        if ( !unit->isFiringInProgress() || w->isSingleShot() ) {
                            firewait =
                                (w->getFireWaitTime()
                                 * ( ( 100
                                       - w->getFireWaitVary() ) + game->gameRandom->nextInt()
                                     % (w->getFireWaitVary() + 1) ) / 100);
                        ...
                        int firereload =
                            (w->getFireReloadTime()
                             * ( ( 100
                                   - w->getFireReloadVary() ) + game->gameRandom->nextInt()
                                 % (w->getFireReloadVary() + 1) ) / 100);
                        if ( w->usesCustomTimeFactor() )
                        ...
                        if ( !w->hasIndependentFireDelay() )
                            for (int attach = 0; attach < UNIT_MAX_WEAPONS; attach++) {
                                if ( unit->getWeaponType(attach) != NULL &&
                                     unit->getWeaponType(attach)->getAttachedWeaponType() != NULL
                                     && unit->getWeaponType(attach)->getAttachedWeaponType()->getPartTypeId() ==
                                     w->getPartTypeId() )
                                {
                                    attachedTo = attach;
                        ...
                        if (w->getAttachedWeaponType() != NULL)
                            if ( !w->usesFireDelayHack() && !w->getAttachedWeaponType()->hasIndependentFireDelay() )
                                for (int attach = 0; attach < UNIT_MAX_WEAPONS; attach++) {
                                    if ( unit->getWeaponType(attach) != NULL
                                         && w->getAttachedWeaponType()->getPartTypeId() ==
                                         unit->getWeaponType(attach)->getPartTypeId() )
                                        if (unit->getFireReloadDelay(attach) < firewait + firereload)
                                            unit->setFireReloadDelay(attach, firewait + firereload);
...
unit->getSelectedWeapon() )->
                                                ...
                                                if (unit->getWeaponType( unit->getSelectedWeapon() )->
                                                    getWeaponAnimationType() >= 0
                                                    && unit->getWeaponType( unit->getSelectedWeapon() )->
                                                    getWeaponAnimationType() < ANIM_TYPES_AMOUNT)
                                                {
                                                    VC3 animvel = unit->getVelocity();
                                                    if ( ( (fabs(animvel.x) < 0.01f
                                                            && fabs(animvel.z) < 0.01f)
                                                           || unit->getWalkDelay() > 0 )
                                                         && unit->getAnimationSet()->isAnimationInSet(
ANIM_SHOOT_TYPE0_STANDING
+unit->getWeaponType( unit->getSelectedWeapon() )->
getWeaponAnimationType() ) )
                                                    {
                                                        ...
                                                        int a = ANIM_SHOOT_TYPE0_STANDING + unit->getWeaponType(
                                                             unit->getSelectedWeapon() )->getWeaponAnimationType();
                                                        ...
                                                        int a = ANIM_SHOOT_TYPE0 + unit->getWeaponType(
                                                             unit->getSelectedWeapon() )->getWeaponAnimationType();
                                                        if ( !unit->getAnimationSet()->isAnimationLooping(a) )
                                                            Animator::endBlendAnimation(unit, 2, true);
                ...
                if (fabs(vel.x) > 0.001f || fabs(vel.z) > 0.001f) {
                    ...
                    if (diffLenSq < SQUASHABLE_MAX_RANGE * SQUASHABLE_MAX_RANGE) {
                        ...
                        if ( !squash->isInherited( getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Bull") ) ) ) {
                            Logger::getInstance()->warning(
                                "UnitActor::actSquashable - B_Squash part is not of bullet type.");
                            return;
                        }
...
//                    && unit->getCurrentVisibilityFadeValue() == 0.0f)
        ...
        if ( unit->doesCollisionCheck() )
            for (failcount = 0; failcount < 1000; failcount++) {
                ...
                //&& !game->gameScene->isBlockedAtScaled(sx,sy+1,0))
                    ...
                    break;
                sx = groundPos.x
                     + ( ( game->gameRandom->nextInt()
                           % ( 51 * (1 + failcount / 100) ) ) - ( 25 * (1 + failcount / 100) ) ) / 50.0f;
                sy = groundPos.z
                     + ( ( game->gameRandom->nextInt()
                           % ( 51 * (1 + failcount / 100) ) ) - ( 25 * (1 + failcount / 100) ) ) / 50.0f;
    ...
    void UnitActor::handlePointerWeapon(Unit *unit)
    {
        ...
        QUAT q;
        q.MakeFromAngles(UNIT_ANGLE_TO_RAD( unit->getLastBoneAimBetaAngle() ),
                         UNIT_ANGLE_TO_RAD( unit->getRotation().y + unit->getLastBoneAimDirection() ), 0);
        dir = VC3(0, 0, -1);
        q.RotateVector(dir);
        dir.Normalize();
        ...
        if ( ve == NULL || !ve->isMuzzleAttachment(unit, "pointer", helperName) ) {
            ...
            ve = game->gameUI->getVisualEffectManager()->createNewVisualEffect(visId,
                                                                               NULL,
                                                                               unit,
                                                                               weaponPosition,
                                                                               target,
        ...
        if ( w->getPointerHitVisualEffect() != NULL && w->getPointerHitVisualEffect()[0] != '\0' &&
             ( ve_hit == NULL || !ve_hit->isMuzzleAttachment(unit, "pointer_hit", helperName) ) ) {
            ...
            ve_hit = game->gameUI->getVisualEffectManager()->createNewVisualEffect(visId,
                                                                                   NULL,
                                                                                   unit,
                                                                                   target,
                                                                                   target,
    ...
    void UnitActor::handleTargetLocker(Unit *unit)
    {
        ...
        if (hitUnit && hitUnit->isActive() && !hitUnit->isDestroyed() && hitUnit->getOwner() == 1) {
            ...
            if ( hitUnit->getTargetLockCounter() == hitUnit->getTargetLockCounterMax() &&
                 !hitUnit->wasTargetLockSoundPlayed() ) {
                VC3 pos = unit->getPosition();
