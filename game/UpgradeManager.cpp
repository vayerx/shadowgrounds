...
namespace game
{
    ...
    bool UpgradeManager::canUpgrade(Unit *unit, int upgradeId, int *costPending)
    {
        ...
        int ret =
            game->gameScripting->runOtherScript( upgradeTypes[upgradeId].getScript(), "can_upgrade", unit, VC3(0, 0, 0) );
    ...
    bool UpgradeManager::isUpgraded(Unit *unit, int upgradeId)
    {
        ...
        int ret =
            game->gameScripting->runOtherScript( upgradeTypes[upgradeId].getScript(), "is_upgraded", unit, VC3(0, 0, 0) );
    ...
    int UpgradeManager::getUpgradedAmount(Unit *unit)
    {
        ...
        for (int i = 0; i < MAX_UPGRADE_TYPES; i++) {
            int ret =
                game->gameScripting->runOtherScript( upgradeTypes[i].getScript(), "is_upgraded", unit, VC3(0, 0, 0) );
...
#endif
        {
            ...
            while ( sp.next() ) {
                ...
                if (k != NULL) {
                    if (!insideUpgrade)
                        sp.error(
                            "UpgradeManager::loadUpgradeTypes - Parse error, key=value pair outside upgrade block.");
                    char *v = sp.getValue();
                    ...
                    if (strcmp(k, "cost") == 0) {
                        if (v != NULL) {
                            ...
                            if (str2int_errno() != 0)
                                sp.error(
                                    "UpgradeManager::loadUpgradeTypes - Cost parameter invalid, integer number expected.");
                        } else {
