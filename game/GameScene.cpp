...
namespace game
{
    ...
    void GameScene::rayTrace(const VC3         &origin,
                             const VC3         &direction,
                             float              rayLength,
                ...
                if (d2 != NULL) {
                    ...
                    if (id2 == &game::unitDataId) {
                        ...
                        Logger::getInstance()->error(
                            "GameScene::rayTrace - Raytrace hit a visual object with unknown data id.");
                        assert(0);
                    }
                } else {
...
// --- HACK, show raytrace with a line ---
        ...
        if ( SimpleOptions::getBool(DH_OPT_B_SHOW_DEBUG_RAYTRACES) ) {
            ...
            if ( ( (mask & 1) != 0 &&
                   !loscheck ) ||
                 ( (mask & 2) != 0 &&
                   loscheck ) || ( (mask & 4) != 0 && accurate ) || ( (mask & 8) != 0 && !accurate ) ) {
                             ...
                             int coverAvoidDistance, int coverBlockDistance, int depth, int lightAvoidAmount)
    {
        ...
        int maxdiff =
            (int)( maxHeightDifference
                   * ( gameMap->getScaledSizeX() / gameMap->getHeightmapSizeX() ) / gameMap->getScaleHeight() );
        float cost = climbPenalty
                     * ( gameMap->getScaledSizeX() / gameMap->getHeightmapSizeX() ) / gameMap->getScaleHeight();
    ...
    void GameScene::modifyTerrainObstaclesImpl(std::vector<TerrainObstacle> &obstacleList, bool add)
    {
        if (add)
            Logger::getInstance()->debug(
                "GameScene::modifyTerrainObstaclesImpl - Adding terrain obstacles, count follows.");
        else
            Logger::getInstance()->debug(
                "GameScene::modifyTerrainObstaclesImpl - Removing terrain obstacles, count follows.");
...
//                    Logger::getInstance()->warning("GameScene::modifyTerrainObstaclesImpl - Terrain object material name unknown or not selected in current material palette.");
                    Logger::getInstance()->debug(
                        "GameScene::modifyTerrainObstaclesImpl - Terrain object material name unknown or not selected in current material palette.");
            ...
            if (to->terrainObstacleType == TERRAIN_OBSTACLE_TYPE_NONE) {
                        ...
                        if (rx * rx + ry * ry <= radsq)
                            if (to->height > 0) {
                                ...
                                if (add) {
                                    ...
                                    gameMap->addObstacleHeight(ox + rx,
                                                               oy + ry,
                                                               hmapheight,
                                                               AREAVALUE_OBSTACLE_TERRAINOBJECT);
                                    gameMap->getAreaMap()->setAreaValue( ox + rx, oy + ry, AREAMASK_MATERIAL,
                                                                         (lastMaterialAtPalette << AREASHIFT_MATERIAL) );
                                } else {
                                    pathFinder->removeObstacle(ox + rx, oy + ry);
                                    gameMap->removeObstacleHeight(ox + rx,
                                                                  oy + ry,
                                                                  hmapheight,
                                                                  AREAVALUE_OBSTACLE_TERRAINOBJECT);
                ...
                if (newRotX != lastRotationX || newRotY != lastRotationY || newRotZ != lastRotationZ) {
                    ...
                    lastBuildingMap = new frozenbyte::BuildingMap(
                        to->modelFilename.c_str(), lastVisualObject->model, lastRotationX, lastRotationY, lastRotationZ);
                ...
                int endTargX =
                    gameMap->scaledToObstacleX( x
                                                + ( ( startMapX
                                                      + (heightMap.size() - 1) ) * lastBuildingMap->getMapResolution() ) );
                int endTargY =
                    gameMap->scaledToObstacleY( y
                                                + ( ( startMapY
                                                      + (heightMap[0].size() - 1) ) * lastBuildingMap->getMapResolution() ) );
#ifdef LEGACY_FILES
                            ...
                            if (blocked) { // this test could be moved further so we can write material without blocking
                                ...
                                if ( gameMap->isWellInScaledBoundaries(xPosition, yPosition) ) {
                                    ...
                                    if (add) {
                                        ...
                                        int heightval =
                                            (int)( (float)heightMap[i][j] * lastBuildingMap->getHeightScale()
                                                   / gameMap->getScaleHeight() );

                                        heightval += (int)( to->heightOffset / gameMap->getScaleHeight() );

                                        if ( heightval > 0 // && blocked
                                             && ( !sideways || heightval >
                                                  (int)( TERR_OBJ_OBST_HEIGHT_BLOCK_THRESHOLD / gameMap->getScaleHeight() ) ) )
                                        {
                                        ...
                                        if (lastModelIsBreakable)
                                            gameMap->getAreaMap()->setAreaValue(ox,
                                                                                oy,
                                                                                AREAMASK_BREAKABLE,
                                                                                AREAVALUE_BREAKABLE_YES);
                                        gameMap->getAreaMap()->setAreaValue( ox, oy, AREAMASK_MATERIAL,
                                                                             (lastMaterialAtPalette
                                                                              << AREASHIFT_MATERIAL) );
                                    } else { // if(blocked)
                                        AREAMAP_DATATYPE mask = AREAVALUE_OBSTACLE_TERRAINOBJECT;
                                        if (lastModelIsFirethru)
                                        ...
                                        if ( !sideways || heightval >
                                             (int)( TERR_OBJ_OBST_HEIGHT_BLOCK_THRESHOLD
                                                    / gameMap->getScaleHeight() ) ) {
                            ...
                            if (useFloormap && add) {
                                ...
                                if ( gameMap->isWellInScaledBoundaries(xPosition, yPosition) ) {
                                    ...
                                    if (floorVal != BUILDINGMAP_NO_FLOOR_BLOCK) {
                                        ...
                                        int floorValInt =
                                            (int)( ( (float)floorVal ) * lastBuildingMap->getHeightScale()
                                                   / gameMap->getScaleHeight() );
 ...
 */
    ...
    void GameScene::initTerrainMaterial(void)
    {
        ...
        if (terrain_material_id < 0) {
            Logger::getInstance()->debug(
                "GameScene::initTerrainMaterial - Terrain material name unknown or not selected in current material palette.");
                        ...
                        if (heightMap[i][j] > 0) {
                            ...
                            //if (gameMap->isWellInScaledBoundaries(xPosition, yPosition))
                            {
                                ...
                                //if (!pathFinder->isBlocked(ox, oy))
                                {
                                    ...
                                    if (floorVal == BUILDINGMAP_NO_FLOOR_BLOCK)
                                        floorVal = 0;
                                    absObstHeightmap[ox + oy
                                                     * obstsizex] =
                                        (int)( ( (float)floorVal
                                                 + (float)heightMap[i][j] ) * builder.getHeightScale()
                                               / gameMap->getScaleHeight() );
                        ...
                        if (heightMap[i][j] > 0) {
                            ...
                            if (terrainCut)
                                ...
                                gameMap->getAreaMap()->setAreaValue(ox,
                                                                    ...
                                                                    AREAVALUE_INBUILDING_YES);
                            gameMap->getAreaMap()->setAreaValue( ox, oy, AREAMASK_MATERIAL,
                                                                 (buildingMaterialInPalette << AREASHIFT_MATERIAL) );
                        }
                    }
...
//                            && floorRelHeight <= 13.0f / gameMap->getScaleHeight());
                        if ( floorRelHeight < -13.0f / gameMap->getScaleHeight()
                            ...
                            Logger::getInstance()->warning(
                                "GameScene::addBuildingObstacle - Building floormap makes height difference over limit.");
