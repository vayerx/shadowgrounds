...
namespace game
{
    ...
    void LightBlinker::run()
    {
        ...
        if (impl->blinkerTimer >=
            (impl->cycleTime + impl->pauseTime + impl->thisFrameRandomPauseTime) / GAME_TICK_MSEC) {
            ...
            impl->thisFrameRandomPauseTime =
                (int)(impl->pauseTimeVariation * (float)(impl->game->gameRandom->nextInt() % 101) / 100.0f);
        }
        ...
        if (impl->enabled) {
            ...
            if (impl->randomVariation > 0.0f) {
                ...
                if (impl->randomTimeLeft <= 0) {
                    ...
                    impl->lastRandomValue = impl->randomVariation
                                            * (float)( (impl->game->gameRandom->nextInt() % 201) - 100 ) / 100.0f;
                }
            ...
            result = COL(defaultColWeight * impl->lightColorDefault.r,
                         defaultColWeight * impl->lightColorDefault.g,
                         defaultColWeight * impl->lightColorDefault.b);

            // color1/2 balance, 0 - 1 (- x if pausetime greater than zero)
            float colorBalanceZeroOne =
                (float)( impl->blinkerTimer
                         % ( (impl->cycleTime + impl->pauseTime
                              + impl->thisFrameRandomPauseTime)
                             / GAME_TICK_MSEC ) ) / (float)(impl->cycleTime / GAME_TICK_MSEC);
            ...
            // lightmap
            {
                ...
                if (impl->outdoor) {
                    impl->game->gameUI->getTerrain()->GetTerrain()->getRenderer().setColorValue(
                        IStorm3D_TerrainRenderer::OutdoorLightmapMultiplierColor,
                        col);
