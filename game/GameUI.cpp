 ...
 */
    ...
    void GameUI::setCursorControllers(bool allowJoystick)
    {
        ...
        for (c = 0; c < MAX_PLAYERS_PER_CLIENT; c++) {
            if ( game::SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + c) ) {
                ...
                //
                if (controller_scheme >= GameController::CONTROLLER_TYPE_KEYBOARD_AND_MOUSE1 && controller_scheme <=
                    GameController::CONTROLLER_TYPE_KEYBOARD_AND_MOUSE4) {
                    // give cursor
                ...
                //
                    ...
                    if (joy == 0)
                        ...
                        ogui->SetCursorControllerKeyboard( c,
                                                           gameController[c]->getBoundKey(DH_CTRL_CAMERA_MOVE_LEFT, 0),
                                                           gameController[c]->getBoundKey(DH_CTRL_CAMERA_MOVE_RIGHT, 0),
                                                           gameController[c]->getBoundKey(DH_CTRL_CAMERA_MOVE_FORWARD,
                                                                                          0),
                                                           gameController[c]->getBoundKey(DH_CTRL_CAMERA_MOVE_BACKWARD,
                                                                                          0),
    ...
    void GameUI::createCameras()
    {
        ...
        if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_TOPDOWN_SHOOTER) ) {
            ...
            cameras[GAMEUI_CAMERA_NORMAL]->setZoom( SimpleOptions::getFloat(
                                                        DH_OPT_F_CAMERA_DEFAULT_ZOOM) + (float)players
                                                    * SimpleOptions::getFloat(DH_OPT_F_CAMERA_DEFAULT_ZOOM_PLAYER_INC) );
        } else {
    ...
    void GameUI::closeCommandWindow(int player)
    {
        if (commandWindows[player] == NULL) {
            ...
            if (renderTerrain)
                this->renderTerrain->GetTerrain()->getRenderer().enableFeature(IStorm3D_TerrainRenderer::RenderTargets,
                                                                               true);
        ...
        if (upgradeWindow == NULL) {
            ...
            if ( upgradeWindow->isVisible() ) {
                ...
                if (this->renderTerrain != NULL)
                    this->renderTerrain->GetTerrain()->getRenderer().enableFeature(
                        IStorm3D_TerrainRenderer::RenderTargets,
                        true);
            }
...
//        updateIngameTabs();
    ...
    void GameUI::openArmorIncompleteConfirm(int  player,
                                            bool notAnyArmor,
                                            bool incompleteArmor,
                                            bool noArmor,
                                            bool notPaid)
    {
        if (armorIncompleteConfirmWindows[player] != NULL) {
            ...
            if (notAnyArmor) {
                ...
                const char *imgs0[3] =
                { "Data/GUI/Buttons/incomplete_back.tga", "Data/GUI/Buttons/incomplete_back_down.tga",
                  "Data/GUI/Buttons/incomplete_back_highlight.tga" };
                const char **images[1];
                images[0] = imgs0;
                armorIncompleteConfirmWindows[player] = new MessageBoxWindow(
                    ogui,
                    ...
                    150,
                    50,
                    "Data/GUI/Windows/incomplete_confirm.dds",
                    MSGBOX_NOARMORS,
                    this);
                msgBoxIsOpen = true;
                ogui->SetCursorImageState(0, DH_CURSOR_ARROW);
                ...
                const char *imgs0[3] =
                { "Data/Buttons/incomplete_back.tga", "Data/Buttons/incomplete_back_down.tga",
                  "Data/Buttons/incomplete_back_highlight.tga" };
                const char *imgs1[3] =
                { "Data/Buttons/incomplete_continue.tga", "Data/Buttons/incomplete_continue_down.tga",
                  "Data/Buttons/incomplete_continue_highlight.tga" };
                const char **images[2];
                images[0] = imgs0;
                images[1] = imgs1;
                const char *text = 0;
                if (incompleteArmor) {
                    ...
                    if (noArmor)
                        ...
                        text =
                            "Some of the armors have parts that have not been paid for. Unpaid parts will be removed. Any armors that will become incomplete will not take part in combat.";
                }
    ...
    void GameUI::setRenderMap(GameMap *gameMap, char *configFile)
    {
        ...
        if (gameMap != NULL) {
            ...
            assert(lightManager == NULL);
            lightManager = new ui::LightManager(*storm3d, *scene,
                                                *renderTerrain->GetTerrain(), gameMap->lightMap, renderTerrain);
        ...
        if (gameMap != NULL)
            this->gridOcclusionCuller = new util::GridOcclusionCuller(
                 gameMap->getScaledSizeX(), gameMap->getScaledSizeY(),
                (gameMap->getObstacleSizeX() >> GRIDOCCLUSIONCULLER_OBSTACLE_DIV_SHIFT),
                (gameMap->getObstacleSizeY() >> GRIDOCCLUSIONCULLER_OBSTACLE_DIV_SHIFT) );
    }
                ...
                if ( gridOcclusionCuller->isWellInScaledBoundaries(camPos.x, camPos.z) ) {
                    ...
                    if ( (game->gameTimer & 63) == 0 )
                        Logger::getInstance()->debug(
                            "GameUI::runUI - Camera outside occlusion culling area (using default area).");
                }
                // HACK: static
...
#  ifdef PROJECT_SURVIVOR
                            // in survival mode
                            int value = 0;
                            if (util::Script::getGlobalIntVariableValue("survival_mode_enabled",
                                                                        &value) && value == 1) {
...
#  endif
...
#ifdef PROJECT_SURVIVOR
                    if (renderTerrain && firstPerson[c] && firstPerson[c]->getMoveState() !=
                        Unit::UNIT_MOVE_STATE_UNCONSCIOUS)
                        if (game->gameTimer > last_container_check) {
                            ...
                            if (uh != UNIFIED_HANDLE_NONE) {
                                ...
                                if (Timer::getTime() - last_container_hint > 5500) {
                                    combatWindows[game->singlePlayerNumber]->showHintMessage( getLocaleGuiString(
                                                                                                  "gui_container_hint") );
        ...
        // Quit pressed, at cinematic (but not while still in load screen)
        {
            ...
            /*
               for (int c = 0; c < MAX_PLAYERS_PER_CLIENT; c++)
               {*/
            if ( ( wasKeyClicked(DH_CTRL_QUIT) || Keyb3_IsKeyPressed(KEYCODE_MOUSE_BUTTON1) ||
                   Keyb3_IsKeyPressed(KEYCODE_MOUSE_BUTTON2) ||
                   Keyb3_IsKeyPressed(KEYCODE_MOUSE_BUTTON3) ) && cinematicScreen != NULL ) {
            ...
            if ( game->isCooperative() && (
                     ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED) &&
                       game->getGameUI()->getController(0)->wasKeyClicked(DH_CTRL_OPEN_MENU) ) ||
                     ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + 1) &&
                       game->getGameUI()->getController(1)->wasKeyClicked(DH_CTRL_OPEN_MENU) ) ||
                     ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + 2) &&
                       game->getGameUI()->getController(2)->wasKeyClicked(DH_CTRL_OPEN_MENU) ) ||
                     ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + 3) &&
                       game->getGameUI()->getController(3)->wasKeyClicked(DH_CTRL_OPEN_MENU) )
                     ) )
                        ...
                        && commandWindows[game->singlePlayerNumber]->wasQuitPressed() ) ) )
            {
                ...
                if (!msgBoxIsOpen) {
                    ...
                    if ( ( armorConstructWindows[game->singlePlayerNumber] != NULL &&
                           armorConstructWindows[game->singlePlayerNumber]->isVisible() )
                         || ( storageWindows[game->singlePlayerNumber] != NULL &&
                              storageWindows[game->singlePlayerNumber]->isVisible() ) )
                    {
                        if ( armorConstructWindows[game->singlePlayerNumber] != NULL &&
                             armorConstructWindows[game->singlePlayerNumber]->isVisible() ) {
                            ...
                            if ( storageWindows[game->singlePlayerNumber] != NULL &&
                                 storageWindows[game->singlePlayerNumber]->isVisible() ) {
                    ...
                    for (int c = 0; c < MAX_PLAYERS_PER_CLIENT; c++) {
                        if ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + c) ) {
                            if ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_HAS_CURSOR + c) ) {
                                ...
                                if (joystickAimer[c] == NULL)
                                    joystickAimer[c] = new JoystickAimer(firstPerson[c],
                                                                         ogui,
                                                                         game,
                                                                         gameController[c],
                                                                         c);
                ...
                if (vo != NULL) {
                    ...
                    if ( unit->hasActed() ) {
                        ...
                        if (shadowStrength > 0.05f && shadowSize.x > 0.1f && shadowSize.y > 0.1f) {
                            ...
                            if (delta < maxHeightDifference) {
                                ...
                                QUAT rotation;
                                VC3 terrainNormal;
                                DecalPositionCalculator::calculateDecalRotation(game->gameScene,
                                                                                shadowPosition,
                                                                                rotation,
                                                                                yAngle,
                                                                                terrainNormal);
                                ...
                                if (terrain) {
                                    ...
                                    if (alpha > 0.005f)
                                        terrain->getDecalSystem().setShadowDecal(shadowPosition,
                                                                                 rotation,
                                                                                 shadowSize,
                                                                                 alpha);
                                }
                    ...
                    if ( Unit::getVisualizationOffsetInUse() ) {
                        ...
                        if ( game->gameMap->isWellInScaledBoundaries(position.x, position.z) ) {
                            ...
                            const float v1 =
                                (float)( (getMaterialByPalette(game->gameMap->getAreaMap()->getAreaValue(x - 1, y - 1,
                                                                                                         AREAMASK_MATERIAL)
                                                               >> AREASHIFT_MATERIAL) == MATERIAL_SAND) ? 1 : 0 );
                            const float v2 =
                                (float)( (getMaterialByPalette(game->gameMap->getAreaMap()->getAreaValue(x + 1, y - 1,
                                                                                                         AREAMASK_MATERIAL)
                                                               >> AREASHIFT_MATERIAL) == MATERIAL_SAND) ? 1 : 0 );
                            ...
                            if (game->gameMap->colorMap) {
                                ...
                                if ( game->gameMap->isWellInScaledBoundaries(position.x, position.z) ) {
                                    if ( vo->isSkyModel() ) {
                                        ...
                                        if ( game->gameMap->getAreaMap()->isAreaAnyValue(ox, oy, AREAMASK_INBUILDING) )
                                            lightManager->getLighting( unit->getPosition(), lights,
                                                                       getRadius(
                                                                            vo->getStormModel() ), true, true,
                                                                       vo->getStormModel() );
                                        else
                                            lightManager->getLighting( unit->getPosition(), lights,
                                                                       getRadius(
                                                                            vo->getStormModel() ), true, false,
                                                                       vo->getStormModel() );
                                    }
                                }
...
//vo->setCAMotionBlur(true, 1.0f);
                    ...
                    if (fl != NULL) {
                        ...
                        float angle = UNIT_ANGLE_TO_RAD(
                             rot.y + unit->getFlashlightDirection() + unit->getUnitType()->getAimBoneDirection() );
                                         ...
                                         && SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_HAS_CURSOR + c) )
                                    {
                                        ...
                                        betaAngle = PI * 0.15f
                                                    - sqrtf( (curx
                                                             ...
                                                             * (curx
                                                                - u_scr_posx)
                                                             + (cury - u_scr_posy) * (cury - u_scr_posy) ) * 0.002f;
                        ...
                        {
                            float jumpNormTime =
                                (float)( unit->getJumpTotalTime()
                                         - unit->getJumpCounter() ) / (float)unit->getJumpTotalTime();
                            ...
                            if (left) {
                                if (backward)
                                    fl->setBetaRotation( betaAngle - 0.3f
                                                         * (float)sinf(jumpNormTime * 3.1415927f * 2.0f) );
                                else
                                    fl->setBetaRotation( betaAngle + 0.3f
                                                         * (float)sinf(jumpNormTime * 3.1415927f * 2.0f) );
                            ...
                            if (left || right)
                                fl->setTemporaryBrightnessFactor( 1.0f
                                                                  - (float)sqrtf( (float)sinf(jumpNormTime * 3.1415927f) ) );
                            else
                        ...
                        if (unit->getUnitType()->getHaloType() == NULL) {
                                 ...
                                 && fl->isFlashlightOn() )
                            {
                                if ( unit->isMuzzleflashVisible() )
                                    ...
                                    halo->setFakelightParams( 14,
                                                              COL(haloColormapMultiplier * 0.20f,
                                                                  haloColormapMultiplier * 0.20f,
                                                                  haloColormapMultiplier * 0.15f) );
                                else
                                    ...
                                    halo->setFakelightParams( 8.0f + 6.0f * fl->getFlashlightIlluminationFactor(),
                                                              COL(haloColormapMultiplier * 0.15f,
                                                                  haloColormapMultiplier * 0.15f,
                                                                  haloColormapMultiplier * 0.15f) );
                            } else {
                                if ( unit->isMuzzleflashVisible() )
                 ...
                 && gameCamera->isFirstPersonMode() )
            {
                ...
                if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_TOPDOWN_SHOOTER) ) {
                    if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_AIM_UPWARD) ) {
                        ...
                        if (thirdPersonView) {
                            combatWindows[game->singlePlayerNumber]->setCrosshairProperties(
                                ogui->getCursorScreenX(0), ogui->getCursorScreenY(
                                    0), firstPerson[0]->getFiringSpreadFactor() );
                ...
                if ( gameCamera->isFirstPersonMode() ) {
                    if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_TOPDOWN_SHOOTER) ) {
                        ...
                        for (int c = 0; c < MAX_PLAYERS_PER_CLIENT; c++) {
                            if (firstPerson[c] != NULL
                                ...
                                if (thirdPersonView) {
                                    ...
                                    //unitpos += aimvec / 3;
                                    aimvec = aimvec * (float)SimpleOptions::getInt(DH_OPT_I_CAMERA_SCROLLY_AMOUNT)
                                             / 100.0f;
                                    ...
                                    float speedfact = (float)SimpleOptions::getInt(DH_OPT_I_CAMERA_SCROLLY_SPEED)
                                                      / 100.0f;
                                    if (scrolly.x < aimvec.x - 0.5f) scrolly.x +=
                                            (0.02f + (float)fabs(scrolly.x - aimvec.x) / 20.0f) * speedfact;
                                    if (scrolly.x > aimvec.x + 0.5f) scrolly.x -=
                                            (0.02f + (float)fabs(scrolly.x - aimvec.x) / 20.0f) * speedfact;
                                    if (scrolly.z < aimvec.z - 0.5f) scrolly.z +=
                                            (0.02f + (float)fabs(scrolly.z - aimvec.z) / 20.0f) * speedfact;
                                    if (scrolly.z > aimvec.z + 0.5f) scrolly.z -=
                                            (0.02f + (float)fabs(scrolly.z - aimvec.z) / 20.0f) * speedfact;
                                    ...
                                    gameCamera->setFirstPersonPosition(
                                        interppos.x,
                                        interppos.z,
                                        scrolledPos.x + positionOffset.x
                                        * POSITION_OFFSET_AMOUNT,
                                        scrolledPos.z + positionOffset.z
                                        * POSITION_OFFSET_AMOUNT,
                                        scrolledPos.y + heightOffset,
                                        alpha,
                                        lookrotx + (aimOffset - oldAimOffset) * 0.0f);
                        ...
                        if (camplayers == 0) {
                            ...
                            lookrotx = 0;
                            alpha = 360 - gameCamera->getAngleY();
                            gameCamera->setFirstPersonPosition(pos.x,
                                                               pos.z,
                                                               pos.x,
                                                               pos.z,
                                                               pos.y + heightOffset,
                    ...
                    if (firstPerson[c] != NULL)
                        if (sceneSelection[c]->hit) {
                            if (sceneSelection[c]->unit != NULL) {
                                ...
                                float aimHeight = game->gameMap->getScaledHeightAt(sceneSelection[c]->scaledMapX,
                                                                                   sceneSelection[c]->scaledMapY);
                                VC3 aimPos = VC3(sceneSelection[c]->scaledMapX,
                                                 aimHeight,
                                                 sceneSelection[c]->scaledMapY);
...
/*                // CAMERA SYSTEM STUFF HERE!
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_MAGIC_SCREENSHOT) ) {
            ...
            if ( !magic_screenshot_queued && wasKeyClicked(DH_CTRL_SCREENSHOT) ) {
                ...
                magic_screenshot_gui_was_visible = combatWindows[game->singlePlayerNumber] &&
                                                   combatWindows[game->singlePlayerNumber]->isGUIVisible();
                ...
                if (magic_screenshot_gui_was_visible) {
                    ...
                    if (combatWindows[game->singlePlayerNumber]) combatWindows[game->singlePlayerNumber]->
                        updateCursorImage();
                }
        ...
        if (game->inCombat) {
            if (game->getClawController() && firstPerson[0])
                game->getClawController()->setIgnoreActor( game->getGamePhysics()->getImplementingObject( firstPerson[0
                                                                                                          ]->
                                                                                                          getGamePhysicsObject() ) );
            ...
            if ( game->getClawController() && firstPerson[0] && firstPerson[0]->getVisualObject() ) {
                 ...
                 */
                {
                    if ( game->getGameScene() && game->getGameScene()->getGameMap() )
                        aimPosition.y = game->getGameScene()->getGameMap()->getScaledHeightAt(aimPosition.x,
                                                                                              aimPosition.z);
...
//                        game->getClawController()->moveToRestPosition();
                    ...
                    //if(!cursorMode)
                    {
                        ...
                        if ( throttle && (fabsf(delta.x) > 0.1f || fabsf(delta.y) > 0.1f) ) {
                            float factor = 0.05f;
                            throttleVector.x += delta.x * factor;
                            throttleVector.z += delta.y * factor;
                            //throttleVector.x = delta.x;
                            //throttleVector.z = delta.y;
                            if (throttleVector.GetSquareLength() > 0.001f)
                                throttleVector.Normalize();
                        }
...
//                    game->getClawController()->setAction(ClawController::TurboHit);
                ...
                if ( game->inCombat && game->getClawController() && firstPerson[0] &&
                     firstPerson[0]->getVisualObject() ) {
        ...
        if ( SimpleOptions::getBool(DH_OPT_B_GAME_MODE_AIM_UPWARD) ) {
            ...
            if ( ctrl->controllerTypeHasJoystick() ) {
                ...
                if (x != 0 || x != 0) {
                    ...
                    // Is this the right way? Seems to work OK
                ...
                *looped = true;
                ...
                if (splitpos[0] > 0 && !invalidStr) {
                    ...
                    Logger::getInstance()->warning(
                        "GameUI::parseSoundFromDefinitionString - Invalid looping sound definition.");
                }
                return -1;
            } else {
                *looped = false;
    ...
    int GameUI::playSoundEffect(const char *filename,
                                float       x,
                                float       y,
                                float       z,
                                bool        loop,
    ...
    int GameUI::playSpeech(const char *filename, float x, float y, float z, bool loop, int volume, bool volume_adjust)
    {
        ...
        ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                          radiospeechlocale.c_str(), &radio_id);
            ...
            if (s == NULL) {
                ...
                if (handle != -1) {
                    ...
                    if (volume_adjust && s->getLength() > 0) {
                        ...
                        SoundMixer::SoundEvent *se_fadein = soundMixer->getLastSoundEventOfType(
                            SoundMixer::Event_SpeechStop);
                    ...
                    if (radio_id) {
                        const char *click1 =
                            getLocaleGuiString( ("radiospeecheffect_" + std::string(
                                                     radio_id) + "_startclick_sound").c_str() );
                        const char *click2 =
                            getLocaleGuiString( ("radiospeecheffect_" + std::string(
                                                     radio_id) + "_endclick_sound").c_str() );
                        const char *noise =
                            getLocaleGuiString( ("radiospeecheffect_" + std::string(radio_id) + "_noise_sound").c_str() );
                        int click1_time =
                            getLocaleGuiInt( ("radiospeecheffect_" + std::string(
                                                  radio_id) + "_startclick_starttime").c_str(), 0 );
                        int click2_time =
                            getLocaleGuiInt( ("radiospeecheffect_" + std::string(
                                                  radio_id) + "_endclick_starttime").c_str(), 0 );
                        int noise_starttime =
                            getLocaleGuiInt( ("radiospeecheffect_" + std::string(
                                                  radio_id) + "_noise_starttime").c_str(), 0 );
                        int noise_endtime =
                            getLocaleGuiInt( ("radiospeecheffect_" + std::string(
                                                  radio_id) + "_noise_endtime").c_str(), 0 );
                        int speech_delay =
                            getLocaleGuiInt( ("radiospeecheffect_" + std::string(radio_id) + "_speech_starttime").c_str(
                                                 ), 0 );
    ...
    bool GameUI::pushUIState()
    {
        ...
        if ( !uiStateStack->isEmpty() ) {
            ...
            char str[1024];
            sprintf(str,
                    "GameUI::pushUIState - State stack is not empty (multiple states stored), storing as ID %i",
                    id);
            Logger::getInstance()->error(str);
            ret = false;
        }
    ...
    void GameUI::nextInterfaceGeneration()
    {
        ...
        Logger::getInstance()->debug(
            "GameUI::nextInterfaceGeneration - Interface recreate done, time used (msec) follows:");
...
//                    && posDiffLenSq >= 24 * 24 && posDiffLenSq <= 30 * 30)
                    ...
                    if (lightningVisualEffect != NULL) {
                        ...
                        Logger::getInstance()->error(
                            "GameUI::setEnvironmentLightning - No envlightning visual effect found.");
                    }
                }
    ...
    int GameUI::playGUISound(const char *filename, int relativeVolume)
    {
        ...
        if (relativeVolume < 0)
            relativeVolume = 0;
        return this->playSoundEffect(filename,
                                     campos.x,
                                     campos.y,
                                     campos.z,
                                     false,
                                     DEFAULT_SOUND_EFFECT_VOLUME * relativeVolume / 100,
                                     999,
                                     DEFAULT_SOUND_PRIORITY_HIGH);
    }
    ...
    int GameUI::playGUISpeech(const char *filename, int relativeVolume)
    {
        ...
        return this->playSpeech(filename,
                                campos.x,
                                campos.y,
                                campos.z,
                                false,
    ...
    void GameUI::enableAlphaTestPass(bool enabled)
    {
        if (this->renderTerrain)
            this->renderTerrain->GetTerrain()->getRenderer().enableFeature(
                IStorm3D_TerrainRenderer::AdditionalAlphaTestPass,
                enabled);
    }
