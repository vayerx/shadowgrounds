...
// (5 chars + 1 for null + 2 for alignment)
    ...
    void solveSlopeType(GameMap    *gameMap,
                        GameScene  *gameScene,
                        const VC3  &pos,
                        bool       &ongroundOut,
                        bool       &onslopeOut,
                        SLOPE_TYPE &slopeTypeOut,
                        VC3        &surfacePositionOut)
    {
                ...
                if (!mismatch) {
                    ...
                    case SLOPE_TYPE_LEFT_45_TEMP: {
                        ...
                        if ( offY <= (scaleX - offX) ) {
                            ongroundOut = true;
                            onslopeOut = true;
                            surfacePositionOut.z =
                                (scaleX - offX) + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f + extraLift;
                        } else {
                    ...
                    case SLOPE_TYPE_RIGHT_45_TEMP: {
                        ...
                        if (offY <= offX) {
                            ...
                            if (offY <= offX + mysteryThreshold) {
                                ongroundOut = true;
                                onslopeOut = true;
                                surfacePositionOut.z = offX + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f
                                                       + extraLift;
                            }
                        }
                    }
                    ...
                    case SLOPE_TYPE_INNER: {
                        bool bu2 =
                            gameScene->isBlockedAtScaled( pos.x, pos.z + scaleY * 2,
                                                          gameMap->getScaledHeightAt(pos.x, pos.z + scaleY * 2) );
                        if (!bu2) {
                            ongroundOut = true;
                            onslopeOut = true;
                            int obstY = gameMap->scaledToObstacleY(pos.z);
                    ...
                    case SLOPE_TYPE_LEFT_22_5_LOWER: {
                        ...
                        if (offY <= (scaleX - offX) * 0.5f) {
                            ...
                            onslopeOut = true;
                            surfacePositionOut.z =
                                (scaleX - offX) * 0.5f + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f + extraLift;
                        } else {
                            if (offY <= (scaleX - offX) * 0.5f + mysteryThreshold) {
                                ...
                                surfacePositionOut.z =
                                    (scaleX
                                     - offX) * 0.5f + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f + extraLift;
                            }
                        }
                    }
                    break;
                    case SLOPE_TYPE_LEFT_22_5_UPPER: {
                        ...
                        if (offY <= scaleY * 0.5f + (scaleX - offX) * 0.5f) {
                            ongroundOut = true;
                            onslopeOut = true;
                            surfacePositionOut.z = scaleY * 0.5f + (scaleX - offX) * 0.5f + gameMap->obstacleToScaledY(
                                obstY) - scaleY * 0.5f + extraLift;
                        } else {
                            if (offY <= scaleY * 0.5f + (scaleX - offX) * 0.5f + mysteryThreshold) {
                                ongroundOut = true;
                                onslopeOut = true;
                                surfacePositionOut.z = scaleY * 0.5f
                                                       + (scaleX
                                                          - offX) * 0.5f + gameMap->obstacleToScaledY(obstY) - scaleY
                                                       * 0.5f + extraLift;
                            }
                        }
                    }
                    ...
                    case SLOPE_TYPE_RIGHT_22_5_LOWER: {
                        ...
                        if (offY <= offX * 0.5f) {
                            ongroundOut = true;
                            onslopeOut = true;
                            surfacePositionOut.z = offX * 0.5f + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f
                                                   + extraLift;
                        } else {
                            if (offY <= offX * 0.5f + mysteryThreshold) {
                                ongroundOut = true;
                                onslopeOut = true;
                                surfacePositionOut.z = offX * 0.5f + gameMap->obstacleToScaledY(obstY) - scaleY
                                                       * 0.5f + extraLift;
                            }
                        }
                    }
                    ...
                    case SLOPE_TYPE_RIGHT_22_5_UPPER: {
                        ...
                        if (offY <= scaleY * 0.5f + offX * 0.5f) {
                            ongroundOut = true;
                            onslopeOut = true;
                            surfacePositionOut.z = scaleY * 0.5f + offX * 0.5f + gameMap->obstacleToScaledY(obstY)
                                                   - scaleY * 0.5f + extraLift;
                        } else {
                            if (offY <= scaleY * 0.5f + offX * 0.5f + mysteryThreshold) {
                                ongroundOut = true;
                                onslopeOut = true;
                                surfacePositionOut.z = scaleY * 0.5f + offX * 0.5f
                                                       + gameMap->obstacleToScaledY(obstY) - scaleY * 0.5f + extraLift;
                            }
                        }
                    }
                    break;
