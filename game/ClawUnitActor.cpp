...
namespace game
{
    ...
    void ClawUnitActor::actDirectClawControls(Unit *unit)
    {
        ...
        if ( game->getClawController()->hasActor() ) {
            if (game->getClawController()->getTargetPosition(TARGET_ADVANCE_TICKS).GetRangeTo( unit->getPosition() ) >
                controlSettings.animationSwitchFarPoseRange)
                claw_near = false;
            if (game->getClawController()->getTargetPosition(TARGET_ADVANCE_TICKS).GetRangeTo( unit->getPosition() ) <
                controlSettings.animationSwitchNearPoseRange && !rotating)
                claw_near = true;
        } else {
            if (game->getClawController()->getTargetPosition(TARGET_ADVANCE_TICKS).GetRangeTo( unit->getPosition() ) >
                controlSettings.animationSwitchFarPoseRange)
...
// SPRINT DISABLED FOR NOW!
        ...
        VC3 cameraFrontVec = game->gameUI->getGameCamera()->getTargetPosition()
                             - game->gameUI->getGameCamera()->getPosition();
        ...
        VC3 globalDirVec =
            ( cameraFrontVec
              * controlDirVec.GetDotWith(controlFrontVec) ) + ( cameraSideVec * controlDirVec.GetDotWith(controlSideVec) );
...
//        VC3 aimPos = game->getClawController()->getClawPosition();
            ...
            if (doMovement) {
                ...
                if (insideFwd == 0) {
                    ...
                    float insideFwdDiag = util::AngleRotationCalculator::getRotationForAngles(moveAngle,
                                                                                              aimDestAngle,
                                                                                              90.0f + 45.0f);
                    if (insideFwdDiag == 0) {
                        if (insideFwd < 0) {
                ...
                while (true) {
                    float tooFarTwist = util::AngleRotationCalculator::getRotationForAngles(rotateToAngle,
                                                                                            aimDestAngle,
                                                                                            maxTwist + 10);
                    if (tooFarTwist > 0) {
        ...
        if (unit->getAnimationSet() != NULL) {
            if ( unit->getAnimationSet()->isAnimationInSet(anim) ) {
                ...
                Logger::getInstance()->warning(
                    "ClawUnitActor::actDirectClawControls - Requested anim not found in animation set.");
...
// HACK: ..
        ...
        if (!unit->isDestroyed()
            || (unit->getDisappearCounter() <
                (SimpleOptions::getInt(DH_OPT_I_CORPSE_DISAPPEAR_TIME)
                 * 1000) / GAME_TICK_MSEC - UNIT_DISAPPEAR_FADE_TICKS)
