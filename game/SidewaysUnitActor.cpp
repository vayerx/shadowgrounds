        ...
        if (unit->getAnimationSet() != NULL) {
            if ( unit->getAnimationSet()->isAnimationInSet(anim) ) {
                ...
                Logger::getInstance()->warning(
                    "SidewaysUnitActor::actDirectSidewaysControls - Requested anim not found in animation set.");
                Logger::getInstance()->debug("anim id number follows:");
                Logger::getInstance()->debug( int2str(anim) );
            }
        } else {
            Logger::getInstance()->warning(
                "SidewaysUnitActor::actDirectSidewaysControls - Unit has null animation set.");
        }
    }
