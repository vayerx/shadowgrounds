    ...
    bool UnitType::setRootData(const char *key, char *value)
    {
        if (strcmp(key, "unitid") == 0) {
            ...
            Logger::getInstance()->warning(
                "UnitType::setRootData - Unit has a manually assigned id (deprecated feature).");
#endif
        ...
        if (strcmp(key, "defaultspeed") == 0) {
            if (strcmp(value, "crawl") == 0) {
                ...
                Logger::getInstance()->error(
                    "UnitType::setRootData - default speed type name unknown (expected \"crawl\", \"slow\", \"fast\" or \"sprint\").");
                return false;
            }
            return true;
        }
