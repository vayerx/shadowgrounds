...
namespace game
{
    ...
    void ObstacleMapUnitObstacle::addObstacle(Game *game,
                                              Unit *unit,
                                              int   radius,
                                              int   height,
                                              bool  lineblock,
            ...
            for (int i = minY; i <= maxY; i++) {
                for (int j = minX; j <= maxX; j++) {
                    ...
                    if (diffX * diffX + diffY * diffY <= radiusSq) {
                        if (lineblock) {
                            if ( obstmap_is_at_line(diffX, diffY, UNIT_ANGLE_TO_RAD(unit->yAngle), leftside, rightside,
                                                    lineWidth) )
// (this hack removed)
...
//                                || obstmap_is_at_line(diffX, diffY + 1, UNIT_ANGLE_TO_RAD(unit->yAngle), leftside, rightside, lineWidth))
    ...
    void ObstacleMapUnitObstacle::removeObstacle(Game *game,
                                                 Unit *unit,
                                                 int   radius,
                                                 int   height,
                                                 bool  lineblock,
        ...
        if (radius == 1) {
            ...
            for (int i = minY; i <= maxY; i++) {
                for (int j = minX; j <= maxX; j++) {
                    ...
                    if (diffX * diffX + diffY * diffY <= radiusSq) {
                        if (lineblock) {
                            if ( obstmap_is_at_line(diffX, diffY, UNIT_ANGLE_TO_RAD(unit->obstacleAngle), leftside,
                                                    rightside, lineWidth) )
// (this hack removed)
...
//                                || obstmap_is_at_line(diffX, diffY + 1, UNIT_ANGLE_TO_RAD(unit->obstacleAngle), leftside, rightside, lineWidth))
    ...
    void ObstacleMapUnitObstacle::moveObstacle(Game *game,
                                               Unit *unit,
                                               int   x,
                                               int   y,
                                               int   radius,
            ...
            if (radius == 1) {
                ...
                for (int i = minY; i <= maxY; i++) {
                    for (int j = minX; j <= maxX; j++) {
                        ...
                        if (diffX * diffX + diffY * diffY <= radiusSq) {
                            if (lineblock) {
                                if ( obstmap_is_at_line(diffX, diffY, UNIT_ANGLE_TO_RAD(unit->yAngle), leftside,
                                                        rightside, lineWidth) ) {
//                                    || obstmap_is_at_line(diffX + 1, diffY, UNIT_ANGLE_TO_RAD(unit->yAngle), leftside, rightside, lineWidth)
//                                    || obstmap_is_at_line(diffX, diffY + 1, UNIT_ANGLE_TO_RAD(unit->yAngle), leftside, rightside, lineWidth))
