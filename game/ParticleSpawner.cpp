...
namespace game
{
    ...
    void ParticleSpawner::setSpawnerWeapon(const Weapon *weapon)
    {
        ...
        if (weapon->getFireWaitTime() > 0) {
            int firewait =
                (weapon->getFireWaitTime()
                 * ( ( 100
                       - weapon->getFireWaitVary() ) + game->gameRandom->nextInt()
                     % (weapon->getFireWaitVary() + 1) ) / 100);
    ...
    Projectile *ParticleSpawner::spawnProjectileWithWeapon(Game         *game,
                                                           const Weapon *weapon,
                                                           const VC3    &position,
                                                           const VC3    &direction,
            ...
            if (fireSound != NULL) {
                    ...
                    || distanceToListener < soundrange + PARTICLESPAWNER_EXTRA_SOUND_RANGE)
                {
                    ...
                    if (distanceToListener < soundrange) {
                        if (soundrange > 0)
                            priority -=
                                (int)( PARTICLESPAWNER_SOUND_DISTANCE_PRIORITY_ADJUST
                                       * (distanceToListener / soundrange) );
                    } else {
                    ...
                    int handle = game->gameUI->parseSoundFromDefinitionString(fireSound,
                                                                              projpos.x,
                                                                              projpos.y,
                                                                              projpos.z,
                                                                              &looped,
                                ...
                                || gameui_sound_effect_errorcode == GAMEUI_SOUND_EFFECT_ERRORCODE_TOO_FAR)
                            {
                                ...
                                if (soundmixer_play_errorcode == SOUNDMIXER_PLAY_ERRORCODE_COUNT_EXCEEDED) {
                                    Logger::getInstance()->debug(
                                        "ParticleSpawner::run - Sound sample simultaneous playing count exceeded.");
                    ...
                    if (looped) {
                        Logger::getInstance()->warning(
                            "ParticleSpawner::run - Firesound definition was a looping sound, not properly supported.");
    ...
    void ParticleSpawner::run()
    {
        ...
        if (waitTime > 0) {
            ...
            int firereload =
                (weapon->getFireReloadTime()
                 * ( ( 100
                       - weapon->getFireReloadVary() ) + game->gameRandom->nextInt()
                     % (weapon->getFireReloadVary() + 1) ) / 100);
            waitTime = firereload;

            Projectile *proj =
                spawnProjectileWithWeapon( game, weapon, this->position, this->direction,
                                           sqrtf(this->distanceToListenerSq) );
