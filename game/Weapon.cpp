 ...
 */
    ...
    Weapon::~Weapon()
    {
        ...
        if (fireSound != NULL) {
            delete[] fireSound; fireSound = NULL;
        }
        if (prepareFireSound != NULL) {
            delete[] prepareFireSound; prepareFireSound = NULL;
        }
        if (repeatSound != NULL) {
            delete[] repeatSound; repeatSound = NULL;
        }
        if (clipReloadSound != NULL) {
            delete[] clipReloadSound; clipReloadSound = NULL;
        }
        if (clipFirstReloadSound != NULL) {
            delete[] clipFirstReloadSound; clipFirstReloadSound = NULL;
        }
        if (clipEmptySound != NULL) {
            delete[] clipEmptySound; clipEmptySound = NULL;
        }
        if (muzzleflashEffect != NULL) {
            delete[] muzzleflashEffect; muzzleflashEffect = NULL;
        }
        if (muzzleflashEffect2 != NULL) {
            delete[] muzzleflashEffect2; muzzleflashEffect2 = NULL;
        }
        if (muzzleflashEffect3 != NULL) {
            delete[] muzzleflashEffect3; muzzleflashEffect3 = NULL;
        }
        if (rechargeSound != NULL) {
            delete[] rechargeSound; rechargeSound = NULL;
        }
        if (remoteTriggerSound != NULL) {
            delete[] remoteTriggerSound; remoteTriggerSound = NULL;
        }
        if (pointerHelper != NULL) {
            delete[] pointerHelper; pointerHelper = NULL;
        }
        ;
        if (pointerVisualEffect != NULL) {
            delete[] pointerVisualEffect; pointerVisualEffect = NULL;
        }
        if (pointerHitVisualEffect != NULL) {
            delete[] pointerHitVisualEffect; pointerHitVisualEffect = NULL;
        }
        if (targetLockSound != NULL) {
            delete[] targetLockSound; targetLockSound = NULL;
        }
        if (reloadFinishedSound != NULL) {
            delete[] reloadFinishedSound; reloadFinishedSound = NULL;
        }
        if (customWeaponBarrelHelper != NULL) {
            delete[] customWeaponBarrelHelper; customWeaponBarrelHelper = NULL;
        }
    }
    ...
    const char *Weapon::getMuzzleflashEffect()
    {
            ...
            && muzzleflashEffect2 != NULL)
        {
            ...
            else if (lastMuzzleflashNumber == 1)
