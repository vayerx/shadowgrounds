        ...
        Unit *some_unit = NULL;
        {
            for (int i = 0; i < 2; i++) {
                ...
                if (o != NULL) {
                    ...
                    PhysicsContactUtils::mapPhysicsObjectToUnitOrTerrainObject(impl->game,
                                                                               o,
                                                                               &unit,
                                                                               &terrObjModelId,
                                                                               &terrObjInstanceId);
            ...
            if (o != NULL)
                ...
                if (true) {
                    ...
                    if (terrObjInstanceId != -1 && terrObjModelId != -1) {
                        UnifiedHandle trackableUH = impl->game->gameUI->getTerrain()->getUnifiedHandle(
                            terrObjModelId,
                            terrObjInstanceId);
                        impl->game->objectTracker->signalToTrackerFromTrackable(trackableUH,
                                                                                TRACKER_SIGNAL_BURN_THE_POLICE);
                    ...
                    if (unitCollisionNumber != -1)
                        if (i != unitCollisionNumber) {
                            ...
                            if ( !o->isDynamic() ) {
                                ...
                                float threshold = SimpleOptions::getFloat(
                                    DH_OPT_F_GAME_CONTACT_DAMAGE_REQUIRED_UNIT_DAMAGE_VELOCITY);
                ...
                if (makeDamage[i]) {
                    ...
                    if (i == 1) {
                        ...
                        if (destroyedObject != -1 && o2 != NULL)
                            // WARNING: unsafe cast!
                            o = (AbstractPhysicsObject *)impl->game->getGamePhysics()->getInterfaceObjectForHandle(
                                o2handle);
                        else
                            o = o2;
                    }
...
//Logger::getInstance()->error("SLOW!!");
                                ...
                                if (othervel.x == 0.0f && othervel.y == 0.0f && othervel.z == 0.0f) {
                                    ...
                                    if (o2 && fabsf(o2->getPosition().y - mapY) <= 0.05f) {
                                        ...
                                        unit->getGamePhysicsObject()->setVelocity(
                                            unit->getGamePhysicsObject()->getVelocity() * 0.25f);
                                        ...
                                        unit->setRotation(unitRot.x,
                                                          unit->getVisualObject()->getRenderYAngle(), unitRot.z);
...
// HACK: !!!
...
#endif
                                if (broken)
                                    broken = impl->game->gameUI->getTerrain()->breakObjects(terrObjModelId,
                                                                                            terrObjInstanceId,
                                                                                            (int)(
                                                                                                CONTACT_TERRAIN_OBJECT_DAMAGE
                                                                                                * damageFactor),
                                                                                            removedObjects,
                                                                                            objectEvents,
                                                                                            dampos2d,
                                ...
                                if (signal_this_terrain_object_break_hack) {
                                    UnifiedHandle trackableUH = impl->game->gameUI->getTerrain()->getUnifiedHandle(
                                        terrObjModelId,
                                        terrObjInstanceId);
                                    impl->game->objectTracker->signalToTrackerFromTrackable(
                                        trackableUH,
                                        TRACKER_SIGNAL_TERRAIN_OBJECT_BREAK_SELF_LOOP);
                                }
                                    ...
                                    for (int i = 0; i < eventAmount; i++) {
                                                ...
                                                if (pt != NULL) {
                                                             ...
                                                             getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Bull") ) ) )
                                                    {
                                                        ...
                                                        Logger::getInstance()->error(
                                                            "PhysicsContactDamageManager::physicsContact - Explosion event projectile, no part type found with given name.");
                                                    }
                                                } else {
                                                    Logger::getInstance()->error(
                                                        "PhysicsContactDamageManager::physicsContact - Explosion event projectile is not of bullet type.");
                                                }
                                            } else {
                                                Logger::getInstance()->error(
                                                    "PhysicsContactDamageManager::physicsContact - Explosion event projectile name is not a valid part type id.");
                                            }
                                        }
                                        if ( !objectEvents[i].effect.empty() ) {
                                            ...
                                            if (!broken && terrObjInstanceId != -1 && terrObjModelId != -1) {
                                                ...
                                                int lastTime =
                                                    impl->game->gameUI->getTerrain()->getLastObjectEffectTime(
                                                        terrObjModelId,
                                                        terrObjInstanceId);
                                                if ( impl->game->gameTimer < lastTime + (maxEffectRate / GAME_TICK_MSEC) )
                                                    effectRateOk = false;
                                                else
                                                    impl->game->gameUI->getTerrain()->setLastObjectEffectTime(
                                                        terrObjModelId,
                                                        terrObjInstanceId,
                                                        impl->game->gameTimer);
                                            }
                                            if (effectRateOk) {
                                                ui::VisualEffectManager *vman =
                                                ...
                                                if (effId == -1) {
                                                    Logger::getInstance()->error(
                                                        "PhysicsContactDamageManager::physicsContact - No visual effect with given name found.");
...
//                                                    VisualEffect *effect = vman->createNewManagedVisualEffect(effId, lifetime, NULL, NULL, pos, pos, VC3(objectEvents[i].rotation.x,objectEvents[i].rotation.y,objectEvents[i].rotation.z), objectEvents[i].velocity, impl->game);
                                                    ...
                                                    VisualEffect *effect = vman->createNewManagedVisualEffect(effId,
                                                                                                              ...
                                                                                                              game);
                                                    if (effect != NULL)
                                                        effect->setParticleExplosion(objectEvents[i].explosionPosition,
                                                                                     objectEvents[i].useExplosion);
                                                }
                                            }
                                        }
                                        if ( !objectEvents[i].script.empty() ) {
                                            ...
                                            impl->game->gameScripting->runOtherScriptForUnifiedHandle(
                                                sname,
                                                "spawn",
                                                uh,
                                                objectEvents[i].
                                        ...
                                        if ( !objectEvents[i].sound.empty() ) {
                                            ...
                                            impl->game->getGameUI()->playSoundEffect(
                                                e.sound.c_str(), e.position.x, e.position.y, e.position.z, false,
                                                DEFAULT_SOUND_SCRIPT_VOLUME, DEFAULT_SOUND_RANGE,
