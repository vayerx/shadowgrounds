...
namespace game
{
    TerrainPhysicsObject::TerrainPhysicsObject(GamePhysics          *gamePhysics,
                                               const unsigned short *buffer_,
                                               int                   samplesX_,
                                               int                   samplesY_,
                                               const VC3            &size_)
    ...
    {
        boost::shared_ptr<frozenbyte::physics::ActorBase> actor = gamePhysics->getPhysicsLib()->createHeightmapActor(
            buffer,
            samplesX,
            samplesY,
            size);
