...
#  endif
            ...
            if ( iter != physicsObjectsHash.end() ) {
                ...
                if (physicsObjects[arraypos].deleted) {
                    Logger::getInstance()->error(
                        "GamePhysicsImpl::getInterfaceObjectForHandle - Requested an object by handle which maps to a deleted object.");
                    return NULL;
                }
        ...
        void createPhysics(IGamePhysicsScriptRunner *scriptRunner)
        {
            ...
            if (scriptRunner != NULL)
                physicsParams.scriptRunner =
                    (frozenbyte::physics::IPhysicsLibScriptRunner *)( scriptRunner->
                                                                      getGamePhysicsScriptRunnerImplementation() );
            ...
            if ( SimpleOptions::getFloat(DH_OPT_F_PHYSICS_SKY_PLANE_HEIGHT) >
                 SimpleOptions::getFloat(DH_OPT_F_PHYSICS_GROUND_PLANE_HEIGHT) )
        ...
        void runPhysics(IStorm3D_Scene *stormScene, frozenbyte::particle::ParticleEffectManager *particleEffectManager)
        {
                    ...
                    if (physicsObjects[i].implObject) {
                        if (physicsObjects[i].implObjectWasActive) {
                            ...
                            if ( water_height > 0 && water_damping > 0
                                 ...
                                 && physicsObjects[i].implObject->getVolumeUnderWater(water_height, volume_min,
                                                                                      volume_max) )
                            {
                            ...
                            if (apo->dynamicActor && apo->dirty)
                                physicsObjects[i].interfaceObject->syncInactiveImplementationObject(
                                    physicsObjects[i].implObject);
                        }
                    }
#else
                    physicsObjects[i].interfaceObject->syncImplementationObject(physicsObjects[i].implObject);
            ...
            for (int i = 0; i < (int)deleteList.size(); i++) {
                ...
                if ( iter != physicsObjectsHash.end() )
                    ...
                    assert(
                        !
                        "GamePhysicsImpl::runPhysics - Unable to find handle to be deleted in hashmap, data integrity error (bug).");
            }
            deleteList.clear();
            ...
            if (fluidPushCurrentTime < fluidPushEndTime) {
                if (fluidPushEndTime > 0) {
                    ...
                    int ang =
                        (int)( ( (float)angTime
                                 / (float)( (int)(fluidPushEndTime / timeDiv) ) ) * 360.0f * (float)rotTimes );
                    //int rotNum = (ang / 360);
                    int sect = (ang % 360);
...
#  if !defined PROJECT_SURVIVOR && !defined PROJECT_SHADOWGROUNDS
            ...
            for (JointCreationList::iterator it = jointCreationList.begin(); it != jointCreationList.end(); ++it) {
                ...
                jointList.push_back( boost::static_pointer_cast<frozenbyte::physics::JointBase>( physicsLib->
                                                                                                 createGeneralJoint(a,
                                                                                                                    b,
                                                                                                                    info
                                                                                                                    .joint) ) );
            }
...
//Logger::getInstance()->error(buf);
        ...
        void removeObject(IGamePhysicsObject *obj)
        {
            ...
            if ( iter != physicsObjectsHash.end() ) {
                ...
                if (!physicsObjects[vecpos].deleted)
                    ...
                    assert(
                        !
                        "GamePhysicsImpl::deleteObject - Attempt to delete an object containing a handle for an already deleted object, data integrity error (bug).");
        ...
        PHYSICS_ACTOR getImplementingBaseObject(IGamePhysicsObject *interfaceObject)
        {
            ...
            if ( iter != physicsObjectsHash.end() ) {
                ...
                if (physicsObjects[arraypos].deleted) {
                    Logger::getInstance()->error(
                        "GamePhysics::getImplementingObject - Requested an object by handle which maps to a deleted object.");
                    return PHYSICS_ACTOR();
                }
    ...
    void GamePhysics::runPhysics(IStorm3D_Scene                              *stormScene,
...
#ifdef PHYSICS_PHYSX
#  if !defined PROJECT_SURVIVOR && !defined PROJECT_SHADOWGROUNDS
    void GamePhysics::addJoint(boost::shared_ptr<AbstractPhysicsObject> &objectA,
                               boost::shared_ptr<AbstractPhysicsObject> &objectB,
                               const frozenbyte::physics::PhysicsJoint  &joint,
                               const std::string                        &id)
    {
...
#  endif
    ...
    boost::shared_ptr<frozenbyte::physics::ActorBase> GamePhysics::getImplementingBaseObject(
        IGamePhysicsObject *interfaceObject)
    {
