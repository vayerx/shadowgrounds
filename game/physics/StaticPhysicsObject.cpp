...
namespace game
{
...
#ifdef PHYSICS_PHYSX
        boost::shared_ptr<frozenbyte::physics::StaticMesh> getMesh(GamePhysics *gamePhysics,
                                                                   const VC3   &position,
                                                                   const QUAT  &rotation)
        {
            if (mesh)
            ...
            boost::shared_ptr<frozenbyte::physics::StaticMesh> m = gamePhysics->getPhysicsLib()->createStaticMesh(
                 cookfile.c_str() );
    ...
    StaticPhysicsObject::StaticPhysicsObject(GamePhysics    *gamePhysics,
                                             const char     *filename,
                                             IStorm3D_Model *model,
                                             const VC3      &position,
                                             const QUAT     &rotation)
    ...
    {
#ifdef PHYSICS_PHYSX
        boost::shared_ptr<frozenbyte::physics::StaticMesh> staticMeshSPtr = impl->getMesh(gamePhysics,
                                                                                          position,
                                                                                          rotation);
        ...
        boost::shared_ptr<frozenbyte::physics::ActorBase> actor = gamePhysics->getPhysicsLib()->createStaticMeshActor(
            staticMeshSPtr,
            position,
            rotation);
...
#endif
#ifdef PHYSICS_ODE
        PhysicsActorOde actor = PhysicsActorOde::createNewStaticPhysicsObject(
            impl->filename.c_str(), position, rotation);
        return actor;
#endif
#ifdef PHYSICS_NONE
