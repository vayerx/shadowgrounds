...
namespace game
{
    CapsulePhysicsObject::CapsulePhysicsObject(GamePhysics *gamePhysics,
                                               float        height,
                                               float        radius,
                                               float        mass,
                                               int          collisionGroup,
...
#ifdef PHYSICS_PHYSX
#  ifdef GAME_SIDEWAYS
        boost::shared_ptr<frozenbyte::physics::ActorBase> actor = gamePhysics->getPhysicsLib()->createCapsuleActor(
            height,
            radius,
            position,
            -( radius + (0.5f * height) ),
