...
namespace game
{
    BoxPhysicsObject::BoxPhysicsObject(GamePhysics *gamePhysics,
                                       const VC3   &sizes,
                                       float        mass,
                                       int          collisionGroup,
                                       const VC3   &position)
...
#  ifdef GAME_SIDEWAYS
        boost::shared_ptr<frozenbyte::physics::ActorBase> actor =
            gamePhysics->getPhysicsLib()->createBoxActor( sizes, position, VC3(0, -sizes.y,
                                                                               0) );
#  else
        boost::shared_ptr<frozenbyte::physics::ActorBase> actor = gamePhysics->getPhysicsLib()->createBoxActor(sizes,
                                                                                                               position);
#  endif
        if (actor) {
            actor->setMass(mass);
