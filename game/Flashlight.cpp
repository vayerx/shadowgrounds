    ...
    void Flashlight::run(const VC3 &position)
    {
        ...
        if ( isFlashlightOn() ) {
            ...
            if ( impl->energy < SimpleOptions::getInt(DH_OPT_I_GAME_FLASHLIGHT_ENERGY_LOW) ) {
                ...
                {
                    float lowFactor = (float)impl->energy / (float)SimpleOptions::getInt(
                        DH_OPT_I_GAME_FLASHLIGHT_ENERGY_LOW);
                    COL lowIntensity = COL(0.20f + lowFactor * 0.80f,
                                           0.15f + lowFactor * 0.85f,
                                           0.10f + lowFactor * 0.90f);
            ...
            float lightmapBri = (float)impl->lightMap->getLightAmount( impl->lightMap->scaledToLightMapX(
                                                                           boundedPos.x),
                                                                       impl->lightMap->scaledToLightMapY(boundedPos.z) );
 ...
 */
            ...
            if (illum >= (float)SimpleOptions::getInt(DH_OPT_I_GAME_FLASHLIGHT_RECHARGE_MIN_ILLUMINATION) / 100.0f) {
                recharging = SimpleOptions::getInt(DH_OPT_I_GAME_FLASHLIGHT_RECHARGE_STATIC)
                             + int( illum * SimpleOptions::getInt(DH_OPT_I_GAME_FLASHLIGHT_RECHARGE_DYNAMIC) );
            } else {
                recharging = 0;
                if ( impl->energy < SimpleOptions::getInt(DH_OPT_I_GAME_FLASHLIGHT_ENERGY_LOW) )
        ...
        if (impl->spotlight.get() != NULL) {
            if (fabs(impl->betaAngle) > 0.001f) {
                ...
                if (impl->shakeFactor != 0.0f) {
                    shakenAngle +=
                        sinf( (float)impl->shakeCurrentTime / (float)impl->shakeTotalTime ) * impl->shakeFactor;
                    shakenBetaAngle +=
                        sinf( (float)impl->shakeCurrentTime / (float)impl->shakeTotalTime ) * impl->shakeFactor;
                }
                if (impl->swayFactor != 0.0f) {
                    shakenAngle +=
                        sinf( (float)impl->swayCurrentTime / (float)impl->swayTotalTime * 0.5f ) * impl->swayFactor;
                    shakenBetaAngle +=
                        sinf( (float)impl->swayCurrentTime / (float)impl->swayTotalTime ) * impl->swayFactor * 0.5f;
                }
    ...
    void Flashlight::setFlashlightOn(bool flashlightOn)
    {
        ...
        if (flashlightOn) {
            ...
            impl->lightCalculator =
                boost::shared_ptr<util::SpotLightCalculator>( new util::SpotLightCalculator( 40.0f, 20.0f,
                                                                                             impl->origin->
                                                                                             getDataObject() ) );
