...
namespace game
{
                            ...
                            if (id != 0) {
                                ...
                                if (u != NULL) {
                                    ...
                                    Logger::getInstance()->error(
                                        "AniRecorder::setRecords - Unit with given id number not found.");
                                    Logger::getInstance()->error(
                                        "AniRecorder::setRecords - Or numeric id-string was misinterpreted as id-number.");
                                }
                            } else {
                                ...
                                if (u != NULL) {
                                    ...
                                    Logger::getInstance()->error(
                                        "AniRecorder::setRecords - Unit with given id-string not found.");
                                }
                            }
                        }
                }
            ...
            if (impl->anis[i] == NULL) {
                ...
                for (int j = 0; j < ANIRECORDER_MAX_ANIS; j++) {
                    ...
                    if (tfm.setStartSelectionNearMarker(seekbuf) != aniUsed)
                        Logger::getInstance()->error(
                            "AniRecorder::addAniRecord - Ani batch file / ani memory integrity failure.");
                }
    ...
    int AniRecorder::dumpCameraPosition()
    {
        ...
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraPositionOffsetXToFloatValue\r\n", posOffset.x);
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraPositionOffsetYToFloatValue\r\n", posOffset.y);
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraPositionOffsetZToFloatValue\r\n", posOffset.z);
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraTargetOffsetXToFloatValue\r\n", targOffset.x);
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraTargetOffsetYToFloatValue\r\n", targOffset.y);
        sprintf(&camdumpbuf[strlen(
                                camdumpbuf)], "setFloatValue %f;setCameraTargetOffsetZToFloatValue\r\n", targOffset.z);
#endif
