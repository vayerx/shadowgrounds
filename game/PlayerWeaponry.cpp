...
namespace game
{
    ...
    void PlayerWeaponry::setPlayerWeaponry(Unit *unit, int i, int weapon)
    {
        if (unit) {
            ...
            it = playerweaponry_weaplist.find(unit_id);
            if ( it == playerweaponry_weaplist.end() )
                playerweaponry_weaplist.insert( std::pair< int, std::vector< int > >( unit_id,
                                                                                      std::vector< int >(
                                                                                          PLAYERWEAPONRY_MAX_WEAPONS) ) );
    ...
    void PlayerWeaponry::initWeaponry()
    {
        ...
        if ( sp.loadFile("data/part/playerweaponry.txt") )
            ...
            sp.error(
                "PlayerWeaponry::initWeaponry - Failed to load player weaponry file, game will not function properly.");
