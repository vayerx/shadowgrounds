...
public:
    virtual game::Projectile *createNewProjectileTrackerInstance()
    {
        ...
        if ( PARTTYPE_ID_STRING_VALID(idstr) ) {
            ...
            if (pt == NULL) {
                LOG_ERROR_W_DEBUG(
                    "GameProjectileTrackerFactory::createNewProjectileTrackerInstance - reference to unloaded part type.",
                    idstr);
            } else {
                if ( pt->isInherited( game::getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Bull") ) ) )
                    ...
                    LOG_ERROR_W_DEBUG(
                        "GameProjectileTrackerFactory::createNewProjectileTrackerInstance - part type is not bullet type.",
                        idstr);
            }
        } else {
            LOG_ERROR_W_DEBUG(
                "GameProjectileTrackerFactory::createNewProjectileTrackerInstance - invalid part type id string.",
                idstr);
        }
...
namespace game
{
    ...
    Game::Game()
    {
        ...
        game_anyBurnableTrackableObjectFactory = new tracking::AnyBurnableTrackableObjectFactory(
            burnableImplementations);
...
//                        gameUI->closeLoadingWindow( singlePlayerNumber );
                ...
                if (!paused && !tacticalMode) {
                    ...
                    physics->runPhysics( gameUI->getStormScene(),
                                         gameUI->getVisualEffectManager()->getParticleEffectManager() );
                                ...
                                if (ua != NULL) {
                                    ...
                                    if (fl != NULL) {
                                            ...
                                            || fabs(vel.z) > 0.5f / GAME_TICKS_PER_SECOND)
                                        {
                                            ...
                                            //fl->setSwayFactor(0);
                                            fl->setSwayFactor(SimpleOptions::getFloat(
                                                                  DH_OPT_F_FLASHLIGHT_SWAY_FACTOR) * 0.5f);
...
//                        projAmount++;
                    ...
                    for (int c = 0; c < MAX_PLAYERS_PER_CLIENT; c++) {
                        if (gameUI->getFirstPerson(c) != NULL) {
                            IUnitListIterator *iter = units->getNearbyOwnedUnits(2, gameUI->getFirstPerson(
                                                                                     c)->getPosition(), 1);
                            while ( iter->iterateAvailable() ) {
                                ...
                                if ( (u->getUnitType()->getTouchBullet() != NULL) && u->isTouchProjectileEnabled()
                                     && (units->getIdForUnit(u) % TOUCHBULLET_INTERVAL == gameTimer
                                         % TOUCHBULLET_INTERVAL) )
                                {
                                    ...
                                    RayTrace(gameUI->getFirstPerson(c)->getPosition()
                                             + VC3(0, 20.0f, 0), VC3(0, -1.0f, 0), 300.0f, collisionInfo, true);
...
#else
                        particleSpawnerManager->setPlayerPosition(
                             gameUI->getGameCamera()->getActualInterpolatedPosition() );
#endif
                    }
                     ...
                     || gameUI->isAbortingMission() )
                {
                    ...
                    if (missionSuccessCounter >= 0 && missionFailureCounter >= 0) {
                        Logger::getInstance()->warning(
                            "Game::run - Combat ended, but with neither failure nor success.");
...
// HACK: ...
    ...
    void Game::startCombat()
    {
        ...
        bool needPhysCacheRecreate = !FileTimestampChecker::isFileUpToDateComparedTo(
             cacheFilename.c_str(), obstFilename.c_str() );
             ...
             || (SimpleOptions::getBool(DH_OPT_B_AUTO_PHYSICS_CACHE_RECREATE) && needPhysCacheRecreate) )
        {
            ...
            if (wasHardware) {
                Logger::getInstance()->debug(
                    "Temporarily disabled hardware physics for cached physics state stabilization.");
...
//        gameMap->saveHideMap();
        ...
        if ( SimpleOptions::getBool(DH_OPT_B_PHYSICS_ENABLED) ) {
            ...
            Logger::getInstance()->debug(
                "Game::startCombat - Physics are not enabled, so skipping physics object creation.");
        }
...
#  endif
            ...
            std::string secrets =
                boost::lexical_cast<std::string>( gameScripting->getGlobalIntVariableValue("secretpart_amount") );

            std::string stats = "savegameStats \"time " + std::string(time) + " kills " + kills + " secrets "
                                + secrets + expstats + "\"\n";
    ...
    bool Game::loadVariablesFromMemory(const char *memoryBuffer,
                                       const char *saveId,
                                       const char *saveType,
                                       const char *applySub)
    {
    ...
    bool Game::loadVariablesFromTemporary()
    {
        if (temporarySaveBuffer == NULL) {
            ...
            strcpy(
                temporarySaveBuffer,
                "script savegame_temp; sub get_info; endSub; sub apply_mission; endSub; sub apply_variables; endSub; endScript;\n\n");
        }
    ...
    void Game::createVisualForParts(Unit *unit, Part *part, bool keepBones)
    {
        ...
        if (keepBones && unit->getVisualObject() == NULL) {
            ...
            Logger::getInstance()->error(
                "Game::createVisualForParts - Unit has null visual object when keepBones parameter given.");
            return;
        }
    ...
    void Game::stopCustomScriptProcessById(int pid)
    {
        ...
        if (deletedProcesses > 1)
            Logger::getInstance()->error(
                "Game::stopCustomScriptProcessById - Internal error, multiple script process found with given id.");
    }
    ...
    void Game::applyPendingLoad()
    {
        ...
        if (!success)
            Logger::getInstance()->warning(
                "Game::applyPendingLoad - Failed to load variables (failed to run apply_variables sub).");
    ...
    bool Game::getInfoForSavegame(const char *saveId, const char *saveType)
    {
        ...
        if ( savegame_version != "x"
            ...
            Logger::getInstance()->warning(
                "Game::getInfoForSavegame - Savegame version mismatch, game may not load correctly.");
