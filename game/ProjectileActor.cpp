...
namespace game
{
    ...
    void ProjectileActor::createVisualForProjectile(Projectile *projectile, bool originToSpecialUnit, Unit *specialUnit)
    {
        ...
        if (ve_id_explosion_cluster == -1)
            ve_id_explosion_cluster = game->gameUI->getVisualEffectManager()->getVisualEffectIdByName(
                "cluster_main_explosion");
...
//            || projectile->getVisualType() == game->gameUI->getVisualEffectManager()->getVisualEffectIdByName("electricmuzzleflash3"))
        ...
        if (projectile->getBulletType() != NULL)
            if (projectile->getBulletType()->getFluidPushTime() > 0
                ...
                game->getGamePhysics()->createFluidPushPoint(position,
                                                             projectile->getBulletType()->getFluidPushRadius(),
                                                             projectile->getBulletType()->getFluidPushTime()
                                                             / GAME_TICK_MSEC);
    ...
    void ProjectileActor::act(Projectile *projectile)
    {
                    ...
                    if (!helperFound) {
                        ...
                        if (!helperFound)
                            Logger::getInstance()->warning(
                                "ProjectileActor::act - Parent unit weapon barrel helper not found.");
                    }
                }
            }
        ...
        if ( projectile->doesFollowOrigin() )
            if (projectile->getOriginUnit() != NULL) {
                VC3 newpos = projectile->getOriginUnit()->getPosition()
                             + projectile->getOriginUnit()->getPointerMiddleOffset();
                {
                    QUAT q;
                    q.MakeFromAxisRotation( VC3(0, -1,
                                                0), UNIT_ANGLE_TO_RAD(projectile->getOriginUnit()->getRotation().y) );
...
//                            || u->getOwner() != 1))
                ...
                if (hitUnit != NULL) {
                    ...
                    if (projectile->getChain() == HITCHAIN_GROUND) {
                        ...
                        if (projectile->getChain() == HITCHAIN_BUILDING)
                            // hit building
                            createChainedProjectile( projectile,
                                                     projectile->getDestination(),
                                                     projectile->getChain(), projectile->getDirection() );
                     ...
                     && !projectile->getBulletType()->doesNoTerrainObjectBreaking() )
                {
                    ...
                    if ( projectile->getBulletType()->getTerrainObjectDamageProbability() >= 100
                         || (game->gameRandom->nextInt() % 100) <
                         projectile->getBulletType()->getTerrainObjectDamageProbability() )
                    ...
                    terrain->BreakTerrainObjects(position, projectile->getVelocity(),
                                                 projectile->getBulletType()->getTerrainHoleRadius(),
                                                 removedObjects, objectEvents,
                                                 projectile->getBulletType()->getTerrainObjectRadiusDamageAmount(),
                                                 false, only_breaktexture);
                    ...
                    if (!overrideHoleRadiusForImpulse)
                        terrain->physicsImpulse(position, projectile->getVelocity(),
                                                projectile->getBulletType()->getTerrainHoleRadius(),
                                                projectile->getBulletType()->getPhysicsImpulseFactor() * push_factor,
                                                false);

                    if ( game && game->getGameUI() && game->getGameUI()->getVisualEffectManager() &&
                         game->getGameUI()->getVisualEffectManager()->getParticleEffectManager() )
                        game->getGameUI()->getVisualEffectManager()->getParticleEffectManager()->addPhysicsExplosion(
                            position,
                            1.f);
                }
                ...
                if (overrideHoleRadiusForImpulse) {
                    if (projectile->getBulletType()->getPhysicsImpulseRadius() > 0) {
                        ...
                        const Vector &position = projectile->getPosition();
                        terrain->physicsImpulse(position, projectile->getVelocity(),
                                                projectile->getBulletType()->getPhysicsImpulseRadius(),
                                                projectile->getBulletType()->getPhysicsImpulseFactor() * push_factor,
                                                false);
                ...
                //else if (hitBreakableObject)
                {
                         ...
                         && !projectile->getBulletType()->doesPoisonDamage() )
                    {
                        ...
                        terrain->BreakTerrainObjects(position, projectile->getVelocity(),
                                                     0.4f,
                                                     removedObjects, objectEvents,
                                                     projectile->getBulletType()->getTerrainObjectDirectDamageAmount(),
                                                     true, only_breaktexture);
                    }
                }
                    ...
                    || hitBreakableObject)
                {
                    const Vector &position = projectile->getPosition();
                    Terrain *terrain = game->gameUI->getTerrain();
                    terrain->physicsImpulse(position, projectile->getVelocity(),  0.4f,
                                            projectile->getBulletType()->getPhysicsImpulseFactor() * push_factor, true);
                }
#endif
    ...
    bool ProjectileActor::doGore(Projectile *projectile,
                                 Unit       *hitUnit,
                                 bool        onlyPartial,
                                 ...
                                 int         additionalProbability)
    {
        ...
        if (projectile->getBulletType() != NULL)
            for (int gor = 0; gor < GORETYPE_AMOUNT; gor++) {
                int prob =
                    (int)( (projectile->getBulletType()->getGoreProbability(gor)
                            + additionalProbability) * probabilityFactor );
                if (prob > 0 && hitUnit->getUnitType()->getGoreAmount(gor) > 0) {
                    ...
                    if ( (game->gameRandom->nextInt() % 100) < prob ) {
                        ...
                        if (goresnd != NULL && goresnd[0] != '\0')
                            ...
                            game->gameUI->playSoundEffect(goresnd,
                                                          upos.x,
                                                          upos.y,
                                                          upos.z,
                                                          false,
                            ...
                            if (visualType >= 0) {
                                VisualEffect *ve =
                                    game->gameUI->getVisualEffectManager()->createNewManagedVisualEffect(
                                        visualType, GAME_TICKS_PER_SECOND * 2, NULL, NULL, upos, upos, urot,
                                        VC3(0, 0, 0), game);

                                if (ve != NULL) {
                                    if (projectile->getBulletType() && projectile->getBulletType()->getDamageRange() >
                                        0)
                        ...
                        if (goreTypeRemovesOrigin[gor]) {
                            ...
                            if (gorepart != NULL && gorepart[0] != '\0')
                                LOG_WARNING_W_DEBUG(
                                    "ProjectileActor::doGore - Gore has gore part defined but removes origin model.",
                                    gorepart);
                        } else {
                            if (gorepart != NULL && gorepart[0] != '\0') {
                                /*int dummy1 = 0;
        ...
        if (hitPart != NULL) {
            ...
            if ( !hitUnit->isDestroyed() ) {
                ...
                if ( heatDam > 0 && !hitUnit->getUnitType()->isVehicle() )
                    if ( !hitUnit->getUnitType()->doesDisableEffectLayer() ) {
                        ...
                        if (SimpleOptions::getInt(DH_OPT_I_LAYER_EFFECTS_LEVEL) >= 50)
                            ...
                            {
                                hitUnit->setUnitEffectLayerType(Unit::UNIT_EFFECT_LAYER_BURNING,
                                                                heatDam / GAME_TICK_MSEC);
                ...
                if (projectile) {
                    ...
                    if ( hitUnit->getTargetLockCounter() > 0 && hitUnit->getTargetLockCounter() ==
                         hitUnit->getTargetLockCounterMax() )
                        percent = 1.0f;
                    ...
                    if (allow_crithit && ( (float)(game->gameRandom->nextInt() % 100) / 100.0f ) < percent) {
                        ...
                        int player = 0;
                        if ( hit && message.empty() == false && game && game->gameUI &&
                             game->gameUI->getCombatWindow(player) &&
                             game->gameUI->getCombatWindow(player)->getSubWindow("TargetDisplayWindow") )
                            ( (TargetDisplayWindowUpdator *)game->gameUI->getCombatWindow(player)->getSubWindow(
                                  "TargetDisplayWindow") )->risingMessage(hitUnit, message);
                            // game->gameUI->getCombatWindow( player )->
            ...
            if (hitUnit->getHitAnimationCounter() > hitUnit->getUnitType()->getHitAnimationTime() / GAME_TICK_MSEC
                * 2 / 4) {
                ...
                if (hitUnit->getHitAnimationCounter() > hitUnit->getUnitType()->getHitAnimationTime()
                    / GAME_TICK_MSEC * 7 / 8) {
...
//                    hitUnit->setPhysicsObjectLock( true );
                    ...
                    if ( hitUnit->isDestroyed() ) {
                             ...
                             && projectile->getBulletType()->doesPoisonDamage() )
                        {
                            ...
                            if (deathSound != NULL) {
                                ...
                                game->gameUI->playSoundEffect(deathSound,
                                                              pos.x,
                                                              pos.y,
                                                              pos.z,
                                                              false,
                    ...
                    if (hpDamage > 0) {
                        if (hpDamage > PLAYER_BIG_POISON_LIMIT) {
                            ...
                            if (hpDamage > PLAYER_MEDIUM_POISON_LIMIT)
                                game->gameUI->getEffects()->startFlashEffect(
                                    UIEffects::FLASH_EFFECT_TYPE_PLAYER_POISON_MEDIUM);
                            else
                                game->gameUI->getEffects()->startFlashEffect(
                                    UIEffects::FLASH_EFFECT_TYPE_PLAYER_POISON_SMALL);
                        }
                    }
                } else {
                    if ( hitUnit->getMoveState() == Unit::UNIT_MOVE_STATE_UNCONSCIOUS
                        ...
                        if (hpDamage > 0) {
                            if (hpDamage > PLAYER_BIG_HIT_LIMIT) {
                                game->gameUI->getEffects()->startFlashEffect(
                                    UIEffects::FLASH_EFFECT_TYPE_PLAYER_HIT_BIG);
                            } else {
                                if (hpDamage > PLAYER_MEDIUM_HIT_LIMIT)
                                    game->gameUI->getEffects()->startFlashEffect(
                                        UIEffects::FLASH_EFFECT_TYPE_PLAYER_HIT_MEDIUM);
                                else
                                    game->gameUI->getEffects()->startFlashEffect(
                                        UIEffects::FLASH_EFFECT_TYPE_PLAYER_HIT_SMALL);
                            }
                        }
                }
            }
                                                         ...
                                                         Unit       *indirectHitUnit)
    {
        ...
        if (projectile->getBulletType()->getChainScript(hitchain) != NULL) {
            ...
            game->gameScripting->runHitChainScript( projectile->getBulletType()->getChainScript(hitchain),
                                                    ...
                                                    chainBullet, position, hitchain, direction,
                                                    projectile->getHitNormal() );
            projectile->getBulletType()->getChainBullet(hitchain);
            ...
            if (chainSoundAmount > 0) {
                ...
                if (chainSound != NULL && chainSound[0] != '\0')
                    if ( (game->gameRandom->nextInt() % 100)
                        ...
                        game->gameUI->playSoundEffect(chainSound,
                                                      position.x,
                                                      position.y,
                                                      position.z,
                                                      false,
                     ...
                     || (ally == 0 && unit->getIdString() != NULL && strcmp(unit->getIdString(), "arwyn") == 0) )
                {
...
#else
                    IUnitListIterator *ownUnitsIter = game->units->getNearbyOwnedUnits(
                        ally,
                        weaponPosition,
                        UNITACTOR_DONT_HIT_FRIENDLY_RANGE);
#endif
                    ...
                    while ( ownUnitsIter->iterateAvailable() ) {
                        ...
                        if ( ownu->isActive() ) {
                            ...
                            if (posdiff.GetSquareLength() < UNITACTOR_DONT_HIT_FRIENDLY_RANGE
                                * UNITACTOR_DONT_HIT_FRIENDLY_RANGE) {
    ...
    void ProjectileActor::doProjectileRaytrace(Unit       *shooter,
                                               Unit       *noCollisionUnit,
                                               Projectile *projectile,
                                               Bullet     *bulletType,
                                               const VC3  &_weaponPosition,
                                               ...
                                               float       velocityFactor)
    {
        ...
        if (parabolic) {
            ...
            VC3 parabolicStartDir =
                VC3(0, (float)bulletType->getParabolicPathHeight() / 2, 0) + ( (target - weaponRayPosition) * 0.5f );
            ...
            if (cinfo.hit) {
                ...
                VC3 parabolicEndDir =
                    VC3(0, -(float)bulletType->getParabolicPathHeight() / 2, 0) + ( (target - weaponPosition) * 0.5f );
                ...
                if (cinfo.hitBuilding) {
                    ...
                    if (cinfo.hitTerrainObject) {
                        ...
                        if (cinfo.terrainInstanceId != -1
                            ...
                            material = game->gameUI->getTerrain()->getMaterialForObject(cinfo.terrainModelId,
                                                                                        cinfo.terrainInstanceId);
                        if (material == -1) {
                            if ( game->gameMap->isWellInScaledBoundaries(cinfo.position.x, cinfo.position.z) ) {
                                ...
                                if ( game->gameMap->getAreaMap()->isAreaAnyValue(areaX, areaY,
                                                                                 AREAMASK_OBSTACLE_BUILDINGWALL) )
                                    proj->setChain(HITCHAIN_BUILDING);
                                else
...
//                        target = weaponPosition + (dir * (float)weapRange);
    ...
    void ProjectileActor::handleTerrainBreaking(Game                         *game,
                                                std::vector<TerrainObstacle> &removedObjects,
        ...
        if (eventAmount > 0)
            for (int i = 0; i < eventAmount; i++) {
                        ...
                        if (pt != NULL) {
                                     ...
                                     getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Bull") ) ) )
                            {
                                ...
                                Logger::getInstance()->error(
                                    "ProjectileActor::act - Explosion event projectile, no part type found with given name.");
                            }
                        } else {
                            Logger::getInstance()->error(
                                "ProjectileActor::act - Explosion event projectile is not of bullet type.");
                        }
                    } else {
                        Logger::getInstance()->error(
                            "ProjectileActor::act - Explosion event projectile name is not a valid part type id.");
                    }
                }
                ...
                if ( !events[i].sound.empty() ) {
                    ...
                    game->getGameUI()->playSoundEffect(
                        e.sound.c_str(), e.position.x, e.position.y, e.position.z, false, DEFAULT_SOUND_SCRIPT_VOLUME,
                        DEFAULT_SOUND_RANGE, DEFAULT_SOUND_PRIORITY_NORMAL);
                }
            }
    }
}
