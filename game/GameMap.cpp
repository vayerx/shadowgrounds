...
//        loadHideMap();
    ...
    void GameMap::createCoverMap()
    {
        ...
        if ( !SimpleOptions::getBool(DH_OPT_B_AUTO_COVER_BIN_RECREATE) )
            if (!upToDate) {
                Logger::getInstance()->warning(
                    "GameMap::createCoverMap - Covermap is not up to date, but it will not be recreated (auto recreate off).");
                upToDate = true;
            }
        ...
        if ( !SimpleOptions::getBool(DH_OPT_B_AUTO_COVER_BIN_RECREATE) )
            ...
            if (upToDate && !loaded)
                Logger::getInstance()->warning(
                    "GameMap::setData - Failed to load covermap (will attempt to recreate it).");
        if (!loaded) {
    ...
    void GameMap::loadObstacleAndAreaMap(frozenbyte::ai::PathFind *pathfinder)
    {
        ...
        if ( !SimpleOptions::getBool(DH_OPT_B_AUTO_OBSTACLE_BIN_RECREATE) )
            if (!upToDate) {
                Logger::getInstance()->warning(
                    "GameMap::loadObstacleAndAreaMap - Obstacle/area map is not up to date, but it will not be recreated (auto recreate off).");
                upToDate = true;
            }
        ...
        if ( SimpleOptions::getBool(DH_OPT_B_FORCE_OBSTACLE_BIN_RECREATE) ) {
            Logger::getInstance()->debug(
                "GameMap::loadObstacleAndAreaMap - Obstacle/area map is recreated (force recreate on).");
            upToDate = false;
        }
        ...
        if ( !SimpleOptions::getBool(DH_OPT_B_AUTO_OBSTACLE_BIN_RECREATE) )
            ...
            if (upToDate && !loaded)
                Logger::getInstance()->warning(
                    "GameMap::loadObstacleAndAreaMap - Failed to load obstacle/area map (will attempt to recreate it).");
    ...
    {
#ifdef _DEBUG
        if (x < 0 || y < 0 || x >= sizeX * GAMEMAP_HEIGHTMAP_MULTIPLIER || y >= sizeY
            * GAMEMAP_HEIGHTMAP_MULTIPLIER) abort();
#endif
    ...
    bool GameMap::loadObstacleAndAreaImpl(const char *filename, frozenbyte::ai::PathFind *pathfinder)
    {
        ...
        if (f != NULL) {
            ...
            for (int i = 0; i < pathfindSizeY; i++) {
                got += rle_packed_fread(&obstacleHeightMap[i * pathfindSizeX],
                                        sizeof(unsigned short) * pathfindSizeX,
                                        1,
                                        f);
            }
                ...
                if (got2 != pathfindSizeY) {
                    ...
                    if (got3 != pathfindSizeY) {
                        Logger::getInstance()->error(
                            "GameMap::loadObstacleAndAreaImpl - Error reading file (at pathfinder).");
                        Logger::getInstance()->debug(filename);
                    } else {
                        int xResolution = this->sizeX * GAMEMAP_HEIGHTMAP_MULTIPLIER;
                        int yResolution = this->sizeY * GAMEMAP_HEIGHTMAP_MULTIPLIER;
                        ...
                        for (int i = 0; i < yResolution; i++) {
                            got4 +=
                                rle_packed_fread(&precalcedPathfindHeightMap[i * xResolution],
                                                 sizeof(unsigned short) * xResolution,
                                                 1,
                                                 f);
                        }
                        if (got4 != yResolution) {
                            Logger::getInstance()->error(
                                "GameMap::loadObstacleAndAreaImpl - Error reading file (at heightmap).");
                            Logger::getInstance()->debug(filename);
                        } else {
                            Logger::getInstance()->debug(
                                "GameMap::loadObstacleAndAreaImpl - Obstacle/area map successfully loaded.");
                            success = true;
                        }
                    }
                }
    ...
    bool GameMap::saveObstacleAndAreaImpl(const char *filename, frozenbyte::ai::PathFind *pathfinder)
    {
        ...
        if (f != NULL) {
            ...
            int got = 0;
            for (int i = 0; i < pathfindSizeY; i++) {
                got += rle_packed_fwrite(&obstacleHeightMap[i * pathfindSizeX],
                                         sizeof(unsigned short) * pathfindSizeX,
                                         1,
                                         f);
            }
            if (got != pathfindSizeY) {
                ...
                if (got2 != pathfindSizeY) {
                    ...
                    if (got3 != pathfindSizeY) {
                        Logger::getInstance()->error(
                            "GameMap::saveObstacleAndAreaImpl - Error writing file (at pathfinder).");
                        Logger::getInstance()->debug(filename);
                    } else {
                        ...
                        int got4 = 0;
                        for (int i = 0; i < yResolution; i++) {
                            got4 += rle_packed_fwrite(&pathfindHeightMap[i * xResolution],
                                                      sizeof(unsigned short) * xResolution,
                                                      1,
                                                      f);
                        }
                        if (got4 != yResolution) {
                            Logger::getInstance()->error(
                                "GameMap::saveObstacleAndAreaImpl - Error writing file (at heightmap).");
                            Logger::getInstance()->debug(filename);
                        } else {
                            Logger::getInstance()->debug(
                                "GameMap::saveObstacleAndAreaImpl - Obstacle/area map successfully saved.");
                            success = true;
                        }
                    }
                }
