    ...
    struct ClawController::Data : public NxUserRaycastReport {
        ...
        void createPhysics()
        {
            ...
            for (int i = 0; i < clawSettings.boxAmount; ++i) {
                ...
                boost::shared_ptr<frozenbyte::physics::ActorBase> actor = physicsLib->createBoxActor(
                    clawSettings.boxSize,
                    pos,
                    localPos);
                if (actor) {
 ...
 */                 {
                        boost::shared_ptr<frozenbyte::physics::SphericalJoint> joint = physicsLib->createSphericalJoint(
                            actors[i - 1].actor,
                            actors[i].actor,
                            anchor);
                        joints.push_back(joint);
...
//VC3 forceDiff = force - clawVelocity;
                ...
                if (grabInfo.actor) {
                    ...
                    if (!grabInfo.actor->isDynamic() && force.GetSquareLength() > 1.0) {
                        frozenbyte::physics::ActorBase *actorBase =
                            static_cast<frozenbyte::physics::ActorBase *>(grabInfo.actor->userData);
                        if ( actorBase && actorBase->getUserData() && game->getGamePhysics() ) {
                            AbstractPhysicsObject *object =
                                static_cast<AbstractPhysicsObject *>( game->getGamePhysics()->
                                                                      getInterfaceObjectForHandle(
                                                                                                   reinterpret_cast
                                                                                                   <int>( actorBase->
                                                                                                          getUserData() ) ) );
 ...
 */
                    ...
                    // Rotation around y
                    {
                        ...
                        if (prepareAction != ClawController::ActionNone || activeAction.action !=
                            ClawController::ActionNone)
            ...
            if (!grabInfo.actor) {
                ...
                if ( !targetInfo.hasTarget() ) { // Auto aiming
                    ...
                    // Find all shapes based on frustum/sphere around claw
                    {
                        ...
                        activeAmount += scene->overlapSphereShapes(worldSphere,
                                                                   NX_ALL_SHAPES,
                                                                   MAX_ACTORS,
                                                                   shapes + activeAmount,
                                                                   NULL,
                    ...
                    for (int i = 0; i < activeAmount; ++i) {
                        ...
                        // Discard based on various properties
                        {
                            ...
                            //
                            frozenbyte::physics::ActorBase *actorBase =
                                static_cast<frozenbyte::physics::ActorBase *>(actor->userData);
                            if ( actorBase && actorBase->getUserData() && game->getGamePhysics() ) {
                                object =
                                    static_cast<AbstractPhysicsObject *>( game->getGamePhysics()->
                                                                          getInterfaceObjectForHandle(
                                                                                                       ...
                                                                                                       <int>( actorBase
                                                                                                              ...
                                                                                                              getUserData() ) ) );
                                if (object)
                                    PhysicsContactUtils::mapPhysicsObjectToUnitOrTerrainObject(game,
                                                                                               object,
                                                                                               &unit,
                                                                                               &terrObjModelId,
                                                                                               &terrObjInstanceId);
                        ...
                        if ( shapes[i]->raycast(rayShapeDirection, clawSettings.clawMaxDistance
                                                + rayShapeDirectionFudge + EXTRA_GRAB_DISTANCE,
                                                NX_RAYCAST_FACE_NORMAL | NX_RAYCAST_IMPACT
                                                | NX_RAYCAST_DISTANCE, hitInfo, false) ) {
                            float distance = model->GetPosition().GetRangeTo( toVC3(hitInfo.worldImpact) );
                            if (distance < clawSettings.clawMaxDistance + EXTRA_GRAB_DISTANCE) {
                                //float clawDistance = grabPos.GetRangeTo(toVC3(hitInfo.worldImpact));
                                ...
                                // Prefer objects from claw direction
                                {
                                    ...
                                    float addFactor =
                                        ( 1.f
                                          - powf(dot,
                                                 clawSettings.preferObjectsFromClawDirectionNonlinearSpread) )
                                        * clawSettings.preferObjectsFromClawDirectionDistanceFactor;
...
actorBase->getUserData() ) ) );
                ...
                if ( targetInfo.hasTarget() ) {
                    ...
                    if ( allowGrab &&
                         ( forceGrab ||
                           targetInfo.shape->raycast(rayClawDirection, rayBehind + rayForward, NX_RAYCAST_FACE_NORMAL
                                                     | NX_RAYCAST_IMPACT | NX_RAYCAST_DISTANCE, hitInfo, false) ) )
                        if ( allowGrab &&
                             (forceGrab || hitInfo.worldNormal.dot(rayClawDirection.dir) < -GRAB_REQUIRED_DOT) ) {
                            ...
                            if (game) {
                                AbstractPhysicsObject *object = 0;
                                frozenbyte::physics::ActorBase *actorBase =
                                    static_cast<frozenbyte::physics::ActorBase *>(targetInfo.actor->userData);
                                if ( actorBase && actorBase->getUserData() && game->getGamePhysics() )
                                    object =
                                        static_cast<AbstractPhysicsObject *>( game->getGamePhysics()->
                                                                              getInterfaceObjectForHandle(
...
actorBase->getUserData() ) ) );
                                ...
                                if (object != 0) {
                                    ...
                                    PhysicsContactUtils::mapPhysicsObjectToUnitOrTerrainObject(
                                        game,
                                        object,
                                        &grabInfo.unit,
                                        &grabInfo.terrObjModelId,
                                        &grabInfo.
                                        terrObjInstanceId);
                            ...
                            if (grabInfo.unit) {
                                ...
                                scene->setActorPairFlags(
                                    *actors[clawSettings.boxAmount - 1].actor->getActor(), *jointDesc.actor[1],
                                    NX_IGNORE_PAIR);
                                scene->setActorPairFlags(
                                    *actors[clawSettings.boxAmount - 2].actor->getActor(), *jointDesc.actor[1],
                                    NX_IGNORE_PAIR);
                                scene->setActorPairFlags(
                                    *actors[clawSettings.boxAmount - 3].actor->getActor(), *jointDesc.actor[1],
                                    NX_IGNORE_PAIR);
                            }
...
//                            actors[clawSettings.boxAmount - 1].actor->setMass(grabInfo.actorMass);
                            ...
                            if (grabInfo.actor->userData) {
                                ...
                                if ( actorBase && actorBase->getUserData() && game->getGamePhysics() ) {
                                    ...
                                    IGamePhysicsObject *gamePhysicsObject =
                                        game->getGamePhysics()->getInterfaceObjectForHandle(handle);

                                    if (gamePhysicsObject) {
                                        AbstractPhysicsObject *abstractObject =
                                            static_cast<AbstractPhysicsObject *>(gamePhysicsObject);
                ...
                if (grabInfo.actor) {
                    ...
                    // Fudge release velocity to autoaim direction
                    {
                        ...
                        if (objectMag > 0.0001f) {
                            ...
                            if ( getAutoAimDirection(getClawTargetPosition(), velocityDir, maxAngleChange * 180.f / PI,
                                                     40.f, autoaimDir) ) {
        ...
        int getFrustumShapes(const NxVec3 &position,
                             const NxVec3 &dir,
                             float         angle,
                             float         range,
                             NxShape     **shapes,
        ...
        bool getAutoAimDirection(const VC3 &position, const VC3 &originalDir, float angle, float range,
                                 VC3 &resultDir) const
        {
            ...
            int activeAmount = getFrustumShapes(fromVC3(position), fromVC3(
                                                    originalDir), angle, range, shapes, MAX_ACTORS);
            for (int i = 0; i < activeAmount; ++i) {
                ...
                object =
                    static_cast<AbstractPhysicsObject *>( game->getGamePhysics()->getInterfaceObjectForHandle(
reinterpret_cast
<int>(
      actorBase->getUserData() ) ) );
                if (!object)
 ...
 */
        ...
        bool updateTargeting()
        {
            ...
            NxRaycastHit hitInfo;
            if ( targetInfo.shape->raycast(rayShapeDirection, clawSettings.clawMaxDistance + rayShapeDirectionFudge
                                           + EXTRA_GRAB_DISTANCE, NX_RAYCAST_FACE_NORMAL | NX_RAYCAST_IMPACT
                                           | NX_RAYCAST_DISTANCE, hitInfo, false) ) {
                float distance = model->GetPosition().GetRangeTo( toVC3(hitInfo.worldImpact) );
                if (distance > clawSettings.clawMaxDistance + EXTRA_GRAB_DISTANCE) {
                    targetInfo.clear();
            ...
            if (grabInfo.actor && grabInfo.joint) {
                ...
                if (++jointBreakDelay > 80 && massEntry.dropForce > 1)
                    if (grabInfo.actor->userData) {
                        ...
                        if ( actorBase && actorBase->getUserData() && game->getGamePhysics() ) {
                            int handle = reinterpret_cast<int>( actorBase->getUserData() );
                            IGamePhysicsObject *gamePhysicsObject = game->getGamePhysics()->getInterfaceObjectForHandle(
                                handle);
...
//Logger::getInstance()->error(foo.c_str());
        ...
        void parseStuff()
        {
            ...
            for (int i = 0; i < groups; ++i) {
                // Claw
                {
                    ...
                    clawSettings.velocityScaleFactor = convertFromString<float>(group.getValue(
                                                                                    "velocity_scale_factor"), 1.0f);
                    clawSettings.maxVelocityFactor = convertFromString<float>(group.getValue(
                                                                                  "max_velocity_factor"), 10.0f);
                    ...
                    clawSettings.clawMaxDistance = convertFromString<float>(group.getValue(
                                                                                "max_distance"),
                                                                            clawSettings.clawLength + 1.0f);
                    ...
                    clawSettings.heightWithoutActor = convertFromString<float>(group.getValue(
                                                                                   "height_without_actor"), 1.0f);
                    ...
                    clawSettings.maxThrowAngleAdjust = convertFromString<float>(group.getValue(
                                                                                    "max_throw_angle_adjust"), 45.0f);
                    clawSettings.maxAutoAimAdjust = convertFromString<float>(group.getValue(
                                                                                 "max_autoaim_angle_adjust"), 20.0f);
                    clawSettings.maxThrowAngleAdjust *= PI / 180.f;
                    clawSettings.maxAutoAimAdjust *= PI / 180.f;

                    clawSettings.maxTurboAutoAimAdjust =
                        convertFromString<float>(group.getValue("max_turbo_autoaim_angle_adjust"), 20.0f);
                    clawSettings.maxAimedThrowAutoAimAdjust =
                        convertFromString<float>(group.getValue("max_aimed_throw_autoaim_angle_adjust"), 15.0f);
                    clawSettings.maxTurboAutoAimAdjust *= PI / 180.f;
                    clawSettings.maxAimedThrowAutoAimAdjust *= PI / 180.f;

                    clawSettings.minAimedThrowBetaAngle =
                        convertFromString<float>(group.getValue("min_aimed_throw_beta_angle"), -5.0f);
                    clawSettings.minAimedThrowBetaAngle *= PI / 180.f;
                    clawSettings.maxAimedThrowBetaAngle =
                        convertFromString<float>(group.getValue("max_aimed_throw_beta_angle"), 20.0f);
                    clawSettings.maxAimedThrowBetaAngle *= PI / 180.f;
                    clawSettings.aimedThrowBetaAngleForceScale =
                        convertFromString<float>(group.getValue("aimed_throw_beta_angle_force_scale"), 1.0f);

                    clawSettings.actionPrepareTicks = convertFromString<int>(group.getValue(
                                                                                 "aimed_throw_prepare_time"), 500) / 15;
                    clawSettings.actionPrepareSpeed =
                        convertFromString<float>(group.getValue("aimed_throw_prepare_speed"), 0.4f);
                    ...
                    clawSettings.turboCooldownTicks =
                        convertFromString<int>(group.getValue("turbo_cooldown"), 1000) / 15;
                    clawSettings.turboTicks = convertFromString<int>(group.getValue("turbo_duration"), 1000) / 15;
                    clawSettings.turboControllerFactor =
                        convertFromString<float>(group.getValue("turbo_acceleration_factor"), 4.0f);
                    clawSettings.turboMaxspeedFactor =
                        convertFromString<float>(group.getValue("turbo_maxvelocity_factor"), 2.0f);
                    ...
                    controlSettings.drawHelperCursor = convertFromString<bool>(group.getValue(
                                                                                   "draw_helper_cursor"), false);

                    clawSettings.preferObjectsFromClawDirectionNonlinearSpread = convertFromString<float>(
                        group.getValue("prefer_objects_from_claw_direction_nonlinear_spread"),
                        1.0f);
                    clawSettings.preferObjectsFromClawDirectionDistanceFactor =
                        convertFromString<float>(group.getValue(
                                                     "prefer_objects_from_claw_direction_distance_factor"), 10.0f);
                    clawSettings.preferObjectsWithMassLimit =
                        convertFromString<float>(group.getValue("prefer_objects_with_mass_limit"), 50.0f);
                    clawSettings.preferObjectsWithMassFactor =
                        convertFromString<float>(group.getValue("prefer_objects_with_mass_factor"), 0.5f);
                }
                ...
                // Controls
                {
                    ...
                    clawSettings.restPoseReturnMethod =
                        convertFromString<float>(controlsGroup.getValue("rest_pose_return_method"), 0.5f);
                    clawSettings.restPoseScaleFactor =
                        convertFromString<float>(controlsGroup.getValue("rest_pose_scale_factor"), 0.005f);
                    clawSettings.restPoseLinearSpeed =
                        convertFromString<float>(controlsGroup.getValue("rest_pose_linear_speed"), 0.015f);

                    controlSettings.animationSwitchFarPoseRange =
                        convertFromString<float>(controlsGroup.getValue("animation_switch_far_pose_range"), 5.5f);
                    controlSettings.animationSwitchNearPoseRange =
                        convertFromString<float>(controlsGroup.getValue("animation_switch_near_pose_range"), 5.0f);
                    ...
                    // Pad
                    {
                        ...
                        controlSettings.padDeltaTurboFactor =
                            convertFromString<float>(group.getValue("delta_turbo_factor"), 1.5f);
                        controlSettings.padTriggerMovementFactor =
                            convertFromString<float>(group.getValue("trigger_movement_factor"), 1.5f);
                    }

                    // Mouse
                    {
                        const ParserGroup &group = controlsGroup.getSubGroup("Mouse");
                        controlSettings.mouseDeltaScale = convertFromString<float>(group.getValue(
                                                                                       "delta_scale"), 0.025f);
                ...
                // Mass
                {
                    ...
                    for (int j = 0; j < group.getValueAmount(); ++j) {
                        ...
                        entry.minimumSpeedTowardsController =
                            convertFromString<float>(valueGroup.getValue("minimum_speed_towards_controller"), 0.f);
                        entry.liftSpeed = convertFromString<float>(valueGroup.getValue("lift_speed"), 0.07f);
                        // Decay
                        entry.accelerationDecayLinear =
                            convertFromString<float>(valueGroup.getValue("acceleration_decay_linear"), 0.f);
                        entry.accelerationDecayFactor =
                            convertFromString<float>(valueGroup.getValue("acceleration_decay_factor"), 0.f);
                        entry.accelerationDecayNonlinearFactor =
                            convertFromString<float>(valueGroup.getValue("acceleration_decay_nonlinear_factor"), 0.f);
                        entry.inactiveAccelerationDecayLinear =
                            convertFromString<float>(valueGroup.getValue("inactive_acceleration_decay_linear"), 0.f);
                        entry.inactiveAccelerationDecayFactor =
                            convertFromString<float>(valueGroup.getValue("inactive_acceleration_decay_factor"), 0.f);
                        entry.inactiveAccelerationDecayNonlinearFactor =
                            convertFromString<float>(valueGroup.getValue(
                                                         "inactive_acceleration_decay_nonlinear_factor"), 0.f);
                        entry.controllerAlignedAccelerationDecayLinear =
                            convertFromString<float>(valueGroup.getValue(
                                                         "controller_aligned_acceleration_decay_linear"), 0.f);
                        entry.controllerAlignedAccelerationDecayFactor =
                            convertFromString<float>(valueGroup.getValue(
                                                         "controller_aligned_acceleration_decay_factor"), 0.f);
                        entry.controllerAlignedAccelerationDecayNonlinearFactor = convertFromString<float>(
                            valueGroup.getValue("controller_aligned_acceleration_decay_nonlinear_factor"),
                            0.f);

                        // Acceleration
                        entry.accelerationIncreaseLinear =
                            convertFromString<float>(valueGroup.getValue("acceleration_increase_linear"), 0.f);
                        entry.accelerationIncreaseFactor =
                            convertFromString<float>(valueGroup.getValue("acceleration_increase_factor"), 0.f);
                        entry.accelerationIncreaseNonlinearFactor =
                            convertFromString<float>(valueGroup.getValue("acceleration_increase_nonlinear_factor"), 0.f);
                        entry.accelerationDisableLinearIncreaseSpeed =
                            convertFromString<float>(valueGroup.getValue(
                                                         "acceleration_disable_linear_increase_speed"), 10000.f);

                        entry.velocityRotationHelperFactor =
                            convertFromString<float>(valueGroup.getValue("velocity_rotation_helper_factor"), 0.f);
                        ...
                        entry.releaseExtraSpeedFactor =
                            convertFromString<float>(valueGroup.getValue("release_extra_speed_factor"), 0.f);

                        entry.clawCenterForceEnabled = convertFromString<bool>(valueGroup.getValue(
                                                                                   "center_force"), true);
    ...
    void ClawController::update()
    {
        if (data->newClawObject && ++data->newClawObjectDelay > 1) {
            NxActor *newActor = 0;
            game::IGamePhysicsObject *obj = data->game->getGamePhysics()->getInterfaceObjectForHandle(
                 data->newClawObject->getHandle() );
            if (obj)
            ...
            if (newActor) {
                ...
                if (data->game) {
                    AbstractPhysicsObject *object = 0;
                    frozenbyte::physics::ActorBase *actorBase =
                        static_cast<frozenbyte::physics::ActorBase *>(newActor->userData);
                    if ( actorBase && actorBase->getUserData() && data->game->getGamePhysics() )
                        object =
                            static_cast<AbstractPhysicsObject *>( data->game->getGamePhysics()->
                                                                  getInterfaceObjectForHandle(
                                                                                               reinterpret_cast
                                                                                               <int>( actorBase->
                                                                                                      getUserData() ) ) );
                    ...
                    if (object != 0) {
                        object->setThrownByClaw(true);
                        PhysicsContactUtils::mapPhysicsObjectToUnitOrTerrainObject(data->game,
                                                                                   object,
                                                                                   &data->grabInfo.unit,
                                                                                   &data->grabInfo.terrObjModelId,
                                                                                   &data->grabInfo.terrObjInstanceId);
                ...
                if (data->grabInfo.actor->userData) {
                    // WARNING: int to void * cast!
                    frozenbyte::physics::ActorBase *actorBase =
                        static_cast<frozenbyte::physics::ActorBase *>(data->grabInfo.actor->userData);
                    if ( actorBase && actorBase->getUserData() && data->game->getGamePhysics() ) {
                        int handle = reinterpret_cast<int>( actorBase->getUserData() );
                        IGamePhysicsObject *gamePhysicsObject =
                            data->game->getGamePhysics()->getInterfaceObjectForHandle(handle);
                ...
                if (data->grabInfo.actor) {
                    for (int j = 1; j < data->clawSettings.boxAmount; ++j) {
                        scene->setActorPairFlags(
                            *data->actors[j].actor->getActor(), *data->grabInfo.actor, NX_IGNORE_PAIR);
                    }
 ...
 */
            ...
            // Acceleration
            {
                ...
                if (deltaAmount > massEntry.accelerationDisableLinearIncreaseSpeed) {
                    ...
                    acceleration *=
                        (massEntry.accelerationIncreaseFactor + deltaAmount
                         * massEntry.accelerationIncreaseNonlinearFactor);
                    ...
                    if (data->activeAction.action == ActionThrow) {
                        if (data->grabInfo.actor && data->grabInfo.joint) {
                            ...
                            if (!data->grabInfo.unit) {
                                ...
                                if (data->game)
                                    data->game->gameScripting->runOtherScript( "claw", "ungrab_unit",
                                                                               data->grabInfo.unit, VC3(0, 0,
                                                                                                        0) );
                            }
 ...
 */
    void ClawController::prepareAction(Action action)
    {
        ...
        if (action == ActionNone) {
            ...
            if (data->prepareAction != ActionNone && data->prepareActionCount >
                data->clawSettings.actionPrepareTicks) {
                ...
                // Get dir where to aim
                {
                    ...
                    // Autoaim
                    {
                        ...
                        if ( data->getAutoAimDirection(clawPosition, dir, maxAngleChange * 180.f / PI, 40.f,
                                                       autoaimDir) ) {
                        ...
                        float strength_scale = powf(cosf(
                                                        data->throwBetaAngle),
                                                    data->clawSettings.aimedThrowBetaAngleForceScale);
    ...
    VC3 ClawController::getClawPosition() const
    {
        ...
        if (data->actors.size() == data->clawSettings.boxAmount && data->joints.size() ==
            data->clawSettings.boxAmount - 1)
