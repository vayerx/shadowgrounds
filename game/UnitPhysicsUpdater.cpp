...
#define UPU_PHYSOBJTYPE_FLUID_CONTAINMENT 1
        ...
        for (int physobjtype = 0; physobjtype < 2; physobjtype++) {
                      ...
                      && SimpleOptions::getBool(DH_OPT_B_PHYSICS_FLUIDS_ENABLED) ) )
            {
                ...
                if (ut->getPhysicsObjectType() == UNITTYPE_PHYSICS_OBJECT_TYPE_CAPSULE) {
                    ...
                    if (ut->getPhysicsObjectImplementationType() ==
                        UNITTYPE_PHYSICS_OBJECT_IMPLEMENTATION_TYPE_SIDEWAYS)
...
#ifdef PHYSICS_NONE
                    physobj =
                        new game::BoxPhysicsObject(physics,
                                                   VC3( ut->getPhysicsObjectSizeX(), ut->getPhysicsObjectSizeY(),
                                                        ut->getPhysicsObjectSizeZ() ),
                                                   ut->getPhysicsObjectMass(), collGroup, pos);
#else
                    physobj = new game::CapsulePhysicsObject(physics, height, radius,
                                                             ut->getPhysicsObjectMass(), collGroup, pos);
#endif
                }
                if (ut->getPhysicsObjectType() == UNITTYPE_PHYSICS_OBJECT_TYPE_BOX) {
                    ...
                    if ( unit->getUnitType()->isPhysicsObjectDoor() ) {
                        ...
                        physobj->setRotation( QUAT( UNIT_ANGLE_TO_RAD(urot.x), UNIT_ANGLE_TO_RAD(urot.y),
                                                    UNIT_ANGLE_TO_RAD(urot.z) ) );
                    }
                }

                if (physobj != NULL) {
                    // WARNING: unsafe int to void * cast!
                    physobj->setCustomData( reinterpret_cast<void *>( PhysicsContactUtils::
                                                                      calcCustomPhysicsObjectDataForUnit(game, unit) ) );
    ...
    {
        assert(
            unit->getUnitType()->getPhysicsObjectImplementationType() ==
            UNITTYPE_PHYSICS_OBJECT_IMPLEMENTATION_TYPE_SIDEWAYS);
        assert(unit->getGamePhysicsObject() != NULL);
...
//                        feedbackdiff.z *= 0.2f;
            ...
            if (setposx || setposz) {
                bp->setVelocity( diff * ut->getPhysicsObjectImpulseFactor()
                                 * ( 1.0f - ut->getPhysicsObjectImpulseFeedbackFactor() ) );
            } else {
    ...
    void UnitPhysicsUpdater::updatePhysics(Unit *unit, GamePhysics *physics, int gameTickTimer)
    {
        ...
        for (int physobjtype = 0; physobjtype < 2; physobjtype++) {
            if ( (physobjtype == UPU_PHYSOBJTYPE_GAME && unit->getGamePhysicsObject() != NULL)
                 || (physobjtype == UPU_PHYSOBJTYPE_FLUID_CONTAINMENT && unit->getFluidContainmentPhysicsObject() !=
                     NULL) )
            {
...
// also don't delete physics until the unit has stopped.
                ...
                if (unit->getUnitType()->getPhysicsObjectImplementationType() ==
                    UNITTYPE_PHYSICS_OBJECT_IMPLEMENTATION_TYPE_SIDEWAYS) {
                ...
                if ( diff.GetSquareLength() > ut->getPhysicsObjectWarpThreshold() ) {
                    ...
                    if ( unit->getUnitType()->isPhysicsObjectDoor() ) {
                        VC3 urot = unit->getRotation();
                        bp->setRotation( QUAT( UNIT_ANGLE_TO_RAD(urot.x), UNIT_ANGLE_TO_RAD(urot.y),
                                               UNIT_ANGLE_TO_RAD(urot.z) ) );
                    } else {
                        bp->setRotation( QUAT(0, 0, 0) );
                    }
                    ...
                    if (diff.GetSquareLength() > threshold * threshold) {
                        bp->addImpulse(
                             bp->getMassCenterPosition(), diff * GAME_TICKS_PER_SECOND
                            * ut->getPhysicsObjectImpulseFactor() );
                        if ( unit->isPhysicsObjectFeedbackEnabled() )
