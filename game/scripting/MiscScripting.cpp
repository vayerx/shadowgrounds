...
namespace game
{
                                ...
                                GameScriptData *gsd, Game *game, bool *pause)
    {
        ...
        case GS_CMD_WAIT: {
            ...
            if (intData >= 15)
                sp->warning(
                    "MiscScripting::process - wait with parameter greater or equal than 15 seconds (probably unintended?).");
        }
            *pause = true;
            break;
        ...
        case GS_CMD_WAITACCURATE: {
            ...
            if (intData < 15)
                sp->warning(
                    "MiscScripting::process - waitAccurate with parameter less than 15 milliseconds (probably unintended?).");
            *pause = true;
            break;
                ...
                if (stringData != NULL) {
                        ...
                        && unit->getCharacter()->getLipsyncId() != NULL)
                    {
                        ...
                        if ( !game->gameUI->getLipsyncManager()->isActive() )
                            sp->warning(
                                "GameScripting::process - Attempt to characterMessage when lipsync manager is inactive.");
            ...
            if (unit != NULL) {
                if (stringData != NULL) {
                    ...
                    if (unit->getCharacter() != NULL
                        && unit->getCharacter()->getLipsyncId() != NULL)
                    {
                        boost::shared_ptr<sfx::AmplitudeArray> amplitudes =
                            game->gameUI->getLipsyncManager()->getAmplitudeBuffer( convertLocaleSpeechString(usestr) );
                        game->gameUI->getLipsyncManager()->playSpeech(
                            unit->getCharacter()->getLipsyncId(), amplitudes, lipsync_start_time);

                        if ( !game->gameUI->getLipsyncManager()->isActive() )
                            sp->warning(
                                "GameScripting::process - Attempt to playLipsync when lipsync manager is inactive.");
                    } else {
                ...
                if (s != NULL) {
                                ...
                                for (int j = i; j < slen; j++) {
                                    if (s[j] == ')') {
                                        ...
                                        for (int k = 0; k < (int)paramVars.size(); k++) {
                                            int val = game->gameScripting->getGlobalIntVariableValue(
                                                 paramVars[k].c_str() );
                    ...
                    *lastValue = game->addCustomScriptProcess(name.c_str(), (Unit *) NULL, params);
                    ...
                    if (!game->inCombat && !game_in_start_combat)
                        sp->warning(
                            "GameScripting::process - startScriptProcess called while in menus (will not run until in mission).");
                } else {
        ...
        case GS_CMD_ISSCRIPTPROCESSOFVALUECINEMATIC:
                ...
                && *lastValue <= SCRIPTPROCESS_MAX_ID)
            {
                ...
                sp->error(
                    "GameScripting::process - isScriptProcessOfValueCinematic value out of range (script process id expected in value).");
                *lastValue = 0;
            }
            break;
                ...
                if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                    if (game->inCombat)
                        ...
                        *lastValue = game->addCustomScriptProcess(stringData, gsd->unifiedHandle, NULL);
                        ...
                        sp->error(
                            "GameScripting::process - Attempt to startScriptProcessForUnifiedHandle while in menus.");
                } else {
                    if (gsd->unifiedHandle == UNIFIED_HANDLE_NONE)
                        sp->warning(
                            "GameScripting::process - Attempt to startScriptProcessForUnifiedHandle for unified handle none.");
                    else
                        sp->warning(
                            "GameScripting::process - Attempt to startScriptProcessForUnifiedHandle for invalid unified handle.");
                }
            } else {
        ...
        case GS_CMD_STOPSCRIPTPROCESSBYID:
            if (*lastValue >= SCRIPTPROCESS_MIN_ID
                ...
                sp->error(
                    "GameScripting::process - stopScriptProcessById value out of range (script process id expected in value).");
            break;
        ...
        case GS_CMD_DISABLEAIFORPLAYER:
            if (intData >= 0 && intData < ABS_MAX_PLAYERS)
                ...
                sp->error(
                    "MiscScripting::process - disableAIForPlayer parameter out of range (player side number expected).");
            break;
        ...
        case GS_CMD_ENABLEAIFORPLAYER:
            if (intData >= 0 && intData < ABS_MAX_PLAYERS)
                ...
                sp->error(
                    "MiscScripting::process - enableAIForPlayer parameter out of range (player side number expected).");
            break;
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->error(
                                "MiscScripting::process - forceControllerKeyEnabled parameter invalid, no such control name.");
                            sp->debug(stringData);
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - forceControllerKeyEnabled parameter missing, control name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - forceControllerKeyEnabled, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
                sp->error(
                    "MiscScripting::process - forceControllerKeyEnabled last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
            }
            break;
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->error(
                                "MiscScripting::process - isControllerKeyDown parameter invalid, no such control name.");
                            sp->debug(stringData);
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - isControllerKeyDown parameter missing, control name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - isControllerKeyDown, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->error(
                                "MiscScripting::process - wasControllerKeyClicked parameter invalid, no such control name.");
                            sp->debug(stringData);
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - wasControllerKeyClicked parameter missing, control name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - wasControllerKeyClicked, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
                sp->error(
                    "MiscScripting::process - wasControllerKeyClicked last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
            }
            break;
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->error(
                                "MiscScripting::process - setControllerKeyDown parameter invalid, no such control name.");
                            sp->debug(stringData);
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - setControllerKeyDown parameter missing, control name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - setControllerKeyDown, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->error(
                                "MiscScripting::process - setControllerKeyUp parameter invalid, no such control name.");
                            sp->debug(stringData);
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - setControllerKeyUp parameter missing, control name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - setControllerKeyUp, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
        ...
        case GS_CMD_getControllerSceneSelectionPosition:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                sp->error(
                    "MiscScripting::process - getControllerSceneSelectionPosition last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
            }
            break;
        ...
        case GS_CMD_getControllerSceneSelectionUnit:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                sp->error(
                    "MiscScripting::process - getControllerSceneSelectionUnit last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
                *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_isControllerSceneSelectionAvailable:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                sp->error(
                    "MiscScripting::process - isControllerSceneSelectionAvailable last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
                *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_getControllerCursorScreenPositionX:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                *lastValue = game->gameUI->getCursorScreenX(*lastValue);
                ...
                sp->error(
                    "MiscScripting::process - getControllerCursorScreenPositionX last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
                *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_getControllerCursorScreenPositionY:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                *lastValue = game->gameUI->getCursorScreenY(*lastValue);
                ...
                sp->error(
                    "MiscScripting::process - getControllerCursorScreenPositionY last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
                *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_BIND:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            if (splitpos != 0) {
                                ...
                                sp->error(
                                    "MiscScripting::process - bind parameter invalid, keycode name expected after control name.");
                                sp->debug(&buf[splitpos]);
                            }
                        } else {
                    ...
                    sp->error(
                        "MiscScripting::process - bind, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
                ...
                if (controller != NULL) {
                    if (stringData != NULL) {
                        ...
                        if (ctrlnum != -1) {
                            ...
                            sp->debug(
                                "MiscScripting::process - unbind parameter was not a control name, assuming it is keycode name.");
                            sp->debug(stringData);
                            ...
                            if (keycode != -1) {
                                ...
                                sp->error(
                                    "MiscScripting::process - unbind parameter invalid, no such control or keycode name.");
                                sp->debug(stringData);
                            }
                        }
                    } else {
                        sp->error(
                            "MiscScripting::process - unbind parameter missing, control or keycode name expected.");
                    }
                } else {
                    sp->error(
                        "MiscScripting::process - unbind, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
        ...
        case GS_CMD_LOADBINDS:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                if (controller != NULL) {
                    ...
                    sp->error(
                        "MiscScripting::process - loadBinds, null controller for given last value (controller number).");
                    sp->debug( int2str(*lastValue) );
                }
            } else {
        ...
        case GS_CMD_PRELOADTEMPORARYTEXTURE:
            if (stringData != NULL)
                ...
                sp->error(
                    "MiscScripting::process - preloadTemporaryTexture, parameter missing, texture filename expected.");
            break;
                ...
                if (pt == NULL) {
                             ...
                             getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Weap") ) ) )
                    {
                        ...
                        if (gsd->stringValue != NULL) {
                            ParticleSpawner *duplicateTest = game->particleSpawnerManager->getParticleSpawnerByName(
                                gsd->stringValue);
                            if (duplicateTest != NULL)
                                sp->warning(
                                    "MiscScripting::process - addParticleSpawner, created particle spawner with duplicate name.");
                        }
        ...
        case GS_CMD_DISABLEPARTICLESPAWNERBYNAME:
            if (stringData != NULL) {
                ...
                if (spawner != NULL)
                    ...
                    sp->error(
                        "MiscScripting::process - disableParticleSpawnerByName, no particle spawner with given name found.");
            } else {
        ...
        case GS_CMD_ENABLEPARTICLESPAWNERBYNAME:
            if (stringData != NULL) {
                ...
                if (spawner != NULL)
                    ...
                    sp->error(
                        "MiscScripting::process - enableParticleSpawnerByName, no particle spawner with given name found.");
            } else {
        ...
        case GS_CMD_spawnProjectileWithShooter:
            if (stringData != NULL) {
                ...
                if (s != NULL) {
                    ...
                    if (pt == NULL) {
                        sp->error(
                            "MiscScripting::process - spawnProjectileWithShooter, reference to unloaded part type.");
                    } else {
                        if ( pt->isInherited( getPartTypeById( PARTTYPE_ID_STRING_TO_INT("Weap") ) ) ) {
                            // WARNING: unsafe cast!
                            Weapon *weap = (Weapon *)pt;
                            ...
                            sp->error(
                                "MiscScripting::process - spawnProjectileWithShooter, attempt to use non-weapon part type.");
                        }
                    }
                } else {
                ...
                sp->error(
                    "MiscScripting::process - spawnProjectileWithShooter parameter missing, weapon name expected.");
            }
            break;

        case GS_CMD_MAKEHEIGHTAREABLOCKED:
            game->gameMap->makeHeightAreaBlocked( game->gameMap->scaledToHeightmapX(
                                                      gsd->position.x),
                                                  game->gameMap->scaledToHeightmapY(gsd->position.z) );
            break;
        ...
        case GS_CMD_HIDESKYMODEL:
            game->gameUI->getTerrain()->GetTerrain()->getRenderer().enableFeature(
                IStorm3D_TerrainRenderer::RenderSkyModel,
                false);
            break;
        ...
        case GS_CMD_SAVEGAMETYPE:
            if (stringData != NULL)
                ...
                sp->warning(
                    "GameScripting::process - savegameType parameter missing, expected 'savegame' or 'newgame'.");
            break;
        ...
        case GS_CMD_SAVEGAMEVERSION:
            if (stringData != NULL)
                ...
                sp->warning(
                    "GameScripting::process - savegameVersion parameter missing, expected savegame version string.");
            break;
        ...
        case GS_CMD_SAVEGAMEDESCRIPTION:
            if (stringData != NULL)
                ...
                sp->warning(
                    "GameScripting::process - savegameDescription parameter missing, expected savegame description string.");
            break;
        ...
        case GS_CMD_SAVEGAMETIME:
            if (stringData != NULL)
                ...
                sp->warning(
                    "GameScripting::process - savegameDescription parameter missing, expected savegame time string.");
            break;
                ...
                if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                    ...
                    if (std::string(stringData).empty() == false) {
                        ...
                        if ( SimpleOptions::getBool(DH_OPT_B_1ST_PLAYER_ENABLED + c) ) {
                            ...
                            SimpleOptions::setInt( DH_OPT_I_1ST_PLAYER_CONTROL_SCHEME + c,
        ...
        case GS_CMD_printStringValueToConsole:
            if (game->gameUI->getErrorWindow() != NULL) {
                ...
                sp->warning(
                    "GameScripting::process - Attempt to printStringValueToConsole when no error window exists.");
            }
            break;
        ...
        case GS_CMD_saveGridOcclusionCulling:
            if (stringData != NULL) {
                if (game->gameUI->getGridOcclusionCuller() != NULL)
                    ...
                    sp->error(
                        "MiscScripting::process - saveGridOcclusionCulling, no grid occlusion culler available (called at menus?).");
            } else {
                sp->error(
                    "MiscScripting::process - saveGridOcclusionCulling, grid occlusion filename parameter expected.");
            }
            break;
        ...
        case GS_CMD_loadGridOcclusionCulling:
            if (stringData != NULL) {
                if (game->gameUI->getGridOcclusionCuller() != NULL)
                    ...
                    sp->error(
                        "MiscScripting::process - loadGridOcclusionCulling, no grid occlusion culler available (called at menus?).");
            } else {
                sp->error(
                    "MiscScripting::process - loadGridOcclusionCulling, grid occlusion filename parameter expected.");
            }
            break;
        ...
        case GS_CMD_applyGridOcclusionCulling:
                ...
                && game->gameUI->getTerrain() != NULL)
            {
                ...
                if ( game->gameUI->getGridOcclusionCuller()->isWellInScaledBoundaries(camPos.x, camPos.z) ) {
                    ...
                    sp->warning(
                        "MiscScripting::process - applyGridOcclusionCulling, camera outside occlusion culling area (using default area).");
                }
                game->gameUI->getTerrain()->updateOcclusionForAllObjects(
                    game->gameUI->getGridOcclusionCuller(), cameraArea);
            } else {
                sp->error(
                    "MiscScripting::process - applyGridOcclusionCulling, no grid occlusion culler or terrain available (called at menus?).");
            }
            break;
        ...
        case GS_CMD_previewGridOcclusionCullingByValue:
            if (game->gameUI->getGridOcclusionCuller() != NULL) {
                ...
                sp->error(
                    "MiscScripting::process - previewGridOcclusionCullingByValue, no grid occlusion culler available (called at menus?).");
            }
            break;
            ...
            if (stringData != NULL) {
                ...
                if (splitted.size() == PHYSICS_MAX_COLLISIONGROUPS + 1) {
                    ...
                    if (row >= 0 && row < PHYSICS_MAX_COLLISIONGROUPS)
                        ...
                        sp->error(
                            "GameScripting::process - physSetGroupColl/Cont parameter invalid, parameter does not start with a valid collision group value.");
                } else {
                    sp->error(
                        "GameScripting::process - physSetGroupColl/Cont parameter invalid, wrong number of entries.");
                }
        ...
        case GS_CMD_setElaborateHintStyle:
            if (game && game->gameUI && game->gameUI->getCombatWindow(gsd->player) &&
                ...
                stringData != NULL)
                ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(gsd->player)->getSubWindow(
                      "ElaborateHintMessageWindow") )->setStyle(stringData);
            else
                Logger::getInstance()->warning(
                    "MiscScripting::process - setElaborateHintStyle failed, probably because there's no ElaborateHintMessageWindow");
        ...
        case GS_CMD_elaborateHint:
                ...
                stringData != NULL)
            {
                ...
                ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(gsd->player)->getSubWindow(
                      "ElaborateHintMessageWindow") )->showMessage( convertLocaleSubtitleString(usestr) );
            } else {
                Logger::getInstance()->warning(
                    "MiscScripting::process - elaborateHint failed, probably because there's no ElaborateHintMessageWindow");
            }
        ...
        case GS_CMD_getUnifiedHandleObjectPosition:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) )
                    ...
                    sp->error(
                        "MiscScripting::process - getUnifiedHandleObjectPosition, object does not exist with given unified handle.");
            } else {
        ...
        case GS_CMD_setUnifiedHandleObjectPosition:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) )
                    ...
                    sp->error(
                        "MiscScripting::process - setUnifiedHandleObjectPosition, object does not exist with given unified handle.");
            } else {
        ...
        case GS_CMD_getUnifiedHandleObjectCenterPosition:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) )
                    ...
                    sp->error(
                        "MiscScripting::process - getUnifiedHandleObjectCenterPosition, object does not exist with given unified handle.");
            } else {
        ...
        case GS_CMD_setUnifiedHandleObjectCenterPosition:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) )
                    ...
                    sp->error(
                        "MiscScripting::process - setUnifiedHandleObjectCenterPosition, object does not exist with given unified handle.");
            } else {
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                    ...
                    *lastValue = (int)diffVec.GetLength();
                    ...
                    sp->error(
                        "MiscScripting::process - getUnifiedHandleObjectDistanceToPosition, object does not exist with given unified handle.");
                }
            } else {
        ...
        case GS_CMD_getUnifiedHandleObjectDistanceToPositionFloat:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                    ...
                    sp->error(
                        "MiscScripting::process - getUnifiedHandleObjectDistanceToPositionFloat, object does not exist with given unified handle.");
                }
            } else {
                sp->error(
                    "MiscScripting::process - getUnifiedHandleObjectDistanceToPositionFloat, invalid unified handle.");
            }
            break;
        ...
        case GS_CMD_getEngineMetaValueInt:
            if (stringData != NULL) {
                if ( !EngineMetaValues::doesMetaValueExist(stringData) )
                    ...
                    sp->warning(
                        "MiscScripting::process - getEngineMetaValueInt, requested meta value did not exist (define it before use to remove this warning).");
                else
                    if ( !EngineMetaValues::isMetaValueTypeInt(stringData) )
                        sp->warning(
                            "MiscScripting::process - getEngineMetaValueInt, requested meta value is not int type (should use a meta key as single type only).");
        ...
        case GS_CMD_setEngineMetaValueInt:
            if (stringData != NULL) {
                if ( !EngineMetaValues::doesMetaValueExist(stringData) )
                    sp->warning(
                        "MiscScripting::process - setEngineMetaValueInt, requested meta value did not exist (define it before use to remove this warning).");
                if ( !EngineMetaValues::isMetaValueTypeInt(stringData) )
                    sp->warning(
                        "MiscScripting::process - getEngineMetaValueInt, requested meta value is not int type (should use a meta key as single type only).");
        ...
        case GS_CMD_defineEngineMetaValueInt:
            if (stringData != NULL) {
                if ( !EngineMetaValues::doesMetaValueExist(stringData) )
                    ...
                    if ( !EngineMetaValues::isMetaValueTypeInt(stringData) )
                        sp->warning(
                            "MiscScripting::process - defineEngineMetaValueInt, requested meta value already exists with different type (should use a meta key as single type only).");
            } else {
        ...
        case GS_CMD_getControllerSceneSelectionUnifiedHandle:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                ...
                sp->error(
                    "MiscScripting::process - getControllerSceneSelectionUnifiedHandle last value (controller number) out of range.");
                sp->debug( int2str(*lastValue) );
                *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_openCombatSubWindowWithText: {
            ...
            if ( stringData != NULL && game->gameUI->getCombatWindow(gsd->player) ) {
                ...
                if (textw != NULL) {
                    ...
                    sp->error(
                        "MiscScripting::process - openCombatSubWindowWithText cannot be used for non-text windows.");
                }
                break;
            }
#endif
