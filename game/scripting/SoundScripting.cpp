                                 ...
                                 GameScriptData *gsd, Game *game)
    {
        ...
        case GS_CMD_PLAYSOUNDEFFECT:
            if (stringData != NULL) {
                ...
                game->gameUI->playSoundEffect(stringData,
                                              pos.x,
                                              pos.y,
                                              pos.z,
                                              false,
        ...
        case GS_CMD_PLAYSPEECH:
            if (stringData != NULL) {
                VC3 pos = gsd->position;
                game->gameUI->playSpeech(convertLocaleSpeechString(
                                             stringData), pos.x, pos.y, pos.z, false, DEFAULT_SPEECH_VOLUME);
            } else {
            ...
            if (stringData != NULL) {
                ...
                for (int i = 0; i < PLAYLIST_AMOUNT; i++) {
                    ...
                    if (strncmp(stringData, playlistNames[i], pllen) == 0
                        && stringData[pllen] == ',')
                    {
                        game->gameUI->getMusicPlaylist(game->singlePlayerNumber)->loadPlaylist(i,
                                                                                               &stringData[pllen + 1]);
                        paramok = true;
                        break;
                    }
        ...
        case GS_CMD_SETAMBIENTSOUNDCLIPTOVALUE: {
            if (intData <= 0 || intData > 15)
                sp->warning(
                    "SoundScripting::process - setAmbientSoundClipToValue parameter out of expected range (probably an erronous value).");
            ui::AmbientSoundManager *asman = game->gameUI->getAmbientSoundManager();
        ...
        case GS_CMD_MAKEAMBIENTSOUND:
            if (stringData != NULL) {
                ...
                sp->error(
                    "SoundScripting::process - makeAmbientSound parameter missing, sound definition string expected.");
            }
            break;
        ...
        case GS_CMD_STARTAMBIENTSOUNDBYNAME:
            if (stringData != NULL) {
                ...
                sp->error(
                    "SoundScripting::process - startAmbientSoundByName parameter missing, ambient sound name expected.");
            }
            break;
        ...
        case GS_CMD_STOPAMBIENTSOUNDBYNAME:
            if (stringData != NULL) {
                ...
                sp->error(
                    "SoundScripting::process - stopAmbientSoundByName parameter missing, ambient sound name expected.");
            }
            break;
        ...
        case GS_CMD_STOPAMBIENTSOUNDIMMEDIATELYBYNAME:
            if (stringData != NULL) {
                ...
                sp->error(
                    "SoundScripting::process - stopAmbientSoundImmediatelyByName parameter missing, ambient sound name expected.");
            }
            break;
        ...
        case GS_CMD_PLAYSOUNDEFFECTWITHVOLUME:
            if (stringData != NULL) {
                ...
                game->gameUI->playSoundEffect(stringData,
                                              ...
                                              pos.z,
                                              false,
                                              SoundScripting::soundScriptVolume,
                                              range,
                                              DEFAULT_SOUND_PRIORITY_NORMAL);
            } else {
                sp->error(
                    "SoundScripting::process - playSoundEffectWithVolume parameter missing, sound filename expected.");
            }
            break;
        ...
        case GS_CMD_LOADAMBIENTPLAYLIST:
            if (stringData != NULL)
                ...
                sp->error(
                    "SoundScripting::process - loadAmbientPlaylist parameter missing, playlist filename expected.");
            break;
        ...
        case GS_CMD_SETAMBIENTOUTDOORAREA:
            if (stringData != NULL)
                game->gameUI->getAmbientAreaManager()->setArea( sfx::AmbientAreaManager::Outdoor,
        ...
        case GS_CMD_SETAMBIENTINDOORAREABYSTRINGVALUE:
            if (gsd->stringValue != NULL)
                game->gameUI->getAmbientAreaManager()->setArea( sfx::AmbientAreaManager::Indoor,
                                                                std::string(gsd->stringValue) );
            else
        ...
        case GS_CMD_setEaxOutdoorArea:
            if (stringData)
                game->gameUI->getAmbientAreaManager()->setEaxArea( sfx::AmbientAreaManager::Outdoor,
                                                                   std::string(stringData) );
            else
        ...
        case GS_CMD_setEaxIndoorArea:
            if (stringData)
                game->gameUI->getAmbientAreaManager()->setEaxArea( sfx::AmbientAreaManager::Indoor,
                                                                   std::string(stringData) );
            else
        ...
        case GS_CMD_playSoundEffectFromStringValue: {
            const char *parse_error =
                "SoundScripting::process - Parse error: string value must be in format \"file,loop,volume,range\".";
            ...
            game->gameUI->playSoundEffect(
                splitParams[0].c_str(), pos.x, pos.y, pos.z, loop, volume, (float)range, DEFAULT_SOUND_PRIORITY_NORMAL);
        }
        break;
