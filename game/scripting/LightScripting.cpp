                                 ...
                                 GameScriptData *gsd, Game *game)
    {
        ...
        case GS_CMD_GETPOSITIONLIGHTAMOUNT:
            if (game->inCombat)
                {
                    *lastValue =
                        int(util::LightAmountManager::getInstance()->getLightAmount(gsd->position, 1.5f) * 100.0f);
                }
        ...
        case GS_CMD_GETPOSITIONDYNAMICLIGHTAMOUNT:
            if (game->inCombat) {
                ...
                *lastValue =
                    int(util::LightAmountManager::getInstance()->getDynamicLightAmount(gsd->position, visData,
                                                                                       1.5f) * 100.0f);
            }
            break;
        ...
        case GS_CMD_GETPOSITIONSTATICLIGHTAMOUNT:
            if (game->inCombat) {
                if ( game->gameMap->isWellInScaledBoundaries(gsd->position.x, gsd->position.z) )
                    *lastValue =
                        int(util::LightAmountManager::getInstance()->getStaticLightAmount(gsd->position, 1.5f) * 100.0f);
                else
                    *lastValue = 0;
            }
            break;
        ...
        case GS_CMD_GETPOSITIONDYNAMICLIGHTORIGINUNIT:
            if (game->inCombat) {
                IVisualObjectData *visData;
                float lightAmount = util::LightAmountManager::getInstance()->getDynamicLightAmount(gsd->position,
                                                                                                   visData,
                                                                                                   1.5f);
            ...
            if (game->inCombat) {
                if (stringData != NULL) {
                    if (gsd->stringValue != NULL) {
                        ...
                        sp->error(
                            "LightScripting::process - swapTextureToStringValue attempt to swap for null string value.");
                    }
                } else {
        ...
        case GS_CMD_SETAMBIENTLIGHTTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f) {
                    ...
                    sp->error(
                        "LightScripting::process - setAmbientLight or setAmbientLightToValue parameter value out of range.");
                }
            }
            break;
        ...
        case GS_CMD_SETFORCEAMBIENTLIGHTTOVALUE:
            if (game->inCombat) {
                ...
                if (command == GS_CMD_SETFORCEAMBIENTLIGHT)
                    ...
                    val = float(*lastValue) / 255.0f;
                if (val >= 0 && val <= 1.0f)
                    game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                        IStorm3D_TerrainRenderer::ForceAmbient,
                        val);
                else
                    sp->error(
                        "LightScripting::process - setAmbientLight or setAmbientLightToValue parameter value out of range.");
        ...
        case GS_CMD_SETAMBIENTLIGHTREDTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f) {
                    ...
                    sp->error(
                        "LightScripting::process - setAmbientLightRed or setAmbientLightRedToValue parameter value out of range.");
                }
            }
            break;
        ...
        case GS_CMD_SETAMBIENTLIGHTGREENTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f) {
                    ...
                    sp->error(
                        "LightScripting::process - setAmbientLightGreen or setAmbientLightGreenToValue parameter value out of range.");
                }
            }
            break;
        ...
        case GS_CMD_SETAMBIENTLIGHTBLUETOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f) {
                    ...
                    sp->error(
                        "LightScripting::process - setAmbientLightBlue or setAmbientLightBlueToValue parameter value out of range.");
                }
            }
            break;
        ...
        case GS_CMD_ADDLIGHT: {
            ...
            if (lightscripting_spot_props.type != SpotProperties::ShadowCaster || lightscripting_spot_props.shadow)
                game->gameUI->getLightManager()->addSpot(lightpos,
                                                         lightscripting_light_angle_y,
                                                         lightscripting_spot_props);
        ...
        case GS_CMD_SETLIGHTMAPBRIGHTNESSTOVALUE: {
            ...
            game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                IStorm3D_TerrainRenderer::LightmapMultiplier,
                val);
        ...
        case GS_CMD_SETOUTDOORLIGHTMAPBRIGHTNESSTOVALUE: {
            ...
            if (command == GS_CMD_SETOUTDOORLIGHTMAPBRIGHTNESSTOVALUE)
                ...
                val = (float(intData) / 100.0f);
            game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                IStorm3D_TerrainRenderer::OutdoorLightmapMultiplier,
                val);
        ...
        case GS_CMD_SETLIGHTMAPBRIGHTNESSREDTOVALUE: {
            ...
            if (command == GS_CMD_SETLIGHTMAPBRIGHTNESSREDTOVALUE)
                ...
                val = (float(intData) / 100.0f);
            COL col = game->gameUI->getTerrain()->GetTerrain()->getRenderer().getColorValue(
                IStorm3D_TerrainRenderer::LightmapMultiplierColor);
            col.r = val;
            game->gameUI->getTerrain()->GetTerrain()->getRenderer().setColorValue(
                IStorm3D_TerrainRenderer::LightmapMultiplierColor,
                col);
        ...
        case GS_CMD_SETBLINKINGLIGHT1TOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight1 or setBlinkingLight1ToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHTDEFAULTTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLightDefault or setBlinkingLightDefaultToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT1REDTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight1Red or setBlinkingLight1RedToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT1GREENTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight1Green or setBlinkingLight1GreenToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT1BLUETOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight1Blue or setBlinkingLight1BlueToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT2TOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight2 or setBlinkingLight2ToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT2REDTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight2Red or setBlinkingLight2RedToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT2GREENTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight2Green or setBlinkingLight2GreenToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHT2BLUETOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLight2Blue or setBlinkingLight2BlueToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHTRANDOMTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLightRandom or setBlinkingLightRandomToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHTSTRENGTHTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLightStrength or setBlinkingLightStrengthToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_SETBLINKINGLIGHTFREQUENCYTOVALUE:
            if (game->inCombat) {
                ...
                if (val >= GAME_TICK_MSEC)
                    ...
                    sp->error(
                        "LightScripting::process - setBlinkingLightFrequency or setBlinkingLightFrequencyToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight1ToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight1 or setBlinkingLight1ToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightDefaultToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightDefault or setBlinkingLightDefaultToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight1RedToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight1Red or setBlinkingLight1RedToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight1GreenToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight1Green or setBlinkingLight1GreenToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight1BlueToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight1Blue or setBlinkingLight1BlueToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight2ToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight2 or setBlinkingLight2ToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight2RedToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight2Red or setBlinkingLight2RedToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight2GreenToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight2Green or setBlinkingLight2GreenToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLight2BlueToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLight2Blue or setBlinkingLight2BlueToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightRandomToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightRandom or setBlinkingLightRandomToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightStrengthToValue:
            if (game->inCombat) {
                ...
                if (val >= 0 && val <= 1.0f)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightStrength or setBlinkingLightStrengthToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightFrequencyToValue:
            if (game->inCombat) {
                ...
                if (val >= GAME_TICK_MSEC)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightFrequency or setBlinkingLightFrequencyToValue parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightPauseTime:
            if (game->inCombat) {
                if (intData >= 0)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightPauseTime parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightPauseVariation:
            if (game->inCombat) {
                if (intData >= 0)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightPauseVariation parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setOutdoorBlinkingLightRandomTime:
            if (game->inCombat) {
                if (intData >= 0)
                    ...
                    sp->error(
                        "LightScripting::process - setOutdoorBlinkingLightRandomTime parameter value out of range.");
            }
            break;
        ...
        case GS_CMD_setGlowFadeFactor:
            if (game->inCombat) {
                float floatData = intFloat.f;
                game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                    IStorm3D_TerrainRenderer::GlowFactor,
                    floatData);
            }
            break;
        ...
        case GS_CMD_setGlowTransparencyFactor:
            if (game->inCombat) {
                float floatData = intFloat.f;
                game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                    IStorm3D_TerrainRenderer::GlowTransparencyFactor,
                    floatData);
            }
            break;
        ...
        case GS_CMD_setGlowAdditiveFactor:
            if (game->inCombat) {
                float floatData = intFloat.f;
                game->gameUI->getTerrain()->GetTerrain()->getRenderer().setFloatValue(
                    IStorm3D_TerrainRenderer::GlowAdditiveFactor,
                    floatData);
            }
            break;
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                    if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                        ...
                        sp->error(
                            "LightScripting::process - changeLightPosition, light with given unified handle does not exist.");
                    }
                } else {
                    sp->error(
                        "LightScripting::process - changeLightPosition, given unified handle is not of light type.");
                }
            } else {
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                    if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                        ...
                        sp->error(
                            "LightScripting::process - changeLightRadiusToValue, light with given unified handle does not exist.");
                    }
                } else {
                    sp->error(
                        "LightScripting::process - changeLightRadiusToValue, given unified handle is not of light type.");
                }
            } else {
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                    if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                        ...
                        sp->error(
                            "LightScripting::process - changeLightRadiusToFloatValue, light with given unified handle does not exist.");
                    }
                } else {
                    sp->error(
                        "LightScripting::process - changeLightRadiusToFloatValue, given unified handle is not of light type.");
                }
            } else {
                    ...
                    if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                        if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                            ...
                            if (success) {
                                ...
                                sp->error(
                                    "LightScripting::process - changeLightColor, invalid parameter (light r,g,b value expected).");
                            }
                        } else {
                            sp->error(
                                "LightScripting::process - changeLightColor, light with given unified handle does not exist.");
                        }
                    } else {
                        sp->error(
                            "LightScripting::process - changeLightColor, given unified handle is not of light type.");
                    }
                } else {
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                    if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                        ...
                        sp->error(
                            "LightScripting::process - deleteLight, light with given unified handle does not exist.");
                    }
                } else {
        ...
        case GS_CMD_getNextLightUnifiedHandle:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_LIGHT(gsd->unifiedHandle) ) {
                    ...
                    sp->error(
                        "LightScripting::process - getNextLightUnifiedHandle, given unified handle is not of light type.");
        ...
        case GS_CMD_spotTypeSetTexture:
            if (stringData != NULL) {
                ...
                if (spotProps != NULL)
                    ...
                    sp->error(
                        "LightScripting::process - spotType..., not currently adding a spot type (use spotTypeAdd first).");
            } else {
            ...
            if (stringData != NULL) {
                ...
                if (spotProps != NULL) {
                    if (strcmp(stringData, "none") == 0)
                        ...
                        sp->error(
                            "LightScripting::process - spotTypeSetType, bad parameter (expected directional, point or flat).");
                } else {
                    sp->error(
                        "LightScripting::process - spotType..., not currently adding a spot type (use spotTypeAdd first).");
                }
                ...
                sp->error(
                    "LightScripting::process - spotTypeSetType, parameter missing (spotlight light model type expected).");
            }
            break;
        ...
        case GS_CMD_spotTypeSetDirection:
            if (stringData != NULL) {
                ...
                sp->error(
                    "LightScripting::process - spotTypeSetDirection, parameter missing (direction vector expected).");
            }
            break;
            ...
            if (stringData != NULL) {
                ...
                if (spotProps != NULL) {
                    ...
                    if ( gs_tricoord_param(stringData, &result) ) {
                        ...
                        sp->error(
                            "LightScripting::process - spotTypeSetColor parameter bad (should be r,g,b in range 0.0 - 1.0).");
                    }
                } else {
                    sp->error(
                        "LightScripting::process - spotType..., not currently adding a spot type (use spotTypeAdd first).");
            ...
            if (stringData != NULL) {
                ...
                if (spotProps != NULL) {
                    ...
                    if ( gs_tricoord_param(stringData, &result) ) {
                        ...
                        sp->error(
                            "LightScripting::process - spotTypeSetFakelightColor parameter bad (should be r,g,b in range 0.0 - 1.0).");
                    }
                } else {
                    sp->error(
                        "LightScripting::process - spotType..., not currently adding a spot type (use spotTypeAdd first).");
        ...
        case GS_CMD_spotTypeSetFlashType:
            if (stringData != NULL) {
                ...
                if (spotProps != NULL) {
                    if (strcmp(stringData, "none") == 0) {
                        ...
                        if (s != NULL) {
                            ...
                            sp->error(
                                "LightScripting::process - spotTypeSetType, bad parameter (expected predefined spotlight flash type or name of loaded script).");
                        }
                    }
                } else {
                    sp->error(
                ...
                sp->error(
                    "LightScripting::process - spotTypeSetFlashType, parameter missing (spotlight flash type expected).");
            }
            break;
        ...
        case GS_CMD_spotTypeSetLowDetailVersion:
            if (stringData != NULL) {
                ...
                sp->error(
                    "LightScripting::process - spotTypeSetLowDetailVersion, parameter missing (spotlight type expected).");
            }
            break;
        ...
        case GS_CMD_spotTypeSetPositionOffset:
            if (stringData != NULL) {
                ...
                sp->error(
                    "LightScripting::process - spotTypeSetPositionOffset, parameter missing (direction vector expected).");
            }
            break;
