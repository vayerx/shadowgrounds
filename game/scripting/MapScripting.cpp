...
namespace game
{
    ...
    struct Portal {
        ...
        Portal(const std::string &fromLayer_,
               const std::string &toLayer_,
               const VC3         &fromPosition_,
               const VC3         &toPosition_)
                               ...
                               GameScriptData *gsd, Game *game)
    {
        ...
        case GS_CMD_ADDSECONDARYMISSIONOBJECTIVE:
            if (stringData != NULL) {
                if (maps_objective_position.x == 0
                    ...
                    sp->warning(
                        "MapScripting::process - addActive/Primary/SecondaryMissionObjective, objective position was not set?");

#ifndef PROJECT_SURVIVOR
                if (maps_objective_range < 0.0f || maps_objective_range > 1000.0f) {
                    sp->error(
                        "MapScripting::process - addActive/Primary/SecondaryMissionObjective, objective range out of bounds.");
                    break;
                }
#endif
                ...
                if ( game->gameUI->getMapWindow()->doesMissionObjectiveExist(idstr) ) {
                    sp->error(
                        "MapScripting::process - addActive/Primary/SecondaryMissionObjective, objective with that name already exists.");
                    break;
                }
                ...
                if (maps_objective_range > 0.0f && maps_objective_range < 1000.0f) {
                    game->gameUI->getMapWindow()->setObjectivePoint(idstr,
                                                                    maps_objective_position,
                                                                    maps_objective_range);
...
#ifdef GUI_BUILD_MAP_WINDOW
            if (game->gameUI->getMapWindow() != NULL)
                game->gameUI->getMapWindow()->addPortal(maps_portal_from_layer,
                                                        maps_portal_to_layer,
                                                        maps_portal_origin,
                                                        maps_portal_destination);
            else
                portalCache.push_back( Portal(maps_portal_from_layer, maps_portal_to_layer, maps_portal_origin,
                                              maps_portal_destination) );
#else
            sp->warning("MapScripting::process - Map window not in build.");
#endif
...
#ifdef GUI_BUILD_MAP_WINDOW
            if (game->gameUI->getMapWindow() != NULL)
                game->gameUI->getMapWindow()->removePortal(maps_portal_from_layer,
                                                           maps_portal_to_layer,
                                                           maps_portal_origin,
                                                           maps_portal_destination);
        ...
        case GS_CMD_renderMapToFile: {
            ...
            if ( !SimpleOptions::getBool(DH_OPT_B_MAPRENDER_USE_USER_RESOLUTION) ) {
                ...
                OptionApplier::applyDisplayOptions(game->gameUI->getStorm3D(),
                                                   game->gameUI->getStormScene(),
                                                   game->gameUI->getStormScene()->ITTerrain->Begin()->GetCurrent(),
                                                   NULL);
                ...
                for (int x = mapBlocksY - 1; x >= 0; x--) {
                    for (int y = 0; y < mapBlocksX; y++) {
                        ...
                        game->gameUI->getStorm3D()->RenderSceneToArray(
                            game->gameUI->getStormScene(), mapBlocks[l++], 512, 512);
                    }
                }
                ...
                IStorm3D_Texture *static_texture = game->gameUI->getStorm3D()->CreateNewTexture(
                    width,
                    height,
                    IStorm3D_Texture::
                    TEXTYPE_BASIC);
