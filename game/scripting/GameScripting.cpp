...
// (some seem to get stuck waiting for path complete???)
        ...
        while ( iter.iterateAvailable() ) {
            ...
            if (u == NULL) {
                Logger::getInstance()->error(
                    "GameScripting::makeAlert - Fatal internal error. Unit list contains null unit pointer.");
                        ...
                        if ( ai != NULL && ai->isThisAndAllEnabled() ) {
                            ...
                            while ( !sp->isFinished() ) {
                                ...
                                if (failCount > ALERTED_SUB_FAIL_LIMIT) {
                                    sp->warning(
                                        "GameScripting::makeAlert - Script sub \"alerted\" did not finish within defined limit.");
                                    sp->warning(
                                        "GameScripting::makeAlert - Script process will be abnormally terminated.");
                                    break;
                                }
                            }
                            if (gsd != NULL)
    ...
    int GameScripting::runTrackerScript(const char             *scriptname,
                                        const char             *subname,
    ...
    int GameScripting::runOtherScriptForUnifiedHandle(const char   *scriptname,
                                                      const char   *subname,
                                                      UnifiedHandle uh,
                                                      const VC3    &position)
    {
        ...
        if (scriptname != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > OTHER_SUBS_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runOtherScriptForUnifiedHandle - Script sub did not finish within defined limit.");
                    sp->warning(
                        "GameScripting::runOtherScriptForUnifiedHandle - Script process will be abnormally terminated.");
                    sp->debug(scriptname);
                    sp->debug(subname);
                    break;
                }
        ...
        if (unit->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > EXECUTE_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runExecuteScript - Script sub \"execute\" did not finish within defined limit.");
    ...
    void GameScripting::runHitMissScript(Unit *unit, Unit *shooter)
    {
        if (unit->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > HIT_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runHitMissScript - Script sub \"hitmiss\" did not finish within defined limit.");
        ...
        if (unit->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > POINTED_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runPointedScript - Script sub \"pointed\" did not finish within defined limit.");
        ...
        if (unit->getScript() != NULL) {
            ...
            if (sp == NULL) {
                Logger::getInstance()->warning(
                    "GameScripting::runEventScript - Could not start event script process (requested script/sub does not exist?).");
    ...
    bool GameScripting::runItemScriptImpl(Unit *unit, Item *item, const char *subName, bool ignoreMissingSub)
    {
        ...
        if (itemType->getScript() != NULL) {
            ...
            if (s == NULL) {
                Logger::getInstance()->warning(
                    "GameScripting::runItemScriptImpl - Script not loaded or does not exist.");
            ...
            if ( ignoreMissingSub && !s->hasSub(subName) ) {
                Logger::getInstance()->debug(
                    "GameScripting::runItemScriptImpl - Item script sub does not exist (but ignoring that).");
                Logger::getInstance()->debug(scriptName);
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > ITEM_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runItemScript - Script sub \"pickup\" did not finish within defined limit.");
    ...
    void GameScripting::runItemUseScript(Unit *unit, Item *item)
    {
        ...
        if (itemType->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > ITEM_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runItemUseScript - Script sub \"useitem\" did not finish within defined limit.");
    ...
    void GameScripting::runHearNoiseScript(Unit *unit, Unit *noisy)
    {
        if (unit->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > HIT_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runHearNoiseScript - Script sub \"hearnoise\" did not finish within defined limit.");
    ...
    void GameScripting::runSpottedScript(Unit *unit, Unit *spotted)
    {
        if (unit->getScript() != NULL) {
            ...
            while ( !sp->isFinished() ) {
                ...
                if (failCount > HIT_SUB_FAIL_LIMIT) {
                    sp->warning(
                        "GameScripting::runSpottedScript - Script sub \"spotted\" did not finish within defined limit.");
    ...
    ScriptProcess *GameScripting::startUnitScript(Unit                   *unit,
                                                  const char             *script,
                                                  const char             *sub,
                                                  const std::vector<int> *paramStack)
    {
        assert(unit != NULL);
        assert(script != NULL);
        assert(sub != NULL);
    ...
    ScriptProcess *GameScripting::startNonUnitScript(const char             *script,
                                                     const char             *sub,
                                                     const std::vector<int> *paramStack)
    {
        assert(script != NULL);
...
//        assert(s != NULL);
    ...
    void GameScripting::setGlobalIntVariableValue(const char *variablename, int value)
    {
        ...
        if (!success) {
            Logger::getInstance()->warning(
                "GameScripting::setGlobalIntVariableValue - Global variable with given name not found.");
    ...
    int GameScripting::getGlobalIntVariableValue(const char *variablename)
    {
        ...
        if (!success) {
            Logger::getInstance()->warning(
                "GameScripting::getGlobalIntVariableValue - Global variable with given name not found.");
            Logger::getInstance()->debug(variablename);
        }
        return ret;
    }
    ...
    void GameScripting::addAreaTrigger(Unit             *unit,
                                       const VC3        &position,
                                       float             range,
                                       int               clipMask,
                                       const LinkedList *unitsToTrack)
    {
        //char buf[256];
    ...
    void GameScripting::runHitChainScript(const char *scriptname,
                                          Projectile *origin,
                                          Unit       *hitUnit,
                                          Unit       *shooter,
                                          Bullet     *chainBullet,
                                          ...
                                          const VC3  &hitPlaneNormal)
    {
        ...
        while ( !sp->isFinished() ) {
            ...
            if (failCount > EXECUTE_SUB_FAIL_LIMIT) {
                sp->warning(
                    "GameScripting::runHitChainScript - Script sub \"hitchain\" did not finish within defined limit.");
    ...
    void GameScripting::runSingleCommand(int             command,
                                         floatint        intFloat,
    ...
    void GameScripting::runSingleSimpleCommand(int         command,
                                               floatint    intFloat,
                                               const char *stringData,
                                               int        *lastValue,
                                               int        *secondaryValue)
    ...
    void GameScripting::runMultipleSimpleCommands(int          commandAmount,
                                                  int         *command,
                                                  floatint    *intFloat,
                                                  const char **stringData,
                                                  int         *lastValue,
    ...
    bool GameScripting::runSingleSimpleStringCommand(const char *command,
                                                     const char *param,
                                                     int        *lastValue,
                                                     int        *secondaryValue)
    {
        ...
        if (gs_datatypes[cmdid] == SCRIPT_DATATYPE_NONE) {
            ...
            if (param != NULL) {
                ...
                if (str2int_errno() != 0)
                    Logger::getInstance()->error(
                        "GameScripting::runSingleSimpleStringCommand - Command expected int, but given parameter is not a valid int value.");
            } else {
                tmp.i = 0;
                Logger::getInstance()->error(
                    "GameScripting::runSingleSimpleStringCommand - Command expected int, but no parameter given.");
            }
            ...
            if (param != NULL) {
                ...
                if ( (param[0] < '0' || param[0] > '9') && param[0] != '-' )
                    Logger::getInstance()->error(
                        "GameScripting::runSingleSimpleStringCommand - Command expected float, but given parameter is not a valid float value.");
            } else {
                Logger::getInstance()->error(
                    "GameScripting::runSingleSimpleStringCommand - Command expected float, but no parameter given.");
            }
            floatint tmp;
            tmp.f = tmpfloat;
            runSingleSimpleCommand(cmdid, tmp, param, lastValue, secondaryValue);
            ...
            Logger::getInstance()->error(
                "GameScripting::runSingleSimpleStringCommand - Unsupported command parameter type.");
            return false;
        }
    ...
    bool GameScripting::runMultipleSimpleStringCommands(int          commandAmount,
                                                        const char **command,
                                                        const char **param,
                                                        int         *lastValue,
                                                        int         *secondaryValue)
    {
        ...
        for (int i = 0; i < commandAmount; i++) {
            ...
            if (cmdid == -1) {
                Logger::getInstance()->error(
                    "GameScripting::runMultipleSimpleStringCommands - No command of given name.");
            ...
            if (gs_datatypes[cmdid] == SCRIPT_DATATYPE_NONE) {
                ...
                if (param[i] != NULL) {
                    ...
                    if (str2int_errno() != 0)
                        Logger::getInstance()->error(
                            "GameScripting::runMultipleSimpleStringCommands - Command expected int, but given parameter is not a valid int value.");
                } else {
                    tmp.i = 0;
                    Logger::getInstance()->error(
                        "GameScripting::runMultipleSimpleStringCommands - Command expected int, but no parameter given.");
                }
                ...
                if (param != NULL) {
                    ...
                    if ( (param[i][0] < '0' || param[i][0] > '9') && param[i][0] != '-' )
                        Logger::getInstance()->error(
                            "GameScripting::runMultipleSimpleStringCommands - Command expected float, but given parameter is not a valid float value.");
                } else {
                    Logger::getInstance()->error(
                        "GameScripting::runMultipleSimpleStringCommands - Command expected float, but no parameter given.");
                }
                floatint tmp;
                tmp.f = tmpfloat;
                runSingleSimpleCommand(cmdid, tmp, param[i], lastValue, secondaryValue);
                ...
                Logger::getInstance()->error(
                    "GameScripting::runMultipleSimpleStringCommands - Unsupported command parameter type.");
                return false;
            }
        }
