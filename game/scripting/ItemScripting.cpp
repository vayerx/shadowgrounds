                                ...
                                GameScriptData *gsd, Game *game, bool *pause)
    {
        ...
        case GS_CMD_DISABLEITEM:
            if (intData >= 0) {
                if (intData == 0)
                    sp->debug(
                        "GameScripting::process - disableItem with zero time parameter, thus item will never re-enable.");
                ...
                sp->error(
                    "GameScripting::process - disableItem value out of range, non-negative disable time expected.");
            }
            break;
            ...
            if (stringData != NULL) {
                ...
                if (itemId != -1) {
                    ...
                    if (i != NULL)
                        ...
                        sp->warning(
                            "ItemScripting::process - removeClosestItemOfType, no item of given type found for deletion.");
                } else {
            ...
            if (stringData != NULL) {
                ...
                if (itemId != -1) {
                    ...
                    if (i != NULL)
                        ...
                        sp->warning(
                            "ItemScripting::process - disableClosestItemOfType, no item of given type found for deletion.");
                } else {
            ...
            if (stringData != NULL) {
                ...
                if (itemId != -1) {
                    ...
                    if (i != NULL) {
                        ...
                        sp->warning(
                            "ItemScripting::process - stop/startBlinkingForClosestItemOfType, no item of given type found.");
                    }
                } else {
                    sp->error(
                        "ItemScripting::process - stop/startBlinkingForClosestItemOfType, item type with given name not found.");
                }
            } else {
                sp->error(
                    "ItemScripting::process - stop/startBlinkingForClosestItemOfType parameter missing, item type expected.");
            }
            break;
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (intData / GAME_TICK_MSEC > 0)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressTotalTime parameter invalid, progress total time expected (msec).");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressTotalTime for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (intData / GAME_TICK_MSEC > 0)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressTickTime parameter invalid, progress tick interval expected (msec).");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressTickTime for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (stringData != NULL)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressDoneLabel parameter missing, label text expected.");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressDoneLabel for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (stringData != NULL)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressInterruptedLabel parameter missing, label text expected.");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressInterruptedLabel for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (stringData != NULL)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressBarImage parameter missing, image filename expected.");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressBarImage for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL) {
                    if (stringData != NULL)
                        ...
                        sp->error(
                            "ItemScripting::process - setProgressBarBorderImage parameter missing, image filename expected.");
                } else {
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressBorderImage for item with no progress started.");
                }
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL)
                    *lastValue = itemScriptItem->getProgressBar()->getProgressDone() * GAME_TICK_MSEC;
                    ...
                    sp->error(
                        "ItemScripting::process - Attempt to getProgressDoneTime for item with no progress started.");
            } else {
            ...
            if (itemScriptItem != NULL) {
                if (gsd->unit != NULL) {
                    ...
                    if ( !itemScriptItem->getProgressBar()->isProgressing() )
                        itemScriptItem->getProgressBar()->startProgress(gsd->unit,
                                                                        gsd->unit->getPosition(),
                                                                        gsd->unit->getRotation().y);
                    else
                        sp->debug(
                            "ItemScripting::process - Attempt to startProgress for item with another progress running.");
                } else {
        ...
        case GS_CMD_INTERRUPTPROGRESS:
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL)
                    ...
                    sp->warning(
                        "ItemScripting::process - Attempt to interruptProgress for item with no progress running.");
            } else {
        ...
        case GS_CMD_CHANGEITEMMODELFORCLOSESTITEMOFTYPETOSTRINGVALUE:
            if (stringData != NULL) {
                ...
                sp->error(
                    "ItemScripting::process - changeItemModelForClosestItemOfTypeToStringValue parameter missing, item type name expected.");
            }
            break;
        ...
        case GS_CMD_setItemHighlightTextBySpecialString:
            if (itemScriptItem != NULL)
                itemScriptItem->setHighlightText(
                    itemScriptItem->getSpecialString() ? itemScriptItem->getSpecialString() : NULL);
            else
                sp->error("ItemScripting::process - Attempt to setItemHighlightText outside item script.");
            break;
            ...
            if (itemScriptItem != NULL) {
                if (game != NULL) {
                    if (itemScriptItem->getSpecialString() != NULL)
                        ...
                        sp->error(
                            "ItemScripting::process - Attempt to openTerminalWindowByItemSpecialString for item with null special string.");
                }
            } else {
                sp->error(
                    "ItemScripting::process - Attempt to openTerminalWindowByItemSpecialString outside item script.");
            }
            break;
        ...
        case GS_CMD_openTerminalWindowByStringValue:
            if (game != NULL) {
                if (gsd->stringValue != NULL)
                    ...
                    sp->error(
                        "ItemScripting::process - Attempt to openTerminalWindowByStringValue with null string value.");
            }
            break;
        ...
        case GS_CMD_setProgressInterruptPercent:
            if (itemScriptItem != NULL) {
                if (itemScriptItem->getProgressBar() != NULL)
                    ...
                    sp->error(
                        "ItemScripting::process - Attempt to setProgressInterruptPercent for item with no progress started.");
            } else {
        ...
        case GS_CMD_createItemSpawner:
            if (stringData != NULL) {
                const char *parse_error =
                    "ItemScripting::process - createItemSpawner expects format \"groupname,radius:itemname,respawntime,itemname,respawntime...\".";
                ...
                while (true) {
                    ...
                    if (item_id != -1 && sscanf(respawn_str.c_str(), "%i", &respawn_time) == 1) {
                        game->itemManager->createSpawner(
                            group_name.c_str(), item_id, item_name, GAME_TICKS_PER_SECOND * respawn_time / 1000, pos,
                            radius);
                    } else {
                        sp->error(parse_error);
                        break;
