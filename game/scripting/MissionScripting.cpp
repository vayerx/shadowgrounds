...
namespace game
{
        ...
        case GS_CMD_setPhysicsWaterSettings:
            if (stringData != NULL) {
                ...
                if (splitPos != NULL) {
                    ...
                    sp->error(
                        "GameScripting::process - setPhysicsWaterSettings expects two parameters; \"height,damping\".");
                }
            } else {
                ...
                // not unlocked yet
                    ...
                    if ( game->gameUI->getCombatWindow(0) ) {
                        ...
                        if ( win && SurvivalMenu::loadMissionInfo(name, mi) ) {
                            ...
                            std::string msg =
                                std::string( getLocaleGuiString("gui_elaboratehint_survival_mission_unlocked") )
                                + "<br><h1>" + mission_name + "</h1> ";
                            win->showMessage(msg);
                        }
                    }
    ...
    int MissionScripting::countHostilesInRange(Game      *game,
                                               const VC3 &position,
                                               int        player,
                                               int        range,
                                               bool       consciousOnly)
