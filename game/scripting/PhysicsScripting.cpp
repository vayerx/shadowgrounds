		...
		GameScriptData *gsd, Game *game)
	{
			...
			if (stringData != NULL)
			{
				...
				if (splitted.size() == PHYSICS_MAX_COLLISIONGROUPS + 1)
				{
					...
					if (row >= 0 && row < PHYSICS_MAX_COLLISIONGROUPS)
					{
						...
						sp->error("PhysicsScripting::process - physSetGroupColl/Cont parameter invalid, parameter does not start with a valid collision group value.");
					}
				} else {
					sp->error("PhysicsScripting::process - physSetGroupColl/Cont parameter invalid, wrong number of entries.");
				}
...
#else
			sp->warning("PhysicsScripting::process - physSetGroupColl/Cont not supported by used physics implementation.");
#endif
			break;
					...
					if (game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle))
					{
						...
						if (IS_UNIFIED_HANDLE_TERRAIN_OBJECT(gsd->unifiedHandle))
						{
							...
							else if (physicsscripting_adding_joint_type == "rope")
							{
								...
								sp->error("PhysicsScripting::process - jointAttachToUnifiedHandleObject, joint type unsupported.");
							}
						} else {
							sp->error("PhysicsScripting::process - jointAttachToUnifiedHandleObject, object with given unified handle is not a terrain object.");
						}
					} else {
						sp->error("PhysicsScripting::process - jointAttachToUnifiedHandleObject, unified handle object does not exist.");
					}
				} else {
					sp->error("PhysicsScripting::process - jointAttachToUnifiedHandleObject, invalid unified handle.");
				}
			} else {
				sp->error("PhysicsScripting::process - jointAttachToUnifiedHandleObject, was not adding a joint (jointAddStart expected before jointAttachToUnifiedHandleObject).");
			}
			break;
		...
		case GS_CMD_jointSetParameterName:
			if (stringData != NULL)
			{
				if (physicsscripting_adding_joint)
				{
					...
					{
						sp->error("PhysicsScripting::process - jointSetParameterName, name set command without setting value for previous one first.");
					}
					...
					if (!paramOk)
					{
						sp->error("PhysicsScripting::process - jointSetParameterName parameter bad (parameter name was unknown or invalid for joint type).");
					}
				} else {
					sp->error("PhysicsScripting::process - jointSetParameterName, was not adding a joint (jointAddStart expected before jointSetParameterName).");
				}
			} else {
				sp->error("PhysicsScripting::process - jointSetParameterName parameter missing (joint parameter name expected).");
			}
			break;
		...
		case GS_CMD_jointSetParameterValue:
			if (physicsscripting_joint_param_name_given)
			{
				...
				sp->error("PhysicsScripting::process - jointSetParameterValue, name was not set (jointSetParameterName expected before value).");
			}
		break;
		...
		case GS_CMD_jointAddDone:
			if (physicsscripting_adding_joint)
			{
				...
				sp->error("PhysicsScripting::process - jointAddDone, was not adding a joint (jointAddStart expected before done).");
			}
			break;
		...
		case GS_CMD_setJointSwing1:
			{
				if(stringData)
				{
					if(sscanf(stringData, "%f,%f,%f", &jointParameters.highAngle.y, &jointParameters.highSpring.y, &jointParameters.highDamping.y) != 3)
		...
		case GS_CMD_setJointSwing2:
			{
				if(stringData)
				{
					if(sscanf(stringData, "%f,%f,%f", &jointParameters.highAngle.z, &jointParameters.highSpring.z, &jointParameters.highDamping.z) != 3)
		...
		case GS_CMD_setJointAxis:
			{
				if(stringData)
				{
					if(sscanf(stringData, "%f,%f,%f", &jointParameters.axis.x, &jointParameters.axis.y, &jointParameters.axis.z) != 3)
		...
		case GS_CMD_setJointNormal:
			{
				if(stringData)
				{
					if(sscanf(stringData, "%f,%f,%f", &jointParameters.normal.x, &jointParameters.normal.y, &jointParameters.normal.z) != 3)
		...
		case GS_CMD_setJointPosition:
			{
				if(stringData)
				{
					if(sscanf(stringData, "%f,%f,%f", &jointParameters.position.x, &jointParameters.position.y, &jointParameters.position.z) != 3)
		...
		case GS_CMD_setJointObjectAToUnifiedHandle:
			if (VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle))
			{
				...
				if (IS_UNIFIED_HANDLE_TERRAIN_OBJECT(gsd->unifiedHandle))
				{
					...
					sp->error("PhysicsScripting::process - setJointObjectAToUnifiedHandle, only terrain objects supported.");
				}
			} else {
		...
		case GS_CMD_setJointObjectBToUnifiedHandle:
			if (VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle))
			{
				...
				if (IS_UNIFIED_HANDLE_TERRAIN_OBJECT(gsd->unifiedHandle))
				{
					...
					sp->error("PhysicsScripting::process - setJointObjectBToUnifiedHandle, only terrain objects supported.");
				}
			} else {
				...
				if(stringData)
				{
					if(game->getGamePhysics())
					{
						...
						if(sscanf(stringData, "%f,%f,%f", &info.bendAngle, &info.breakAngle, &info.durability) != 3)
							sp->error("PhysicsScripting::process - invalid parameters for setJointDeforming (expecting \"bendAngle,breakAngle,durability\")");
