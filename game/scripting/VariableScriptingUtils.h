    ...
    public:
                ...
                if (variable->getVariableType() == game::IScriptVariable::VARTYPE_BOOLEAN) {
                    ...
                    *resultValue = variable->getIntValue();
                    ...
                    Logger::getInstance()->warning(
                        "VariableScriptingUtils::getVariableValueToInt - Converting float to int.");
                    *resultValue = (int)variable->getFloatValue();
                } else if (variable->getVariableType() == game::IScriptVariable::VARTYPE_STRING) {
                    Logger::getInstance()->warning(
                        "VariableScriptingUtils::getVariableValueToInt - Converting string to int.");
                    *resultValue = str2int( variable->getStringValue() );
                } else {
                    Logger::getInstance()->error(
                        "VariableScriptingUtils::getVariableValueToInt - Unknown variable type.");
                    *resultValue = 0;
                }
            } else {
                Logger::getInstance()->error(
                    "VariableScriptingUtils::getVariableValueToInt - Null parameter variable given.");
            }
        }
        ...
        void setVariableValueToInt(IScriptVariable *variable, int toValue)
        {
            if (variable != NULL) {
                if (variable->getVariableType() == game::IScriptVariable::VARTYPE_BOOLEAN) {
                    ...
                    Logger::getInstance()->warning(
                        "VariableScriptingUtils::setVariableValueToInt - Converting int to float.");
                    variable->setFloatValue( (float)toValue );
                } else if (variable->getVariableType() == game::IScriptVariable::VARTYPE_STRING) {
                    Logger::getInstance()->warning(
                        "VariableScriptingUtils::setVariableValueToInt - Converting int to string.");
                    variable->setStringValue( int2str(toValue) );
                } else {
                    Logger::getInstance()->error(
                        "VariableScriptingUtils::setVariableValueToInt - Unknown variable type.");
                }
            } else {
                Logger::getInstance()->error(
                    "VariableScriptingUtils::setVariableValueToInt - Null parameter variable given.");
            }
        }
        ...
        void logVariableValue(IScriptVariable *variable)
        {
            if (variable != NULL) {
                ...
                Logger::getInstance()->error(
                    "VariableScriptingUtils::logVariableValue - Null parameter variable given.");
            }
        }
                    ...
                    if (stringData[i] == ',' && !atSecond) {
                        ...
                        if (stringData[i] == ',')
                            Logger::getInstance()->debug(
                                "VariableScriptingUtils::splitFromComma - Multiple comma seperators ignored.");
                        if (atSecond) {
