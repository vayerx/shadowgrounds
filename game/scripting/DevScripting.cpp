                               ...
                               GameScriptData *gsd, Game *game, bool *pause)
    {
        ...
        case GS_CMD_reloadParticleEffects:
                ...
                && game->gameUI->getVisualEffectManager() != NULL)
            {
                ...
                Logger::getInstance()->info(
                    "GameScripting::process - reloadParticleEffects, particle effects reloaded (existing effects may not be affected).");
            } else {
                sp->warning(
                    "GameScripting::process - reloadParticleEffects called while in menus or no visual effect manager loaded (call ignored).");
            }
            break;
        ...
        case GS_CMD_dumpEffectsInfo:
            if (game->getGameScene() != NULL && game->inCombat) {
                ...
                Logger::getInstance()->warning(
                    "DevScripting::process - dumpEffectsInfo cannot get info for null scene.");
            }
            break;
        ...
        case GS_CMD_clearSelectionVisualizationForUnifiedHandle:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) )
                ...
                sp->error(
                    "DevScripting::process - clearSelectionVisualizationForUnifiedHandle, unified handle not valid.");
            break;
