                                ...
                                GameScriptData *gsd, Game *game, bool *pause)
    {
                ...
                if (strcmp(stringData, "idvalue") == 0) {
                        ...
                        && *lastValue <= UNITID_HIGHEST_POSSIBLE_VALUE)
                    {
                        ...
                        if (o != NULL)
                            ...
                            sp->warning(
                                "GameScripting::process - setTarget idvalue, no unit found with given id number.");
                    } else {
                ...
                if (strcmp(stringData, "idstring") == 0) {
                    if (gsd->stringValue != NULL) {
                        ...
                        if (o != NULL) {
                            ...
                            std::string temp =
                                "GameScripting::process - setTarget idvalue, no unit found with given id string: ";
                        ...
                        sp->error(
                            "UnitScripting::process - setTarget called with idstring parameter, but string value is null.");
                    }
                    break;
                }
        ...
        case GS_CMD_MOVETOPATH:
            if (unit != NULL) {
                ...
                if (ua != NULL) {
                    ...
                    bool success =
                        ua->setPathTo( unit,
                                       unit->scriptPaths.getStoredPathEndPosition( unit->scriptPaths.
                                                                                   getStoredPathNumber() ) );
                        ...
                        if (unit->getMoveState() != Unit::UNIT_MOVE_STATE_UNCONSCIOUS) {
                            ...
                            if (strcmp(stringData, "crawl") == 0) {
                                     ...
                                     && unit->getAnimationSet()->isAnimationInSet(ANIM_PRONE) )
                                {
                                    ...
                                    sp->warning(
                                        "UnitScripting::process - setSpeed, attempt to set crawl speed when no proper animation.");
                                }
                                speedok = true;
                            }
        ...
        case GS_CMD_CHOOSEPATH:
            if (unit != NULL) {
                ...
                if (intData == 0) {
                    ...
                    sp->warning(
                        "UnitScripting::process - choosePath with zero parameter deprecated (use choosePathByValue instead).");
                } else {
        ...
        case GS_CMD_RESCRIPTMAIN:
            if (unit != NULL) {
                ...
                ai->requestReScriptMain();
                sp->warning(
                    "UnitScripting::process - reScriptMain, main script will restart once the current main script process finishes.");
            } else {
        ...
        case GS_CMD_TERMINATEMAINSCRIPT:
            if (unit != NULL) {
                ...
                if ( !ai->isScriptProcessMainScriptProcess(sp) ) {
                    sp->warning(
                        "UnitScripting::process - terminateMainScript, main script process will be immediately terminated.");
                    ...
                    sp->warning(
                        "UnitScripting::process - terminateMainScript, main script self termination not allowed.");
                }
            } else {
                    ...
                    if (ua != NULL) {
                        ...
                        if (copyAmount > 0) {
                            ...
                            VC3 lastPos = VC3( game->gameMap->pathfindToScaledX( partialpath->getPointX(copyAmount - 1) ),
                                               0,
                                               game->gameMap->pathfindToScaledX( partialpath->getPointY(copyAmount - 1) ) );
                ...
                if (intData > 100 || intData <= 0) {
                    ...
                    if (ua != NULL) {
                        ...
                        if (copyAmount > 0) {
                            ...
                            VC3 lastPos = VC3( game->gameMap->pathfindToScaledX( partialpath->getPointX(copyAmount - 1) ),
                                               0,
                                               game->gameMap->pathfindToScaledY( partialpath->getPointY(copyAmount - 1) ) );
                            //VC3 lastPos = VC3(game->gameMap->pathfindToScaledX(partialpath->getPointX(0)),
                            //    0, game->gameMap->pathfindToScaledX(partialpath->getPointY(0)));
                            unit->setPath(partialpath);
                            unit->setPathIndex(unit->getPathIndex() + 1);
        ...
        case GS_CMD_setOwnedUnitOfTypeIncludingInActiveUnits:
            if (stringData != NULL) {
                ...
                if (ut == NULL) {
                    sp->error(
                        "UnitScripting::process - setOwnedUnitOfType / setOwnedUnitOfTypeIncludingInActiveUnits, reference to unknown unit type.");
                    break;
                }
                ...
                sp->error(
                    "UnitScripting::process - setOwnedUnitOfType / setOwnedUnitOfTypeIncludingInActiveUnits parameter missing (unit type expected).");
            }
            break;
                ...
                if (ut == NULL) {
                    ...
                    if ( ut->doesAllowRespawn() ) {
                        if ( !ut->doesRemoveDestroyed() ) {
                            sp->warning(
                                "UnitScripting::process - addUnit, unit type has \"allowrespawn\" on, but does not have \"removedestroyed\" on (useless combination).");
                            sp->debug(stringData);
                        }
            ...
            if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                ...
                if (pt == NULL) {
                    ...
                    if (unit == NULL) {
                        ...
                        if (unit->getRootPart() != NULL) {
                            sp->debug(
                                "UnitScripting::process - addUnitRootPart, detaching old root part and sub parts.");
        ...
        case GS_CMD_SETSELECTEDSECONDARYWEAPON:
            if (unit != NULL) {
                if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                    ...
                    if (pt == NULL) {
                        sp->error(
                            "UnitScripting::process - setSelectedSecondaryWeapon, reference to unloaded part type.");
                    } else {
                        assert( unit->isActive() );
                        int newweap = unit->getWeaponByWeaponType( pt->getPartTypeId() );
                        if (newweap != -1) {
                            ...
                            sp->warning(
                                "UnitScripting:process - setSelectedSecondaryWeapon, unit did not have such weapon");
                        }
                    }
                }
            } else {
        ...
        case GS_CMD_SEEKSUBPART:
            if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                ...
                if (pt == NULL) {
                    ...
                    if (gsd->part == NULL) {
                        sp->error(
                            "UnitScripting::process - seekSubPart, attempted to seek sub-part to null parent part.");
                    } else {
                        Part *parentPart = gsd->part;
                        int slotamount = parentPart->getType()->getSlotAmount();
                        int slot;
                ...
                if (stringData != NULL) {
                    ...
                    if (sep != NULL) {
                        *sep = '\0';
                        ...
                        sp->error(
                            "UnitScripting::process - setPartTypeData parameter bad (part type data key=value pair expected).");
                    }
                    delete[] tmpbuf;
                } else {
                ...
                if (gsd->unit != NULL) {
                        ...
                        || strcmp(stringData, "null") == 0)
                    {
                        ...
                        if (s != NULL) {
                            ...
                            if (prevu != NULL)
                                sp->warning(
                                    "UnitScripting::process - setUnitIdString, Another unit with given id-string already exists.");
                            unit->setIdString(s);
                        } else {
        ...
        case GS_CMD_POSITIONDISTANCETONEARESTCOVER:
            if (unit != NULL) {
                ...
                if (dist > 120) // not meters, pathfind blocks
                    dist = 120;
                *lastValue =
                    (int)( (float)dist * ( game->gameMap->getScaledSizeX() / (float)game->gameMap->getPathfindSizeX() ) );
            } else {
            ...
            if (stringData != NULL) {
                if (unit != NULL) {
                    ...
                    if (otherUnit != NULL) {
                        ...
                        sp->warning(
                            "UnitScripting::process - setUnitRotationToFace, no unit found with given id string.");
                    }
                } else {
                ...
                sp->error(
                    "UnitScripting::process - setUnitRotationToFace parameter missing, another unit id string expected.");
            }
            break;
        ...
        case GS_CMD_SETUNITITEMCOUNTTOVALUE:
            if (unit != NULL) {
                if (stringData != NULL) {
                    ...
                    sp->error(
                        "GameScripting::process - setUnitItemCountToValue parameter missing, item type name expected.");
                }
            } else {
        ...
        case GS_CMD_SETSELECTEDUNITITEM:
            if (unit != NULL) {
                if (stringData != NULL)
                    ...
                    sp->error(
                        "GameScripting::process - setSelectedUnitItem parameter missing, item type name expected.");
            } else {
            ...
            if (stringData != NULL) {
                ...
                if (str2int(stringData) > 0) {
                    ...
                    Logger::getInstance()->warning(
                        "GameScripting::process - setUnitByIdString, integer parameter given, treating it as id number (deprecated).");
                } else {
                    const char *s = stringData;
                    if (stringData[0] == '$'
                        && stringData[1] == '\0')
        ...
        case GS_CMD_doesUnitExistWithIdString:
            if (stringData != NULL) {
                ...
                *lastValue = 0;
                sp->error(
                    "GameScripting::process - doesUnitExistWithIdString, parameter missing (unit id string expected).");
            }
            break;
                ...
                if (gsd->unit != NULL) {
                    ...
                    if (tmp != NULL) {
                        if (gsd->unit != tmp) {
                            ...
                            *lastValue = 0;
                            sp->error(
                                "GameScripting::process - setTargetByIdString, Unit with given id is the same as current unit (cannot target self).");
                        }
                    } else {
                        *lastValue = 0;
            ...
            if (unit != NULL) {
                if (*lastValue > 1) {
                    ...
                    if (gsd->unit != tmp) {
                        ...
                        *lastValue = 0;
                        sp->error(
                            "GameScripting::process - setTargetByIdValue, Unit with given id is the same as current unit (cannot target self).");
                    }
                } else {
        ...
        case GS_CMD_ABSOLUTEINTERPOLATEPOSITIONANTICLOCKWISE:
            if (unit != NULL) {
                ...
                if (command == GS_CMD_ABSOLUTEINTERPOLATEPOSITIONANTICLOCKWISE)
                    sp->warning(
                        "UnitScripting::process - Attempt to absoluteInterpolatePositionAnticlockwise for null unit.");
                else
                    sp->warning(
                        "UnitScripting::process - Attempt to absoluteInterpolatePositionClockwise for null unit.");
            }
            break;
                ...
                if (unit != NULL) {
                    if ( !unit->isAtPathEnd() ) {
                        ...
                        if (unit->getUnitType()->getAimHeightStanding() < 0.5f)
                            height = 0.5f;
                        util::PathDeformer::deformForDarkness(game->getGameScene()->getPathFinder(),
                                                              unit->getPath(), unit->getPathIndex(),
                                                              game->gameUI->getTerrain()->GetTerrain(), game->gameMap,
                                                              height);
                    }
                } else {
        ...
        case GS_CMD_makeUnitImmortalWithHitScript:
            if (unit != NULL) {
                ...
                sp->warning(
                    "UnitScripting::process - Attempt to makeUnitImmortal / makeUnitImmortalWithHitScript for null unit.");
            }
            break;
            ...
            if (unit != NULL) {
                if (intData > 0) {
                    if (unit->getJumpCounter() > 0) {
                        ...
                        sp->warning(
                            "UnitScripting::process - makeJump/makeJumpTowardPosition command when already jumping.");
                    }
                    UnitActor *ua = getUnitActorForUnit(unit);
                    ...
                    if (command == GS_CMD_MAKEJUMPTOWARDPOSITION) {
                        VC3 rot = unit->getRotation();
                        float angle = util::PositionDirectionCalculator::calculateDirection(
                            unit->getPosition(), gsd->position);
                        float relativeAngle = util::AngleRotationCalculator::getFactoredRotationForAngles(rot.y,
                                                                                                          angle,
                                                                                                          0.0f);
                    ...
                    *pause = true;
                    ...
                    sp->warning(
                        "UnitScripting::process - Attempt to makeJump/makeJumpTowardPosition with zero length parameter.");
                }
            } else {
        ...
        case GS_CMD_ISUNITFACINGTARGET:
            if (unit != NULL) {
                ...
                if (targUnit != NULL) {
                    ...
                    float targAngle = util::PositionDirectionCalculator::calculateDirection(
                        unit->getPosition(), targPos);
        ...
        case GS_CMD_ISTARGETFACINGUNIT:
            if (unit != NULL) {
                ...
                if (targUnit != NULL) {
                    ...
                    float rotAngle = util::AngleRotationCalculator::getFactoredRotationForAngles(targRot.y,
                                                                                                 targAngle,
                                                                                                 0.0f);
        ...
        case GS_CMD_SETUNITFIRSTPERSONCONTROL:
            if (unit != NULL) {
                if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT) {
                    ...
                    sp->warning(
                        "UnitScripting::process - setUnitFirstPersonControl last value (controller number) out of range.");
                }
            } else {
        ...
        case GS_CMD_setFirstPersonControlledUnit:
            if (*lastValue >= 0 && *lastValue < MAX_PLAYERS_PER_CLIENT)
                ...
                sp->warning(
                    "UnitScripting::process - setFirstPersonControledUnit last value (controller number) out of range.");
            break;
        ...
        case GS_CMD_FINDCLOSESTUNITOFTYPEWITHSTRINGVALUEVARIABLESET:
            if (stringData == NULL) {
                sp->warning(
                    "UnitScripting::process - findClosestUnitOfTypeWithStringValueVariableSet parameter missing.");
            } else {
                if (gsd->stringValue == NULL) {
                    sp->warning(
                        "UnitScripting::process - findClosestUnitOfTypeWithStringValueVariableSet called with null string value (unitvariable name expected).");
                } else {
                    gsd->unit = findClosestUnitOfTypeWithVariableSet(game,
                                                                     gsd->position,
                                                                     stringData,
                                                                     gsd->stringValue,
                                                                     true);
        ...
        case GS_CMD_FINDCLOSESTUNITOFTYPEWITHSTRINGVALUEVARIABLENOTSET:
            if (stringData == NULL) {
                sp->warning(
                    "UnitScripting::process - findClosestUnitOfTypeWithStringValueVariableNotSet parameter missing.");
            } else {
                if (gsd->stringValue == NULL) {
                    sp->warning(
                        "UnitScripting::process - findClosestUnitOfTypeWithStringValueVariableNotSet called with null string value (unitvariable name expected).");
                } else {
                    gsd->unit = findClosestUnitOfTypeWithVariableSet(game,
                                                                     gsd->position,
                                                                     stringData,
                    ...
                    if ( PARTTYPE_ID_STRING_VALID(s) ) {
                        ...
                        if (pt != NULL) {
                            ...
                            if (unit != NULL) {
                                ...
                                sp->error(
                                    "UnitScripting::process - Attempt to isSelectedUnitWeaponOfType for null unit.");
                            }
                        } else {
                            sp->error(
                                "UnitScripting::process - isSelectedUnitWeaponOfType, no part type loaded with given id.");
                            LOG_DEBUG(s);
                        }
                    } else {
        ...
        case GS_CMD_WARPUNITTOPOSITIONFROMSECONDARYUSINGOFFSET:
            if (unit != NULL) {
                if (gsd->secondaryPosition.x == 0
                    ...
                    sp->warning(
                        "UnitScripting::process - warpUnitToPositionFromSecondaryUsingOffset, secondary position is origo, probably an error.");
                ...
                if ( !ua->warpToPosition(unit, groundPos) )
                    sp->warning(
                        "UnitScripting::process - warpUnitToPositionFromSecondaryUsingOffset did not find suitable place for unit.");
            } else {
                sp->warning(
                    "UnitScripting::process - Attempt to warpUnitToPositionFromSecondaryUsingOffset for null unit.");
            }
            break;
        ...
        case GS_CMD_FADEUNITVISIBILITYUSINGTIMEVALUE:
            if (unit != NULL) {
                ...
                if (timeValue < GAME_TICK_MSEC)
                    sp->warning(
                        "UnitScripting::process - fadeUnitVisibilityUsingTimeValue, value given for time invalid (less than game tick).");
                float floatData = intFloat.f;
        ...
        case GS_CMD_FADEUNITLIGHTINGUSINGTIMEVALUE:
            if (unit != NULL) {
                ...
                if (timeValue < GAME_TICK_MSEC)
                    sp->warning(
                        "UnitScripting::process - fadeUnitLightingUsingTimeValue, value given for time invalid (less than game tick).");
                float floatData = intFloat.f;
        ...
        case GS_CMD_AREALISTENTOPLAYERUNITS:
            if (unit != NULL) {
                if (gsd->originalUnit != NULL && unit != gsd->originalUnit)
                    sp->warning(
                        "UnitScripting::process - areaListenToPlayerUnits, unit is not the original script unit (possibly missing restoreUnit somewhere).");
                if (unit->getAreaClipMask() >= 1 && unit->getAreaClipMask() <= 15 && unit->getAreaRange() > 0) {
                    if (game->units->getOwnedUnitAmount(gsd->player) > 0) {
                        ...
                        game->gameScripting->addAreaTrigger(unit, unit->getAreaCenter(),
                                                            unit->getAreaRange(), unit->getAreaClipMask(), &ulistfixed);
                                ...
                                if (ap != NULL) {
                                    ...
                                    if (p != NULL) {
                                        ...
                                        *lastValue = apo->getAmount();
                                        ...
                                        sp->debug(
                                            "UnitScripting:process - getAmmoForUnitWeapon, unit (inactive) did not have such weapon");
                                        *lastValue = 0;
                                    }
                                } else {
                                    sp->debug(
                                        "UnitScripting:process - getAmmoForUnitWeapon, weapon does not use any ammo type.");
                                    *lastValue = 0;
                                }
                            } else {
                                sp->error(
                                    "UnitScripting:process - getAmmoForUnitWeapon, part type is of not weapon type.");
                            }
                        }
                    }
                }
        ...
        case GS_CMD_SETUNITEXECUTETIPTOSTRINGVALUE:
            if (unit != NULL) {
                if (gsd->stringValue != NULL)
                    ...
                    sp->warning(
                        "UnitScripting::process - Attempt to setUnitExecuteTipToStringValue for null string value (use clearUnitExecuteTip instead).");
            } else {
                        ...
                        if (stringData != NULL) {
                                ...
                                for (int i = 1; i < allocedOrigPaths; i++) {
                                    if ( gsd->originalUnit->scriptPaths.isStoredPathUsed(i)
                                        ...
                                        if (gsd->originalUnit->scriptPaths.getStoredPathName(i) != NULL
                                            && strcmp( stringData,
                                                       gsd->originalUnit->scriptPaths.getStoredPathName(i) ) == 0)
                                        {
                                            foundPath = i;
                                            break;
                                        }
                            ...
                            if (foundPath != -1) {
                                ...
                                if (endPathNum != -1) {
                                    ...
                                    for (int i = foundPath; i < endPathNum + 1; i++) {
                                        ...
                                        if (i == foundPath) {
                                            ...
                                            if (ua != NULL) {
                                                ...
                                                if (path == NULL)
                                                    sp->debug(
                                                        "UnitScripting::process - copyExtPathFromOriginalUnit failed to find path.");
                                            }
                                        }
                                    ...
                                    sp->error(
                                        "UnitScripting::process - copyExtPathFromOriginalUnit failed to find end point for given path (internal error).");
                                }
                            } else {
                                //if (alreadyHasExt)
                                //{
                                ...
                                //} else {
                                sp->warning(
                                    "UnitScripting::process - copyExtPathFromOriginalUnit failed to find path with given name.");
                                //}
                            }
                        } else {
                            sp->error(
                                "UnitScripting::process - copyExtPathFromOriginalUnit parameter missing, path name expected.");
                        }
                    } else {
                        // NOTE: this might still work? copying unit's another path to name "ext_path"..
                        sp->warning(
                            "UnitScripting::process - Attempt to copyExtPathFromOriginalUnit when unit is the same as original unit.");
                    }
                } else {
                    sp->warning(
                        "UnitScripting::process - Attempt to copyExtPathFromOriginalUnit for null original unit.");
                }
            } else {
                    ...
                    if (*lastValue >= GAME_TICK_MSEC) {
                        ...
                        if ( unit->isActive() ) {
                            ...
                            if (effTypeOk) {
                                ...
                                sp->warning(
                                    "UnitScripting::process - createUnitEffectLayerUsingTimeValue parameter invalid, effect layer type expected.");
                            }
                        }
                    } else {
                        sp->warning(
                            "UnitScripting::process - Attempt to createUnitEffectLayerUsingTimeValue with time value less than one game tick.");
                    }
                } else {
                    sp->warning(
                        "UnitScripting::process - createUnitEffectLayerUsingTimeValue parameter missing, effect layer type expected.");
                }
            } else {
        ...
        case GS_CMD_moveOriginalUnitToUnitHelper:
            if (stringData != NULL) {
                        ...
                        if (gsd->originalUnit != unit) {
                                ...
                                && unit->getVisualObject()->getStormModel() != NULL)
                            {
                                ...
                                if (!helperFound) {
                                    ...
                                    sp->warning(
                                        "UnitScripting::process - moveOriginalUnitToUnitHelper, helper with given name not found.");
                                } else {
                                ...
                                sp->warning(
                                    "UnitScripting::process - Attempt to moveOriginalUnitToUnitHelper for unit with no visual object or model.");
                            }
                        } else {
                            sp->warning(
                                "UnitScripting::process - Attempt to moveOriginalUnitToUnitHelper for same unit and original unit.");
                        }
                    } else {
                        sp->warning(
                            "UnitScripting::process - Attempt to moveOriginalUnitToUnitHelper for null original unit.");
                    }
                } else {
                ...
                sp->warning(
                    "UnitScripting::process - moveOriginalUnitToUnitHelper parameter missing, helper name expected.");
            }
            break;
        ...
        case GS_CMD_setUnitWeaponOperable:
            if (unit != NULL) {
                if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                    ...
                    if (pt == NULL) {
                        sp->error(
                            "UnitScripting::process - setUnitWeaponInoperable/Operable, reference to unloaded part type.");
                    } else {
                        assert( unit->isActive() );
                        int newweap = unit->getWeaponByWeaponType( pt->getPartTypeId() );
                        if (newweap != -1) {
                            ...
                            WeaponWindow *win;
                            win = (WeaponWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                                "SurvivorMarineWeaponWindow");
                            if (win) win->forceUpdate();
                            win = (WeaponWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                                "SurvivorNapalmWeaponWindow");
                            if (win) win->forceUpdate();
                            win = (WeaponWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                                "SurvivorSniperWeaponWindow");
                            if (win) win->forceUpdate();
#endif
                        } else {
                            sp->warning(
                                "UnitScripting:process - setUnitWeaponInoperable/Operable, unit did not have such weapon");
                        }
                    }
                }
            } else {
        ...
        case GS_CMD_isUnitHitByBullet:
            if (unit != NULL) {
                if (unit != gsd->originalUnit)
                    sp->error(
                        "UnitScripting::process - isUnitHitByBullet can only be called inside hit script for original hit script unit.");
                if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                    if (stringData == NULL)
        ...
        case GS_CMD_isUnitHitByElectricBullet:
            if (unit != NULL) {
                if (unit != gsd->originalUnit)
                    sp->error(
                        "UnitScripting::process - isUnitHitByElectricBullet can only be called inside hit script for original hit script unit.");
                if (gs_hitscript_hit_bullet_type != NULL) {
                    ...
                    sp->warning(
                        "UnitScripting::process - Attempt to isUnitHitByElectricBullet when null hit bullet type info.");
                }
            } else {
            ...
            if (stringData != NULL) {
                if (unit != NULL) {
                    ...
                    if (otherUnit != NULL) {
                        ...
                        sp->warning(
                            "UnitScripting::process - rotateUnitAroundUnitByValue, no unit found with given id string.");
                    }
                } else {
                ...
                sp->error(
                    "UnitScripting::process - rotateUnitAroundUnitByValue parameter missing, another unit id string expected.");
            }
            break;
        ...
        case GS_CMD_risingMessage: {
            if ( game && game->gameUI && game->gameUI->getCombatWindow(game->singlePlayerNumber) &&
                 game->gameUI->getCombatWindow(0)->getSubWindow("TargetDisplayWindow") ) {
        ...
        case GS_CMD_activateUnitByIdString:
            if (stringData != NULL) {
                ...
                *lastValue = 0;
                sp->error(
                    "GameScripting::process - activateUnitByIdString, parameter missing (unit id string expected).");
            }
            break;
        ...
        case GS_CMD_setFirstOwnedUnitOfTypeIncludingInActiveUnits:
            if (stringData == NULL) {
                sp->error(
                    "UnitScripting::process - setFirstOwnedUnitOfTypeIncludingInActiveUnits parameter missing (unit type expected).");
            } else {
                UnitType *ut = getUnitTypeByName(stringData);
                if (ut == NULL) {
                    sp->error(
                        "UnitScripting::process - setFirstOwnedUnitOfTypeIncludingInActiveUnits, reference to unknown unit type.");
                } else {
                    ...
                    if (gsd->unit != NULL) {
                        ...
                        *lastValue = 0;
                        sp->error(
                            "UnitScripting::process - setFirstOwnedUnitOfTypeIncludingInActiveUnits, could not find unit type.");
                    }
                }
            }
            break;
        ...
        case GS_CMD_setUnitJumpCounterTime:
            if (unit != NULL) {
                ...
                sp->warning(
                    "UnitScripting::process - setUnitJumpCounterTime, should not be used (inteded to use setUnitJumpNotAllowedTime?).");
            } else {
        ...
        case GS_CMD_deleteUnitPhysicsObject:
            if (unit != NULL) {
                if (unit->getGamePhysicsObject() != NULL) {
                    ...
                    sp->warning(
                        "UnitScripting::process - deleteUnitPhysicsObject, unit does not have a physics object.");
                }
            } else {
            ...
            if (unit != NULL) {
                if (stringData != NULL) {
                    if ( PARTTYPE_ID_STRING_VALID(stringData) ) {
                        ...
                        sp->warning(
                            "UnitScripting::process - Attempt to doesWeaponExistInPlayerWeaponry with invalid weapon id.");
                    }
                } else {
                    sp->warning(
                        "UnitScripting::process - Attempt to doesWeaponExistInPlayerWeaponry with invalid string parameter.");
                }
            ...
            if (unit != NULL) {
                if (unit->getCharacter() != NULL)
                    unit->setMaxHP(unit->getUnitType()->getHP()
                                   * unit->getCharacter()->getSkillAmount(CHAR_SKILL_STAMINA) / 100);
                else
                    unit->setMaxHP( unit->getUnitType()->getHP() );
            } else {
        ...
        case GS_CMD_setUnitHPGainMode:
            if (unit != NULL) {
                if (strcmp(stringData, "disabled") == 0) {
                    ...
                    unit->enableHPGain(limit,
                                       amount,
                                       delay * GAME_TICKS_PER_SECOND,
                                       startdelay * GAME_TICKS_PER_SECOND,
                                       damagefactor);
                }
            } else {
        ...
        case GS_CMD_setCustomWeaponTypeDataForType:
            if (stringData != NULL) {
                ...
                // find parttype before : separator
                {
                    ...
                    if (sep == NULL) {
                        sp->error(
                            "UnitScripting::process - setCustomWeaponTypeDataForType parameter bad (\"parttype:datakey=value\" expected).");
                        break;
                    }
                    *sep = 0;
        ...
        case GS_CMD_changeUnitRootPartModel:
            if (unit != NULL) {
                if ( unit->getRootPart() ) {
                    ...
                    if (unit->getVisualObject() == NULL) {
                        sp->error(
                            "UnitScripting::process - Attempt to changeUnitRootPartModel for unit without visual object.");
                        break;
                    }
                    ...
                    if (vom == NULL) {
                        sp->error(
                            "UnitScripting::process - Attempt to changeUnitRootPartModel with invalid model name.");
                        break;
                    }
                ...
                if (stringData != NULL) {
                    ...
                    if (sep != NULL) {
                        *sep = '\0';
                        ...
                        sp->error(
                            "UnitScripting::process - setCustomWeaponTypeData parameter bad (part type data key=value pair expected).");
                    }
                    delete[] tmpbuf;
                } else {
        ...
        case GS_CMD_setUnitWeaponVisible:
            if (unit != NULL) {
                if ( !PARTTYPE_ID_STRING_VALID(stringData) ) {
                    ...
                    if (pt == NULL) {
                        sp->error(
                            "UnitScripting::process - setUnitWeaponInvisible/Visible, reference to unloaded part type.");
                    } else {
                        assert( unit->isActive() );
                        int newweap = unit->getWeaponByWeaponType( pt->getPartTypeId() );
                        if (newweap != -1) {
        ...
        case GS_CMD_restoreDefaultSpeed:
            if (unit != NULL) {
                ...
                if (defSpeedStr != NULL)
                    ...
                    sp->warning(
                        "UnitScripting::process - restoreDefaultSpeed, invalid default speed type (internal error).");
            } else {
        ...
        case GS_CMD_isUnitHitByPoison:
            if (unit != NULL) {
                if (unit != gsd->originalUnit)
                    sp->error(
                        "UnitScripting::process - isUnitHitByPoison can only be called inside hit script for original hit script unit.");
                if (gs_hitscript_hit_bullet_type != NULL) {
...
//                && u->getOwner() == player)
    ...
    Unit *UnitScripting::findClosestUnitWithVariableSet(Game       *game,
                                                        const VC3  &position,
                                                        const char *unitvar,
                                                        bool        varIsNonZero)
    {
    ...
    Unit *UnitScripting::findClosestUnitOfTypeWithVariableSet(Game       *game,
                                                              const VC3  &position,
                                                              const char *unittype,
                                                              const char *unitvar,
                                                              bool        varIsNonZero)
