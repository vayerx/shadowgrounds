...
namespace game
{
                                    ...
                                    bool                *pause)
    {
        ...
        case GS_CMD_createScriptableTrackerType:
            if (stringData != NULL) {
                if (game->objectTracker->getTrackerTypeNumberByName(stringData) == -1) {
                    ...
                    if (s == NULL)
                        sp->warning(
                            "TrackingScripting::process - createScriptableTrackerType, no script loaded with given name (creating the scriptable tracker type anyway, but initialization will be incomplete).");
                    ...
                    *lastValue = game->objectTracker->addTrackerType(st);
                    ...
                    sp->error(
                        "TrackingScripting::process - createScriptableTrackerType, another tracker type already exists with given name.");
                    *lastValue = 0;
                }
            } else {
                    ...
                    if (tnum != -1) {
                        ...
                        if (gsd->unifiedHandle != UNIFIED_HANDLE_NONE) {
                            ...
                            if (t->getType()->getTypeId() == ScriptableTrackerObjectType::typeId) {
                                ...
                                UnifiedHandle originalTrackerBeforeCall =
                                    ScriptableTrackerObject::trackerForCurrentlyRunningScript;
                            ...
                            sp->error(
                                "TrackingScripting::process - createTracker, failed to create a tracker (too many trackers or invalid tracker type parameter).");
                        }
                    } else {
                ...
                if (s != NULL) {
                    ...
                    if (tnum != -1) {
                        ...
                        if (gsd->unifiedHandle != UNIFIED_HANDLE_NONE) {
                            ...
                            if ( VALIDATE_UNIFIED_HANDLE_BITS(*lastValue) ) {
                                ITrackableObject *trackable = TrackableUnifiedHandleObject::getInstanceFromPool(
                                    *lastValue);
                                if (trackable != NULL)
                                    game->objectTracker->attachTrackerToTrackable(gsd->unifiedHandle, trackable);
                                else
                                    sp->error(
                                        "TrackingScripting::process - createTrackerAttachingToTrackableByValue, failed to get trackable object by given value.");
                            } else {
                                sp->error(
                                    "TrackingScripting::process - createTrackerAttachingToTrackableByValue, value is not a valid unified handle.");
                            }
                            ...
                            sp->error(
                                "TrackingScripting::process - createTrackerAttachingToTrackableByValue, failed to create a tracker (too many trackers or invalid tracker type parameter).");
                        }
                    } else {
                        sp->error(
                            "TrackingScripting::process - createTrackerAttachingToTrackableByValue, no tracker type of given name.");
                        gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                    }
                } else {
                    sp->error(
                        "TrackingScripting::process - createTrackerAttachingToTrackableByValue, null string value parameter.");
                    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                }
            } else {
        ...
        case GS_CMD_doesTrackerExist:
            ...
            sp->error(
                "TrackingScripting::process - doesTrackerExist, object with given unified handle is not a tracker.");
            *lastValue = 0;
        }
    } else {
        ...
        *lastValue = 0;
...
#endif
            {
                ...
                if (trackable != NULL) {
                    if (trackable->getTypeId() == TrackableUnifiedHandleObject::typeId) {
                        ...
                        sp->warning(
                            "TrackingScripting::process - doesTrackerAttachedTrackableExist, attached trackable is not of unified handle type trackable.");
                        *lastValue = 1;
                    }
                } else {
                    sp->warning(
                        "TrackingScripting::process - doesTrackerAttachedTrackableExist, tracker is not attached to any trackable.");
                    *lastValue = 0;
                }
            }
#ifndef UNSAFE_TRACKER_SCRIPTING
        ...
        sp->error(
            "TrackingScripting::process - doesTrackerAttachedTrackableExist, object with given unified handle is not a tracker.");
        *lastValue = 0;
    }
} else {
    ...
    *lastValue = 0;
                    ...
                    if (game->objectTracker->getTrackerByUnifiedHandle(gsd->unifiedHandle) != NULL) {
                        if ( VALIDATE_UNIFIED_HANDLE_BITS(*lastValue) ) {
                            ...
                            if (trackable != NULL) {
                                ...
                                sp->error(
                                    "TrackingScripting::process - attachTrackerToTrackableByValue, failed to get trackable object by given value.");
                            }
                        } else {
                            sp->error(
                                "TrackingScripting::process - attachTrackerToTrackableByValue, value is not a valid unified handle.");
                        }
                    } else {
                        sp->error(
                            "TrackingScripting::process - attachTrackerToTrackableByValue, tracker does not exist.");
                    }
                } else {
                    sp->error(
                        "TrackingScripting::process - attachTrackerToTrackableByValue, object with given unified handle is not a tracker.");
                }
            } else {
        ...
        case GS_CMD_setTrackerToPosition:
            ...
            sp->error(
                "TrackingScripting::process - setTrackerToPosition, object with given unified handle is not a tracker.");
        }
    } else {
        ...
        case GS_CMD_getTrackerPosition:
            ...
            sp->error(
                "TrackingScripting::process - getTrackerPosition, object with given unified handle is not a tracker.");
            gsd->position = VC3(0, 0, 0);
        }
    } else {
        ...
        case GS_CMD_deleteTracker:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_TRACKER(gsd->unifiedHandle) ) {
                    ...
                    sp->error(
                        "TrackingScripting::process - deleteTracker, object with given unified handle is not a tracker.");
                }
            } else {
    ...
    *lastValue = 0;
...
#endif
            {
                ...
                if (trackable != NULL) {
                    if (trackable->getTypeId() == TrackableUnifiedHandleObject::typeId) {
                        ...
                        sp->warning(
                            "TrackingScripting::process - getTrackerAttachedTrackable, attached trackable is not of unified handle type trackable.");
                        gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                    }
                } else {
                    sp->warning(
                        "TrackingScripting::process - getTrackerAttachedTrackable, tracker is not attached to any trackable.");
                    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                }
            }
#ifndef UNSAFE_TRACKER_SCRIPTING
        ...
        sp->error(
            "TrackingScripting::process - getTrackerAttachedTrackable, object with given unified handle is not a tracker.");
        gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
    }
} else {
    ...
    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
        ...
        case GS_CMD_detachTracker:
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_TRACKER(gsd->unifiedHandle) ) {
                    if (game->objectTracker->getTrackerByUnifiedHandle(gsd->unifiedHandle) != NULL) {
                        ITrackableObject *trackable = game->objectTracker->getTrackerAttachedTrackable(
                            gsd->unifiedHandle);
                        if (trackable != NULL) {
                            UnifiedHandle originalTrackerBeforeCall =
                                ScriptableTrackerObject::trackerForCurrentlyRunningScript;
                            ...
                            sp->warning(
                                "TrackingScripting::process - detachTracker, tracker is not attached to any trackable.");
                        }
                    } else {
                    ...
                    sp->error(
                        "TrackingScripting::process - detachTracker, object with given unified handle is not a tracker.");
                }
            } else {
        ...
        case GS_CMD_restoreTracker:
            ...
            if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                if ( IS_UNIFIED_HANDLE_TRACKER(gsd->unifiedHandle) ) {
                    ...
                    sp->error(
                        "TrackingScripting::process - restoreTracker, restored tracker is not a tracker (internal error).");
                    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                }
            } else {
                sp->error(
                    "TrackingScripting::process - restoreTracker, restored tracker handle is invalid (internal error).");
                gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
            }
        }
#endif
                    ...
                    if ( PARTTYPE_ID_STRING_VALID(s) ) {
                        ...
                        if (pt != NULL) {
                            ...
                            if ( bulletpt != NULL && pt->isInherited(bulletpt) )
                                ...
                                sp->error(
                                    "UnitScripting::process - setTrackerProjectileBullet, given part type is non-bullet type.");
                        } else {
                            sp->error(
                                "UnitScripting::process - setTrackerProjectileBullet, reference to unloaded part type.");
                        }
                    } else {
        ...
        case GS_CMD_iterateNextTrackable:
            ...
            if ( ScriptableTrackerObject::trackableIteratorForCurrentlyRunningScript->iterateAvailable() ) {
                *lastValue = 1;
                ...
                ITrackableObject *trackable =
                    ScriptableTrackerObject::trackableIteratorForCurrentlyRunningScript->iterateNext();
                if (trackable->getTypeId() == TrackableUnifiedHandleObject::typeId)
                    // WARNING: unsafe cast! (based on check above)
                    gs_currentlyIteratedTrackableUnifiedHandle =
                        ( (TrackableUnifiedHandleObject *)trackable )->getUnifiedHandle();
                else
        ...
        case GS_CMD_getIteratedTrackableUnifiedHandle:
            ...
            sp->error(
                "TrackingScripting::process - getIteratedTrackableUnifiedHandle called outside tracker iterate script.");
            gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
            *lastValue = 0;
        }
#endif
            ...
            if (stringData != NULL) {
                if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                    if ( IS_UNIFIED_HANDLE_TRACKER(gsd->unifiedHandle) ) {
                        ...
                        sp->error(
                            "TrackingScripting::process - isTrackerType, object with given unified handle is not a tracker.");
                        *lastValue = 0;
                    }
                } else {
        ...
        case GS_CMD_getScriptableTrackerVariable:
                    ...
                    if (t->getType()->getTypeId() == ScriptableTrackerObjectType::typeId) {
                        ...
                        if (varNum != -1) {
                            *lastValue = st->getVariable(varNum);
                            ...
                            sp->error(
                                "TrackingScripting::process - getScriptableTrackerVariable, scriptable tracker variable with given name does not exist.");
                            *lastValue = 0;
                        }
                    } else {
                        sp->error(
                            "TrackingScripting::process - getScriptableTrackerVariable, tracker is not of scriptable tracker type.");
                        *lastValue = 0;
                    }
                } else {
            ...
            sp->error(
                "TrackingScripting::process - getScriptableTrackerVariable, object with given unified handle is not a tracker.");
            *lastValue = 0;
        }
    } else {
        ...
        *lastValue = 0;
        ...
        case GS_CMD_setScriptableTrackerVariable:
                ...
                if (t != NULL) {
                    ...
                    if (t->getType()->getTypeId() == ScriptableTrackerObjectType::typeId) {
                        ...
                        if (varNum != -1)
                            ...
                            sp->error(
                                "TrackingScripting::process - setScriptableTrackerVariable, scriptable tracker variable with given name does not exist.");
                    } else {
                        sp->error(
                            "TrackingScripting::process - setScriptableTrackerVariable, tracker is not of scriptable tracker type.");
                    }
                } else {
            ...
            sp->error(
                "TrackingScripting::process - setScriptableTrackerVariable, object with given unified handle is not a tracker.");
        }
    } else {
        ...
        case GS_CMD_isUnifiedHandleObjectTrackedByValue:
            ...
            {
                // HACK: ...
                TrackableUnifiedHandleObject *tuho = TrackableUnifiedHandleObject::getInstanceFromPool(
                    gsd->unifiedHandle);
                if (tuho != NULL) {
                    ...
                    sp->error(
                        "TrackingScripting::process - isUnifiedHandleObjectTrackedByValue, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                    *lastValue = 0;
                }
            }
#ifndef UNSAFE_TRACKER_SCRIPTING
        ...
        case GS_CMD_isUnifiedHandleObjectTrackedByTrackerInterestedInTrackableTypes:
...
#endif
            {
                ...
                if (tuho != NULL) {
                    ...
                    sp->error(
                        "TrackingScripting::process - isUnifiedHandleObjectTrackedByTrackerInterestedInTrackableTypes, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                    *lastValue = 0;
                }
            }
#ifndef UNSAFE_TRACKER_SCRIPTING
            ...
            sp->error(
                "TrackingScripting::process - isUnifiedHandleObjectTrackedByTrackerInterestedInTrackableTypes, object does not exist.");
            *lastValue = 0;
        }
    } else {
        sp->error(
            "TrackingScripting::process - isUnifiedHandleObjectTrackedByTrackerInterestedInTrackableTypes, invalid unified handle.");
        *lastValue = 0;
    }
} else {
    sp->error(
        "TrackingScripting::process - isUnifiedHandleObjectTrackedByTrackerInterestedInTrackableTypes parameter missing (non-zero bit mask value expected).");
    *lastValue = 0;
}
#endif
            break;
    ...
    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
            ...
            if (stringData != NULL) {
...
#endif
                {
                    ...
                    if (tuho != NULL) {
                        ...
                        sp->error(
                            "TrackingScripting::process - isUnifiedHandleObjectTrackedByName, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                        *lastValue = 0;
                    }
                }
#ifndef UNSAFE_TRACKER_SCRIPTING
        ...
        *lastValue = 0;
        ...
        case GS_CMD_getTrackedByForUnifiedHandleObjectByValue:
...
#endif
            {
                ...
                if (tuho != NULL) {
                    ...
                    sp->error(
                        "TrackingScripting::process - getTrackedByForUnifiedHandleObjectByValue, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                    gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                }
            }
#ifndef UNSAFE_TRACKER_SCRIPTING
            ...
            if (stringData != NULL) {
...
#endif
                {
                    ...
                    if (tuho != NULL) {
                        ...
                        sp->error(
                            "TrackingScripting::process - getTrackedByForUnifiedHandleObjectByName, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                        gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
                    }
                }
#ifndef UNSAFE_TRACKER_SCRIPTING
                ...
                sp->error(
                    "TrackingScripting::process - getTrackedByForUnifiedHandleObjectByName, object does not exist.");
                gsd->unifiedHandle = UNIFIED_HANDLE_NONE;
            }
        } else {
                    ...
                    if ( VALIDATE_UNIFIED_HANDLE_BITS(gsd->unifiedHandle) ) {
                        if ( game->unifiedHandleManager->doesObjectExist(gsd->unifiedHandle) ) {
                            ...
                            if (tuho != NULL) {
                                ...
                                sp->error(
                                    "TrackingScripting::process - deleteUnifiedHandleObjectTrackerByName, object with given unified handle is not of supported type (TrackableUnifiedHandleObject).");
                                *lastValue = 0;
                            }
                        } else {
                            sp->error(
                                "TrackingScripting::process - deleteUnifiedHandleObjectTrackerByName, object does not exist.");
                            *lastValue = 0;
                        }
                    } else {
                        sp->error(
                            "TrackingScripting::process - deleteUnifiedHandleObjectTrackerByName, invalid unified handle.");
                        *lastValue = 0;
                    }
                } else {
        ...
        case GS_CMD_addScriptableTrackerTypeVariable:
            if (stringData != NULL) {
                if (ScriptableTrackerObjectType::trackerTypeForCurrentlyRunningScript != NULL)
                    ...
                    ScriptableTrackerObjectType::trackerTypeForCurrentlyRunningScript->addScriptableTrackerVariable(
                        stringData);
                else
                    sp->error(
                        "TrackingScripting::process - addScriptableTrackerTypeVariable called outside appropriate scriptable tracker type init script.");
            } else {
        ...
        case GS_CMD_iterateTrackablesForTracker:
            ...
            sp->error(
                "TrackingScripting::process - iterateTrackablesForTracker, object with given unified handle is not a tracker.");
        }
    } else {
