            ...
            if (stringData != NULL) {
                ...
                if (game && game->bonusManager) {
                    ...
                    if ( game->gameUI->getCombatWindow(0) ) {
                        ElaborateHintMessageWindow *win =
                            ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                                  "ElaborateHintMessageWindow") );
                        if (win && !already_unlocked) {
                            win->setStyle("upgradeunlock");

                            std::string msg = "<h1>"
                                              + std::string( getLocaleGuiString("gui_elaboratehint_bonus_unlocked") )
                                              + "</h1>"
                                              + std::string( getLocaleGuiString(
                                                                 "gui_elaboratehint_bonus_unlocked_image") )
                                              + std::string( getLocaleGuiString( ("gui_newgamemenu_bonus_"
                                                                                  + name).c_str() ) );
                            if ( !game->bonusManager->areOptionsAvailable() )
                                msg += "<br><br><i>"
                                       + std::string( getLocaleGuiString("gui_elaboratehint_bonus_complete") )
                                       + "</i> ";
                            else
                                msg += " ";
                            win->showMessage(msg);
