...
namespace game
{
    ...
    public:
        ...
        void addSpawnPoint(const VC3 &position, const char *spawnerScript)
        {
            ...
            if (scriptId != 0) {
                ...
                Logger::getInstance()->warning(
                    "AlienSpawnerImpl::addSpawnPoint - Too many different spawner scripts or invalid script parameter given.");
            }
        ...
        void run(const VC3 &playerPosition)
        {
                ...
                if (spawnPoint[i].spawnerScriptId != 0) {
                    if (scriptEnabled[spawnPoint[i].spawnerScriptId]) {
                            ...
                            && distSq > ALIENSPAWNER_MIN_SPAWN_RANGE * ALIENSPAWNER_MIN_SPAWN_RANGE)
                        {
                            ...
                            float d = sqrtf(closestDistSq) + ALIENSPAWNER_SPAWN_RANGE_RAND
                                      * float(game->gameRandom->nextInt() % 101) / 100.0f;
                            closestDistSq = d * d;
                        }
                    }
                } else {
            ...
            if (closestId != -1)
                game->gameScripting->runOtherScript(
                    spawnerScripts[spawnPoint[closestId].spawnerScriptId].c_str(), "spawn", NULL,
                    spawnPoint[closestId].position);
        ...
        void spawnRandomAt(const std::string &spawner_name)
        {
            ...
            if (scriptId != -1) {
                ...
                if (results.empty() == false) {
                    ...
                    game->gameScripting->runOtherScript(spawnerScripts[scriptId].c_str(), "spawn", NULL,
                                                        spawnPoint[random_i].position);
                }
        ...
        void setSpawnerScriptEnabled(const char *spawnerScript, bool enabled)
        {
            ...
            if (!foundScript) {
                Logger::getInstance()->warning(
                    "AlienSpawnerImpl::setSpawnerScriptEnabled - No given spawner scripts found.");
