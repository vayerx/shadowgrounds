...
namespace game
{
            ...
            if (strcmp(key, "type") == 0) {
                ...
                if (parent == NULL) {
                    Logger::getInstance()->error(
                        "PartType::setRootData - Part parent type does not exist or has not yet been loaded.");
                    return false;
                }
                parentType = parent;
    ...
    {
        if (visualObjectModel != NULL) {
            delete visualObjectModel; visualObjectModel = NULL;
        }
        if (mirrorVisualObjectModel != NULL) {
            delete mirrorVisualObjectModel; mirrorVisualObjectModel = NULL;
        }
    }
