    ...
    struct EnvironmentalEffectManagerImpl {
...
#ifdef LEGACY_FILES
            filesystem::InputStream f = filesystem::FilePackageManager::getInstance().getFile(
                "Data/Effects/environmental_effect_groups.txt");
            f >> parser;
#else
            filesystem::InputStream f = filesystem::FilePackageManager::getInstance().getFile(
                "data/effect/environmental_effect_groups.txt");
            f >> parser;
#endif
        ...
        void setEffectGroup(const char *name, bool fade)
        {
            ...
            if ( list.lightningEnabled && SimpleOptions::getBool(DH_OPT_B_WEATHER_EFFECTS) ) {
                ...
                game->gameScripting->setGlobalIntVariableValue("random_lightning_wait_variation",
                                                               list.lightningVariation);
            ...
            if (particleFilename) {
                ...
                if (visualEffectId != -1) {
                    ...
                    Logger::getInstance()->error(
                        "EnvironmentalEffectManager::addParticleEffect - Effect with given name not found.");
                }
            } else {
                Logger::getInstance()->error(
                    "EnvironmentalEffectManager::addParticleEffect - Null particle filename given.");
            }
        }
...
// --
    ...
    void EnvironmentalEffectManager::disableSunlight()
    {
        ...
        impl->game->gameUI->getTerrain()->GetTerrain()->getRenderer().enableFeature(
            IStorm3D_TerrainRenderer::ForcedDirectionalLightEnabled,
            false);
#endif
    }
...
//#if !defined(PROJECT_CLAW_PROTO)
        ...
        if (impl->sunlightEnabled) {
            impl->game->gameUI->getTerrain()->GetTerrain()->getRenderer().setColorValue(
                IStorm3D_TerrainRenderer::ForcedDirectionalLightColor,
                impl->sunlightColor);
            impl->game->gameUI->getTerrain()->GetTerrain()->getRenderer().setVectorValue(
                IStorm3D_TerrainRenderer::ForcedDirectionalLightDirection,
                -impl->sunlightDirection);
        }
#endif
    }
