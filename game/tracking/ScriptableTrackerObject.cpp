...
namespace game
{
    namespace tracking
    {
            ...
            for (int i = 0; i < (int)this->type->variableNames.size(); i++) {
                if (this->type->variableNames[i] == variableName) {
                    if ( this->type->variableNames.size() == variables.size() ) {
                        ...
                        LOG_ERROR(
                            "ScriptableTrackerObject::getVariableNumberByName - Scriptable tracker object type has different amount of variables than its instance.");
                        return -1;
                    }
                }
            }
            LOG_WARNING_W_DEBUG(
                 "ScriptableTrackerObject::getVariableNumberByName - No variable found with given name.",
                variableName.c_str() );
            return -1;
        }
        ...
        void ScriptableTrackerObject::tick()
        {
            ...
            ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                this->type->script.c_str(), "tick", this->unifiedHandle);
        ...
        void ScriptableTrackerObject::trackerSignal(int trackerSignalNumber)
        {
            ...
            ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                this->type->script.c_str(), "signal", this->unifiedHandle, &params);
            trackerForCurrentlyRunningScript = UNIFIED_HANDLE_NONE;
        }
        ...
        void ScriptableTrackerObject::lostTracked()
        {
            ...
            trackerForCurrentlyRunningScript = this->unifiedHandle;
            ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                this->type->script.c_str(), "lost_tracked", this->unifiedHandle);
            trackerForCurrentlyRunningScript = UNIFIED_HANDLE_NONE;
        }
        ...
        void ScriptableTrackerObject::attachedToTrackable(ITrackableObject *trackable)
        {
            ...
            if (trackable != NULL)
                ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                    this->type->script.c_str(), "attached", this->unifiedHandle);
            else
                ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                    this->type->script.c_str(), "detached", this->unifiedHandle);
            trackerForCurrentlyRunningScript = UNIFIED_HANDLE_NONE;
        }
        ...
        void ScriptableTrackerObject::iterateTrackables(ITrackableObjectIterator *iter)
        {
            ...
            ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                this->type->script.c_str(), "iterate_trackables", this->unifiedHandle);
        ...
        void ScriptableTrackerObject::initVariableValues()
        {
            ...
            trackerForCurrentlyRunningScript = this->unifiedHandle;
            ScriptableTrackerObjectType::gameScripting->runTrackerScript(
                this->type->script.c_str(), "init_variable_values", this->unifiedHandle);
            trackerForCurrentlyRunningScript = UNIFIED_HANDLE_NONE;
        }
