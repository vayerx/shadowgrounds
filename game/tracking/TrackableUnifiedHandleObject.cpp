    ...
    namespace tracking
    {
        ...
        VC3 TrackableUnifiedHandleObject::getTrackableObjectPosition() const
        {
            ...
            if ( IS_UNIFIED_HANDLE_UNIT(handle) ) {
                ...
                assert(
                    !
                    "TrackableUnifiedHandleObject::getTrackableObjectPosition - Unified handle maps to unsupported object type.");
                return VC3(0, 0, 0);
            }
        ...
        QUAT TrackableUnifiedHandleObject::getTrackableObjectRotation() const
        {
            ...
            if ( IS_UNIFIED_HANDLE_UNIT(handle) ) {
                ...
                assert(
                    !
                    "TrackableUnifiedHandleObject::getTrackableObjectRotation - Unified handle maps to unsupported object type.");
                return QUAT();
            }
 ...
 */
        ...
        TrackableUnifiedHandleObject *TrackableUnifiedHandleObject::getInstanceFromPool(int unifiedHandle)
        {
            ...
            if (tuho_pool_used >= TUHO_POOL_SIZE * 3 / 4) {
                Logger::getInstance()->error(
                    "TrackableUnifiedHandleObject::getInstanceFromPool - Maximum allowed pool size exceeded.");
            ...
            if (tuho_pool_used == TUHO_POOL_SIZE / 2)
                Logger::getInstance()->warning(
                    "TrackableUnifiedHandleObject::getInstanceFromPool - Approaching maximum pool size.");
        ...
        void TrackableUnifiedHandleObject::addTrackedBy(UnifiedHandle tracker)
        {
            ...
            assert(
                !
                "TrackableUnifiedHandleObject::addTrackedBy - did not fit in cache... currently non-cached not supported?");
        }
        ...
        void TrackableUnifiedHandleObject::removeTrackedBy(UnifiedHandle tracker)
        {
            ...
            assert(
                !
                "TrackableUnifiedHandleObject::removeTrackedBy - was not in cache... currently non-cached not supported?");
        }
        ...
        UnifiedHandle TrackableUnifiedHandleObject::getTrackedByForType(TrackerTypeNumber trackerTypeNumber)
        {
            ...
            for (int i = 0; i < TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE; i++) {
                if (this->trackedBy[i] != UNIFIED_HANDLE_NONE) {
                    ...
                    if (t != NULL) {
                        if (tuho_game->objectTracker->getTrackerTypeNumberForTracker(this->trackedBy[i]) ==
                            trackerTypeNumber)
                        ...
                        assert(
                            !
                            "TrackableUnifiedHandleObject::getTrackedByForType - data integrity lost, tracker has been deleted but is still in cache! (bug)");
                        return UNIFIED_HANDLE_NONE;
                    }
                }
            }
            ...
            if (this->trackedByCount > TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE) {
                ...
                LOG_ERROR(
                    "TrackableUnifiedHandleObject::getTrackedByForType - TODO, need to look for the tracker the non-cached way...");
                assert(
                    !
                    "TrackableUnifiedHandleObject::getTrackedByForType - TODO, need to look for the tracker the non-cached way...");
            }
        ...
        UnifiedHandle TrackableUnifiedHandleObject::getTrackedByForTrackableTypes(
            TRACKABLE_TYPEID_DATATYPE trackableTypes)
        {
            if (this->trackedByCount == 0)
                return UNIFIED_HANDLE_NONE;
            ...
            for (int i = 0; i < TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE; i++) {
                if (this->trackedBy[i] != UNIFIED_HANDLE_NONE) {
                    ...
                    if (t != NULL) {
                        ...
                        assert(
                            !
                            "TrackableUnifiedHandleObject::getTrackedByForTrackableType - data integrity lost, tracker has been deleted but is still in cache! (bug)");
                        return UNIFIED_HANDLE_NONE;
                    }
                }
            }
            ...
            if (this->trackedByCount > TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE) {
                ...
                // change the ObjectTracker::getAllTrackers to return an iterator instead of vector and make it public...
                LOG_ERROR(
                    "TrackableUnifiedHandleObject::getTrackedByForTrackableType - TODO, need to look for the tracker the non-cached way...");
                assert(
                    !
                    "TrackableUnifiedHandleObject::getTrackedByForTrackableType - TODO, need to look for the tracker the non-cached way...");
            }
        ...
        UnifiedHandle TrackableUnifiedHandleObject::getTrackedByWithIndex(int index)
        {
            ...
            for (int i = 0; i < TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE; i++) {
                if (this->trackedBy[i] != UNIFIED_HANDLE_NONE) {
                    ...
                    if (t != NULL) {
                        ...
                        assert(
                            !
                            "TrackableUnifiedHandleObject::getTrackedByWithIndex - data integrity lost, tracker has been deleted but is still in cache! (bug)");
                        return UNIFIED_HANDLE_NONE;
                    }
                }
            }
            ...
            if (this->trackedByCount > TRACKABLEUNIFIEDHANDLEOBJECT_TRACKED_BY_CACHE_SIZE) {
                ...
                // change the ObjectTracker::getAllTrackers to return an iterator instead of vector and make it public...
                LOG_ERROR(
                    "TrackableUnifiedHandleObject::getTrackedByWithIndex - TODO, need to look for the tracker the non-cached way...");
                assert(
                    !
                    "TrackableUnifiedHandleObject::getTrackedByWithIndex - TODO, need to look for the tracker the non-cached way...");
            }
