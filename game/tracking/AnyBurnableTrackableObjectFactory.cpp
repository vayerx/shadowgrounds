...
namespace game
{
    namespace tracking
    {
        ...
        AnyBurnableTrackableObjectFactory::AnyBurnableTrackableObjectFactory(std::vector<
                                                                                 ITrackableUnifiedHandleObjectImplementationManager
                                                                                 *> implementations)
        {
        ...
        void AnyBurnableTrackableObjectFactory::addImplementation(
            ITrackableUnifiedHandleObjectImplementationManager *implementation)
        {
            // TODO: assert that the implementation is not already in the list
            impl->implementations.push_back(implementation);
        }

        void AnyBurnableTrackableObjectFactory::removeImplementation(
            ITrackableUnifiedHandleObjectImplementationManager *implementation)
        {
            ...
            assert(
                !"AnyBurnableTrackableObjectFactory::removeImplementation - implementation was not found in the list.");
        }
        ...
        ITrackableObjectIterator *AnyBurnableTrackableObjectFactory::getTrackablesFromArea(const VC3 &globalPosition,
                                                                                           float      radius)
        {
            ...
            for (int i = 0; i < (int)impl->implementations.size(); i++) {
                ITrackableUnifiedHandleObjectIterator *iter =
                    impl->implementations[i]->getTrackableUnifiedHandleObjectsFromArea(globalPosition,
                                                                                       radius,
                                                                                       TRACKABLE_TYPE_BURNABLE);
