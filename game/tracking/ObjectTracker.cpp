    ...
    namespace tracking
    {
        ...
        private:
            ...
            void addTrackableObjectFactory(ITrackableObjectFactory *trackableObjectFactory)
            {
                ...
                LOG_ERROR(
                    "ObjectTracker::addTrackerType - Factory had invalid trackable object type value (expected single bit value).");
                assert(
                    !
                    "ObjectTracker::addTrackerType - Factory had invalid trackable object type value (expected single bit value).");
                    ...
                    if (trackers[i] != NULL) {
                        ...
                        if (attachedTrackables[i] != NULL) {
                            if (attachedTrackables[i]->getTypeId() == TrackableUnifiedHandleObject::typeId)
                                ...
                                ( (TrackableUnifiedHandleObject *)attachedTrackables[i] )->removeTrackedBy(
                                    UNIFIED_HANDLE_FIRST_TRACKER + i);
            ...
            void deleteTracker(UnifiedHandle trackerUnifiedHandle)
            {
                ...
                if (i < OBJECTTRACKER_MAX_TRACKERS) {
                    if (trackers[i] != NULL) {
                        ...
                        LOG_WARNING(
                            "ObjectTracker::deleteTracker - Tracker with given handle not found in list (already deleted?).");
                    }
                } else {
            ...
            void iterateTrackablesForTracker(ITrackerObject *tracker)
            {
                ...
                if (typeMask == 0) {
                    ...
                    Logger::getInstance()->warning(
                        "ObjectTracker::getTrackablesForTracker - Tracker is not interested in any trackable object types but iterateTrackablesForTracker was called.");
                    return;
                }
                ...
                if (!foundFactories) {
                    Logger::getInstance()->warning(
                        "ObjectTracker::getTrackablesForTracker - No trackable object factory for any of the tracker's requested trackable types.");
                    return;
                }
            ...
            void signalToTrackerFromTrackable(UnifiedHandle trackable, int trackerSignalNumber)
            {
                ...
                if (trackableObject != NULL) {
                    if (trackableObject->getTypeId() == TrackableUnifiedHandleObject::typeId) {
                        ...
                        int index = 0;
                        while (true) {
                            UnifiedHandle trackerUH =
                                ( (TrackableUnifiedHandleObject *)trackableObject )->getTrackedByWithIndex(index);
                        ...
                        Logger::getInstance()->error(
                            "ObjectTracker::signalToTrackerFromTrackable - Only TrackableUnifiedHandleObject type trackable supported.");
                        assert(
                            !
                            "ObjectTracker::signalToTrackerFromTrackable - Only TrackableUnifiedHandleObject type trackable supported.");
                    }
                    ...
                    Logger::getInstance()->error(
                        "ObjectTracker::signalToTrackerFromTrackable - Failed to get TrackableUnifiedHandleObject from pool with given unified handle.");
                    assert(
                        !
                        "ObjectTracker::signalToTrackerFromTrackable - Failed to get TrackableUnifiedHandleObject from pool with given unified handle.");
                }
            ...
            ITrackerObject *getTrackerByUnifiedHandle(UnifiedHandle handle)
            {
                ...
                if (i >= 0 && i < OBJECTTRACKER_MAX_TRACKERS) {
                    ...
                    Logger::getInstance()->error(
                        "ObjectTracker::getTrackerByUnifiedHandle - Given unified handle out of range.");
            ...
            TrackerTypeNumber getTrackerTypeNumberForTracker(UnifiedHandle handle)
            {
                ...
                if (i >= 0 && i < OBJECTTRACKER_MAX_TRACKERS) {
                    ...
                    Logger::getInstance()->error(
                        "ObjectTracker::getTrackerTypeNumberByUnifiedHandle - Given unified handle out of range.");
                            ...
                            if ( trackers[i]->getType()->doesAllowTickBalancing() )
                                if (runTrackerTicks >= OBJECTTRACKER_BALANCING_PREFERRED_SIMULTANEOUS_TRACKERS) {
                                    ...
                                    if (balancingTickDelay > 0)
                                        runNow = false;
                                    if (currentTime < trackerLastCallTimes[i]
                                        + ( trackers[i]->getType()->getTickInterval()
                                            * (100
                                               + OBJECTTRACKER_BALANCING_MAX_TICK_INTERVAL_VARIATION_PERCENTAGE) )
                                        / 100)
                                        runNow = false;
                                }
                                ...
                                if (trackers[i] != NULL) {
                                    if ( trackers[i]->getType()->doesAllowTickBalancing() ) {
                                        ...
                                        if (runTrackerTicks >= OBJECTTRACKER_BALANCING_PREFERRED_SIMULTANEOUS_TRACKERS)
                                            ...
                                            trackerLastCallTimes[i] = currentTime
                                                                      + (SystemRandom::getInstance()->nextInt()
                                                                         & 1) * msec;
                                        else
                                            trackerLastCallTimes[i] = currentTime;
                                    } else {
        ...
        std::string ObjectTracker::getStatusInfo()
        {
                ...
                && impl->runsSinceLastDump > 0)
            {
                ...
                ret += std::string("\r\n");
                ret += std::string("Tracker balancing, peak tracker ticks per run: ") + int2str(
                    impl->peakTrackerTicksPerRun);
                ret += std::string("\r\n");
                ...
                ret += std::string("Tracker balancing, avg tracker ticks per run: ")
                       + "N/A (first tracker dump or no runs)";
                ret += std::string("\r\n");
                ret += std::string("Tracker balancing, peak tracker ticks per run: ")
                       + "N/A (first tracker dump or no runs)";
                ret += std::string("\r\n");
            }
#ifdef OBJECTTRACKER_COLLECT_PERF_STATS
