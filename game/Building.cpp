...
namespace game
{
    ...
    void Building::addPhysics(GamePhysics *gamePhysics)
    {
        if (visualObject != NULL) {
            ...
            QUAT rot = QUAT();
            physicsObject = new StaticPhysicsObject(gamePhysics, modelFilename,
                                                    visualObject->getStormModel(), position, rot);
            ...
            if (soundindex == SOUNDMATERIALPARSER_NO_SOUND_INDEX)
                Logger::getInstance()->warning(
                    "Building::addPhysics - Failed to solve sound material index for \"Building\" sound material name (material definition missing).");
