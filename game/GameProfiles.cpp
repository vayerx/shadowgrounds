                        ...
                        if (sp.getValue() != NULL) {
                            if ( doesProfileExist( sp.getValue() ) )
                                ...
                                Logger::getInstance()->warning(
                                    "GameProfilesImpl::loadCurrentProfile - Current profile data does not exist (falling back to Default).");
                        }
...
#else
            copyFile( "data/misc/default_keybinds.txt",
                      igios_mapUserDataPrefix("profiles/" + profile + "/config/keybinds.txt") );
#endif
            ...
            // ( currently does not delete its subfolders )
            {
                ...
                for (i = 0; i < (int)tmplist.size(); i++) {
                    ...
                    if (DeleteFile( tmplist[i].c_str() ) == false)
                        Logger::getInstance()->error( ("GameProfilesImpl::removeDirectory() Could not remove file: "
                                                       + tmplist[i]).c_str() );
                }
            }
