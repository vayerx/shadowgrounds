    ...
    public:
        ...
        void runCollect()
        {
            ...
            if (u != NULL) {
                ...
                this->exp[currentEntry] =
                    game->gameScripting->getGlobalIntVariableValue( ("survivor_current_expo["
                                                                     + boost::lexical_cast<std::string>(playerNum)
                                                                     + "]").c_str() );
#endif
            } else {
...
#endif
                {
                    for (int i = 0; i < GAMESTATS_MAX_AMMO; i++) {
                        if (u != NULL
                            ...
                            && u->getWeaponType( u->getWeaponByWeaponType( PlayerWeaponry::getWeaponIdByUINumber(u,
                                                                                                                 i) ) )
                            != NULL)
                        {
                            fprintf( f, "%s;",
                                     u->getWeaponType( u->getWeaponByWeaponType( PlayerWeaponry::getWeaponIdByUINumber(
                                                                                     u,
                                                                                     i) ) )
                                     ->getPartTypeIdString() );
                        } else {
                    ...
                    for (int e = 0; e < totalEntries; e++) {
...
#endif
                        {
                            for (int i = 0; i < GAMESTATS_MAX_AMMO; i++) {
                                if (u != NULL
                                    && PlayerWeaponry::getWeaponIdByUINumber(u, i) != -1
                                    && u->getWeaponByWeaponType( PlayerWeaponry::getWeaponIdByUINumber(u, i) ) != -1
                                    && u->getWeaponType( u->getWeaponByWeaponType( PlayerWeaponry::
                                                                                   getWeaponIdByUINumber(u,
                                                                                                         i) ) ) != NULL)
                                {
                        ...
                        }
                        {
                            for (int i = 0; i < GAMESTATS_MAX_AMMO; i++) {
                                ...
                                {
                                    int maxammo =
                                        u->getWeaponMaxAmmoAmount( u->getWeaponByWeaponType( PlayerWeaponry::
                                                                                             getWeaponIdByUINumber(
                                                                                                 u, i) ) );
                                    if (maxammo <= 0)
                                        maxammo = 1;
        ...
        void updateScoreList(std::vector<GameStats::ScoreData> &scores,
