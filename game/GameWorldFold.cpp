        ...
        if (position.x > 0) {
            ...
            for (int i = 0; i < positiveFoldsUsed; i++) {
                if (positiveFoldPositions[i].x > position.x) {
                    LOG_ERROR(
                        "GameWorldFold::addFold - Attempt to add a positive position fold between existing folds (need to add after the last positive position fold).");
                    return 0;
                }
            }
            ...
            for (int i = 0; i < negativeFoldsUsed; i++) {
                if (negativeFoldPositions[i].x < position.x) {
                    LOG_ERROR(
                        "GameWorldFold::addFold - Attempt to add a negative position fold between existing folds (need to add after the last negative position fold).");
                    return 0;
                }
            }
