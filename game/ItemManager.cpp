...
namespace game
{
    ...
    Item *ItemManager::createNewItem(int itemId, const VC3 &position)
    {
        ...
        if (itemId >= 0 && itemId < MAX_ITEM_TYPES) {
            ...
            if (itemTypes[itemId].getModelFilename() != NULL) {
                VisualObjectModel *vom = game->visualObjectModelStorage->getVisualObjectModel(
                     itemTypes[itemId].getModelFilename() );
                VisualObject *vo = vom->getNewObjectInstance();
                item->setVisualObject(vo);
                        ...
                        while ( uiter.iterateAvailable() ) {
                            ...
                            if ( u->isActive() && !u->isDestroyed() ) {
                                    ...
                                    && heightdiff < ITEM_PICKUP_HEIGHT / 2.0f)
                                {
                                    ...
                                    game->gameUI->gameMessage(getLocaleGuiString(txt), NULL,
                                                              it->getTipPriority(), 700,
                                                              GameUI::MESSAGE_TYPE_EXECUTE_TIP);
                                    break;
                                }
                            }
                        }
    ...
    Item *ItemManager::getNearestItemOfType(const VC3 &position, ItemType *itemType)
    {
        ...
        if (itemTypeId == -1) {
            Logger::getInstance()->warning(
                "ItemManager::getNearestItemOfType - Failed to solve proper id for given itemType (internal error?).");
            fb_assert(
                !"ItemManager::getNearestItemOfType - Failed to solve proper id for given itemType (internal error?).");
            return NULL;
        }
    ...
    ItemSpawner *ItemManager::createSpawner(const char        *group_name,
                                            int                item_id,
