...
namespace frozenbyte {
    namespace physics {
        ...
        D6Joint::D6Joint(NxScene &scene, const NxD6JointDesc &desc, boost::shared_ptr<ActorBase> &a,
        ...
        {
            deformData.reset( new DeformData(info->bendAngle, info->breakAngle, info->durability,
                                             info->resetCollisionGroupOnBreak, info->resetAngularDampingOnBreak) );
        }
        ...
        void D6Joint::handleDeforming(NxScene &scene)
        {
            ...
            if (dotToOriginal < deformData->breakCosAngle || deformData->durability < 0 || joint->getState() ==
                NX_JS_BROKEN) {
                // break
                scene.releaseJoint(*joint);
                joint = NULL;
