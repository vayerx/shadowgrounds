...
// trying to get some sense into physics lib init with this... --jpk
        ...
        public:
            ...
            boost::shared_ptr<BoxActor> createBoxActor( const VC3 &sizes, const VC3 &position,
                                                        const VC3 &localPosition = VC3() );
            boost::shared_ptr<CapsuleActor> createCapsuleActor(float      height,
                                                               float      radius,
                                                               const VC3 &position,
                                                               float      offset = 0.0f,
                                                               int        axisNumber = 1);
            boost::shared_ptr<ConvexActor> createConvexActor(const boost::shared_ptr<ConvexMesh> &mesh,
                                                             const VC3                           &position);
            boost::shared_ptr<StaticMeshActor> createStaticMeshActor(const boost::shared_ptr<StaticMesh> &mesh,
                                                                     const VC3                           &position,
                                                                     const QUAT                          &rotation);
            boost::shared_ptr<HeightmapActor> createHeightmapActor(const unsigned short *buffer,
                                                                   int                   samplesX,
                                                                   int                   samplesY,
                                                                   const VC3            &size);
            ...
            // Joints
            boost::shared_ptr<SphericalJoint> createSphericalJoint(boost::shared_ptr<ActorBase> &a,
            ...
            boost::shared_ptr<Fluid> createFluid(FluidType fluidType,
                                                 int       maxParticles,
            ...
            bool checkOverlapOBB(const VC3    &center,
                                 const VC3    &radius,
                                 const QUAT   &rotation,
