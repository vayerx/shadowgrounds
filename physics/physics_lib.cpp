...
int frozenbyte::physics::physicslib_group_cont[PHYSICS_MAX_COLLISIONGROUPS][PHYSICS_MAX_COLLISIONGROUPS] = {
    { PHYSICSLIB_GROUP_CONTACT_NONE }
};
bool frozenbyte::physics::physicslib_group_coll[PHYSICS_MAX_COLLISIONGROUPS][PHYSICS_MAX_COLLISIONGROUPS] = {
    { false }
};
...
//            if (maxForceLen > 0.0f)
                if (maxForceLen >= 0.0f) {
                    ...
                    ContactListEntry entry(contactActor1,
                                           contactActor2,
                                           contactNormal,
                                           contactPosition,
                                           contactForceLen);
...
// physx appears to use uninitialized memory
// which causes crashes
// therefore this hack which zeroes out all memory before handing it to PhysX
// TODO: should use C++ new[] / delete[]
// TODO: should have debug logging which dumps all this
// in case we want to try and track down the real issue
        class HaxAllocator : public NxUserAllocator {
        public:
            ...
            {
                // FIXME: can't zero out without keeping track of all areas
                // because we would need to know size of old area
                void *newptr = ::realloc(memory, size);
                return newptr;
            }
        ...
        static HaxAllocator hackAllocator;
                ...
                physicslib_fluid_containment_sphere_shape(0)
            {
                ...
                if (params->scriptRunner == NULL)
                    ::Logger::getInstance()->warning(
                        "PhysicsLib - No script runner given, collision groups, etc. will be initialized to default values.");

                NxPhysicsSDKDesc sdkDesc;
                if (!useHardware)
                    sdkDesc.flags |= NX_SDKF_NO_HARDWARE;

                sdk = NxCreatePhysicsSDK(NX_PHYSICS_SDK_VERSION, &hackAllocator, getLogger(), sdkDesc);
                //sdk = NxCreatePhysicsSDK(NX_PHYSICS_SDK_VERSION);

                if (sdk == NULL) {
                    ::Logger::getInstance()->error("PhysicsLib - Failed to create PhysX SDK");
                    sysFatalError("PhysicsLib - Failed to create PhysX SDK");
                }
                ...
                if (sdk) {
                    ...
                    if (scene) {
                        ...
                        if (params->scriptRunner != NULL) {
                            ...
                            if (!ok)
                                ::Logger::getInstance()->error(
                                    "PhysicsLib - Failed to run physics_init:set_contacts script (script/sub may not be loaded or did not return expected value).");
                        }
                        ...
                        for (int i = 0; i < PHYSICS_MAX_COLLISIONGROUPS; i++) {
                            for (int j = 0; j < PHYSICS_MAX_COLLISIONGROUPS; j++) {
                                if (physicslib_group_cont[i][j] != physicslib_group_cont[j][i]) {
                                    ::Logger::getInstance()->error(
                                        "PhysicsLib - Improperly mirrored physics contacts data (group1 -> group2 does not have same flag as group2 -> group1).");
                        ...
                        if (params->scriptRunner != NULL) {
                            ...
                            if (!ok)
                                ::Logger::getInstance()->error(
                                    "PhysicsLib - Failed to run physics_init:set_collision script (script/sub may not be loaded or did not return expected value).");
                        }
                        ...
                        for (int i = 0; i < PHYSICS_MAX_COLLISIONGROUPS; i++) {
                            for (int j = 0; j < PHYSICS_MAX_COLLISIONGROUPS; j++) {
                                if (physicslib_group_coll[i][j] != physicslib_group_coll[j][i]) {
                                    ::Logger::getInstance()->error(
                                        "PhysicsLib - Improperly mirrored physics collision data (group1 -> group2 does not have same flag as group2 -> group1).");
 ...
 */
        boost::shared_ptr<BoxActor> PhysicsLib::createBoxActor(const VC3 &sizes,
                                                               const VC3 &position,
        ...
        boost::shared_ptr<CapsuleActor> PhysicsLib::createCapsuleActor(float      height,
                                                                       float      radius,
                                                                       const VC3 &position,
                                                                       float      offset,
                                                                       int        axisNumber)
        ...
        boost::shared_ptr<ConvexActor> PhysicsLib::createConvexActor(const boost::shared_ptr<ConvexMesh> &mesh,
                                                                     const VC3                           &position)
        {
            ConvexActor *actor = 0;
        ...
        boost::shared_ptr<StaticMeshActor> PhysicsLib::createStaticMeshActor(
        ...
        boost::shared_ptr<HeightmapActor> PhysicsLib::createHeightmapActor(const unsigned short *buffer,
                                                                           int                   samplesX,
                                                                           int                   samplesY,
                                                                           const VC3            &size)
        {
        ...
        boost::shared_ptr<SphericalJoint> PhysicsLib::createSphericalJoint(boost::shared_ptr<ActorBase> &a,
                                                                           boost::shared_ptr<ActorBase> &b,
                                                                           const VC3                    &globalAnchor)
        {
...
#ifndef NX_DISABLE_FLUIDS
        boost::shared_ptr<Fluid> PhysicsLib::createFluid(FluidType fluidType,
                                                         int       maxParticles,
...
// HACK: fluid containment plane quick hack, should rewrite this...
        ...
        void PhysicsLib::createOrMoveFluidContainmentSphere(const VC3 &position, float radius)
        {
            ...
            if (data->physicslib_fluid_containment_sphere_actor == NULL) {
                ...
                if (data->physicslib_fluid_containment_sphere_actor) {
                    data->physicslib_fluid_containment_sphere_actor->setGroup(
                        PHYSICS_COLLISIONGROUP_FLUID_CONTAINMENT_PLANES);
                ...
                if (data->physicslib_fluid_containment_sphere_actor) {
                    ...
                    data->physicslib_fluid_containment_sphere_actor->setGlobalPosition( NxVec3(position.x, position.y,
                                                                                               position.z) );
        ...
        void PhysicsLib::startSimulation(float timeDelta)
        {
            ...
            data->startSimulationTime = SDL_GetTicks();
            data->scene->simulate(timeDelta);
            data->scene->flushStream();

            data->statsSimulationStartWaitTime = SDL_GetTicks() - data->startSimulationTime;
        }
        ...
        void PhysicsLib::finishSimulation()
        {
            ...
            int startWaitTime = SDL_GetTicks();

            NxU32 errorState = 0;
            data->scene->fetchResults(NX_RIGID_BODY_FINISHED, true, &errorState);

            int endSimulationTime = SDL_GetTicks();
        ...
        std::string PhysicsLib::getLoggableStatistics() const
        {
            ...
            if (data->scene) {
                result += std::string("") + boost::lexical_cast<std::string>(data->statsNumDynamicActors) + std::string(
                    ";");
                result += std::string("")
                          + boost::lexical_cast<std::string>(data->statsNumDynamicActorsInAwakeGroups) + std::string(
                    ";");
                ...
                result += std::string("") + boost::lexical_cast<std::string>( physics::getFluidParticleCount() )
                          + std::string(";");
#endif
                result += std::string("") + boost::lexical_cast<std::string>(data->statsSimulationStartWaitTime)
                          + std::string(";");
                result += std::string("") + boost::lexical_cast<std::string>(data->statsSimulationWaitTime)
                          + std::string(";");
            }
        ...
        std::string PhysicsLib::getStatistics() const
        {
            ...
            if (data->scene) {
                result += std::string("Actors: ") + boost::lexical_cast<std::string>(data->statsNumActors)
                          + std::string("\n");
                result += std::string("Dynamic actors: ") + boost::lexical_cast<std::string>(
                    data->statsNumDynamicActors) + std::string("\n");
                result += std::string("Active actors: ") + boost::lexical_cast<std::string>(
                    data->statsNumDynamicActorsInAwakeGroups) + std::string("\n");
                result += std::string("Max active actors: ") + boost::lexical_cast<std::string>(
                    data->statsMaxDynamicActorsInAwakeGroups) + std::string("\n");
                result += std::string("Contacts reported: ") + boost::lexical_cast<std::string>(data->statsContacts)
                          + std::string("\n");
#ifndef NX_DISABLE_FLUIDS
                result += std::string("Fluids: ") + boost::lexical_cast<std::string>( getFluidBaseCount() )
                          + std::string("\n");
                result += std::string("Fluid particles: ") + boost::lexical_cast<std::string>(
                     physics::getFluidParticleCount() ) + std::string("\n");
#endif
                result += std::string("Physics time: ") + boost::lexical_cast<std::string>(data->statsSimulationTime)
                          + std::string("\n");
                result += std::string("Physics wait for start: ") + boost::lexical_cast<std::string>(
                    data->statsSimulationStartWaitTime) + std::string("\n");
                result += std::string("Physics wait for end: ") + boost::lexical_cast<std::string>(
                    data->statsSimulationWaitTime);
        ...
        bool PhysicsLib::checkOverlapOBB(const VC3    &center,
                                         const VC3    &radius,
                                         const QUAT   &rotation,
        ...
        void PhysicsLib::enableFeature(PhysicsLib::Feature feature, bool enable)
        {
            ...
            if ( !data->scene->isWritable() ) {
                ::Logger::getInstance()->warning(
                    "PhysicsLib::enableFeature - Attempt to set physics features while physics running (pause game/physics update first).");
                return;
            }
