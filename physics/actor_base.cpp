...
namespace frozenbyte {
    namespace physics {
        ...
        {
            actor->addForceAtPos(NxVec3(impulse.x, impulse.y, impulse.z), NxVec3(position.x,
                                                                                 position.y,
                                                                                 position.z),
                                 smooth ? NX_SMOOTH_IMPULSE : NX_IMPULSE);
        }
        ...
        {
            actor->addForceAtPos(NxVec3(force.x, force.y, force.z), NxVec3(position.x,
                                                                           position.y,
                                                                           position.z), NX_ACCELERATION);
        }
        ...
        void ActorBase::addVelocityChange(const VC3 &position, const VC3 &force, bool smooth)
        {
            actor->addForceAtPos(NxVec3(force.x, force.y, force.z), NxVec3(position.x,
                                                                           position.y,
                                                                           position.z),
                                 smooth ? NX_SMOOTH_VELOCITY_CHANGE  : NX_VELOCITY_CHANGE);
        }
        ...
        {
            actor->addTorque(NxVec3(torque.x,
                                    torque.y,
                                    torque.z), smooth ? NX_SMOOTH_VELOCITY_CHANGE  : NX_VELOCITY_CHANGE);
        }
