...
namespace frozenbyte {
    namespace particle {
        ...
        void PointParticleRenderer::render(IStorm3D_Scene             *scene,
                                           IStorm3D_Material          *mtl,
                                           GenParticleSystemEditables &eds,
        ...
        void QuadParticleRenderer::render(IStorm3D_Scene             *scene,
                                          IStorm3D_Material          *mtl,
                                          GenParticleSystemEditables &eds,
                                          std::vector<Particle>      &parts,
                                          const COL                  &factor,
        ...
        void LineParticleRenderer1::render(IStorm3D_Scene             *scene,
                                           IStorm3D_Material          *mtl,
                                           GenParticleSystemEditables &eds,
                                           std::vector<Particle>      &parts,
                                           const COL                  &factor,
        ...
        void LineParticleRenderer2::render(IStorm3D_Scene             *scene,
                                           IStorm3D_Material          *mtl,
                                           GenParticleSystemEditables &eds,
                                           std::vector<Particle>      &parts,
                                           const COL                  &factor,
 ...
 */
        ...
        void GenParticleSystem::defaultPrepareForLaunch(IStorm3D                         *s3d,
                                                        IStorm3D_Scene                   *scene,
                                                        const GenParticleSystemEditables &eds)
        {
            int reserveAmount = eds.maxParticles;
            if (eds.physicsType == GenParticleSystemEditables::PHYSICS_TYPE_FLUID || eds.physicsType ==
                GenParticleSystemEditables::PHYSICS_TYPE_FLUID_INTERACTION)
...
// TEMP TEST
                    ...
                    if (eds.particleAnimationType == GenParticleSystemEditables::ANIMATION_LOOP)
                        ...
                        frameSpeed =
                            ( (float)eds.particleAnimationFrameCount
                              - eds.particleAnimationStartFrame ) / eds.particleLife;
                    ...
                    for (int i = 0; i < fluid_part_amount; ++i) {
                        ...
                        // Hax the variation
                        {
                            ...
                            p.frame += eds.particleAnimationStartFrameVar
                                       * (float)(gameRand->nextInt() % 10301) / 10300.0f;
                        }
                    ...
                    if ( fluid && fluid->canSpawn() ) {
                        ...
                        if (create_amount > 0) {
                            ...
                            for (int i = 0; i < create_amount; ++i) {
                                ...
                                if (eds.launchDirectionType == GenParticleSystemEditables::DIRECTION_EXPLOSION ||
                                    eds.launchDirectionType ==
                                    GenParticleSystemEditables::DIRECTION_NEGATIVE_EXPLOSION) {
                                    dir = p.position;
                                    dir -= explosion_position;

                                    if (dir.GetSquareLength() > 0.01f) {
                                        dir.Normalize();
                                        if (eds.launchDirectionType ==
                                            GenParticleSystemEditables::DIRECTION_NEGATIVE_EXPLOSION)
                                            dir = -dir;
                                    } else {
                                        dir = VC3();
                                    ...
                                    || eds.emitterVariation.z != 0)
                                {
                                    p.position.x +=
                                        ( (float)(rand()
                                                  % 1001)
                                          / 1000.0f ) * eds.emitterVariation.x * 2 - eds.emitterVariation.x;
                                    p.position.y +=
                                        ( (float)(rand()
                                                  % 1001)
                                          / 1000.0f ) * eds.emitterVariation.y * 2 - eds.emitterVariation.y;
                                    p.position.z +=
                                        ( (float)(rand()
                                                  % 1001)
                                          / 1000.0f ) * eds.emitterVariation.z * 2 - eds.emitterVariation.z;
                                }
...
#  endif
                    ...
                    for (int i = 1; i < bufferSize + 1; ++i) {
                        ...
                        if (!p.alive) {
                            ...
                            p.life = eds.particleLife +  eds.particleLifeVar
                                     * (float)(rand() - RAND_MAX / 2) / (float)RAND_MAX;
                            p.angle = eds.particleStartAngle + eds.particleStartAngleVar
                                      * (float)(rand() - RAND_MAX / 2) / (float)RAND_MAX;
                            p.angleSpeed = eds.particleSpin + eds.particleSpinVar
                                           * (float)(rand() - RAND_MAX / 2) / (float)RAND_MAX;
                            ...
                            if (eds.particleAnimationType == GenParticleSystemEditables::ANIMATION_LOOP)
                                ...
                                p.frameSpeed =
                                    ( (float)eds.particleAnimationFrameCount - p.frame ) / p.life * PARTICLE_TIME_SCALE;
                                ...
                                || eds.emitterVariation.z != 0)
                            {
                                p.position.x +=
                                    ( (float)(rand()
                                              % 1001) / 1000.0f ) * eds.emitterVariation.x * 2 - eds.emitterVariation.x;
                                p.position.y +=
                                    ( (float)(rand()
                                              % 1001) / 1000.0f ) * eds.emitterVariation.y * 2 - eds.emitterVariation.y;
                                p.position.z +=
                                    ( (float)(rand()
                                              % 1001) / 1000.0f ) * eds.emitterVariation.z * 2 - eds.emitterVariation.z;
                            }
                            m_rotation.RotateVector(p.position);
 ...
 */
        ...
        void GenParticleSystem::defaultParseFrom(const ParserGroup &pg, GenParticleSystemEditables &eds) {
            ...
            eds.dieAfterEmission =
                static_cast<bool>( convertFromString<int>(pg.getValue("die_after_emission", ""), eds.dieAfterEmission) );
            eds.velocityInheritanceFactor = convertFromString<float>(pg.getValue("velocity_inheritance_factor",
                                                                                 ""), eds.velocityInheritanceFactor);
            eds.launchSpeed =
                convertFromString<float>(pg.getValue("launch_speed", ""), eds.launchSpeed) * PARTICLE_TIME_SCALE;
            eds.launchSpeedVar =
                convertFromString<float>(pg.getValue("launch_speed_var", ""), eds.launchSpeedVar) * PARTICLE_TIME_SCALE;
            ...
            eds.particleStartAngle = convertFromString<float>(pg.getValue("particle_start_angle",
                                                                          ""), eds.particleStartAngle);
            eds.particleStartAngleVar = convertFromString<float>(pg.getValue("particle_start_angle_var", ""), 0);
            eds.particleSpin =
                convertFromString<float>(pg.getValue("particle_spin", ""), eds.particleSpin) * PARTICLE_TIME_SCALE;
            eds.particleSpinVar =
                convertFromString<float>(pg.getValue("particle_spin_var",
                                                     ""), eds.particleSpinVar) * PARTICLE_TIME_SCALE;
            eds.particleTexture = pg.getValue("texture", eds.particleTexture);
            eds.particleTextureUSubDivs = convertFromString<int>(pg.getValue("texture_sub_div_u",
                                                                             "1"), eds.particleTextureUSubDivs);
            eds.particleTextureVSubDivs = convertFromString<int>(pg.getValue("texture_sub_div_v",
                                                                             "1"), eds.particleTextureVSubDivs);
            eds.particleTextureAlphaType = convertFromString<int>(pg.getValue("texture_alpha_type",
                                                                              ""), eds.particleTextureAlphaType);
            eds.particleAnimationFrameCount = convertFromString<int>(pg.getValue("animation_frame_count",
                                                                                 ""), eds.particleAnimationFrameCount);
            eds.particleAnimationStartFrame = convertFromString<float>(pg.getValue("animation_start_frame",
                                                                                   ""), eds.particleAnimationStartFrame);
            eds.particleAnimationStartFrameVar = convertFromString<float>(pg.getValue("animation_start_frame_var",
                                                                                      ""),
                                                                          eds.particleAnimationStartFrameVar);
            eds.particleAnimationFps = convertFromString<float>(pg.getValue("animation_fps",
                                                                            ""),
                                                                eds.particleAnimationFps) * PARTICLE_TIME_SCALE;
...
#  pragma message ("***************************************************")
            ...
            eds.fluidStaticRestitution = convertFromString<float>(pg.getValue("fluid_static_restitution",
                                                                              ""), eds.fluidStaticRestitution);
            eds.fluidStaticAdhesion = convertFromString<float>(pg.getValue("fluid_static_adhesion",
                                                                           ""), eds.fluidStaticAdhesion);
            eds.fluidDynamicRestitution = convertFromString<float>(pg.getValue("fluid_dynamic_restitution",
                                                                               ""), eds.fluidDynamicRestitution);
            eds.fluidDynamicAdhesion = convertFromString<float>(pg.getValue("fluid_dynamic_adhesion",
                                                                            ""), eds.fluidDynamicAdhesion);
            ...
            eds.fluidPacketSizeMultiplier = convertFromString<int>(pg.getValue("fluid_packet_size_multiplier",
                                                                               ""), eds.fluidPacketSizeMultiplier);
            eds.fluidMaxEmitterAmount = convertFromString<int>(pg.getValue("fluid_max_emitter_amount",
                                                                           ""), eds.fluidMaxEmitterAmount);
            eds.fluidDetailCollisionGroup = convertFromString<int>(pg.getValue("fluid_detail_collision_group",
                                                                               ""), eds.fluidDetailCollisionGroup);
            ...
            eds.fluidKernelRadiusMultiplier = convertFromString<float>(pg.getValue("fluid_kernel_radius_multiplier",
                                                                                   ""), eds.fluidKernelRadiusMultiplier);
            eds.fluidRestParticlesPerMeter = convertFromString<float>(pg.getValue("fluid_rest_particles_per_meter",
                                                                                  ""), eds.fluidRestParticlesPerMeter);
