 ...
 */
            boost::shared_ptr<PhysicsActor> createActor(boost::shared_ptr<PhysicsMesh> &mesh,
                                                        const VC3                      &position,
                                                        const QUAT                     &rotation,
                                                        const VC3                      &velocity,
                                                        const VC3                      &angularVelocity,
                                                        float                           mass,
                                                        int                             collisionGroup,
                                                        int                             soundGroup)
            {
...
#ifndef NX_DISABLE_FLUIDS
            boost::shared_ptr<PhysicsFluid> createFluid(int   type,
                                                        int   maxParticles,
                                                        float fluidStaticRestitution,
                                                        float fluidStaticAdhesion,
                                                        float fluidDynamicRestitution,
        ...
        void ParticlePhysics::physicsExplosion(const VC3 &explosionPosition, float forceFactor, float radius)
        {
            ...
            for (PhysicsActorList::iterator it = data->physicsActorList.begin();
                 it != data->physicsActorList.end();
                 ++it) {
        ...
        boost::shared_ptr<PhysicsMesh> ParticlePhysics::createConvexMesh(const char            *filename,
                                                                         IStorm3D_Model_Object *object)
        {
            boost::shared_ptr<PhysicsMesh> mesh( new PhysicsMesh() );
        ...
        boost::shared_ptr<PhysicsActor> ParticlePhysics::createActor(boost::shared_ptr<PhysicsMesh> &mesh,
                                                                     const VC3                      &position,
                                                                     const QUAT                     &rotation,
                                                                     const VC3                      &velocity,
                                                                     const VC3                      &angularVelocity,
                                                                     ...
                                                                     int                             soundGroup)
        {
            boost::shared_ptr<PhysicsActor> actor = data->createActor(mesh,
                                                                      position,
                                                                      rotation,
                                                                      velocity,
                                                                      angularVelocity,
...
#ifndef NX_DISABLE_FLUIDS
        boost::shared_ptr<PhysicsFluid> ParticlePhysics::createFluid(int   type,
                                                                     int   maxParticles,
                                                                     float fluidStaticRestitution,
                                                                     float fluidStaticAdhesion,
                                                                     float fluidDynamicRestitution,
                                                                     ...
                                                                     int   collGroup)
        {
            boost::shared_ptr<PhysicsFluid> fluid = data->createFluid(type,
                                                                      maxParticles,
                                                                      fluidStaticRestitution,
                                                                      fluidStaticAdhesion,
                                                                      fluidDynamicRestitution,
                                                                      fluidDynamicAdhesion,
                                                                      fluidDamping,
                                                                      fluidStiffness,
                                                                      fluidViscosity,
                                                                      fluidKernelRadiusMultiplier,
                                                                      fluidRestParticlesPerMeter,
                                                                      fluidRestDensity,
                                                                      fluidMotionLimit,
        ...
        void ParticlePhysics::update()
        {
...
#ifndef NX_DISABLE_FLUIDS
                physics::resetFluidParticleCount();
                for (FluidActorList::iterator it = data->fluidActorList.begin(); it != data->fluidActorList.end();
                     ++it) {
                    PhysicsFluid *fluid = *it;
                    if (!fluid->fluid)
                        continue;
            ...
            // Actors
            {
                ...
                for (ActorList::iterator it = data->actorList.begin(); it != data->actorList.end(); ++it) {
                    ...
                    for (unsigned int i = 0; i < previousBoxes.size(); ++i) {
                        ...
                        if ( NxBoxBoxIntersect(currentBox.extents, currentBox.center, currentBox.rotation, b.extents,
                                               b.center, b.rotation, true) ) {
                            hit = true;
                            break;
                        }
                    }
                    ...
                    boost::shared_ptr<physics::BoxActor> boxActor = data->physics->createBoxActor(mesh->size,
                                                                                                  actor->position,
                                                                                                  mesh->localPosition);
                    if (boxActor) {
                        ++data->createdActors;
            ...
            // Fluids
            {
                ...
                for (FluidList::iterator it = data->fluidList.begin(); it != data->fluidList.end(); ++it) {
                    ...
                    boost::shared_ptr<physics::Fluid> fluidActor = data->physics->createFluid(
                        physics::PhysicsLib::FluidType(fluidData.type),
                        fluidData.maxParticles,
                        fluidData.fluidStaticRestitution,
                        fluidData.fluidStaticAdhesion,
                        fluidData.fluidDynamicRestitution,
                        fluidData.fluidDynamicAdhesion,
                        fluidData.fluidDamping,
                        fluidData.fluidStiffness,
                        fluidData.fluidViscosity,
                        fluidData.fluidKernelRadiusMultiplier,
                        fluidData.fluidRestParticlesPerMeter,
                        fluidData.fluidRestDensity,
                        fluidData.fluidMotionLimit,
                        fluidData.fluidPacketSizeMultiplier,
                        fluidData.collisionGroup);
