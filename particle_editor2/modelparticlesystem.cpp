        ...
        void ModelParticleSystem::spawn()
        {
            ...
            while (needed >= 1.f) {
                for (; a < int( particles.size() ); ++a) {
                    ...
                    if (m_eds->launchDirectionType == GenParticleSystemEditables::DIRECTION_EXPLOSION ||
                        m_eds->launchDirectionType == GenParticleSystemEditables::DIRECTION_NEGATIVE_EXPLOSION) {
                        //VC3 fudge;
                    ...
                    p.object->SetNoRender(false);
                    p.alive = true;
                    p.life = m_eds->particleLife + m_eds->particleLifeVar
                    ...
                    if (m_eds->collision && physics && p.shape) {
                        ...
                        p.actor = physics->createActor(p.shape,
                                                       p.position + position,
                                                       m_rotation_quat,
                                                       velocity,
                                                       angularVelocity,
                ...
                if (p.moving) {
...
#endif
                    {
                        ...
                        if (m_eds->collision)
                            ...
                            if ( collision->getCollision(oldPosition, p.position, p.velocity, m_eds->slowFactor,
                                                         m_eds->slowFactorY, m_eds->slowFactorXZ) ) {
                                p.rotateSpeed[0] *= .85f;
                                p.rotateSpeed[1] *= .85f;
                            }
                    ...
                    if (collision && p.velocity.GetSquareLength() < 0.00001f && p.rotateSpeed[0] < 0.01f &&
                        p.rotateSpeed[1] < 0.01f)
                        p.moving = false;
                }
...
//    emitterTm.RotateVector(position);
        ...
        void ModelParticleSystem::getVelocity(VC3 &velocity, const VC3 &dir, float speed, int index)
        {
            ...
            bool forceDirection = !use_explosion &&
                                  (m_eds->launchDirectionType == GenParticleSystemEditables::DIRECTION_EXPLOSION ||
            m_eds->launchDirectionType == GenParticleSystemEditables::DIRECTION_NEGATIVE_EXPLOSION);
        ...
        void ModelParticleSystem::parseFrom(const editor::ParserGroup       &pg,
