...
// cached so we don't recalculate them constantly
        static float global_wind_angle_cos = cosf(45.0f * 3.1415f / 180.0f);
        static float global_wind_angle_sin = cosf(45.0f * 3.1415f / 180.0f);

        static bool wind_timer_changed = true;
        static float wind_timer_spiral_speed_cos = 0.0f;
        ...
        void WindParticleForce::calcForce(Vector &force, const Vector &pos, const Vector &vel)
        {
            ...
            force.y = 0.0f;
            force.z = global_wind_velocity * global_wind_angle_sin;
            force *= m_wind_effect_factor;

            if (wind_timer_changed) {
                wind_timer_spiral_speed_cos = cosf(wind_timer * m_spiral_speed);
                wind_timer_spiral_speed_sin = sinf(wind_timer * m_spiral_speed);
                wind_timer_changed = false;
            }
            ...
            if (m_spiral_amount != 0.0f) {
                force.x += m_spiral_amount * wind_timer_spiral_speed_cos;
                force.z += m_spiral_amount * wind_timer_spiral_speed_sin;
            }
        ...
        void WindParticleForce::parseFrom(const editor::ParserGroup &pg) {
            ...
            setSpiralSpeed( convertFromString<float>(pg.getValue("spiral_speed", ""), 0.0f) );
        }
        ...
        void WindParticleForce::setGlobalWindAngle(float angleDegrees)
        {
            ...
            global_wind_angle_cos = cosf(WindParticleForce::global_wind_angle);
            global_wind_angle_sin = sinf(WindParticleForce::global_wind_angle);
        }
