        ...
        namespace {
            ...
            struct GenParticleSystemParamDesc {
                ...
                GenParticleSystemParamDesc()
                {
                    ...
                    params.push_back( ParamDesc("velocity_inheritance_factor", IDC_VELOCITY_INHERITANCE_FACTOR,
                                                PARAM_FLOAT) );
...
//    params.push_back(ParamDesc("particle_size", IDC_PARTICLE_SIZE, PARAM_ANIMATED_FLOAT));
...
#ifdef LEGACY_FILES
                    params.push_back( ParamDesc("texture", IDC_PARTICLE_TEXTURE, IDC_PARTICLE_TEXTURE_FILE,
                                                "image files\0*.png\0*.jpg\0*.tga\0*.bmp\0*.dds\0\0",
                                                "Data\\Textures\\Particles") );
#else
                    params.push_back( ParamDesc("texture", IDC_PARTICLE_TEXTURE, IDC_PARTICLE_TEXTURE_FILE,
                                                "image files\0*.png\0*.jpg\0*.tga\0*.bmp\0*.dds\0\0",
                                                "data\\texture\\particle") );
#endif
                    ...
                    params.push_back( ParamDesc("element_type", IDC_ELEMENT_TYPE, "point,quad,line_from_origin,line",
                                                PARAM_SELECTION) );
                    params.push_back( ParamDesc("launch_direction", IDC_LAUNCH_DIRECTION,
                                                "default,velocity,negative_velocity,explosion,negative_explosion",
                                                PARAM_SELECTION) );
            ...
            struct SprayParticleSystemParamDesc {
                ...
                SprayParticleSystemParamDesc()
                {
                    ...
                    // Fluids
                    params.push_back( ParamDesc("physics_type", IDC_PHYSICS_TYPE,
                                                "Disabled,Physics,Fluid without interaction,Fluid with interaction",
                                                PARAM_SELECTION) );
                    params.push_back( ParamDesc("fluid_static_restitution", IDC_FLUID_STATIC_RESTITUTION, PARAM_FLOAT,
                                                "0.5") );
                    params.push_back( ParamDesc("fluid_static_adhesion", IDC_FLUID_STATIC_ADHESION, PARAM_FLOAT, "0.05") );
                    params.push_back( ParamDesc("fluid_dynamic_restitution", IDC_FLUID_DYNAMIC_RESTITUTION, PARAM_FLOAT,
                                                "0.5") );
                    params.push_back( ParamDesc("fluid_dynamic_adhesion", IDC_FLUID_DYNAMIC_ADHESION, PARAM_FLOAT,
                                                "0.5") );
                    ...
                    params.push_back( ParamDesc("fluid_packet_size_multiplier", IDC_FLUID_PACKET_SIZE_MULTIPLIER,
                                                PARAM_INT, "8") );
                    params.push_back( ParamDesc("fluid_max_emitter_amount", IDC_FLUID_MAX_EMITTER_AMOUNT, PARAM_INT,
                                                "40") );
                    params.push_back( ParamDesc("fluid_detail_collision_group", IDC_FLUID_DETAIL_COLLISION_GROUP,
                                                PARAM_INT, "0") );
                    ...
                    params.push_back( ParamDesc("fluid_kernel_radius_multiplier", IDC_FLUID_KERNEL_RADIUS_MULTIPLIER,
                                                PARAM_FLOAT, "2.3") );
                    params.push_back( ParamDesc("fluid_rest_particles_per_meter", IDC_FLUID_REST_PARTICLES_PER_METER,
                                                PARAM_FLOAT, "10.0") );
            ...
            struct PointArrayParticleSystemParamDesc {
                ...
                PointArrayParticleSystemParamDesc()
                {
                    ...
                    params = gen.params;
#ifdef LEGACY_FILES
                    params.push_back( ParamDesc("model", IDC_MODEL, IDC_MODEL_FILE, "s3d files\0*.s3d\0\0",
                                                "Data\\Models\\Particles\\Emitter_shapes") );
#else
                    params.push_back( ParamDesc("model", IDC_MODEL, IDC_MODEL_FILE, "s3d files\0*.s3d\0\0",
                                                "data\\model\\effect\\emitter_shape") );
#endif
            ...
            struct ModelParticleSystemParamDesc {
                ...
                ModelParticleSystemParamDesc()
                {
                    ...
                    params.push_back( ParamDesc("model", IDC_MODEL, IDC_MODEL_FILE, "s3d files\0*.s3d\0\0",
                                                "Data\\Models\\Particles\\Emitter_shapes") );
                    params.push_back( ParamDesc("particle_model", IDC_PARTICLE_MODEL, IDC_PARTICLE_MODEL_FILE,
                                                "s3d files\0*.s3d\0\0", "Data\\Models") );
#else
                    params.push_back( ParamDesc("model", IDC_MODEL, IDC_MODEL_FILE, "s3d files\0*.s3d\0\0",
                                                "data\\model\\effect\\emitter_shape") );
                    params.push_back( ParamDesc("particle_model", IDC_PARTICLE_MODEL, IDC_PARTICLE_MODEL_FILE,
                                                "s3d files\0*.s3d\0\0", "data\\model") );
#endif
                    params.push_back( ParamDesc("scale", IDC_SCALE_X, IDC_SCALE_Y, IDC_SCALE_Z) );
                    params.push_back( ParamDesc("rotation", IDC_ROTATION_X, IDC_ROTATION_Y, IDC_ROTATION_Z) );
                    params.push_back( ParamDesc("direction_from_normals", IDC_DIRECTION_NORMALS, PARAM_INT) );
                    ...
                    params.push_back( ParamDesc("sound_material", IDC_SOUND_MATERIAL, soundMaterialString,
                                                PARAM_SELECTION) );
                }
            };
...
//    static CouldParticleSystemParamDesc theCloudParticleSystemParamDesc;
            ...
            struct ParticleCollision : public IParticleCollision {
                ...
                bool getCollision(const VC3 &oldPos,
                                  VC3       &position,
                                  VC3       &velocity,
                                  float      slowFactor,
                                  float      groundSlowFactor,
                                  float      wallSlowFactor) const
                {
                    bool result = false;
                    if (position.y < 0) {
                        position.y = 0;
 ...
 */
                ...
                void addForce() {
                    ...
                    int selection = 0;
                    if ( IDOK ==
                         DialogBoxParam(NULL, MAKEINTRESOURCE(IDD_FORCE_SELECT), NULL, forceDlgProc,
                                        (LONG)&selection) ) {
                        std::string fileName;
#ifdef LEGACY_FILES
                        if (selection == 0)
            ...
            public:
                ReloadCommand(Storm                 &storm_,
                              Dialog                &dlg_,
                              ParticleEffectManager &mgr_,
                              CrappyFont            &font_,
                              SharedData            &data_,
                              LoadModelCommand      &loadCommand_,
                              ShowEmittersCommand   &showCommand_)
                    :   storm(storm_),
                    dlg(dlg_),
                    mgr(mgr_),
                    font(font_),
        ...
        struct ApplicationData {
            ...
            ApplicationData()
                ...
                reloadCommand(storm, renderDialog, effectManager, font, sharedData, loadModelCommand,
                              showEmittersCommand),
            ...
            void createStuff(boost::shared_ptr<physics::PhysicsLib> &physics)
            {
                ...
                for (int i = 0; i < BOX_AMOUNT; ++i) {
                    ...
                    if (i != 0) {
                        ...
                        boost::shared_ptr<physics::SphericalJoint> joint = physics->createSphericalJoint(actors[i - 1],
                                                                                                         actors[i],
                                                                                                         anchor);
...
//            m->SetLighting(i, -1);
...
#ifdef PHYSICS_PHYSX
                    data->storm.terrain->getRenderer().enableFeature( IStorm3D_TerrainRenderer::Collision,
                                                                      isCheckEnabled(data->sharedData.dialog,
                                                                                     IDC_GLOW) );
                    data->storm.terrain->getRenderer().enableFeature( IStorm3D_TerrainRenderer::Wireframe,
                                                                      isCheckEnabled(data->sharedData.dialog,
                                                                                     IDC_GLOW) );
                    physics->enableFeature( frozenbyte::physics::PhysicsLib::VISUALIZE_FLUIDS,
                                            isCheckEnabled(data->sharedData.dialog, IDC_FLUID_STUFF) );
#endif

                    data->storm.terrain->getRenderer().enableFeature( IStorm3D_TerrainRenderer::Glow,
                                                                      isCheckEnabled(data->sharedData.dialog, IDC_GLOW) );
                    data->storm.terrain->getRenderer().enableFeature( IStorm3D_TerrainRenderer::Distortion,
                                                                      isCheckEnabled(data->sharedData.dialog,
                                                                                     IDC_DISTORTION) );
...
#ifdef PHYSICS_PHYSX
                data->sharedData.effectManager.enableParticlePhysics( isCheckEnabled(data->sharedData.dialog,
                                                                                     IDC_DISTORTION) );
#endif
                ...
                std::string maxParticlesPerSystem = "max particles per system = " + convertToString<int>(
                    stats.maxParticlesPerSystem);
