...
namespace frozenbyte {
    namespace particle {
        ...
        ParamDesc::ParamDesc(const std::string  name_,
                             int                id1_,
                             int                id2_,
        ...
        struct ParamUIData {
                ...
                updateCommand(data)
            {
                for (unsigned int i = 0; i < pd.size(); i++) {
                    if (pd[i].type == PARAM_INT) {
                        int value =
                            convertFromString<int>( parser.getValue(pd[i].name,
                                                                    ""), convertFromString<int>(pd[i].defaultValue, 0) );
                        setDialogItemInt(dlg, pd[i].id1, value);
                    }
                    if (pd[i].type == PARAM_BOOL) {
                        bool value =
                            convertFromString<bool>( parser.getValue(pd[i].name,
                                                                     ""),
                                                     convertFromString<bool>(pd[i].defaultValue, false) );
                        enableCheck(dlg, pd[i].id1, value);
                    }
                    if (pd[i].type == PARAM_FLOAT) {
                        float value =
                            convertFromString<float>( parser.getValue(pd[i].name,
                                                                      ""),
                                                      convertFromString<float>(pd[i].defaultValue, 0.f) );
