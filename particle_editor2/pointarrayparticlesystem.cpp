...
#  ifndef NX_DISABLE_FLUIDS
            ...
            if ( !fluid && physics &&
                 (m_eds->physicsType == GenParticleSystemEditables::PHYSICS_TYPE_FLUID || m_eds->physicsType ==
                ...
                fluid = physics->createFluid(m_eds->physicsType - 2,
                                             m_eds->maxParticles * m_eds->fluidMaxEmitterAmount,
                                             m_eds->fluidStaticRestitution,
                                             m_eds->fluidStaticRestitution,
                                             m_eds->fluidDynamicRestitution,
                                             m_eds->fluidDynamicAdhesion,
                                             m_eds->fluidDamping,
                                             m_eds->fluidStiffness,
                                             m_eds->fluidViscosity,
                                             m_eds->fluidKernelRadiusMultiplier,
                                             m_eds->fluidRestParticlesPerMeter,
                                             m_eds->fluidRestDensity,
                                             m_eds->fluidMotionLimit,
                                             m_eds->fluidPacketSizeMultiplier,
                                             collGroup);
                if (fluid)
                    m_render_fluid_parts.reset( new std::vector<Particle>(m_eds->maxParticles
                                                                          * m_eds->fluidMaxEmitterAmount) );
            }
...
//            float tertiaryFactor = (float)(rand() % 7307) / 7306.0f;
        ...
        void PointArrayParticleSystem::setParticleVelocity(Vector                           &vel,
                                                           const Vector                     &dir,
                                                           float                             speed,
                                                           const GenParticleSystemEditables &eds)
        {
            ...
            bool forceDirection = !use_explosion &&
        ...
        void PointArrayParticleSystem::parseFrom(const ParserGroup               &pg,
                                                 const util::SoundMaterialParser &materialParser) {
            ...
            m_eds->useNormalsAsDirection =
                static_cast<bool>( convertFromString<int>(pg.getValue("direction_from_normals", ""), 0) );
            m_eds->randomizeBetweenVertices =
                static_cast<bool>( convertFromString<int>(pg.getValue("positions_between_vertices", ""), 0) );
