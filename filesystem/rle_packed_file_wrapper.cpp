...
size_t rle_packed_fread(void *buffer, size_t size, size_t count, RLE_PACKED_FILE *stream)
{
    ...
    if (!isPacked && packedSize != unpackedSize) {
        assert(
            !
            "rle_packed_fread - invalid data (supposed to be unpacked chunk, but packed size and unpacked size mismatch).");
        stream->errorFlag = true;
        return 0;
    }
    ...
    if (got == 1) {
        if (isPacked) {
            ...
            if (resultSize != unpackedSize) {
                assert(
                    !
                    "rle_packed_fread - invalid data (chunk header unpacked size did not match actual resulting unpacked size).");
                stream->errorFlag = true;
                delete[] packedbuf;
                return 0;
            }
