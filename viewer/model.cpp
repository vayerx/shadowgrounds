...
//int BLEND_TIME = 1000;
        ...
        struct ModelData {
            ...
            IStorm3D_BoneAnimation *getAnimation(int groupIndex, int index)
            {
                ...
                boost::shared_ptr<Animation> animation( new Animation( storm.storm->CreateNewBoneAnimation(
                                                                            fileName.c_str() ) ) );
        ...
        void Model::save(const std::string &fileName)
        {
            ...
            for (int i = 0; i < 2; ++i) {
                for (unsigned int j = 0; j < data->animations[i].size(); ++j) {
                    parser.getGlobals().getSubGroup("Animations").getSubGroup( std::string(
                                                                                   "Animation")
                                                                               + boost::lexical_cast<std::string>(i) ).
                    addLine(data->animations[i][j]);
                }
            }
        ...
        void Model::addAnimation(int groupIndex, const std::string &fileName)
        {
            ...
            if ( std::find(data->animations[groupIndex].begin(), data->animations[groupIndex].end(),
                           fileName) != data->animations[groupIndex].end() )
                return;
