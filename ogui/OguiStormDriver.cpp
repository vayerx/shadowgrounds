...
int OguiStormFont::getStringWidth(const wchar_t *text)
{
    if (fnt && text) {
        ...
        int length = fnt->GetCharacterWidth( text, wcslen(text) );
...
throw (OguiException *)
{
    ...
    if (filename && strlen(filename) > 4 && strcmp(&filename[strlen(filename) - 4], ".fbf") == 0) {
        ...
        std::string subgroup_name = game::DHLocaleManager::getInstance()->getLocaleIdString(
             game::SimpleOptions::getInt(DH_OPT_I_MENU_LANGUAGE) );
...
IOguiImage *OguiStormDriver::ConvertVideoToImage(IStorm3D_VideoStreamer *video_str,
...
throw (OguiException *)
{
    ...
    while ( it.iterateAvailable() ) {
        ...
        if (fnt->isUnicode) {
            ...
            if (fnt->tex == NULL)
                throw new OguiException("OguiStormDriver::nextStormGeneration - Could not reload font texture.",
                                        fnt->texfname);
 ...
 */
        if (img->filename != NULL) {
            if (img->deletedVideo) {
                //delete img->video;
                IStorm3D_VideoStreamer *video_str = og_storm3d->CreateVideoStreamer(img->filename,
                                                                                    img->streamBuilder,
                                                                                    true);
                if (video_str) {
