...
void OguiFormattedText::parseTextToButtons()
{
    ...
    while ( data.cur_pos < (int)text.size() ) {
             ...
             (data.next_tag == text.npos) )
        {
            ...
            int i =
                data.currentFont ? data.currentFont->getStringWidth( text.substr(data.cur_pos,
                                                                                 break_here
                                                                                 - data.cur_pos).c_str() ) : 0;
            ...
            if (i > 0)
                createTextButton(data.mx, data.my, i, data.currentFont->getHeight(),
                                 text.substr(data.cur_pos, break_here - data.cur_pos), data.currentFont);
            ...
            data.cur_w = data.currentFont->getStringWidth( text.substr(data.cur_pos,
                                                                       data.next_tag - data.cur_pos).c_str() );

            createTextButton(data.mx, data.my, data.cur_w, data.currentFont->getHeight(),
                             text.substr(data.cur_pos, data.next_tag - data.cur_pos), data.currentFont);
...
std::pair< int, int > OguiFormattedText::findLineWidthBreak(std::string::size_type pos, int width, IOguiFont *cur_font)
{
    ...
    while (true) {
        ...
        if (cur_font == NULL ||
            ( temp_w = cur_font->getStringWidth( text.substr(cur_pos, pos - cur_pos).c_str() ) ) > width) {
            break;
        } else {
            next_linebrk = pos;
            mw = temp_w;
...
void OguiFormattedText::createTextButton(int                button_x,
                                         int                button_y,
                                         int                button_w,
                                         int                button_h,
                                         const std::string &button_text,
                                         IOguiFont         *button_font)
{
    FB_ASSERT(ogui);
    FB_ASSERT(window);
...
void OguiFormattedText::createImageButton(int                button_x,
                                          int                button_y,
                                          int                button_w,
                                          int                button_h,
...
void OguiFormattedText::createImageButton(int button_x, int button_y, int button_w, int button_h, IOguiImage *image)
{
    ...
    OguiButton *b;
    b = ogui->CreateSimpleImageButton(window,
                                      button_x,
                                      button_y,
                                      button_w,
                                      button_h,
