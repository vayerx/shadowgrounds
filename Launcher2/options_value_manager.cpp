...
public:
	...
	// HACK: We probably want to load the data from file only once
	// per run + be them global.
	static std::map< std::string, OptionsValueList > data;
	static bool dataInitialized;

};
std::map< std::string, OptionsValueList > OptionsValueManagerImpl::data;
bool OptionsValueManagerImpl::dataInitialized = false;
...
void OptionsValueManager::load()
{
	...
	*/
	...
	if(!OptionsValueManagerImpl::dataInitialized)
	{
