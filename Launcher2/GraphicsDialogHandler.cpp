...
namespace frozenbyte
{
    namespace launcher
    {
        ...
        void GraphicsDialogHandler::initDialog()
        {
            ...
            // Load combobox items.
            addComboItems( GetDlgItem(hwnd, IDC_COMBO1), manager.getOptionNames(
                               "Fake Shadows Texture Quality"), manager.getTheOneInUse("Fake Shadows Texture Quality") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO2), manager.getOptionNames("Misc"), manager.getTheOneInUse("Misc") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO3), manager.getOptionNames(
                               "Lighting Level"), manager.getTheOneInUse("Lighting Level") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO4), manager.getOptionNames(
                               "Anisotropic Filtering"), manager.getTheOneInUse("Anisotropic Filtering") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO5), manager.getOptionNames(
                               "Lighting Texture Quality"), manager.getTheOneInUse("Lighting Texture Quality") );
            addComboItems( GetDlgItem(hwnd,
                                      IDC_COMBO6), manager.getOptionNames("Antialiasing"),
                           manager.getTheOneInUse("Antialiasing") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO7), manager.getOptionNames(
                               "Particle Effects Level"), manager.getTheOneInUse("Particle Effects Level") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO8), manager.getOptionNames(
                               "Shadows Texture Quality"), manager.getTheOneInUse("Shadows Texture Quality") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO9), manager.getOptionNames(
                               "Shadows Level"), manager.getTheOneInUse("Shadows Level") );
            addComboItems( GetDlgItem(hwnd, IDC_COMBO10), manager.getOptionNames(
