...
DialogData::DialogData(int resourceId, const HWND parentHandle, ResizeType type, IDlgHandler* handler )
{
	impl = new DialogDataImpl;
	impl->resizeType = type;
	impl->handler = handler;

	CreateDialogParam(GetModuleHandle(0), MAKEINTRESOURCE(resourceId), parentHandle, DialogDataImpl::DialogHandler, reinterpret_cast<LPARAM> (impl) );
...
void DialogData::getPosition( int &x, int &y) const
{
	RECT windowSize = { 0 };
...
void DialogData::forceDialogRect( RECT &rc )
{
	MoveWindow( impl->windowHandle, rc.left, rc.top, rc.right-rc.left, rc.bottom-rc.top, TRUE );
}
