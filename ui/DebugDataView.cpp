...
namespace ui
{
    ...
    void DebugDataView::run()
    {
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_DEBUG_DATA_VIEW_PERF_STATS) )
            if ( curTime >= impl->lastPerfUpdateTime
                 + game::SimpleOptions::getInt(DH_OPT_I_DEBUG_DATA_VIEW_PERF_STATS_RATE) ) {
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_SHOW_DEBUG_DATA_VIEW) ) {
            ...
            if (impl->debugDataTexture == NULL) {
                ...
                impl->debugDataTexture = s3d->CreateNewTexture(DEBUG_DATA_VIEW_SIZE_X,
                                                               DEBUG_DATA_VIEW_SIZE_Y,
            ...
            if (show_occlusion)
                if (impl->game->gameUI->getGridOcclusionCuller() != NULL) {
                    ...
                    GRIDOCCLUSIONCULLER_DATATYPE areaBitMask =
                        impl->game->gameUI->getGridOcclusionCuller()->getAreaForOrderNumber(impl->currentOcclusionArea);
                    ...
                    if ( impl->game->gameUI->getController(0)->wasKeyClicked(DH_CTRL_SELECT_UNIT_4) )
                        if (impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(impl->currentOcclusionPosX,
                                                                                        impl->currentOcclusionPosY) !=
                            areaBitMask)
                            for (int ty = occminy; ty < occmaxy + 1; ty++) {
                                for (int tx = occminx; tx < occmaxx + 1; tx++) {
                                    impl->game->gameUI->getGridOcclusionCuller()->makeOccludedToArea(tx,
                                                                                                     ty,
                                                                                                     areaBitMask);
                                }
                            }
                    ...
                    if ( impl->game->gameUI->getController(0)->wasKeyClicked(DH_CTRL_SELECT_UNIT_6) )
                        impl->game->gameUI->getTerrain()->updateOcclusionForAllObjects(
                            impl->game->gameUI->getGridOcclusionCuller(),
                            areaBitMask);
            ...
            if ( curTime >= impl->lastUpdateTime + game::SimpleOptions::getInt(DH_OPT_I_DEBUG_DATA_VIEW_RATE) ) {
                        ...
                        for (int x = 0; x < DEBUG_DATA_VIEW_SIZE_X; x++) {
                            ...
                            if (show_special) {
                                ...
                                if (show_forcemap)
                                    ...
                                    impl->debugData[(x + y
                                                     * DEBUG_DATA_VIEW_SIZE_X) * 4
                                                    + 1] = 32 + forcebuf[(mapx / 2) + (mapy / 2) * obstSizeX / 2] / 128;
                                else
                                    impl->debugData[(x + y * DEBUG_DATA_VIEW_SIZE_X) * 4 + 1] = 32;
                                if (show_buildingarea) {
                                    if ( impl->game->gameMap->getAreaMap()->isAreaAnyValue(mapx, mapy,
                                                                                           AREAMASK_INBUILDING) )
                                ...
                                if (show_heightmap)
                                    ...
                                    impl->debugData[(x + y
                                                     * DEBUG_DATA_VIEW_SIZE_X) * 4
                                                    + 2] = 32
                                                           + heightbuf[(mapx / 2) + (mapy / 2) * obstSizeX / 2] / 128;
                                else
                                ...
                                if (show_obstmap) {
                                        ...
                                        && impl->game->gameMap->obstacleHeightMap[mapx + mapy * obstSizeX] != 0)
                                    {
                                        ...
                                        if (impl->game->gameMap->getAreaMap()->isAreaAnyValue(mapx, mapy,
                                                                                              AREAMASK_OBSTACLE_UNHITTABLE)
                                            && impl->game->gameMap->obstacleHeightMap[mapx + mapy * obstSizeX] != 0)
                                        {
                                            if ( (x & 1) != (y & 1) )
                                                impl->debugData[(x + y
                                                                 * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] = 32 + val / 2;
                                        } else {
                                            if (impl->game->gameMap->getAreaMap()->isAreaAnyValue(mapx, mapy,
                                                                                                  AREAMASK_OBSTACLE_SEETHROUGH)
                                                && impl->game->gameMap->obstacleHeightMap[mapx + mapy * obstSizeX] != 0)
                                                if ( (x & 1) == (y & 1) )
                                                    impl->debugData[(x + y
                                                                     * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] = 32 + val / 2;
                                ...
                                if (show_light)
                                    if ( (mapx & 1) == 0 && (mapy & 1) == 0 ) {
                                        float lAmount;
                                        VC3 pos;
                                        pos = VC3( impl->game->gameMap->obstacleToScaledX(
                                                       mapx), 0, impl->game->gameMap->obstacleToScaledY(mapy) );
                                ...
                                if (show_covermap) {
                                    int col = 255 - impl->game->gameMap->getCoverMap()->getDistanceToNearestCover(mapx,
                                                                                                                  mapy);
                                    if (impl->debugData[(x + y * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] < col)
                                        impl->debugData[(x + y * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] = col;
                                    impl->debugData[(x + y * DEBUG_DATA_VIEW_SIZE_X) * 4 + 1] = col;
                                ...
                                if (show_material) {
                                    int mat = impl->game->gameMap->getAreaMap()->getAreaValue(mapx,
                                                                                              mapy,
                                                                                              AREAMASK_MATERIAL)
                                              >> AREASHIFT_MATERIAL;
                                    impl->debugData[(x + y
                                                     * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] = ( mat & (1 << 0) ) * 255;
                                    impl->debugData[(x + y
                                                     * DEBUG_DATA_VIEW_SIZE_X) * 4
                                                    + 1] = ( ( mat & (1 << 1) ) >> 1 ) * 255;
                                    impl->debugData[(x + y
                                ...
                                if (show_occlusion)
                                    if (impl->game->gameUI->getGridOcclusionCuller() != NULL) {
                                        ...
                                        int occx = impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionX(
                                            scaledX);
                                        int occy = impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionY(
                                            scaledY);
                                        bool isVis = impl->game->gameUI->getGridOcclusionCuller()->isVisibleToArea(
                                             occx, occy,
                                            impl->game->gameUI->getGridOcclusionCuller()->getAreaForOrderNumber(impl->
                                                                                                                currentOcclusionArea) );
                                        GRIDOCCLUSIONCULLER_DATATYPE mapArea =
                                            impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(occx, occy);
                                        bool isArea = false;
                                        if ( mapArea ==
                                             impl->game->gameUI->getGridOcclusionCuller()->getAreaForOrderNumber(impl->
                                                                                                                 currentOcclusionArea) )
                                        ...
                                        int occyUp = impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionY(
                                            scaledYUp);
                                        float scaledXLeft = impl->game->gameMap->obstacleToScaledX(mapx - 1);
                                        int occxLeft = impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionX(
                                            scaledXLeft);
                                        float scaledYDown = impl->game->gameMap->obstacleToScaledY(mapy + 1);
                                        int occyDown = impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionY(
                                            scaledYDown);
                                        float scaledXRight = impl->game->gameMap->obstacleToScaledX(mapx + 1);
                                        int occxRight =
                                            impl->game->gameUI->getGridOcclusionCuller()->scaledToOcclusionX(
                                                scaledXRight);
                                            ...
                                            || occxRight != occx || occyDown != occy)
                                        {
                                                ...
                                                && occx <= occmaxx && occy <= occmaxy)
                                            {
                                                ...
                                                GRIDOCCLUSIONCULLER_DATATYPE selarea =
                                                    impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(occx,
                                                                                                                occy);
                                                bool borderArea = false;
                                                if ( impl->game->gameUI->getGridOcclusionCuller()->
                                                     isInOcclusionBoundaries(occx, occyUp)
                                                     && selarea !=
                                                     impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(occx,
                                                                                                                 occyUp) )
                                                    borderArea = true;
                                                if ( impl->game->gameUI->getGridOcclusionCuller()->
                                                     isInOcclusionBoundaries(occx, occyDown)
                                                     && selarea !=
                                                     impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(occx,
                                                                                                                 occyDown) )
                                                    borderArea = true;
                                                if ( impl->game->gameUI->getGridOcclusionCuller()->
                                                     isInOcclusionBoundaries(occxLeft, occy)
                                                     && selarea !=
                                                     impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(
                                                         occxLeft,
                                                         occy) )
                                                    borderArea = true;
                                                if ( impl->game->gameUI->getGridOcclusionCuller()->
                                                     isInOcclusionBoundaries(occxRight, occy)
                                                     && selarea !=
                                                     impl->game->gameUI->getGridOcclusionCuller()->getCameraArea(
                                                         occxRight,
                                                         occy) )
                                ...
                                impl->debugData[(x + y
                                                 * DEBUG_DATA_VIEW_SIZE_X) * 4
                                                + 3] = (unsigned char)( (int)(255 * viewAlpha) / 100 );
                            }
                        }
                    }
                ...
                //{
                impl->debugData[(DEBUG_DATA_VIEW_SIZE_X / 2
                                 + (DEBUG_DATA_VIEW_SIZE_Y / 2) * DEBUG_DATA_VIEW_SIZE_X) * 4 + 0] = 32;
                impl->debugData[(DEBUG_DATA_VIEW_SIZE_X / 2
                                 + (DEBUG_DATA_VIEW_SIZE_Y / 2) * DEBUG_DATA_VIEW_SIZE_X) * 4 + 1] = 32;
                impl->debugData[(DEBUG_DATA_VIEW_SIZE_X / 2
                                 + (DEBUG_DATA_VIEW_SIZE_Y / 2) * DEBUG_DATA_VIEW_SIZE_X) * 4 + 2] = 255;
                impl->debugData[(DEBUG_DATA_VIEW_SIZE_X / 2
                                 + (DEBUG_DATA_VIEW_SIZE_Y / 2) * DEBUG_DATA_VIEW_SIZE_X) * 4 + 3] = 255;
                //}

                impl->debugDataTexture->Copy32BitSysMembufferToTexture( (Uint32 *)impl->debugData );
            }
            scene->Render2D_Picture( impl->debugDataMaterial,
                                     VC2(0, 0), VC2(DEBUG_DATA_VIEW_SIZE_X * 2, DEBUG_DATA_VIEW_SIZE_Y * 2) );
        } else {
            cleanup();
        }
    }
