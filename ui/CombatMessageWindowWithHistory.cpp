...
///////////////////////////////////////////////////////////////////////////////
    ...
    CombatMessageWindowWithHistory::CombatMessageWindowWithHistory(Ogui              *ogui,
                                                                   const std::string &textConfName,
                                                                   const std::string &iconConfName) :
    ...
    void CombatMessageWindowWithHistory::update(int)
    {
        ...
        for (i = 0; i < (int)messageBuffer.size(); i++) {
            if (messageBuffer[ i ] != NULL && messageBuffer[ i ]->text != NULL) {
                float transparency =
                    ( (float)( (Timer::getTime()
                                - messageBuffer[ i ]->initTime)
                               - messageStartFadeTime )
                 / (messageEndFadeTime - messageStartFadeTime) ) * messageBuffer[ i ]->alpha * 100.0f;
...
///////////////////////////////////////////////////////////////////////////////
    ...
    void CombatMessageWindowWithHistory::pushMessage(const std::string &message)
    {
        ...
        messageBuffer[ 0 ]->text = ogui->CreateTextLabel( win, textXPosition, getVerticalPosition(
                                                              0), textXSize, textHeight, message.c_str() );
