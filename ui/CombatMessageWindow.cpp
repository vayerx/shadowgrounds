...
namespace ui
{
        ...
        noiseTime(0)
    {
        ...
        effectWindow =
            new GUIEffectWindow( ogui, l1Image, 0, l3Image,
                                 VC2I(iconXPosition, iconYPosition), VC2I(iconXSize, iconYSize) );
        effectWindow->hide();

        {
#ifdef LEGACY_FILES
            frozenbyte::filesystem::InputStream stream =
                frozenbyte::filesystem::FilePackageManager::getInstance().getFile("Data/Effects/conversation_noise.txt");
#else
            frozenbyte::filesystem::InputStream stream =
                frozenbyte::filesystem::FilePackageManager::getInstance().getFile("data/effect/conversation_noise.txt");
#endif
