...
//        data->terrain->LoadTextureAtBank((*it).second.get()->getTexture(), index++);
            ...
            filesystem::InputStream stream = filesystem::createInputFileStream( Parser::GetString(properties,
                                                                                                  "map").c_str() );

            for (int y = data->mapSize.y * 2 - 1; y >= 0; --y) {
                for (int x = 0; x < data->mapSize.x * 2; ++x) {
                    VC2 position =
                        VC2( (float)( float(x) / 2.0f - (data->mapSize.x / 2) ) * data->terrainScale,
                             (float)( float(y) / 2.0f - (data->mapSize.y / 2) ) * data->terrainScale );
