...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        GenericBarTimerUpdator(game::Game        *game,
                               const std::string &global_value,
                               const std::string &global_timer_length) :
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        GenericBarScriptsGlobalUpdator(game::Game        *game,
                                       GenericBarWindow  *win,
                                       const std::string &global_value,
                                       const std::string &global_max,
                                       const std::string &global_min,
                                       bool               hide_during_converse) :
            valueVarName(global_value),
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        GenericBarTimerWindowConstructor(const std::string &window_name,
                                         const std::string &locales_name,
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        GenericBarGlobalsFromScriptsWindowConstructor(const std::string &window_name,
                                                      const std::string &locales_name,
                                                      const std::string &value_var,
                                                      const std::string &max_var,
                                                      const std::string &min_var,
                                                      bool               hide_during_converse) :
            localesName(locales_name),
            valueVarName(value_var),
            maxVarName(max_var),
        ...
        ICombatSubWindow *CreateNewWindow(Ogui *ogui, game::Game *game, int player)
        {
            GenericBarWindow *result = new GenericBarWindow(ogui, game, player);
            result->loadDataFromLocales(localesName);
            result->setUpdator( new GenericBarScriptsGlobalUpdator(game, result, valueVarName, maxVarName, minVarName,
                                                                   hideDuringConverse) );
            return result;
        }
            ...
            if (weapon) {
                ...
                if (unit) {
                    ...
                    if (false) {
                        ...
                        std::string crap =
                            (std::string)( ("") + boost::lexical_cast< std::string >(b) + " / "
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        GenericBarWeaponReloadWindowConstructor(const std::string &window_name,
                                                const std::string &locales_name,
...
//
    ...
    public:
        ...
        ICombatSubWindow *CreateNewWindow(Ogui *ogui, game::Game *game, int player)
        {
            ...
            windows[2] = (GenericBarWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                "SurvivorKillingSpreeReloadBar");
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        KillCountUpdater(int client, game::Game *game, const std::string &vocal_locale,
                         Ogui *ogui) : client(client), game(game), ogui(ogui)
        {
            desc_text = NULL;
        ...
        void update(GenericTextWindow *win)
        {
            ...
            if (!aligned) {
                ...
                // get timer
                {
                    GenericTextWindow *win2 = (GenericTextWindow *)game->gameUI->getCombatWindow(0)->getSubWindow(
                        "CountdownTimer");
                ...
                desc_text = ogui->CreateSimpleTextButton(win->getWindow(),
                                                         ...
                                                         win->getText()->getW(),
                                                         win->getText()->getH(), NULL, NULL, NULL,
                                                         description.c_str(), 0, 0, false);
            ...
            if ( nextVocalTrigger < vocalTriggers.size() ) {
                ...
                if (kills >= trigger.first) {
                    ...
                    if ( playingVocal == -1 || !game->gameUI->getSoundMixer()->isSoundPlaying(playingVocal) )
                        playingVocal = game->gameUI->playSpeech(
                            trigger.second.c_str(), pos.x, pos.y, pos.z, false, DEFAULT_SPEECH_VOLUME);
                    nextVocalTrigger++;
                }
            }
        }
...
///////////////////////////////////////////////////////////////////////////////
        ...
        void updateValues()
        {
            ...
            if (barwindow) {
                ...
                float interpolate_factor =
                    (Timer::getTime() - game->lastTickTime) / (1000.0f / (float) GAME_TICKS_PER_SECOND);
    ...
    public:
        SurvivorTimerWindowConstructor(const std::string &window_name,
                                       const std::string &locales_name,
                                       bool               countdown,
                                       bool               bar) :
            localesName(locales_name),
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        SurvivorTimerTextWindowConstructor(const std::string &window_name,
                                           const std::string &locales_name,
                                           bool               countdown) :
            localesName(locales_name),
            countdown(countdown)
        ...
        void update(GenericTextWindow *win)
        {
            if (first_run)

                {
                    ...
                    img1 = ogui->CreateSimpleImageButton(
                        win->getWindow(), x, y, w, h, NULL, NULL, NULL, txt, 0, 0, false);
...

                {
                    ...
                    img2 = ogui->CreateSimpleImageButton(
                        win->getWindow(), x, y, w, h, NULL, NULL, NULL, txt, 0, 0, false);
...

                {
                    ...
                    desc = ogui->CreateSimpleTextButton(
                        win->getWindow(), x, y, w, h, NULL, NULL, NULL, txt, 0, 0, false);
    ...
    public:
        ...
        float update()
        {
            ...
            if (!game->isPaused() && value > 0) {
                ...
                win->moveBy(game->gameUI->getCursorScreenX(client,
                                                           exact) - x + offset_x,
                            game->gameUI->getCursorScreenY(client, exact) - y + offset_y);
                if ( win->isHidden() )
                    win->show(0);
            } else {
                if ( !win->isHidden() )
...
///////////////////////////////////////////////////////////////////////////////
    ...
    GenericBarGlobalsFromScriptsWindowConstructor *surv_shield_bar = new GenericBarGlobalsFromScriptsWindowConstructor(
        "SurvivorShieldBar",
        "survivor_shield_bar",
        "player_shield_counter",
        "player_shield_max",
        "",
        false);
    GenericBarTimerWindowConstructor *surv_napalm_reload = new GenericBarTimerWindowConstructor(
        "SurvivorNapalmFlameBar",
        "survivor_napalmflame_bar",
        "survivor_napalm_flame_last_time",
        "survivor_napalm_flame_reload_length");
    GenericBarTimerWindowConstructor *surv_killingspree_reload = new GenericBarTimerWindowConstructor(
        "SurvivorKillingSpreeReloadBar",
        "survivor_killingspree_reloadbar",
        "survivor_slomo_last_time",
        "survivor_slomo_reload_length");
    GenericBarTimerWindowConstructor *surv_killingspree_usebar = new GenericBarTimerWindowConstructor(
        "SurvivorKillingSpreeUseBar",
        "survivor_killingspree_usebar",
        "survivor_slomo_current_time",
        "survivor_slomo_duration");
    KillingSpreeCountConstructor *surv_killingspree_counter = new KillingSpreeCountConstructor(
        "SurvivorKillingSpreeCounter");
    StupidWindowAligner *surv_aligner1 = new StupidWindowAligner("SurvivorShieldBarAligner", "SurvivorShieldBar");
    StupidWindowAligner *surv_aligner2 = new StupidWindowAligner("SurvivorNapalmFlameBarAligner",
                                                                 "SurvivorNapalmFlameBar");
    StupidWindowAligner *surv_aligner3 = new StupidWindowAligner("SurvivorKillingSpreeReloadBarAligner",
                                                                 "SurvivorKillingSpreeReloadBar");

// one for each player..
    GenericBarGlobalsFromScriptsWindowConstructor *surv_expo_bar0 = new GenericBarGlobalsFromScriptsWindowConstructor(
        "SurvivorExpoBar_0",
        "survivor_expo_bar_0",
        "survivor_current_expo[0]",
        "survivor_expo_max[0]",
        "survivor_expo_min[0]",
        true);
    GenericBarGlobalsFromScriptsWindowConstructor *surv_expo_bar1 = new GenericBarGlobalsFromScriptsWindowConstructor(
        "SurvivorExpoBar_1",
        "survivor_expo_bar_1",
        "survivor_current_expo[1]",
        "survivor_expo_max[1]",
        "survivor_expo_min[1]",
        true);
    GenericBarGlobalsFromScriptsWindowConstructor *surv_expo_bar2 = new GenericBarGlobalsFromScriptsWindowConstructor(
        "SurvivorExpoBar_2",
        "survivor_expo_bar_2",
        "survivor_current_expo[2]",
        "survivor_expo_max[2]",
        "survivor_expo_min[2]",
        true);
    GenericBarGlobalsFromScriptsWindowConstructor *surv_expo_bar3 = new GenericBarGlobalsFromScriptsWindowConstructor(
        "SurvivorExpoBar_3",
        "survivor_expo_bar_3",
        "survivor_current_expo[3]",
        "survivor_expo_max[3]",
        "survivor_expo_min[3]",
        true);

    KillCountConstructor *surv_kill_counter = new KillCountConstructor("SniperKillCounter",
                                                                       "Surv_Sniper",
                                                                       "sniper_kill_counter_vocals");

    SurvivorTimerWindowConstructor *survivor_timerwindow_down = new SurvivorTimerWindowConstructor(
        "CountdownTimer",
        "survivor_countdowntimer",
        true,
        false);
    SurvivorTimerWindowConstructor *survivor_timerwindow_up = new SurvivorTimerWindowConstructor(
        "CountupTimer",
        "survivor_countuptimer",
        false,
        false);
    SurvivorTimerWindowConstructor *survivor_barwindow_down = new SurvivorTimerWindowConstructor(
        "CountdownBar",
        "gui_survivor_countdownbar",
        true,
        true);
    SurvivorTimerWindowConstructor *survivor_barwindow_up = new SurvivorTimerWindowConstructor(
        "CountupBar",
        "gui_survivor_countupbar",
        false,
        true);

    SurvivorTimerTextWindowConstructor *survivor_barwindow_text_down = new SurvivorTimerTextWindowConstructor(
        "CountdownText",
        "survivor_countertext",
        true);
    SurvivorTimerTextWindowConstructor *survivor_barwindow_text_up = new SurvivorTimerTextWindowConstructor(
        "CountupText",
        "survivor_countertext",
        false);
    SurvivorTimerTextWindowConstructor *survivor_barwindow_text_down2 = new SurvivorTimerTextWindowConstructor(
        "CountdownText2",
        "survivor_countertext2",
        true);
    SurvivorTimerTextWindowConstructor *survivor_barwindow_text_up2 = new SurvivorTimerTextWindowConstructor(
        "CountupText2",
        "survivor_countertext2",
        false);

    SurvivorWeaponWindowConstructor *surv_marine_weapons = new SurvivorWeaponWindowConstructor(
        "SurvivorMarineWeaponWindow",
        "marine_");
    SurvivorWeaponWindowConstructor *surv_napalm_weapons = new SurvivorWeaponWindowConstructor(
        "SurvivorNapalmWeaponWindow",
        "napalm_");
    SurvivorWeaponWindowConstructor *surv_spiner_weapons = new SurvivorWeaponWindowConstructor(
