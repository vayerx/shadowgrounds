...
namespace ui
{
    ...
    void UIEffects::setGammaEffect(float                     brightness,
                                   float                     contrast,
                                   float                     redCorrection,
                                   float                     greenCorrection,
                                   float                     blueCorrection,
                                   IStorm3D_TerrainRenderer *terrainRenderer)
    {
        for (int i = 0; i < NUM_FILTER_EFFECTS; i++) {
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_SHADER_GAMMA_EFFECTS) ) {
            if (terrainRenderer != NULL && game::SimpleOptions::getInt(DH_OPT_I_LIGHTING_LEVEL) > 0)
                terrainRenderer->setColorEffect( contrast, brightness,
                                                 COL(redCorrection, greenCorrection, blueCorrection) );
        } else {
            ...
            storm3d->SetGammaRamp(opt_gamma,
                                  opt_brightness + brightness,
                                  opt_contrast + contrast,
                                  opt_red_correction + redCorrection,
                                  opt_green_correction + greenCorrection,
                                  opt_blue_correction + blueCorrection,
                                  calibrate);
        }
    }
    ...
    void UIEffects::run(int msec, IStorm3D_TerrainRenderer *terrainRenderer, IStorm3D_Camera *stormCamera)
    {
        ...
        if (flashEffectRunning || hasFilterEffect) {
            ...
            if (hasFilterEffect) {
                this->setGammaEffect(brightness,
                                     contrast,
                                     red_correction,
                                     green_correction,
                                     blue_correction,
            ...
            case FLASH_EFFECT_TYPE_EXPLOSION:
                if (ctime < EXPLOSION_FLASH_EFFECT_DURATION) {
                    ...
                    if ( ctime < (EXPLOSION_FLASH_EFFECT_DURATION / 3) )
                        ...
                        timeFactor = 1.0f
                                     - ( (ctime - EXPLOSION_FLASH_EFFECT_DURATION
                                          / 3) / float(EXPLOSION_FLASH_EFFECT_DURATION * 2 / 3) );
            ...
            case FLASH_EFFECT_TYPE_NUKE_EXPLOSION:
                if (ctime < NUKE_EXPLOSION_FLASH_EFFECT_DURATION) {
                    ...
                    if ( ctime < (NUKE_EXPLOSION_FLASH_EFFECT_DURATION / 3) )
                        ...
                        timeFactor = 1.0f
                                     - ( (ctime - NUKE_EXPLOSION_FLASH_EFFECT_DURATION
                                          / 3) / float(NUKE_EXPLOSION_FLASH_EFFECT_DURATION * 2 / 3) );
            ...
            case FLASH_EFFECT_TYPE_PLAYER_HIT_BIG:
                if (ctime < PLAYER_HIT_FLASH_EFFECT_DURATION) {
                    ...
                    if ( ctime < (PLAYER_HIT_FLASH_EFFECT_DURATION / 4) )
                        ...
                        timeFactor = 1.0f
                                     - ( (ctime - PLAYER_HIT_FLASH_EFFECT_DURATION
                                          / 4) / float(PLAYER_HIT_FLASH_EFFECT_DURATION * 3 / 4) );
            ...
            case FLASH_EFFECT_TYPE_PLAYER_POISON_BIG:
                if (ctime < PLAYER_POISON_FLASH_EFFECT_DURATION) {
                    ...
                    if ( ctime < (PLAYER_POISON_FLASH_EFFECT_DURATION / 4) )
                        ...
                        timeFactor = 1.0f
                                     - ( (ctime - PLAYER_POISON_FLASH_EFFECT_DURATION
                                          / 4) / float(PLAYER_POISON_FLASH_EFFECT_DURATION * 3 / 4) );
    ...
    void UIEffects::startFlashEffect(FLASH_EFFECT_TYPE flashEffectType)
    {
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_EXTRA_GAMMA_EFFECTS) ) {
            ...
            if ( this->flashEffectType == FLASH_EFFECT_TYPE_ENVIRONMENT_LIGHTNING
                 ...
                 ( isPoisonEffect &&
                   (!wasPoisonEffect ||
                    (Timer::getTime() - flashStartTime) > PLAYER_POISON_FLASH_EFFECT_DURATION * 6 / 8) ) )
    ...
    void UIEffects::setMaskPictureText(const char *text, OguiButton::TEXT_H_ALIGN align)
    {
        ...
        if (maskWindow[activeMaskLayer] == NULL) {
            Logger::getInstance()->warning(
                "UIEffects::setMaskPictureText - Cannot set mask picture text when no picture set.");
        ...
        if (maskTextButton[activeMaskLayer] == NULL) {
            /*
               ...
               if (maskFont[activeMaskLayer] != NULL)
                ...
                font = ui::defaultFont;
             */
            maskTextButton[activeMaskLayer] =
                ogui->CreateTextArea(maskWindow[activeMaskLayer],
    ...
    void UIEffects::startMaskPictureFadeOut(int duration)
    {
        if (maskWindow[activeMaskLayer] != NULL)
            ...
            Logger::getInstance()->warning(
                "UIEffects::startMaskPictureFadeOut - Active mask layer has no mask picture.");
    }
    ...
    void UIEffects::startMaskPictureMoveOut(int duration)
    {
        if (maskWindow[activeMaskLayer] != NULL)
            ...
            Logger::getInstance()->warning(
                "UIEffects::startMaskPictureMoveOut - Active mask layer has no mask picture.");
    }
