...
namespace ui
{
        ...
        game(game)
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            characterVideos[i
                            + 3] =
                ogui->LoadOguiImage( getLocaleGuiString( ("gui_characterselectionwindow_" + buttonname + "_idle").c_str() ) );
    ...
    void CharacterSelectionWindow::updateProfileButtons()
    {
        for (unsigned int k = 0; k < MAX_PLAYERS_PER_CLIENT; k++) {
            ...
            if (chosenCharacter[i] != -1) {
                ...
                profileButtons[i]->Move(
                    move_over_button->GetX() + move_over_button->GetSizeX() / 2 - profileButtons[i]->GetSizeX() / 2,
                    move_over_button->GetY() + move_over_button->GetSizeY() / 2
                    - profileButtons[i]->GetSizeY() / 2 + offsetY);
    ...
    void CharacterSelectionWindow::closeWindow()
    {
            ...
            for (int i = 0; i < MAX_PLAYERS_PER_CLIENT; i++) {
                ...
                if (chosenCharacter[i] == charNum) {
                    ...
                    if (unit != NULL) {
                        ...
                        if (i == 0)
                            game->gameScripting->setGlobalIntVariableValue( "singleplayer_unitid",
                                                                            game->units->getIdForUnit(
                                                                                unit) );
                    } else {
                        util::Script::setGlobalArrayVariableValue("player_units", i, 0);
                    }
        ...
        * /
        ...
        readyToCloseTimer = game->gameTimer + GAME_TICKS_PER_SECOND *getLocaleGuiInt(
            "gui_characterselectionwindow_wait_before_close",
            0) / 1000;
    }
    ...
    void CharacterSelectionWindow::updateCharacterButtonChosen(int charNum)
    {
        ...
        if (SimpleOptions::getBool(DH_OPT_B_VIDEO_ENABLED) && chosen) {
            ...
            characterVideoStreams[charNum] =
                game->gameScene->getStorm3D()->CreateVideoStreamer(
                     ("Data/Videos/" + animName + ".wmv").c_str(), builder, !chosen );
