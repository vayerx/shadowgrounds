                ...
                if (key != NULL) {
                    ...
                    if (str2int_errno() == 0) {
                        ...
                        if (keyNum > 0 && keyNum < TERRAIN_OBJECT_VARIABLES_AMOUNT) {
                            ...
                            sp.error(
                                "init_terrain_object_variables - key value out of range (expected positive integer value below "
                                TERRAIN_STRINGIFY(TERRAIN_OBJECT_VARIABLES_AMOUNT) ").");
                        }
                    } else {
...
struct ObjectData {
    ...
    bool hasExplosion() const
    {
        ...
        if ( !explosionObstacle.empty() || !explosionScript.empty() || !explosionProjectile.empty() ||
             !explosionEffect.empty() || !explosionSounds.empty() )
            return true;
...
struct ObjectInstance {
    ...
    // TODO: some of these should be unsigned because they are indices to a std::vector
...
static ExplosionEvent createEvent(const ObjectData     &data,
                                  const ObjectInstance &instance,
                                  const VC2            &position,
                                  const VC3            &velocity,
                                  IStorm3D_Terrain     *terrain,
                                  const VC3            &explosionPosition,
                                  bool                  useExplosion,
...
struct BlendPass {
    ...
    BlendPass(int textureA_, int textureB_, const std::vector<Uint32> &weights_)
        :   textureA(textureA_),
        textureB(textureB_),
        weights(weights_)
    {
...
struct TerrainData {
    ...
    void unifiedHandleToTerrainIds(int unifiedHandle, int *terrainModelIdOut, int *terrainObstacleIdOut) const
    {
        ...
        *terrainModelIdOut =
            ( (unifiedHandle
               & UNIFIED_HANDLE_TERRAIN_OBJECT_MODEL_ID_MASK) >> UNIFIED_HANDLE_TERRAIN_OBJECT_MODEL_ID_SHIFT );
        *terrainObstacleIdOut =
            ( (unifiedHandle
               & UNIFIED_HANDLE_TERRAIN_OBJECT_INSTANCE_ID_MASK) >> UNIFIED_HANDLE_TERRAIN_OBJECT_INSTANCE_ID_SHIFT );
        ...
        assert(
            *terrainObstacleIdOut >= 0 && *terrainObstacleIdOut <= UNIFIED_HANDLE_TERRAIN_OBJECT_INSTANCE_ID_LAST_VALUE);
    }
    ...
    UnifiedHandle getUnifiedHandle(int terrainModelId, int terrainObstacleId) const
    {
        ...
        assert( IS_UNIFIED_HANDLE_TERRAIN_OBJECT( UNIFIED_HANDLE_BIT_TERRAIN_OBJECT | terrainModelId
                                                  | (terrainObstacleId
                                                     << UNIFIED_HANDLE_TERRAIN_OBJECT_INSTANCE_ID_SHIFT) ) );

        return ( UNIFIED_HANDLE_BIT_TERRAIN_OBJECT | terrainModelId
                 | (terrainObstacleId << UNIFIED_HANDLE_TERRAIN_OBJECT_INSTANCE_ID_SHIFT) );
    }
    ...
    UnifiedHandle createTerrainObject(int           newModelId,
                                      UnifiedHandle cloneFrom,
                                      const VC3    &position,
                                      const QUAT   &rotation,
                                      const VC3    &velocity)
    {
        ...
        for (int i = 0; i < (int)splitted.size(); i++) {
            ...
            if (tid != 0)
                ...
                Logger::getInstance()->error(
                    "TerrainData::parseTrackableTypeFromString - trackable type mask name unknown.");
        }
...
#  endif
        ...
        if ( stream.isEof() ) {
            ...
            Logger::getInstance()->warning(
                "This is a fatal error, program will terminate now to prevent crashing later on.");
...
#define INCORRECT_TERRAIN_OBST_MAP_AREA_MULT 16
        ...
        boost::scoped_array<uint16_t> newObstacleMap(
            new uint16_t[INCORRECT_TERRAIN_OBST_MAP_AREA_MULT * mapSize.x * mapSize.y]);
        obstacleMap.swap(newObstacleMap);

        boost::scoped_array<uint16_t> newForceMap(
            new uint16_t[GAMEMAP_HEIGHTMAP_MULTIPLIER * GAMEMAP_HEIGHTMAP_MULTIPLIER * mapSize.x * mapSize.y]);
        ...
        for (int i = 0; i < blockAmount; ++i) {
            ...
            for (int j = 0; j < passes; j += 2) {
                ...
                std::vector<Uint32> weights(BLOCK_SIZE * BLOCK_SIZE);
            ...
            if (version >= 5) {
                ...
                if (version >= 6) {
                    ...
                    if (version >= 10) {
                        ...
                        if ( !game::SimpleOptions::getBool(DH_OPT_B_ENVIRONMENT_ANIMATIONS) &&
                             data.explosionAnimations.empty() ) {
                            data.bones.clear();
 ...
 */
    ...
    void applyTexturing()
    {
...

        {
            ...
            for (; it != blendings.end(); ++it) {
                ...
                for (unsigned int i = 0; i < passes.size(); ++i) {
                    ...
                    pass.texture1 =
                        boost::shared_ptr<IStorm3D_Texture>( storm->CreateNewTexture(BLOCK_SIZE, BLOCK_SIZE,
...

        {
            ...
            std::vector<Uint32> buffer(textureSize * textureSize);
            boost::shared_ptr<IStorm3D_Texture> black( storm->CreateNewTexture(1,
                                                                               1,
                                                                               IStorm3D_Texture::TEXTYPE_BASIC),
                                                       std::mem_fun(&IStorm3D_Texture::Release) );
            ...
            for (int blockIndex = 0; blockIndex < blockAmount; ++blockIndex) {
                ...
                if ( it != lightMaps.end() && !it->second.values.empty() ) {
                    ...
                    map.texture =
                        boost::shared_ptr<IStorm3D_Texture>( storm->CreateNewTexture(textureSize,
                                                                                     textureSize,
                                                                                     IStorm3D_Texture::TEXTYPE_BASIC),
                                                             std::mem_fun(&IStorm3D_Texture::Release) );
    ...
    void apply()
    {
        ...
        terrain->setHeightMap(heightMap.get(), mapSize, realSize, textureRepeat,
                              forceMap.get(), GAMEMAP_HEIGHTMAP_MULTIPLIER, GAMEMAP_HEIGHTMAP_MULTIPLIER
    ...
    bool physicsImpulse(int        modelId,
                        int        objectId,
                        float      factor,
                        const VC3 &velocity_,
                        const VC3 &explosion_position,
                        bool       use_explosion)
    {
    ...
    void updateLatestReplacementInfo(int modelId, int objectId, int newModelId, int newInstanceId)
    {
        ...
        while (true) {
            ...
            if (failureCount > 1000) {
                LOG_ERROR(
                    "TerrainData::updateLatestReplacementInfo - failure count exceeded while trying to solve the very original model/instance.");
                assert(
                    !
                    "TerrainData::updateLatestReplacementInfo - failure count exceeded while trying to solve the very original model/instance.");
                break;
                      ...
                      bool                          only_breaktexture)
    {
        ...
        if ( originalObject.data.hasExplosion() ) {
            ExplosionEvent event =
                createEvent( originalObject.data, instance, position, velocity, terrain, explosion_position,
                             use_explosion,
                             this->getUnifiedHandle(modelId,
                                                    objectId) );
        ...
        for (int i = 0;; ++i) {
            ...
            if ( i > 0 && previousObject.data.hasExplosion() ) {
                ExplosionEvent event =
                    createEvent( previousObject.data, instance, position, velocity, terrain, explosion_position,
                                 use_explosion, this->getUnifiedHandle(modelId,
                                                                       objectId) );
                events.push_back(event);
            }
...
// HACK: _loop
        ...
        if (newIndex >= 0) {
            ...
            if (newObject.data.type == TERRAIN_OBSTACLE_TYPE_NONE)
                if (!instance.movedByPhysics)
                    if ( instance.originalInstance < (int)originalObject.instances.size() ) {
                        ...
                        TerrainObstacle obstacle =
                            createObstacle(originalObject.data, originalObject.instances[instance.originalInstance]);
            ...
            // We need to break objects which are layed on top of broken one
            {
                ...
                for (; !it->end(); ) {
                    ...
                    if ( breakObjects(modelId, instanceId, 100000, removedObjects, events,  position2, velocity,
                                      explosion_position, use_explosion, only_breaktexture) )
                        it->erase();
                    else
                        it->next();
#else
                    VC2 position2(objectPosition.x, objectPosition.z);
                    if ( !collisionObject.data.hasPhysics() &&
                         breakObjects(modelId, instanceId, 100000, removedObjects, events,  position2, velocity,
                                      explosion_position, use_explosion, false) )
                        it->erase();
                    else
                        it->next();
#endif
    ...
    void createAmbientSoundForObject(int modelId, int instanceId_)
    {
        ...
        ambientSoundManager->makeAmbientSoundFromDefString( instance.ambientSound,
                                                            ("!10000000*" + data.ambientSound + ",*,*").c_str() );
        ambientSoundManager->setAmbientSoundPosition( instance.ambientSound,
                                                      VC3(instance.position.x, instance.height, instance.position.y) );
...
#  endif
    ...
    void createPhysicsForObject(game::GamePhysics            *gamePhysics,
                                int                           modelId,
                                int                           instanceId_,
                                bool                          sleepPhysicsObject,
                                ...
                                int                          &totalCreated)
    {
        ...
        if ( data.hasPhysics() ) {
            ...
            if (data.physicsType == TERRAIN_OBJECT_PHYSICS_TYPE_BOX) {
                game::BoxPhysicsObject *bp = new game::BoxPhysicsObject(gamePhysics,
                                                                        data.physicsData1,
                                                                        data.physicsMass,
                ...
                bp->setCustomData( reinterpret_cast<void *>( game::PhysicsContactUtils::
                                                             calcCustomPhysicsObjectDataForTerrainObject(modelId,
                                                                                                         instanceId) ) );
                ...
                game::CylinderPhysicsObject *cp = new game::CylinderPhysicsObject(
                    gamePhysics,
                    height,
                    radius,
                    data.physicsMass,
...
#endif
#ifdef PHYSICS_PHYSX
                game::ConvexPhysicsObject *cp = new game::ConvexPhysicsObject(gamePhysics,
                                                                              cylinderFile.c_str(), data.physicsMass,
                                                                              PHYSICS_COLLISIONGROUP_DYNAMIC_TERRAINOBJECTS,
                                                                              pos);
                if (sleepPhysicsObject)
                    cp->setToSleep();
                ...
                int instanceId = i;
                // WARNING: unsafe int to void * cast!
                cp->setCustomData( (void *)game::PhysicsContactUtils::calcCustomPhysicsObjectDataForTerrainObject(
                                       modelId, instanceId) );
                ...
                game::CapsulePhysicsObject *cp = new game::CapsulePhysicsObject(
                    gamePhysics,
                    height,
                    radius,
                    data.physicsMass,
                ...
                int instanceId = i;
                // WARNING: unsafe int to void * cast!
                cp->setCustomData( reinterpret_cast<void *>( game::PhysicsContactUtils::
                                                             calcCustomPhysicsObjectDataForTerrainObject(modelId,
                                                                                                         instanceId) ) );
                ...
                game::RackPhysicsObject *rp = new game::RackPhysicsObject(gamePhysics,
                                                                          data.physicsMass,
                                                                          PHYSICS_COLLISIONGROUP_DYNAMIC_TERRAINOBJECTS,
                                                                          pos);
                if (sleepPhysicsObject)
                ...
                int instanceId = i;
                // WARNING: unsafe int to void * cast!
                rp->setCustomData( (void *)game::PhysicsContactUtils::calcCustomPhysicsObjectDataForTerrainObject(
                                       modelId, instanceId) );
...
#  ifdef PROJECT_CLAW_PROTO
                ...
                game::CarPhysicsObject *rp = new game::CarPhysicsObject(gamePhysics,
                                                                        data.physicsMass,
                                                                        PHYSICS_COLLISIONGROUP_DYNAMIC_TERRAINOBJECTS,
                                                                        pos);
                if (sleepPhysicsObject)
...
#  endif
            ...
            if (data.physicsType == TERRAIN_OBJECT_PHYSICS_TYPE_STATIC) {
...
#endif
#ifdef PHYSICS_ODE
                game::StaticPhysicsObject *sp = new game::StaticPhysicsObject(gamePhysics,
                                                                              data.fileName.c_str(), NULL, pos, rot);
                ...
                int instanceId = i;
                // WARNING: unsafe int to void * cast!
                sp->setCustomData( (void *)game::PhysicsContactUtils::calcCustomPhysicsObjectDataForTerrainObject(
                                       modelId, instanceId) );
#endif
#ifdef PHYSICS_PHYSX
                game::StaticPhysicsObject *sp = new game::StaticPhysicsObject(gamePhysics,
                                                                              data.fileName.c_str(), meshmodel, pos,
                                                                              rot);
                instance.physicsObject.reset(sp);
                sp->setSoundMaterial( soundmp.getMaterialIndexByName(data.physicsSoundMaterial) );
                sp->setDurabilityType( durp.getDurabilityTypeIndexByName(data.durabilityType) );
...
Terrain::Terrain(IStorm3D                *storm,
                 IStorm3D_Scene          *scene,
                 const char              *dirName,
                 const char              *forceMapName,
                 const util::AreaMap     *areaMap,
                 game::GameMap           *gameMap,
                 ui::LightManager        *lightManager,
                 ui::AmbientSoundManager *ambientSoundManager)
    ...
    for (; it != data->objects.end(); ++it) {
        ...
        for (unsigned int i = 0; i < object.instances.size(); ++i) {
            ...
            if (curpos + (int)sizeof(bool) + 7 * (int)sizeof(float) > filesize) {
                ...
                Logger::getInstance()->error(
                    "Terrain::loadPhysicsCache - Cache data amount did not match expected data amount.");
            }
        ...
        for (unsigned int j = 0; j < o.instances.size(); ++j) {
            ...
            if (data->lightManager && data->gameMap) {
                ...
                if ( data->gameMap->isWellInScaledBoundaries(pos.x, pos.z) ) {
                    ...
                    lights.ambient = data->gameMap->colorMap->getUnmultipliedColor(
                        pos.x / data->gameMap->getScaledSizeX() + .5f,
                        pos.z / data->gameMap->getScaledSizeY() + .5f);

                    if ( data->gameMap->getAreaMap()->isAreaAnyValue(ox, oy, AREAMASK_INBUILDING) )
                        data->lightManager->getLighting( pos, lights, o.data.radius, false, false,
                                                         data->terrain->getInstanceModel(i,
                                                                                         j) );
                    else
                        data->lightManager->getLighting( pos, lights, o.data.radius, false, false,
                                                         data->terrain->getInstanceModel(i,
...
void Terrain::createPhysics(game::GamePhysics *gamePhysics,
                            unsigned char     *clipMap,
                            bool               sleepPhysicsObjects,
...
#  ifdef PHYSICS_PHYSX
        ...
        {
            boost::shared_ptr<frozenbyte::physics::StaticMesh> mesh = gamePhysics->getPhysicsLib()->createStaticMesh(
...
//    for(; it != data->objects.end(); ++it)
    ...
    for (int modelId = 0; modelId < (int)data->objects.size(); modelId++) {
        ...
        for (unsigned int i = 0; i < object.instances.size(); ++i) {
            this->data->createPhysicsForObject(gamePhysics,
                                               modelId,
                                               i,
                                               sleepPhysicsObjects,
                                               soundmp,
...
void Terrain::updatePhysics(game::GamePhysics            *gamePhysics,
                            std::vector<TerrainObstacle> &objectsMovedFirstTime,
                            util::GridOcclusionCuller    *culler)
{
    ...
    for (unsigned int i = 0; i < data->objects.size(); ++i) {
        ...
        for (unsigned int j = 0; j < o.instances.size(); ++j) {
            ...
            if (instance.physicsObject) {
                ...
                if (fabs(instance.position.x - pos.x) > UPDATE_PHYSICS_RANGE || fabs(instance.position.y - pos.z) >
                    UPDATE_PHYSICS_RANGE || fabs(instance.height - pos.y) > UPDATE_PHYSICS_RANGE)
                    needsUpdate = true;
...
//#endif
                    ...
                    if (fabs(instance.position.x - instance.lightUpdatePosition.x) > UPDATE_LIGHT_RANGE ||
                        fabs(instance.position.y - instance.lightUpdatePosition.z) > UPDATE_LIGHT_RANGE ||
                        fabs(instance.height - instance.lightUpdatePosition.y) > UPDATE_LIGHT_RANGE) {
                        ui::PointLights lights;
                        //lights.ambient = instance.ambient;
...
void Terrain::BlastTerrainObjects(const VC3                    &position3,
                                  float                         radius,
                                  std::vector<TerrainObstacle> &removedObjects,
                                  float                         blastHeight)
{
    ...
    boost::shared_ptr<IStorm3D_TerrainModelIterator> it = data->terrain->getModelIterator(position3,
...
UnifiedHandle Terrain::findTerrainObjectByIdString(const char *idString)
{
    ...
    for (unsigned int i = 0; i < data->objects.size(); ++i) {
        ...
        if (strcasecmp(object.data.fileName.c_str(),
                       "data\\models\\terrain_objects\\statue\\obelisk_monument\\obelisk_chainholder.s3d") != 0)
            continue;
        ...
        if (instance.deleted) {
            if (instance.latestReplacementModel > 0 && instance.latestReplacementInstance >= 0) {
                ObjectInstance temp =
                    data->objects[instance.latestReplacementModel].instances[instance.latestReplacementInstance];
                if (!temp.deleted) {
...
void Terrain::BreakTerrainObject(UnifiedHandle                 uh,
                                 std::vector<TerrainObstacle> &removedObjects,
                                 std::vector<ExplosionEvent>  &events,
                                 int                           damage,
                                 bool                          only_breaktexture)
{
    ...
    if ( data->breakObjects(modelId, objectId, damage, removedObjects, events, position, velocity, position3, false,
                            only_breaktexture) )
        data->terrain->removeInstance(modelId, objectId);
}

void Terrain::BreakTerrainObjects(const VC3                    &position3,
                                  const VC3                    &velocity,
                                  float                         radius,
                                  std::vector<TerrainObstacle> &removedObjects,
                                  std::vector<ExplosionEvent>  &events,
    ...
    if (!closestOnly) {
        ...
        for (; !it->end(); ) {
...

            {
                ...
                if ( data->breakObjects(modelId, instanceId, damage, removedObjects, events, position, velocity,
                                        position3, !closestOnly, only_breaktexture) )
                    it->erase();
                else
                    it->next();
            }
        ...
        if (closestModel >= 0 && closestInstance >= 0)
            if ( data->breakObjects(closestModel, closestInstance, damage, removedObjects, events, position, velocity,
                                    position3, !closestOnly, only_breaktexture) )
...
int Terrain::getObjectVariableNumberByName(const std::string &variableName)
{
    ...
    LOG_WARNING_W_DEBUG( "Terrain::getObjectVariableNumberByName - no terrain object variable of given name.",
                         variableName.c_str() );
    return -1;
}
...
game::tracking::ITrackableUnifiedHandleObjectIterator *Terrain::getTrackableUnifiedHandleObjectsFromArea(
    const VC3                &position,
    float                     radius,
...
void Terrain::BlastHoleCircle(const VC3                    &position,
                              const VC3                    &direction,
                              float                         radius,
                              float                         depth,
                              std::vector<TerrainObstacle> &removedObjects)
...
void Terrain::BlastHoleSphere(const VC3                    &position,
                              const VC3                    &direction,
                              float                         radius,
                              float                         depth,
                              std::vector<TerrainObstacle> &removedObjects)
...
void Terrain::setAmbient(COL ambientColor)
{
    ...
    data->ambientColor =
        TColor<unsigned char>( (unsigned char)ambientCharColor.r, (unsigned char)ambientCharColor.g,
                               (unsigned char)ambientCharColor.b );
...
bool Terrain::breakObjects(int                           modelId,
                           int                           objectId,
                           int                           damage,
                           std::vector<TerrainObstacle> &removedObjects,
                           std::vector<ExplosionEvent>  &events,
                           ...
                           bool                          only_breaktexture)
{
    if ( data->breakObjects(modelId, objectId, damage, removedObjects, events, position, velocity_, explosion_position,
                            use_explosion, only_breaktexture) ) {
        data->terrain->removeInstance(modelId, objectId);
        return true;
    } else {
        return false;
...
void Terrain::updateOcclusionForObject(util::GridOcclusionCuller   *culler,
                                       int                          terrainModelId,
                                       int                          terrainObstacleId,
                                       GRIDOCCLUSIONCULLER_DATATYPE cameraArea)
{
        ...
        for (unsigned int j = 0; j < object.instances.size(); ++j) {
            ...
            if (this->data->lightManager && this->data->gameMap) {
                ...
                if ( this->data->gameMap->isWellInScaledBoundaries(pos.x, pos.z) ) {
                    lights.ambient = this->data->gameMap->colorMap->getUnmultipliedColor(
                        pos.x / this->data->gameMap->getScaledSizeX() + .5f,
                        pos.z / this->data->gameMap->getScaledSizeY() + .5f);

                    if ( this->data->gameMap->getAreaMap()->isAreaAnyValue(ox, oy, AREAMASK_INBUILDING) )
                        this->data->lightManager->getLighting( pos, lights, data.radius, false, false,
                                                               this->data->terrain->getInstanceModel(i,
                                                                                                     j) );
                    else
                        this->data->lightManager->getLighting( pos, lights, data.radius, false, false,
                                                               this->data->terrain->getInstanceModel(i,
    ...
    for ( ; !it->end(); it->next() ) {
        ...
        if (this->data->lightManager && this->data->gameMap) {
            ...
            if ( this->data->gameMap->isWellInScaledBoundaries(pos.x, pos.z) ) {
                ...
                if ( this->data->gameMap->getAreaMap()->isAreaAnyValue(ox, oy, AREAMASK_INBUILDING) )
                    this->data->lightManager->getLighting( pos, lights, data.radius, false, false,
                                                           this->data->terrain->getInstanceModel(modelId,
                                                                                                 instanceId) );
                else
                    this->data->lightManager->getLighting( pos, lights, data.radius, false, false,
                                                           this->data->terrain->getInstanceModel(modelId,
...
UnifiedHandle Terrain::createTerrainObject(int           newModelId,
                                           UnifiedHandle cloneFrom,
                                           const VC3    &position,
                                           const QUAT   &rotation,
                                           const VC3    &velocity)
