...
namespace ui
{
    ...
    void GameCamera::setFirstPersonPosition(float personX,
                                            float personY,
                                            float scrolledX,
                                            float scrolledY,
                                            float height,
        ...
        if (interpolateCamera != NULL) {
            ...
            if (smooth) {
                ...
                interTarg = otherTarg + (targ - otherTarg)
                            * ( (float)sinf( 3.1415926f
                                             * (interpolateTime / interpolateDuration - 0.5f) ) * 0.5f + 0.5f );
                ...
                interPosOffset = otherPosOffset
                                 + (positionOffset - otherPosOffset) * (interpolateTime / interpolateDuration);
                interTargOffset = otherTargOffset
                                  + (targetOffset - otherTargOffset) * (interpolateTime / interpolateDuration);
            }
    ...
    void GameCamera::applyToScene()
    {
        ...
        if (!forceMapView) {
            ...
            if (implData->shakeAmount > 0) {
                ...
                float shakeFact = float(implData->shakeAmount)
                                  * ( 1.0f - (implData->shakeTime / implData->totalShakeTime) );
                VC3 shakeVec = VC3(
            ...
            gameMap->getTerrain()->getRenderer().setFloatValue(IStorm3D_TerrainRenderer::ForceAmbient,
                                                               game::SimpleOptions::getFloat(
                                                                   DH_OPT_F_MAPVIEW_AMBIENT_LIGHTING)
                                                               );
            ...
            if ( SimpleOptions::getBool(DH_OPT_B_MAPRENDER_USE_USER_RESOLUTION) ||
                ...
                scene->GetCamera()->ForceOrthogonalProjection(true,
                                                              -mapheight / 2 + orthoOffsetY,
                                                              -mapheight / 2 + orthoOffsetY + orthoHeight,
                                                              -mapwidth / 2 + orthoOffsetX,
                                                              -mapwidth / 2 + orthoOffsetX + orthoWidth);
            }
        }
