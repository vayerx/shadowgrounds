            ...
            if ( (float)radarUnitDist[i] > ( ratio * (RADAR_SIZE / 2 - RADAR_EDGE) ) ) {
                ...
                int vis =
                    (int)( ( ( ratio
                               * (RADAR_SIZE / 2
                                  - RADAR_EDGE) )
                             - (float)radarUnitDist[i] ) / (float)(RADAR_SIZE / 2 - RADAR_EDGE) * 150.0f );
                if (vis < 0) vis = 0;
                if (vis > 100) vis = 100;
...
#ifdef LEGACY_FILES
#  ifndef PROJECT_SURVIVOR
                            game->gameUI->playGUISound( "Data/Sounds/Defaultfx/Radar/radar_beep.wav", 100
                                                        - (perc * 3 / 4) );
#  endif
#else
                            game->gameUI->playGUISound( "data/audio/sound/gui/hud/radar/radar_beep.wav", 100
                                                        - (perc * 3 / 4) );
#endif
                        }
                }
