        ...
        if (renderNeedPositionUpdate) {
            ...
            if (sideways) {
                ...
                if (interpolatePerFrame) {
                    interpolatedRenderPosition.x = renderPosition.x * (interpolate_factor) + lastRenderPosition.x
                                                   * (1 - interpolate_factor);
                    interpolatedRenderPosition.y = renderPosition.y * (interpolate_factor) + lastRenderPosition.y
                                                   * (1 - interpolate_factor);
                    interpolatedRenderPosition.z = renderPosition.z * (interpolate_factor) + lastRenderPosition.z
                                                   * (1 - interpolate_factor);
...
//                #endif
                ...
                for ( helper_iterator = from->ITHelper->Begin(); !helper_iterator->IsEnd(); helper_iterator->Next() ) {
                    ...
                    //if(source_helper->GetHelperType() == IStorm3D_Helper::HTYPE_CAMERA)
                    {
                        ...
                        IStorm3D_Helper_Camera *new_helper = to->Helper_Camera_New(
                            source_helper->GetName(), position, dir, up);
                ...
                Iterator<IStorm3D_Model_Object *> *object_iterator;
                for ( object_iterator = srcModel->ITObject->Begin(); !object_iterator->IsEnd();
        ...
        if (this->rotationInterpolationAmount > 1) {
                ...
                && this->interpolatedRotateBone != bone)
            {
                Logger::getInstance()->warning(
                    "VisualObject::rotateBone - Multiple manual bone rotatations not supported on a visualobject with rotation interpolation on.");
                assert(
                    !
                    "VisualObject::rotateBone - Multiple manual bone rotatations not supported on a visualobject with rotation interpolation on.");
    ...
    void VisualObject::advanceHistory()
    {
        ...
        if (positionInterpolationAmount > 1) {
            ...
            for (int i = 1; i < positionInterpolationAmount; i++) {
                VC3 tmp =
                    positionHistory[( atPositionHistory
                                      + (positionInterpolationAmount - i) ) % positionInterpolationAmount];
        ...
        if (rotationInterpolationAmount > 1) {
            ...
            for (int i = 1; i < rotationInterpolationAmount; i++) {
                float tmp_abs =
                    rotationYHistory[( atRotationHistory
                                       + (rotationInterpolationAmount - i) ) % rotationInterpolationAmount];
                float tmp =
                    util::AngleRotationCalculator::getFactoredRotationForAngles(rotationYHistory[atRotationHistory],
                                                                                tmp_abs,
                                                                                0.1f);
            ...
            if (this->interpolatedRotateBone != NULL) {
                ...
                if (sideways) {
                    if (renderYAngle < 180)
                        rotationq = QUAT(UNIT_ANGLE_TO_RAD(
                                             this->interpolatedBoneAngle - angleDifference),
                                         UNIT_ANGLE_TO_RAD(this->interpolatedBoneBetaAngle), 0);
                    else
                        rotationq = QUAT(-UNIT_ANGLE_TO_RAD(
                                             this->interpolatedBoneAngle - angleDifference),
                                         UNIT_ANGLE_TO_RAD(this->interpolatedBoneBetaAngle), 0);
                    ...
                    rotationq =
                        QUAT(UNIT_ANGLE_TO_RAD(this->interpolatedBoneBetaAngle),
                             UNIT_ANGLE_TO_RAD(this->interpolatedBoneAngle - angleDifference), 0);
                }
    ...
    void VisualObject::setCAMotionBlur(bool motionBlur, float length)
    {
        ...
        if (!this->motionBlurred && motionBlur) {
            ...
            if (this->model != NULL) {
                Iterator<IStorm3D_Model_Object *> *object_iterator;
                for ( object_iterator = model->ITObject->Begin(); !object_iterator->IsEnd();
                      object_iterator->Next() ) {
                    IStorm3D_Model_Object *object = object_iterator->GetCurrent();
            ...
            if (this->model != NULL) {
                ...
                for ( object_iterator = model->ITObject->Begin(); !object_iterator->IsEnd();
                    ...
                    if (this->positionInterpolationAmount > 1) {
                        histPos =
                            positionHistory[( this->positionInterpolationAmount + atPositionHistory
                                              - int( (this->positionInterpolationAmount
                                                      - 1) * 0.3f
                                                     * this->motionBlurLength ) ) % this->positionInterpolationAmount];
