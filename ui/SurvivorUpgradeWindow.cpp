    ...
    struct UpgradeButtonThingie {
        ...
        void showCostText(game::Game *game, Unit *unit, int upgradesPendingCost)
        {
            if (text) {
                ...
                if ( game->upgradeManager->canUpgrade(unit, upgradeId,
                                                      &upgradesPendingCost) &&
                     !game->upgradeManager->isLocked(unit, upgradeId) )
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
            ...
            locked_img(NULL)
        {
            ...
            sniperItems.push_back( SurvivorItem("enemyintelligence", SurvivorItem::TYPE_TOGGLE_ITEM,
                                                "item_enemyintelligence", false) );
            sniperItems[sniperItems.size() - 1].script_use = std::pair<std::string, std::string>(
                "item_enemyintelligence",
                "useitem");
                        ...
                        if (weapon) {
                            if ( weapon->getBulletType() ) {
                                ...
                                Logger::getInstance()->error(
                                    "SurvivorUpgradeWindow::getItemCount - Bullet type not found");
        ...
        void loadDataForWindow(const std::string &window_name)
        {
            ...
            // stats
            {
                ...
                std::string secrets =
                    boost::lexical_cast<std::string>( game->gameScripting->getGlobalIntVariableValue(
                                                          "secretpart_amount") );
        ...
        void loadDataForUnit()
        {
            ...
            for (int i = 0; i < 2; i++) {
                OguiButton *deco_temp = oguiLoader.LoadButton(std::string(
                                                                  "charupgdecorationbar_")
                                                              + boost::lexical_cast< std::string >(i), window,  0);
                decorationList.push_back(deco_temp);

                if (i == 0) {
                    deco_temp =
                        oguiLoader.LoadButton(std::string("charupgdecorationtext_") + boost::lexical_cast< std::string >(
                                                  i), window,  0);
            ...
            // load units upgrade parts
            {
                ...
                for (int i = 0; i < (signed)temp_vector.size(); ++i) {
                    ...
                    OguiButton *selected_button = NULL;
                    {
                        ...
                        IOguiImage *img =
                            ogui->LoadOguiImage( getLocaleGuiString("gui_upgradewindow_characterupgrade_background") );
                        selected_button = ogui->CreateSimpleImageButton(window,
                                                                        0,
                                                                        0,
                                                                        w,
                                                                        h,
                    ...
                    IOguiImage *imgs =
                        ogui->LoadOguiImage( getLocaleGuiString( ("gui_upgradewindow_" + upgButton
                                                                  + "_img_normal").c_str() ) );
                    IOguiImage *imgsh =
                        ogui->LoadOguiImage( getLocaleGuiString( ("gui_upgradewindow_" + upgButton
                                                                  + "_img_normal").c_str() ) );
                    ...
                    temp->textFontNormal =
                        oguiLoader.LoadFont( getLocaleGuiString( (std::string("gui_") + oguiLoader.GetWindowName()
                                                                  + "_"
                                                                  + upgButton + "_cost_text_font_normal").c_str() ) );
                    temp->textFontDisabled =
                        oguiLoader.LoadFont( getLocaleGuiString( (std::string("gui_") + oguiLoader.GetWindowName()
                                                                  + "_"
                                                                  + upgButton + "_cost_text_font_disabled").c_str() ) );
                    temp->selected_button = selected_button;
                    temp->selected_offset_x = getLocaleGuiInt(
                        "gui_upgradewindow_characterupgrade_background_button_offset_x",
                        0);
                    temp->selected_offset_y = getLocaleGuiInt(
                        "gui_upgradewindow_characterupgrade_background_button_offset_y",
                        0);
                    ...
                    // Cost icon
                    {
                        int x = temp->button->GetX() + getLocaleGuiInt(
                            "gui_upgradewindow_characterupgrade_costicon_offset_x",
                            0);
                        int y = temp->button->GetY() + getLocaleGuiInt(
                            "gui_upgradewindow_characterupgrade_costicon_offset_y",
                            0);
                    ...
                    if ( game->upgradeManager->isLocked(unit, temp_vector[ i ]) ) {
                        if (locked_img == NULL)
                            locked_img =
                                ogui->LoadOguiImage( game::getLocaleGuiString("gui_upgradewindow_upgrade_locked_img") );
        ...
        void loadWeaponUpgradesForUnit()
        {
            ...
            for (int i = 0; i < SURVIVOR_UPGRADEWINDOW_MAX_WEAPONS; i++) {
                ...
                if (wnum != -1) {
                    ...
                    {
                        OguiButton *deco_temp = oguiLoader.LoadButton(std::string(
                                                                          "weapondecorationbar_")
                                                                      + boost::lexical_cast< std::string >(
                                                                          weapon_count), window,  0);
                        // deco_temp->SetDisabled( true );
                        decorationList.push_back(deco_temp);
                        ...
                        deco_temp =
                            oguiLoader.LoadButton(std::string("weapondecorationtext_")
                                                  + boost::lexical_cast< std::string >(weapon_count), window,  0);
                        deco_temp->SetDisabled(true);
                    ...
                    if ( !unit->isWeaponOperable(wnum) ) {
                        ...
                        std::string temp = std::string("gui_upgradewindow_") + boost::to_lower_copy(weapName)
                                           + "_image_hintmessage_disabled";
                    ...
                    // upgrades for that weapon
                    {
                        ...
                        for (int j = 0; j < (int)upgIds.size(); j++) {
                            ...
                            OguiButton *selected_button = NULL;
                            {
                                ...
                                int h = getLocaleGuiInt("gui_upgradewindow_weaponupgrade_background_h", 0);
                                IOguiImage *img =
                                    ogui->LoadOguiImage( getLocaleGuiString(
                                                             "gui_upgradewindow_weaponupgrade_background") );
                                selected_button = ogui->CreateSimpleImageButton(window,
                                                                                0,
                                                                                0,
                                                                                w,
                            ...
                            IOguiImage *imgs =
                                ogui->LoadOguiImage( getLocaleGuiString( ("gui_upgradewindow_" + upgScript
                                                                          + "_img_normal").c_str() ) );
                            IOguiImage *imgsh =
                                ogui->LoadOguiImage( getLocaleGuiString( ("gui_upgradewindow_" + upgScript
                                                                          + "_img_normal").c_str() ) );
                            ...
                            temp->text->SetDisabled(true);
                            temp->textFontNormal =
                                oguiLoader.LoadFont( getLocaleGuiString( (std::string("gui_")
                                                                          + oguiLoader.GetWindowName()
                                                                          + "_" + upgScript
                                                                          + "_cost_text_font_normal").c_str() ) );
                            temp->textFontDisabled =
                                oguiLoader.LoadFont( getLocaleGuiString( (std::string("gui_")
                                                                          + oguiLoader.GetWindowName()
                                                                          + "_" + upgScript
                                                                          + "_cost_text_font_disabled").c_str() ) );
                            temp->selected_button = selected_button;
                            temp->selected_offset_x = getLocaleGuiInt(
                                "gui_upgradewindow_weaponupgrade_background_button_offset_x",
                                0);
                            temp->selected_offset_y = getLocaleGuiInt(
                                "gui_upgradewindow_weaponupgrade_background_button_offset_y",
                                0);
                            temp->original_x = temp->button->GetX();
                            temp->original_y = temp->button->GetY();
                            ...
                            {
                                int x = temp->button->GetX() + getLocaleGuiInt(
                                    "gui_upgradewindow_weaponupgrade_costicon_offset_x",
                                    0);
                                int y = temp->button->GetY() + getLocaleGuiInt(
                                    "gui_upgradewindow_weaponupgrade_costicon_offset_y",
                                    0);
                            ...
                            if ( game->upgradeManager->isLocked(unit, upgIds[ j ]) ) {
                                if (locked_img == NULL)
                                    locked_img =
                                        ogui->LoadOguiImage( game::getLocaleGuiString(
                                                                 "gui_upgradewindow_upgrade_locked_img") );
            ...
            if (button_id >= items_first_id && button_id < items_last_id) {
                ...
                for (unsigned int i = 0; i < items.size(); i++) {
                    if ( items[i].button == NULL
                         ...
                         || (items[i].varType != SurvivorItem::TYPE_ITEM && items[i].varType !=
                             SurvivorItem::TYPE_TOGGLE_ITEM) ) continue;
                    ...
                    if ( !items[i].script_can_use.first.empty() ) {
                        ...
                        int return_value = game->gameScripting->runOtherScript(
                             items[i].script_can_use.first.c_str(), items[i].script_can_use.second.c_str(), unit,
                            VC3(0, 0, 0) );
                        if (return_value == 0) {
                            game->gameUI->playGUISound( game::getLocaleGuiString("gui_upgrades_sound_upgrade_failed") );
                            continue;
                    ...
                    if ( !items[i].script_use.first.empty() ) {
                        ...
                        game->gameScripting->runOtherScript(
                             items[i].script_use.first.c_str(), items[i].script_use.second.c_str(), unit, VC3(0, 0, 0) );
                    ...
                    if ( !items[i].use_text.empty() ) {
                        ...
                        int x = items[i].button->button->GetX() - insufficientPartsButton->GetSizeX() / 2
                                + items[i].button->button->GetSizeX() / 2;
        ...
        void updateTheButtons()
        {
            ...
            // upgrade amounts
            {
                ...
                int tmp2 = game->upgradeManager->getCharacterPartsAmount(this->unit)
                           - this->characterUpgradesPendingCost;
            ...
            for (i = upgradesPending.begin(); i != upgradesPending.end(); ++i) {
                ...
                if ( game->upgradeManager->canUpgrade(unit, upgid) && !game->upgradeManager->isLocked(unit, upgid) )
                    ...
                    Logger::getInstance()->warning(
                        "SurvivorUpgradeWindow::applyUpgrades - Pending upgrade cannot be applied, internal error.");
            }
