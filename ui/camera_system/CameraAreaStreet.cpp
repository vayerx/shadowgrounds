...
//v3:=vectorTo(vectorSum(corner[i*2], vMult(line, dot(line, vectorTo(corner[i*2], v2)) / sqrvLen(line))), v2);
            ...
            VC3 temp = VC3(1, 1, 1);
            float w = 1
                      + (v3.GetDotWith( ( ( temp.GetCrossWith(line) ).GetNormalized() ) )
                         - ROTATE_OFFSET) / ROTATE_DISTANCE;
            bool outside = false;
            for (int k = 0; k < 2; k++) {
                ...
                if ( ( line.GetCrossWith(point) ).y < 0 ) {
                    ...
                    v3 = v2
                         - corner[k * 2 + 1] + line * ( line.GetDotWith(v2 - corner[k * 2 + 1]) / line.GetSquareLength() );
                    float w2 = 1 + ( v3.GetDotWith( ( ( temp.GetCrossWith(line) ).GetNormalized() ) ) )
                               / ROTATE_DISTANCE_SIDE;
                    if (w2 < 0) w2 = 0;
