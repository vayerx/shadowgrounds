...
// FIXME: FRAMERATE DEPENDENT MAGIC NUMBER HERE
                    ...
                    if ( fp->doesKeepReloading() )
                        if ( fp->getSelectedWeapon() != -1
                             ...
                             && ( fp->getWeaponType( fp->getSelectedWeapon() )->getAttachedWeaponType() == NULL
                                  || !fp->getWeaponType( fp->getSelectedWeapon() )->getAttachedWeaponType()->
                                  isSingleReloading() ) )
                    ...
                    if ( game->gameUI->getController(c)->wasKeyClicked(DH_CTRL_FLASHLIGHT) )
                             ...
                             && !fp->isDestroyed() )
                        {
                            ...
                            if (fl != NULL) {
                                ...
                                //{
#ifdef LEGACY_FILES
                                game->gameUI->playSoundEffect("Data/Sounds/Defaultfx/flashlight_switch.wav",
                                                              campos.x,
                                                              campos.y,
                                                              campos.z,
                                                              false,
...
#else
                                game->gameUI->playSoundEffect("data/audio/sound/gui/flashlight_switch.wav",
                                                              campos.x,
                                                              campos.y,
                                                              campos.z,
                                                              false,
                        ...
                        while (nextSelection != curSelection) {
                            ...
                            if (failCount > WEAPON_CTRL_AMOUNT) {
                                Logger::getInstance()->warning(
                                    "CombatWindow::doCombatControls - Failed to change next/prev weapon.");
                                break;
                            }
                        }
                    ...
                    if ( fp->getMoveState() != Unit::UNIT_MOVE_STATE_UNCONSCIOUS
                        ...
                        if (!SimpleOptions::getBool(DH_OPT_B_2ND_PLAYER_ENABLED)
                            || SimpleOptions::getInt(DH_OPT_I_1ST_PLAYER_CONTROL_SCHEME + c) <
                            GameController::CONTROLLER_TYPE_JOYSTICK1)
                                ...
                                if ( game->gameUI->getController(c)->wasKeyClicked(ctrl + DH_CTRL_WEAPON_1) ) {
                                    ...
                                    if (weapId != 0) {
                                        ...
                                        if ( newweap != -1 && fp->getWeaponType(newweap) ) {
                                            ...
                                            if (!w->isSelectableWithoutAmmo() && fp->getWeaponAmmoAmount(newweap) ==
                                                0) {
                                                newweap = -1;
                                                weapId = 0;
                                            }
                                ...
                                if (game->unitSelections[player]->getUnitsSelected() > 0) {
                                         ...
                                         isKeyDown(DH_CTRL_FORCE_ATTACK) )
                                    {
                                        ...
                                        VC3 targ = VC3(sel->scaledMapX,
                                                       game->gameMap->getScaledHeightAt(sel->scaledMapX,
                                                                                        sel->scaledMapY),
                                                       sel->scaledMapY);
                                        ...
                                        {
                                            game->formations.addMovePoint(&unitVector,
                                                                          targ,
                                                                          game::Unit::MoveTypeStealth);
                                        } else {
                                            ...
                                            if ( now < lastMoveClickTime + 400
                                                ...
                                                game->formations.addMovePoint(&unitVector,
                                                                              targ,
                                                                              game::Unit::MoveTypeFast);
                                            else
                                                game->formations.addMovePoint(&unitVector,
                                                                              targ,
                                                                              game::Unit::MoveTypeNormal);
                        ...
                        && now < dragStartTime + 400)
                    {
                        ...
                        if (unitsSelected.empty() == false) {
                            ...
                            if (selection->hit) {
                                ...
                                tacticalUnitWindow->tacticalDone();
                                // open tactical menu
                                tacticalUnitWindow->showAt(eve->cursorScreenX,
                                                           eve->cursorScreenY,
                                                           &unitsSelected,
                                                           selection->unit);
#endif
                                  ...
                                  && w2->isHeavyWeapon() ) )
                        {
                            ...
                            if ( u->isSelected() )
                                gamePointers->addPointer(tpos,
                                                         GPOINTER_GROUNDTARGET, u->targeting.getTargetUnit(), lineFrom,
                                                         (float)maxDist);
                            else
                                gamePointers->addPointer(tpos,
                                                         GPOINTER_UNSEL_GROUNDTARGET,
                                                         u->targeting.getTargetUnit(), lineFrom, (float)maxDist);
                        } else {
                            if ( u->isSelected() )
                                gamePointers->addPointer(tpos,
                                                         GPOINTER_TARGET, u->targeting.getTargetUnit(), lineFrom,
                                                         (float)maxDist);
                            else
                                gamePointers->addPointer(tpos,
                                                         GPOINTER_UNSEL_TARGET, u->targeting.getTargetUnit(), lineFrom,
                                                         (float)maxDist);
                        }
                    }
    ...
    void CombatWindow::setTacticalModeButton(bool tactical)
    {
        if (tactical) {
            if (tacticalModeBut == NULL) {
                tacticalModeBut = ogui->CreateSimpleTextButton(
                    win.get(),
                    4, UNITSTAT_BOTTOM_START_Y - 20, 64, 16, NULL, NULL,
                    NULL,
