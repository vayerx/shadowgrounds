 ...
 */
    ...
    void Animator::setAnimation(IAnimatable *animatable,
            ...
            if (animNumber < 0 || animNumber >= MAX_ANIMATIONS) {
                ...
                if (visualObject == NULL) {
                        ...
                        && animfiles[animNumber] != NULL)
                    {
                        ...
                        if (animations[animNumber] == NULL) {
                            Logger::getInstance()->warning(
                                "Animator::setAnimation - Failed to create new bone animation.");
                    ...
                    if (visualObject->animation != animations[animNumber]) {
                        ...
                        if (blend) {
                            visualObject->model->BlendToAnimation(transitionAnim,
                                                                  visualObject->animation,
                                                                  blendTime,
                                                                  loop);
        ...
        if (animations != NULL) {
            if (animNumber < 0 || animNumber >= MAX_ANIMATIONS) {
                Logger::getInstance()->error(
                    "Animator::setAnimationSpeedFactor - Animation number out of bounds (internal error).");
            } else {
                if (visualObject == NULL) {
    ...
    void Animator::setBlendAnimation(IAnimatable *animatable,
                                     int          blendNumber,
                                     int          transitionNumber,
                                     int          animNumber,
                                     bool         blend,
            ...
            if (animNumber < 0 || animNumber >= MAX_ANIMATIONS) {
                ...
                if (visualObject == NULL) {
                    ...
                    if (!animations[animNumber]) {
                        Logger::getInstance()->error(
                            "Animator::endBlendAnimation - Requested animation was not loaded (animation file missing?), possibly causing erronous animation behaviour.");
