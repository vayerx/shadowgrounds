...
namespace ui
{
    ...
    void GameConsole::enter()
    {
        if (inputBufUsed + inputBufRightUsed != 0) {
            ...
            if (historyBufs[historyNumber % GAMECONSOLE_HISTORY] != NULL
                ...
                && strcmp(historyBufs[historyNumber % GAMECONSOLE_HISTORY],
                          historyBufs[(historyNumber + GAMECONSOLE_HISTORY - 1) % GAMECONSOLE_HISTORY]) == 0)
            {
                ...
                if (opt != NULL) {
                             ...
                             || opt->getVariableType() == game::IScriptVariable::VARTYPE_STRING) )
                    {
                        ...
                        Logger::getInstance()->debug(
                            "GameConsole::enter - No new value for option given, so nothing done.");
                    }
                } else {
    ...
    void GameConsole::loadHistory(const char *filename)
    {
        ...
        if (f != NULL) {
            ...
            Logger::getInstance()->debug(
                "GameConsole::loadHistory - Console history file open failed (assuming no previous history saved).");
            return;
        }
        ...
        if ( sp.loadFile(filename) ) {
            ...
            Logger::getInstance()->warning(
                "GameConsole::loadHistory - Error loading history file even though it existed.");
        }
    }
    ...
    void GameConsole::saveHistory(const char *filename)
    {
        ...
        if (f != NULL) {
            ...
            Logger::getInstance()->warning(
                "GameConsole::saveHistory - Failed to open console history file for writing.");
        }
    }
