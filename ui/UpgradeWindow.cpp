...
#define UPGRADEW_UPGRADEBUT_LAST  (UPGRADEW_UPGRADEBUT_FIRST \
                                   + (UPGRADEWINDOW_MAX_WEAPONS * UPGRADEWINDOW_MAX_UPGRADES_PER_WEAPON) - 1)
...
namespace ui
{
    UpgradeWindow::UpgradeWindow(Ogui *ogui, game::Game *game, game::Unit *unit)
    {
                ...
                if (wnum != -1) {
                    {
...

                        {
                            ...
                            weaponButtons[i] =
                                ogui->CreateSimpleImageButton( win, wx + woffx, wy, sizex, sizey, NULL, NULL, NULL,
                                                               NULL, id,
                                                               reinterpret_cast<void *>(partTypeId) );
                    ...
                    // upgrades for that weapon
                    {
                        ...
                        for (int j = 0; j < (int)upgIds.size(); j++) {
                            ...
                            upgradeSelections[i][j] = ogui->CreateSimpleImageButton(win,
                                                                                    x,
                                                                                    y,
                                                                                    sizex,
                                                                                    sizey,
                            ...
                            int upgid = upgIds[j];
                            upgradeButtons[i][j] =
                                ogui->CreateSimpleImageButton( win, x, y, sizex, sizey, NULL, NULL, NULL, NULL, id,
                                                               reinterpret_cast<void *>(upgid) );
        ...
        // info picture and text
        {
            ...
            infoBackground = ogui->CreateSimpleImageButton(win,
                                                           x,
                                                           y,
                                                           bgsizex,
                                                           bgsizey,
            ...
            // picture
            {
                ...
                infoIcon = ogui->CreateSimpleImageButton(win,
                                                         x + offx,
                                                         y + offy,
                                                         sizex,
                                                         sizey,
        ...
        // finally undo button
        {
            ...
            undoButton = ogui->CreateSimpleTextButton(win,
                                                      x,
                                                      y,
                                                      sizex,
                                                      sizey,
        ...
        // and close button
        {
            ...
            closeButton = ogui->CreateSimpleTextButton(win,
                                                       x,
                                                       y,
                                                       sizex,
                                                       sizey,
    ...
    void UpgradeWindow::CursorEvent(OguiButtonEvent *eve)
    {
        ...
        if (eve->eventType == OguiButtonEvent::EVENT_TYPE_OVER) {
            ...
            if (id >= UPGRADEW_UPGRADEBUT_FIRST
                && id <= UPGRADEW_UPGRADEBUT_LAST)
            {
                highlightedWeaponSlot =
                    (eve->triggerButton->GetId() - UPGRADEW_UPGRADEBUT_FIRST) / UPGRADEWINDOW_MAX_UPGRADES_PER_WEAPON;
                highlightedUpgradeSlot =
                    (eve->triggerButton->GetId() - UPGRADEW_UPGRADEBUT_FIRST) % UPGRADEWINDOW_MAX_UPGRADES_PER_WEAPON;
        ...
        while ( !upgradesPending->isEmpty() ) {
            ...
            if ( game->upgradeManager->canUpgrade(unit, upgid) )
                ...
                Logger::getInstance()->warning(
                    "UpgradeWindow::applyUpgrades - Pending upgrade cannot be applied, internal error.");
        }
        this->upgradesPendingCost = 0;
