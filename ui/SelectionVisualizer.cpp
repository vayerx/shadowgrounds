...
namespace ui
{
    ...
    void SelectionVisualizer::clearSelectionForUnifiedHandle(UnifiedHandle uh)
    {
        if ( !VALIDATE_UNIFIED_HANDLE_BITS(uh) ) {
            LOG_WARNING_W_DEBUG(
                 "SelectionVisualizer::clearSelectionForUnifiedHandle - Given unified handle is not valid.", int2str(uh) );
            return;
        }
        ...
        LOG_WARNING_W_DEBUG(
             "SelectionVisualizer::clearSelectionForUnifiedHandle - Given unified handle not found in list.",
            int2str(uh) );
    }
    ...
    void SelectionVisualizer::setSelectionForUnifiedHandle(UnifiedHandle uh)
    {
        if ( !VALIDATE_UNIFIED_HANDLE_BITS(uh) ) {
            LOG_WARNING_W_DEBUG(
                 "SelectionVisualizer::setSelectionForUnifiedHandle - Given unified handle is not valid.", int2str(uh) );
            return;
        }
        ...
        for (int i = 0; i < (int)selectionUnifiedHandles.size(); i++) {
            if (selectionUnifiedHandles[i] == uh) {
                LOG_WARNING_W_DEBUG(
                     "SelectionVisualizer::setSelectionForUnifiedHandle - Given unified handle already found in list.",
                    int2str(uh) );
                return;
            }
        }
