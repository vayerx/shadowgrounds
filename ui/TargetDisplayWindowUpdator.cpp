...
///////////////////////////////////////////////////////////////////////////////
    namespace {
        ...
        TargetDisplayWindowUpdatorRegisterer *__attribute__( (used) ) temp_static_haxoring_constructing_thing =
            new TargetDisplayWindowUpdatorRegisterer("TargetDisplayWindow");
            ...
            for (it = itemsToBeUpdated.begin(); it != itemsToBeUpdated.end(); ++it) {
                ...
                if (distance < updateDistance)
                    ...
                    if ( item && item->getVisualObject() && item->getVisualObject()->getStormModel() ) {
                        ...
                        if ( window->setRect( item, rect.x, rect.y, rect.w, rect.h, (distance / updateDistance),
                                              item->getHighlightStyle() ) )
                ...
                if (distance < updateDistance) {
                    ...
                    if ( unit && unit->getVisualObject() && unit->getVisualObject()->getStormModel() ) {
                        ...
                        for (int i = 0; i < 2; i++) {
                            ...
                            const TargetDisplayButtonManager::ButtonCreator &bc = window->getManager()->getButtonStyle(
                                style);
                            if (bc.rect_style == 0) {
                                ...
                                float radius = rect_scale * ut->getTargetRectScale() * sqrtf(
                            ...
                            if ( window->setRect(p, rect.x, rect.y, rect.w, rect.h, (distance / updateDistance),
                                                 style) ) {
                            ...
                            window->setSliderValue( p,
                                                    ( (float)unit->getHP() / (float)unit->getMaxHP() ),
                                                    unit->getUnitType()->getHealthSliderScale() );
                        }
    ...
    void TargetDisplayWindowUpdator::risingMessage(game::Unit *unit, const std::string &text, int style)
    {
        if (unit == NULL) {
            Logger::getInstance()->error(
                "TargetDisplayWindowUpdator::risingMessage() - could not create a message for a null unit");
            return;
        }
