...
namespace ui
{
    ...
    void VisualEffectManager::loadVisualEffectTypes()
    {
        ...
        // read in files...
        boost::shared_ptr<filesystem::IFileList> files = filesystem::FilePackageManager::getInstance().findFiles(
            "data/effect/visualeffect",
            "*.vef");
                ...
                if (k != NULL) {
                    ...
                    if (strcmp(k, "lighteffect") == 0) {
                        ...
                        Logger::getInstance()->warning(
                            "VisualEffectManager::loadVisualEffectTypes - \"lighteffect\" key deprecated (use \"spotlight\" instead).");
#endif
                    }
    ...
    VisualEffect *VisualEffectManager::createNewManagedVisualEffect(int               visualEffectId,
                                                                    int               lifetimeInTicks,
                                                                    IPointableObject *object,
                                                                    IPointableObject *origin,
                                                                    const VC3        &position,
                                                                    const VC3        &endPosition,
                                                                    const VC3        &rotation,
                                                                    const VC3        &velocity,
                                                                    game::Game       *game,
                                                             ...
                                                             IStorm3D_Model   *spawnModel)
    {
        ...
        if (visualEffectId >= 0 && visualEffectId < MAX_VISUAL_EFFECT_TYPES) {
            ...
            if (particleEffectID != -1) {
#ifdef PHYSICS_PHYSX
                if ( game::SimpleOptions::getBool(DH_OPT_B_PHYSICS_PARTICLES) && game->getGamePhysics() &&
                     game->getGamePhysics()->getPhysicsLib() ) {
                ...
                if (v->particleEffect) {
                    boost::shared_ptr<frozenbyte::particle::IParticleCollision> ptr =
                        boost::static_pointer_cast<frozenbyte::particle::IParticleCollision>(particleCollision);
                    ...
                    boost::shared_ptr<frozenbyte::particle::IParticleArea> area =
                        boost::static_pointer_cast<frozenbyte::particle::IParticleArea>(particleArea);
                    ...
                    if ( visualEffectTypes[visualEffectId].doesAttachToSpawnModel() ) {
                        if (spawnModel != NULL)
                            ...
                            LOG_WARNING_W_DEBUG(
                                ...
                                "VisualEffectManager::createNewVisualEffect - Effect should be attached to spawn model, but null spawn model parameter was given.",
                                visualEffectTypes[visualEffectId].getName() );
                    }
            ...
            if (visualEffectTypes[visualEffectId].getCameraShakeAmount() > 0) {
                ...
                int shakeTime = 500;
                game->gameUI->getGameCamera()->setShakeEffect(
                    visualEffectTypes[visualEffectId].getCameraShakeAmount(), shakeTime, position);
            }
            ...
            if (decalEffectID != -1 && decalAmount > 0) {
                ...
                int randomVariation =
                    (SystemRandom::getInstance()->nextInt() % (amountVariation * 2 + 1) - amountVariation);
            ...
            if (visualEffectTypes[visualEffectId].getSpotLightEffect() != NULL) {
                ...
                if (terrain != NULL) {
                    ui::Spotlight *sp = new ui::Spotlight( *storm3d, *terrain,
                                                           *scene, NULL,
                                                           std::string( visualEffectTypes[visualEffectId].
                                                                        getSpotLightEffect() ) );
                    v->setSpotlight(sp);
            ...
            if (visualEffectTypes[visualEffectId].getPointLightEffect() != NULL) {
                ...
                if (game->gameUI->getDynamicLightManager() != NULL) {
                    ...
                    if (dLightType != -1) {
                        ...
                        LOG_ERROR_W_DEBUG(
                             "VisualEffectManager::loadVisualEffectTypes - Given dynamic point light type not defined.",
                            visualEffectTypes[visualEffectId].getPointLightEffect() );
                    }
                }
            }
        ...
        for (int i = 0; i < MAX_VISUAL_EFFECT_TYPES; i++) {
            ...
            if (particleEffect != NULL) {
                ...
                if ( !stream.isEof() ) {
                    ...
                    if (efId == -1) {
                        Logger::getInstance()->warning(
                            "VisualEffectManager::loadParticleEffects - Particle effect load failed.");
                        Logger::getInstance()->debug(particleEffect);
                    }
                } else {
                    Logger::getInstance()->warning(
                        "VisualEffectManager::loadParticleEffects - Particle effect was defined for visual effect, but was not found.");
                    Logger::getInstance()->debug(particleEffect);
                }
            }
            ...
            if ( game::SimpleOptions::getBool(DH_OPT_B_PHYSICS_FLUIDS_ENABLED) ) {
                ...
                if (particleEffectHardwareFluid != NULL) {
                    ...
                    filesystem::InputStream stream = filesystem::FilePackageManager::getInstance().getFile(
                        particleEffectHardwareFluid);
                    if ( !stream.isEof() ) {
                        stream >> parser;
                        ...
                        if (efId == -1) {
                            Logger::getInstance()->warning(
                                "VisualEffectManager::loadParticleEffects - Hardware fluid particle effect load failed.");
                            Logger::getInstance()->debug(particleEffectHardwareFluid);
                        }
                    } else {
                        Logger::getInstance()->warning(
                            "VisualEffectManager::loadParticleEffects - Hardware fluid particle effect was defined for visual effect, but was not found.");
                        Logger::getInstance()->debug(particleEffectHardwareFluid);
                    }
                }
            }
            ...
            if ( game::SimpleOptions::getBool(DH_OPT_B_PHYSICS_PARTICLES) ) {
                ...
                if (particleEffectHardwareRigid != NULL) {
                    ...
                    if ( !stream.isEof() ) {
                        ...
                        if (efId == -1) {
                            Logger::getInstance()->warning(
                                "VisualEffectManager::loadParticleEffects - Hardware rigid particle effect load failed.");
                            Logger::getInstance()->debug(particleEffectHardwareRigid);
                        }
                    } else {
                        Logger::getInstance()->warning(
                            "VisualEffectManager::loadParticleEffects - Hardware rigid particle effect was defined for visual effect, but was not found.");
                        Logger::getInstance()->debug(particleEffectHardwareRigid);
                    }
                }
            }
        ...
        for (int i = 0; i < MAX_VISUAL_EFFECT_TYPES; i++) {
            ...
            if (decalEffect != NULL) {
                ...
                if (id == -1) {
                    ...
                    Logger::getInstance()->warning(
                        "VisualEffectManager::loadDecalEffects - Decal effect was defined for visual effect, but was not found.");
