...
namespace ui
{
            ...
            && pp_filename.substr(pp_filename.length() - 4, 4) == ".ast")
        {
            ...
            if ( game::SimpleOptions::getBool(DH_OPT_B_FORCE_AST_PREPROCESS) ) {
                Logger::getInstance()->warning(
                    "AnimationSet - Preprocessed animation set is recreated (force preprocess on).");
            ...
            if (!upToDate)
                if (ppWarning) {
                    Logger::getInstance()->warning(
                        "AnimationSet - Preprocessed animation set is not up to date, but it will not be recreated (auto preprocess off).");
                    Logger::getInstance()->debug(filename);
                }
                        ...
                        for (int j = 0; j < ANIM_AMOUNT; j++) {
                            if (strcmp(key, anim_names[j]) == 0) {
                                parseValue(value,
                                           ...
                                           &speedFactorTable[j],
                                           &staticFactorTable[j],
                                           &staticFactorValueTable[j],
                                           &blendTimeTable[j],
                                           &noInterpolateTable[j],
                                           &parser);
                                keyok = true;
                                break;
                            }
    ...
    void AnimationSet::parseValue(char               *value,
                                  int                *fileNumber,
                                  bool               *looping,
                                  bool               *immediate,
                                  int                *blend,
