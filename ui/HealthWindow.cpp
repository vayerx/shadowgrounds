...
namespace ui
{
        ...
        healthTextMultiplier(0.5f)
    {
        ...
        healthBackgroundButton =
            ogui->CreateSimpleImageButton(win, 0, 0,
                                          getLocaleGuiInt( (prefix  + "size_x").c_str(), 0 ), getLocaleGuiInt(
                                               (prefix  + "size_y").c_str(), 0 ), NULL, NULL, NULL);
        ...
        fillupButton = ogui->CreateSimpleImageButton(win, getLocaleGuiInt(
                                                          (prefix  + "fillup_offset_x").c_str(), 0 ), getLocaleGuiInt(
                                                          (prefix  + "fillup_offset_y").c_str(), 0 ), getLocaleGuiInt(
                                                          (prefix  + "fillup_size_x").c_str(), 0 ), getLocaleGuiInt(
                                                          (prefix  + "fillup_size_y").c_str(), 0 ), NULL, NULL, NULL);
        fillupButton->SetDisabled(true);
        fillupButton->SetDisabledImage(fillupImage);

        curveButton = ogui->CreateSimpleImageButton(win, getLocaleGuiInt(
                                                         (prefix  + "curve_offset_x").c_str(), 0 ), getLocaleGuiInt(
                                                         (prefix  + "curve_offset_y").c_str(), 0 ), getLocaleGuiInt(
                                                         (prefix  + "curve_size_x").c_str(), 0 ), getLocaleGuiInt(
                                                         (prefix  + "curve_size_y").c_str(), 0 ), NULL, NULL, NULL);
        ...
        healthAmountText = ogui->CreateTextLabel(win, 0, 0, getLocaleGuiInt(
                                                      (prefix  + "size_x").c_str(),
                                                     0 ), getLocaleGuiInt( (prefix  + "size_y").c_str(), 0 ), "");
    ...
    void HealthWindow::update()
    {
        ...
        if ( true && game && game->gameUI && game->gameUI->getFirstPerson(clientNum) &&
             game->gameUI->getFirstPerson(clientNum)->getUnitType() )
    ...
    void HealthWindow::updateCurve()
    {
        ...
        {
            float health_percentage = 100.0f * game->gameUI->getFirstPerson(clientNum)->getHP()
                                      / (float)game->gameUI->getFirstPerson(clientNum)->getMaxHP();
