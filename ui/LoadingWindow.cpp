...
namespace ui
{
    ...
    void LoadingWindow::createWindows()
    {
        ...
        // close (start)
        {
            ...
            closebut = ogui->CreateSimpleTextButton(win, x, y, w, h,
                                                    ...
                                                    close_high.empty() ? NULL : close_high.c_str(),
                                                    close_text.c_str(), LOADINGW_CLOSE);
        ...
        // loading
        {
            ...
            loadingbut = ogui->CreateSimpleTextButton(win, x, y, w, h,
                                                      ...
                                                      high.empty() ? NULL : high.c_str(),
                                                      loadingtext.c_str(), LOADINGW_PROGRESS);
        ...
        // hack: find alt index from savegame
            ...
            std::string filename = std::string( game->getGameProfiles()->getProfileDirectory(0) ) + "/Save/save_"
                                   + std::string( game->getPendingLoad() ) + ".dhs";
            ...
            if (f != NULL) {
                ...
                {
                    std::string new_locale = mission_brief2_locale + "_alt" + std::string( int2str(value) );
                    if ( ::game::DHLocaleManager::getInstance()->hasString( ::game::DHLocaleManager::BANK_GUI,
                                                                            new_locale.c_str() ) )
                        mission_brief2_locale = new_locale;
                }
                delete[] buf;
        ...
        bool has_scrolling_text = ::game::DHLocaleManager::getInstance()->hasString( ::game::DHLocaleManager::BANK_GUI,
                                                                                     mission_brief2_locale.c_str() );
        ...
        if ( ::game::DHLocaleManager::getInstance()->hasString( ::game::DHLocaleManager::BANK_GUI,
                                                                mission_cinematic_locale.c_str() ) ) {
    ...
    void LoadingWindow::enableClose()
    {
        ...
        game->gameUI->getTerrain()->GetTerrain()->getRenderer().enableFeature(IStorm3D_TerrainRenderer::RenderTargets,
                                                                              false);
    ...
    void LoadingWindow::startScrolling()
    {
        ...
        // play briefing sound
        {
            ...
            if ( ::game::DHLocaleManager::getInstance()->hasString( ::game::DHLocaleManager::BANK_GUI,
                                                                    (mission_brief2_locale + "_sound_time").c_str() ) )
        ...
        if ( ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                               font_normal_locale.c_str(), &scrolling_font_file)
             || ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                                  font_bold_locale.c_str(), &scrolling_font_bold_file)
             || ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                                  font_italic_locale.c_str(),
                                                                  &scrolling_font_italic_file)
             || ::game::DHLocaleManager::getInstance()->getString(::game::DHLocaleManager::BANK_GUI,
                                                                  font_h1_locale.c_str(), &scrolling_font_h1_file) )
        {
            scrollingText->deleteRegisteredFonts();
            delete scrollingText->getFont();
    ...
    void LoadingWindow::update()
    {
        ...
        if ( !cinematic.empty() ) {
            ...
            // cinematic ended
                ...
                if (background_image) {
                    ...
                    std::string mission = game->getMissionId() ? game->getMissionId() : "";
                    background_image =
                        ogui->LoadOguiImage( getLocaleGuiString( ("gui_loadingwindow_" + mission + "_background").c_str() ) );
                    win->setBackgroundImage(background_image);
                    delete bg_old;
        ...
        if (scrollingStarted && !fadingOut) {
            ...
            if (speechSoundStartTime != -1 && Timer::getTime() > speechSoundStartTime) {
                ...
                speechSound = game->gameUI->playSpeech(convertLocaleSpeechString(
                                                            mission_sound.c_str() ), pos.x, pos.y, pos.z, false,
                                                       DEFAULT_SPEECH_VOLUME, false);
                speechSoundStartTime = -1;
            }
        }
    }
    ...
    void LoadingWindow::createUpgradeButton()
    {
        // upgrade button thingie
        {
            ...
            upgradeMenuBut = ogui->CreateSimpleTextButton(win, x, y, w, h,
                                                          ...
                                                          down.empty() ? NULL : down.c_str(),
                                                          high.empty() ? NULL : high.c_str(),
                                                          upgradetext.c_str(), LOADINGW_UPGRADEMENU);
