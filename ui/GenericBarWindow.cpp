...
//=============================================================================
    ...
    void GenericBarWindow::loadDataFromLocales(const std::string &locale_name)
    {
        const std::string prefix = locale_name + "_";
        {
            ...
            if ( DHLocaleManager::getInstance()->getString(DHLocaleManager::BANK_GUI, (prefix + "img_hidden").c_str(),
                                                           &hidden_img) ) {
        ...
        }
        {
            ...
            if ( DHLocaleManager::getInstance()->hasString( DHLocaleManager::BANK_GUI,
                                                            (prefix + "message_on_show").c_str() ) ) {
                messageOnShow = getLocaleGuiString( (prefix + "message_on_show").c_str() );

                if ( messageOnShowStyle.empty() == false && game && game->gameUI &&
                     game->gameUI->getCombatWindow(player) &&
                     game->gameUI->getCombatWindow(player)->getSubWindow("ElaborateHintMessageWindow") )
                    ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(player)->getSubWindow(
                          "ElaborateHintMessageWindow") )->showMessage(messageOnShowStyle, messageOnShow);
                    // game->gameUI->getCombatWindow( player )->
            }
        ...
        if (isOfFadingType) {
            if ( (getTime() - lastTime) > fadeOffTime ) {
                if (hidden == false) {
                    ...
                    if ( messageOnHideStyle.empty() == false && game && game->gameUI &&
                         game->gameUI->getCombatWindow(player) &&
                         game->gameUI->getCombatWindow(player)->getSubWindow("ElaborateHintMessageWindow") )
                    {
                        ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(player)->getSubWindow(
                              "ElaborateHintMessageWindow") )->showMessage(messageOnHideStyle, messageOnHide);

                        OguiFormattedText *text =
                            ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(player)->getSubWindow(
                             "ElaborateHintMessageWindow") )->getFormattedTextForStyle(messageOnHideStyle);
                ...
                if (hidden == true) {
                         ...
                         game->gameUI->getCombatWindow(player)->getSubWindow("ElaborateHintMessageWindow") )
                    {
                        ...
                        OguiFormattedText *text =
                            ( (ElaborateHintMessageWindow *)game->gameUI->getCombatWindow(player)->getSubWindow(
                             "ElaborateHintMessageWindow") )->getFormattedTextForStyle(messageOnShowStyle);
