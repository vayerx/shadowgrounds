...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        CinematicScreenImpl(Ogui              *ogui,
                            Game              *game,
                            const std::string &cinematic_name,
            ...
            currentSoundEffect(0)
        {
            ...
            window =
                ogui->CreateSimpleWindow(getLocaleGuiInt( (prefix + "_window_x").c_str(),
                                                          0 ), getLocaleGuiInt( (prefix + "_window_y").c_str(), 0 ),
                                         getLocaleGuiInt( (prefix + "_window_w").c_str(),
                                                          0 ), getLocaleGuiInt( (prefix + "_window_h").c_str(), 0 ),
                                         NULL);

            {
                ...
                backgroundImage =
                    ogui->CreateSimpleImageButton(window, getLocaleGuiInt(
                                                       (prefix + "_window_x").c_str(), 0 ), getLocaleGuiInt(
                                                       (prefix + "_window_y").c_str(), 0 ), getLocaleGuiInt(
                                                       (prefix + "_window_w").c_str(), 0 ), getLocaleGuiInt(
                                                       (prefix + "_window_h").c_str(), 0 ), NULL, NULL, NULL);
            ...
            if ( SimpleOptions::getBool(DH_OPT_B_VIDEO_ENABLED) ) {
                ...
                if ( ::DHLocaleManager::getInstance()->getString(::DHLocaleManager::BANK_GUI,
                                                                 (prefix + "_window_video_aspect").c_str(),
                                                                 &video_aspect_str) ) {
                    float normal_aspect =
                        ( window->GetSizeX() * ogui->GetScaleX() ) / (float)( window->GetSizeY() * ogui->GetScaleY() );
                    ...
                    if (texcoord_multiplier > 1.0f) {
                        ...
                        wideScreenBars[0] = ogui->CreateSimpleImageButton(window,
                                                                          0,
                                                                          0,
                                                                          w,
                                                                          h,
                        ...
                        wideScreenBars[1] = ogui->CreateSimpleImageButton(window,
                                                                          0,
                                                                          y,
                                                                          w,
                                                                          h,
            ...
            // images
            {
                ...
                for (int i = 0; i < num_of; i++) {
                    ...
                    {
                        pictureFadeByTime.insert( std::pair< int,
                                                             std::pair< std::string,
                                                                        int > >( time,
                                                                                 std::pair< std::string, int >(file,
                                                                                                               length) ) );
                    }
                }
            }
            ...
            // effects
            {
                ...
                for (int i = 0; i < num_of; i++) {
                    ...
                    if (time <= 0)
                        ...
                        effectsByTime.insert( std::pair< int,
                                                         std::pair< int,
                                                                    int > >( time, std::pair< int, int >(effect, length) ) );
                }
            }
            ...
            // text areas
            {
                ...
                for (int i = 0; i < num_of; i++) {
                    ...
                    textAreas[ i ] = new OguiFormattedText( window, ogui,
                                                            getLocaleGuiInt(
                                                                 (pre_text + "_x").c_str(),
                                                                0 ), getLocaleGuiInt( (pre_text + "_y").c_str(), 0 ),
                                                            getLocaleGuiInt(
                                                                 (pre_text + "_w").c_str(),
                                                                0 ), getLocaleGuiInt( (pre_text + "_h").c_str(), 0 ) );
                    ...
                    if ( ::DHLocaleManager::getInstance()->hasString( ::DHLocaleManager::BANK_GUI,
                                                                      (pre_text + "_typingsound").c_str() ) )
                    ...
                    for (int j = 0; j < num_of_texts; j++) {
                        // get time
                        int time =
                            getLocaleGuiInt( (pre_text + "_text_" + boost::lexical_cast< std::string >(
                                                  j)  + "_insert_time").c_str(), 0 );

                        // get text
                        std::string text =
                            getLocaleGuiString( ( pre_text + "_text_" + boost::lexical_cast< std::string >(j) ).c_str() );
                        ...
                        // credits hack
                        if ( ( pre_text + "_text_" + boost::lexical_cast< std::string >(j) ) ==
                             "cinematic_screen_credits_textarea_0_text_0" )
                            text = G_UiCredits;
                        ...
                        if (cinematic_name == "SGS_credits") {
                            ...
                            std::string secrets = boost::lexical_cast<std::string>(
                                 game->gameScripting->getGlobalIntVariableValue("secretpart_amount") );
                        ...
                        // sound file hack
                        std::string sound_file_locale = pre_text + "_text_" + boost::lexical_cast< std::string >(j)
                                                        + "_audiofile";
                        std::string speech_file_locale = pre_text + "_text_" + boost::lexical_cast< std::string >(j)
                                                         + "_speechfile";
                        const char *sound_file = NULL;
                        bool audiofile = ::DHLocaleManager::getInstance()->getString(::DHLocaleManager::BANK_GUI,
                                                                                     sound_file_locale.c_str(),
                                                                                     &sound_file);
                        bool audiofile_stream =
                            ::DHLocaleManager::getInstance()->getString(::DHLocaleManager::BANK_GUI,
                                                                        (sound_file_locale + "stream").c_str(),
                                                                        &sound_file);
                        bool audiofile_speech = ::DHLocaleManager::getInstance()->getString(::DHLocaleManager::BANK_GUI,
                                                                                            speech_file_locale.c_str(),
                                                                                            &sound_file);
                        ...
                        if (time <= 0)
                            ...
                            textsByTime.insert( std::pair< int,
                                                           std::pair< int,
                                                                      std::string > >( time,
                    ...
                    if ( ::DHLocaleManager::getInstance()->hasString( ::DHLocaleManager::BANK_GUI, locale.c_str() ) ) {
                        ...
                        for (; t < textQueue.size(); t++) {
                            ...
                            std::string error_location = "locale " + locale + " line "
                                                         + boost::lexical_cast<std::string>(t - entry_offset + 1);
                                ...
                                if ( c + 1 == entry.length() ) {
                                    ...
                                    if (inside_quote)
                                        Logger::getInstance()->error( (
                                                                          "CinematicScreen - Missing terminating quote in "
                            ...
                            if (params.size() < 2) {
                                Logger::getInstance()->error( ("CinematicScreen - Expecting 2 or more parameters in "
                                                               + error_location).c_str() );
                                continue;
                            }
                            ...
                            for (unsigned int p = 2; p < params.size(); p++) {
                                ...
                                if (define.size() != 2) {
                                    Logger::getInstance()->error( ("CinematicScreen - Error parsing parameter "
                                ...
                                if (key == "typingdelay") {
                                    ...
                                    Logger::getInstance()->error( ("CinematicScreen - Unknown parameter "
                                                                   + boost::lexical_cast<std::string>(p
                                                                                                      + 1) + " in "
                                                                   + error_location).c_str() );
                                }
                            ...
                            if (textQueue[t].time <= last_time) {
                                Logger::getInstance()->error(
                                     (
                                        "CinematicScreen - Expecting a time value larger than previous in "
                                        + error_location).c_str() );
                                continue;
                            }
            ...
            // text faders
            if ( ::DHLocaleManager::getInstance()->hasString( ::DHLocaleManager::BANK_GUI,
                                                              (prefix + "_textfader_amount").c_str() ) ) {
            ...
            // sound effects
            if ( ::DHLocaleManager::getInstance()->hasString( ::DHLocaleManager::BANK_GUI,
                                                              (prefix + "_soundeffect_amount").c_str() ) ) {
                ...
                for (unsigned int i = offset; i < soundEffects.size(); i++) {
                    const std::string &pre_fader =
                        ( prefix + "_soundeffect_" + boost::lexical_cast< std::string >(i - offset) );
                    ...
                    soundEffects[i].file =
                        game::convertLocaleSpeechString( getLocaleGuiString( (pre_fader + "_file").c_str() ) );
                    soundEffects[i].handle = -1;
                    soundEffects[i].speech = false;
                    const char *stream = NULL;
                    if (::DHLocaleManager::getInstance()->getString(::DHLocaleManager::BANK_GUI,
                                                                    (pre_fader + "_stream").c_str(), &stream)
                        && stream[0] == '1')
        ...
        void update()
        {
            ...
            while ( currentSoundEffect < soundEffects.size() ) {
                ...
                if (time - initTime >= se.time) {
                    ...
                    else if (se.speech) se.handle = game->gameUI->playSpeech(
                            se.file.c_str(), pos.x, pos.y, pos.z, false, 100, false);
            ...
            if ( ( timeToDie == -1 && backgroundVideoStream &&
                   !backgroundVideoStream->isPlaying() ) || (timeToDie > 0 && timeToDie + initTime < time) )
                close();
...
///////////////////////////////////////////////////////////////////////////////
    ...
    CinematicScreen::CinematicScreen(Ogui              *ogui,
                                     game::Game        *game,
