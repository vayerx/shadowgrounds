                ...
                // Create screen-size visibility buffer
                {
                    for (int y = 0; y < info.height; ++y) {
                        for (int x = 0; x < info.width; ++x) {
                            ...
                            if ( visPos.x < 0 || visPos.y < 0 ||
                                 (visPos.x >= visibilitySize.x && visPos.y >= visibilitySize.y) )
                                continue;
                ...
                if ( !layer1.empty() && !layer2.empty() ) {
                    ...
                    for (int y = 0; y < info.height; ++y) {
                        for (int x = 0; x < info.width; ++x) {
                            ...
                            FB_ASSERT(
                                 colPos.x >= 0 && colPos.y >= 0 && colPos.x < int(MAP_RESOLUTION) && colPos.y <
                                int(MAP_RESOLUTION) );

                            int layerIndex = colPos.y * MAP_RESOLUTION + colPos.x;
                            FB_ASSERT( layerIndex >= 0 && layerIndex < int( layer1.size() ) && layerIndex <
                                       int( layer2.size() ) );
