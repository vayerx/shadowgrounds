...
namespace ui
{
    AniRecorderWindow::AniRecorderWindow(Ogui *ogui, game::Game *game)
    {
        ...
        sliderBut->SetEventMask(
            OGUI_EMASK_PRESS | OGUI_EMASK_OVER | OGUI_EMASK_LEAVE | OGUI_EMASK_OUT | OGUI_EMASK_CLICK);

        timeLabel = ogui->CreateTextArea(win,
                                         ANIRECW_SLIDER_WIDTH + 16 + 8,
                                         80 + 16 * 6,
                                         512 - ANIRECW_SLIDER_WIDTH - 16,
                                         16,
                                         "00:00.00");
        ...
        scrollUpStyle = new OguiButtonStyle(scrollUpImage,
                                            scrollUpPressedImage,
                                            scrollUpDisabledImage,
                                            scrollUpImage,
                                            defaultFont,
                                            16,
                                            16);
        scrollDownStyle = new OguiButtonStyle(scrollDownImage,
                                              scrollDownPressedImage,
                                              scrollDownDisabledImage,
                                              scrollDownImage,
                                              defaultFont,
                                              16,
                                              16);
        ...
        smallListStyle = new OguiSelectListStyle(smallUnselStyle,
                                                 smallSelStyle,
                                                 scrollUpStyle,
                                                 scrollDownStyle,
                                                 158,
        ...
        tinyListStyle = new OguiSelectListStyle(tinyUnselStyle,
                                                tinySelStyle,
                                                scrollUpStyle,
                                                scrollDownStyle,
                                                38 + 16,
    ...
    void AniRecorderWindow::createMiniDependantButtons()
    {
        ...
        animationSelectList = ogui->CreateSelectList(win,
                                                     8 + 8 + 8 + 8 + 40 * 7,
                                                     80 + 24 + 16 * 7,
                                                     smallListStyle,
                                                     0,
        ...
        smoothPositionBut = ogui->CreateSimpleImageButton(
            win,
            8 + 16 + 8 + 40 * 4 - 2,
            80 + 24 + 16 * 7,
            32,
        ...
        smoothRotationBut = ogui->CreateSimpleImageButton(
            win,
            8 + 16 + 8 + 40 * 5 - 4,
            80 + 24 + 16 * 7,
            32,
        ...
        deletePositionBut = ogui->CreateSimpleImageButton(
            win,
            8 + 8 + 40 * 2,
            80 + 24 + 8 + 16 * 9,
            32,
    ...
    void AniRecorderWindow::CursorEvent(OguiButtonEvent *eve)
    {
        ...
        if (eve->eventType == OguiButtonEvent::EVENT_TYPE_CLICK) {
            ...
            if (eve->triggerButton->GetId() == ANIRECW_CAMERADELETE) {
                ...
                const char *imgs0[3] =
                { "Data/GUI/Buttons/quit_yes.tga", "Data/GUI/Buttons/quit_yes_down.tga",
                  "Data/GUI/Buttons/quit_yes_highlight.tga" };
                const char *imgs1[3] =
                { "Data/GUI/Buttons/quit_no.tga", "Data/GUI/Buttons/quit_no_down.tga",
                  "Data/GUI/Buttons/quit_no_highlight.tga" };
                const char **images[2];
                images[0] = imgs0;
                images[1] = imgs1;
                const char *text = "Delete selected camera?";
                cameraDelBox = new MessageBoxWindow(ogui,
                                                    text,
                                                    2,
                                                    choices,
                                                    images,
                                                    ...
                                                    96,
                                                    27,
                                                    "Data/GUI/Windows/quit_confirm.tga",
                                                    MSGBOX_CAMERADELETE,
                                                    this);
            ...
            if (eve->triggerButton->GetId() == ANIRECW_ADDFEWTICKS) {
                if (this->selectedUnit != NULL) {
                    if (currentSelectedTicks > 0) {
                        std::string comment = std::string("// added ")
                                              + int2str(currentSelectedTicks * 15) + std::string(" msec ticks\r\n");
                        std::string undodesc = std::string("add ") + int2str(currentSelectedTicks) + std::string(
                            " ticks");
                    ...
                    Logger::getInstance()->warning(
                        "AniRecorderWindow::CursorEvent - Cannot \"add few ticks\" as no unit selected.");
                }
            }
            /*
            ...
            if (eve->triggerButton->GetId() == ANIRECW_ADDANIM) {
                if (this->selectedUnit != NULL) {
                    ...
                    Logger::getInstance()->warning(
                        "AniRecorderWindow::CursorEvent - Cannot \"add anim\" as no unit selected.");
                }
            }
            if (eve->triggerButton->GetId() == ANIRECW_ADDCOMMANDS) {
            ...
            if (eve->triggerButton->GetId() == ANIRECW_DELETEPOSITION) {
                if (this->selectedUnit != NULL)
                    ...
                    Logger::getInstance()->warning(
                        "AniRecorderWindow::CursorEvent - Cannot \"delete position\" as no unit selected.");
            }
            if (eve->triggerButton->GetId() == ANIRECW_DROPONGROUND) {
                if (this->selectedUnit != NULL)
                    ...
                    Logger::getInstance()->warning(
                        "AniRecorderWindow::CursorEvent - Cannot \"drop unit on ground\" as no unit selected.");
            }
            if (eve->triggerButton->GetId() == ANIRECW_RECORDPATH) {
                game->gameUI->showConsole();
                game->gameUI->getConsole()->setMiniQueryMode();
