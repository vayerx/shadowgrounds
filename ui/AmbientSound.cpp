...
namespace ui
{
    ...
    void AmbientSound::tick(game::GameUI *gameUI, SoundLooper *looper,
        ...
        if (inArea) {
            if (!played) {
                if (rollOff == 0) {
                    if (gameUI->parseSoundFromDefinitionString(const_cast<char *>( defString.c_str() ),
                                                               listenerPosition.x, listenerPosition.y,
                                                               listenerPosition.z,
                                                               &looped, &handle, &key, false, range,
                                                               DEFAULT_SOUND_PRIORITY_LOW, false, true) != -1)
                    {
                        assert(looped);
                        played = true;
                    }
                } else {
                    if (gameUI->parseSoundFromDefinitionString(const_cast<char *>( defString.c_str() ), position.x,
