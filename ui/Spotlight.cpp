...
#  endif
                ...
                IStorm3D_Texture *texture = storm.CreateNewTexture(
                    "Data\\Textures\\Projective_Lights\\lightninglight.tga");
                if (!texture)
                    return;
                ...
                if (game::SimpleOptions::getInt(DH_OPT_I_LIGHTING_LEVEL) <= 50) {
                    IStorm3D_Texture *texture = storm.CreateNewTexture(
                        "Data\\Textures\\Lights\\PlayerDefaultVision.dds");
                    if (!texture)
                        return;
                    ...
                    IStorm3D_Texture *texture = storm.CreateNewTexture(
                        "Data\\Textures\\Projective_Lights\\muzzleflash_light.tga");
                    if (!texture)
                        return;
                ...
                if (game::SimpleOptions::getInt(DH_OPT_I_LIGHTING_LEVEL) <= 50) {
                    ...
                    IStorm3D_Texture *texture = storm.CreateNewTexture(
                        "Data\\Textures\\Projective_Lights\\electricflash_light.tga");
                    if (!texture)
                        return;
                ...
                if (game::SimpleOptions::getInt(DH_OPT_I_LIGHTING_LEVEL) <= 50) {
                    ...
                    IStorm3D_Texture *texture = storm.CreateNewTexture(
                        "Data\\Textures\\Projective_Lights\\flamerflash_light.tga");
                    if (!texture)
                        return;
                        ...
                        || props.type == SpotTypeProperties::SPOT_LIGHT_MODEL_TYPE_FLAT)
                    {
                        ...
                        else
                        {
                            if ( !props.coneTextureFilename.empty() ) {
                                ...
                                if (coneStormTexture != NULL) {
                                    ...
                                    LOG_ERROR_W_DEBUG( "Spotlight - Failed to load cone texture.",
                        ...
                        if (!props.fakelight)
                            LOG_WARNING_W_DEBUG(
                                ...
                                "Spotlight - Spot lighting model is none, and fakelight is not on (probably unintended?).",
                                spottype.c_str() );
                    } else {
    ...
    void Spotlight::setSpotlightParams(COL color)
    {
        ...
        if (data->spot) {
            ...
            if (data->lightModel) {
                ...
                boost::scoped_ptr<Iterator<IStorm3D_Model_Object *> > object_iterator(
                     data->lightModel->ITObject->Begin() );
