 ...
 */
    ...
    float getFadeFactor(const VC2 &point, const VC2 &planeMin, const VC2 &planeMax, float radius)
    {
        ...
        if (
            intersectX(planeCenter, dir, planeMin.y - planeCenter.y - radius, planeMax.x - planeCenter.x + radius,
                       limitDistance)
            |
            intersectX(planeCenter, dir, planeMax.y - planeCenter.y + radius, planeMax.x - planeCenter.x + radius,
                       limitDistance)
            ||
            intersectY(planeCenter, dir, planeMin.x - planeCenter.x - radius, planeMax.y - planeCenter.y + radius,
                       limitDistance)
            ||
            intersectY(planeCenter, dir, planeMax.x - planeCenter.x + radius, planeMax.y - planeCenter.y + radius,
                       limitDistance)
            )
    ...
    float fakeRange(const VC3       &point,
                    IStorm3D_Camera *camera,
                    const SpotImp   &imp,
                    IStorm3D_Scene  *scene = NULL,
...
//#pragma message("*** this assert seems to be false 'somewhat randomly' while loading mission - uninitialized data or scene changing randomly while loading based on input?. --jpk")
        ...
        if ( scene && game::SimpleOptions::getBool(DH_OPT_B_DEBUG_VISUALIZE_POINTLIGHT_SHADOW_AREAS) ) {
            ...
            for (int i = 0; i < shadowPlaneVertexAmount; i++) {
                VC3 offset(0, 0.1f, 0);
                scene->AddLine( clippedPolygon[i] + offset,
                                clippedPolygon[ (i + 1) % shadowPlaneVertexAmount ] + offset, COL(color, color,
                                                                                                  color) );
            }
        }
#endif
 ...
 */
    ...
    struct LightManager::Data {
        ...
        Data(IStorm3D         &storm_,
             IStorm3D_Scene   &scene_,
             IStorm3D_Terrain &terrain_,
             util::LightMap   *lightMap_,
             Terrain          *uiTerrain_)
            :   storm(storm_),
            scene(scene_),
            terrain(terrain_),
                    ...
                    if ( game::SimpleOptions::getBool(DH_OPT_B_DEBUG_VISUALIZE_POINTLIGHT_SHADOW_AREAS) )
                        for (int l = 0; l < LIGHTING_FAKE_SPOT_AMOUNT + 1; l++) {
                            if ( closestFakeLights[l] >= 0 && closestFakeLights[l] < int( fakeSpots.size() ) )
                                fakeRange(
                                    VC3(), camera, fakeSpots[ closestFakeLights[l] ], &scene, 1.0f - (float)l
                                    / LIGHTING_FAKE_SPOT_AMOUNT);
                        }
...
#define GETLIGHTING_POINT_TEST_OUTSIDE_AREA(a) \
    (p ## a ## .x < l.minPlane.x \
     ...
     || p ## a ## .z < l.minPlane.y  \
        ...
        void getLighting(const VC3      &position,
                         PointLights    &pointLights,
                         bool            includeFactor,
                         float           radius,
                         bool            smoothedTransitions,
                         IStorm3D_Model *model) const
        {
 ...
 */
    ...
    LightManager::LightManager(IStorm3D         &storm,
                               IStorm3D_Scene   &scene,
                               IStorm3D_Terrain &terrain,
                               util::LightMap   *lightMap,
                               Terrain          *uiTerrain)
 ...
 */
    ...
    void LightManager::getLighting(const VC3      &position,
                                   PointLights    &lights,
                                   float           range,
                                   bool            smoothedTransitions,
                                   bool            includeFactor,
                                   IStorm3D_Model *override_model) const
    {
