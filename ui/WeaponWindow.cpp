...
namespace ui
{
        ...
        lastUpdateWeaponAmount(0)
    {
        ...
        this->win =
            ogui->CreateSimpleWindow(xPosition, yPosition, getLocaleGuiInt(
                                          (prefix + "icon_window_size_x").c_str(), 0 ), getLocaleGuiInt(
                                          (prefix + "icon_window_size_y").c_str(), 0 ), NULL);
        this->win->SetUnmovable();

        this->listWin = ogui->CreateSimpleWindow(getLocaleGuiInt(
                                                      (prefix + "list_position_x").c_str(), 0 ), getLocaleGuiInt(
                                                      (prefix + "list_position_y").c_str(), 0 ), getLocaleGuiInt(
                                                      (prefix + "list_size_x").c_str(),
                                                     0 ), getLocaleGuiInt( (prefix + "list_size_y").c_str(), 0 ), NULL);
...

        {
            ...
            for (int i = 0; i < WEAPONWINDOW_MAX_WEAPONS; i++) {
                ...
                weaponNames[ i ] =
                    getLocaleGuiString( ( std::string("gui_weapon_select_text_") + boost::lexical_cast< std::string >(
                                              i) ).c_str() );
#else
...
#  endif
                weaponNames[ i ] =
                    getLocaleGuiString( ( std::string(prefix2 + "select_text_") + boost::lexical_cast< std::string >(
                                              i) ).c_str() );
        ...
        weaponIconButton = ogui->CreateSimpleImageButton(win, getLocaleGuiInt(
                                                              (prefix + "icon_pos_x").c_str(),
                                                             0 ), getLocaleGuiInt(
                                                              (prefix + "icon_pos_y").c_str(), 0 ), getLocaleGuiInt(
                                                              (prefix + "icon_size_x").c_str(),
                                                             0 ), getLocaleGuiInt(
                                                              (prefix + "icon_size_y").c_str(), 0 ), NULL, NULL, NULL);
...

        {
            ...
            for (int i = 0; i < WEAPONWINDOW_MAX_WEAPONS; i++) {
                ...
                if (i == 4) {
                    if (game->gameUI->getFirstPerson(clientNum) &&
                        game->gameUI->getFirstPerson(clientNum)->getWeaponByWeaponType( PARTTYPE_ID_STRING_TO_INT(
                                                                                            "W_Elect3") )
                        != -1) {
                    } else {
                        weaponListButtons[i] = NULL;
...
#endif
                weaponListButtons[i] = ogui->CreateSimpleImageButton(listWin,
                                                                     0 + i * offsetX,
                                                                     0 + i * offsetY,
                                                                     sizeX,
                                                                     sizeY,
        ...
        if ( getLocaleGuiInt( (prefix + "use_text").c_str(), 0 ) ) {
            weaponSelectionText = ogui->CreateTextLabel(win, getLocaleGuiInt(
                                                             (prefix + "text_offset_x").c_str(), 0 ), getLocaleGuiInt(
                                                             (prefix + "text_offset_y").c_str(), 0 ), getLocaleGuiInt(
                                                             (prefix + "text_size_x").c_str(),
                                                            0 ), getLocaleGuiInt(
                                                             (prefix + "text_size_y").c_str(), 0 ), "");
        ...
        if ( getLocaleGuiInt( (prefix + "use_label").c_str(), 0 ) ) {
            labelButton = ogui->CreateSimpleTextButton(win, getLocaleGuiInt(
                                                            (prefix + "label_offset_x").c_str(), 0 ),  getLocaleGuiInt(
                                                            (prefix + "label_offset_y").c_str(), 0 ), getLocaleGuiInt(
                                                            (prefix + "label_size_x").c_str(), 0 ), getLocaleGuiInt(
                                                            (prefix + "label_size_y").c_str(),
    ...
    void WeaponWindow::update()
    {
...

        {
            for (int w = 0; w < UNIT_MAX_WEAPONS; w++) {
                ...
                if ( (weap->isSelectableWithoutAmmo() ||
                      game->gameUI->getFirstPerson(clientNum)->getWeaponAmmoAmount(w) > 0)
#ifdef PROJECT_SURVIVOR
                     && true )
                 #else
        ...
        if (selWeap != lastUpdateValue || lastUpdateWeaponAmount != weaponAmount) {
            ...
            for (int i = 0; i < WEAPONWINDOW_MAX_WEAPONS; i++) {
                ...
                if (weapNum != -1) {
                    ...
                    if (weap && !weap->isSelectableWithoutAmmo() &&
                        game->gameUI->getFirstPerson(clientNum)->getWeaponAmmoAmount(weapNum) == 0)
                        weapNum = -1;
                }
            ...
            if (weaponAmount == 0 || selWeap == -1) {
                ...
                int uinum = game::PlayerWeaponry::getUINumberByWeaponId( game->gameUI->getFirstPerson(
                                                                             clientNum),
                                                                         game->gameUI->getFirstPerson(
                                                                             clientNum)->getWeaponType(
                                                                             selWeap)->getPartTypeId() );
                if (uinum >= 0 && uinum < WEAPONWINDOW_MAX_WEAPONS) {
                    if ( !game->gameUI->getFirstPerson(clientNum)->isWeaponOperable(selWeap) ) {
                        ...
                        if ( ::game::DHLocaleManager::getInstance()->hasString( DHLocaleManager::BANK_GUI,
                                                                                buf.c_str() ) ) {
