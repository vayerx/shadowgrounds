...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification 
{
	...
	{
		std::string optdir = getenv("HOME") + std::string("/Library/Survivor");
	...
	std::string options = getenv("HOME") + std::string("/Library/Survivor/config/options.txt");
	...
	[languageCombo addItemWithObjectValue: @"Danish"];
	[languageCombo addItemWithObjectValue: @"Norwegian"];
	[languageCombo addItemWithObjectValue: @"Svenska"];
...
- (void)changeLanguage:(int)index
{
	...
	if(index == 0)
		...
		parser = new LauncherParser("data/locales/dk/launcher.txt");
	else if(index == 5)
		parser = new LauncherParser("data/locales/no/launcher.txt");
	else if(index == 6)
		parser = new LauncherParser("data/locales/se/launcher.txt");
	...
	setStringValue(launchButton, parser, "launcher_launch_tab");
...
- (IBAction)launch:(id)sender
{
	...
	// HACK: disable reflections on Mac as they are broken
	//parser.setProperty("render_reflection", 0);
