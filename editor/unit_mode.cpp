    ...
    namespace editor {
        namespace {
            struct SharedData {
                ...
                void updateTypes(int primary, int secondary)
                {
                    ...
                    for (int p = 0; p < unitHierarchy.getPrimaryAmount(); ++p) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_TYPE_PRIMARY, CB_ADDSTRING, 0,
                            reinterpret_cast<long>( str.c_str() ) );
                    }
                    ...
                    for (int s = 0; s < unitHierarchy.getSecondaryAmount(primary); ++s) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_TYPE_SECONDARY, CB_ADDSTRING, 0,
                            reinterpret_cast<long>( str.c_str() ) );
                    }
                    ...
                    for (int i = 0; i < unitHierarchy.getUnitScriptAmount(); ++i) {
                        ...
                        SendDlgItemMessage( dialog.getWindowHandle(), IDC_UNIT_LIST, LB_ADDSTRING, 0,
                                    ...
                                    if (getConfig(UnitScripts::Hit, IDC_UNIT_HIT) == scripts.hitScript)
                                        if (getConfig(UnitScripts::Hitmiss, IDC_UNIT_HITMISS) == scripts.hitmissScript)
                                            if (getConfig(UnitScripts::Noise, IDC_UNIT_NOISE) == scripts.noiseScript)
                                                if (getConfig(UnitScripts::Execute,
                                                              IDC_UNIT_EXECUTE) == scripts.executeScript)
                                                    if (getConfig(UnitScripts::Special,
                                                                  IDC_UNIT_SPECIAL) == scripts.specialScript)
                                                        if (getConfig(UnitScripts::Pointed,
                                                                      IDC_UNIT_POINTED) == scripts.pointedScript) {
                                                            SendDlgItemMessage(
                                                                dialog.getWindowHandle(), IDC_UNIT_CONFIGURATION,
                                                                CB_SETCURSEL, i, 0);
                                                            return;
                                                        }
                    }
                ...
                void setUnit(const UnitHandle &unitHandle)
                {
                    ...
                    SendDlgItemMessage(dialog.getWindowHandle(), IDC_UNIT_SIDE, CB_SETCURSEL,
                                       terrainUnits.getUnitSide(activeUnit), 0);
                    updateHelpers();
                }
                ...
                void updateHelpers()
                {
                    ...
                    if (index >= 0) {
                        int length = SendDlgItemMessage(
                            dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_GETTEXTLEN, index, 0);
                        activeText.resize(length + 1);

                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_GETTEXT, index,
                            reinterpret_cast<LPARAM>(&activeText[0]) );
                    }
                    ...
                    for (int i = 0; i < helpers.getHelperAmount(); ++i) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_ADDSTRING, 0,
                            reinterpret_cast<LPARAM>( string.c_str() ) );
                    }
                    ...
                    if ( !activeText.empty() )
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_SELECTSTRING, 0,
                            reinterpret_cast<LPARAM>( activeText.c_str() ) );
                }
                ...
                void initScriptBox(UnitScripts::ScriptType type, int itemId)
                {
                    ...
                    for (int i = 0; i < unitScripts.getScriptCount(type); ++i) {
                        const std::string &string = unitScripts.getScript(type, i).name;
                        SendDlgItemMessage( dialog.getWindowHandle(), itemId, CB_ADDSTRING, 0,
                                            reinterpret_cast<LPARAM>( string.c_str() ) );
                    }
                ...
                void initDialog()
                {
                    ...
                    for (i = 0; i < unitScripts.getScriptConfigCount(); ++i) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_UNIT_CONFIGURATION, CB_ADDSTRING, 0,
                            reinterpret_cast<LPARAM>( string.c_str() ) );
                    }
                    ...
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_UNIT_SIDE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Hostile") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_UNIT_SIDE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Neutral") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_UNIT_SIDE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Ally") );
                    ...
                    for (unsigned int j = 0; j < types.size(); ++j) {
                        SendDlgItemMessage( dialog.getWindowHandle(), IDC_UNIT_TYPE, CB_ADDSTRING, 0,
                                            reinterpret_cast<LPARAM>( types[j].c_str() ) );
                    }
                }
            ...
            public:
                ...
                void execute(int id)
                {
                    int index = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_UNIT_LIST, LB_GETCURSEL, 0, 0);
                    if ( sharedData.activeUnit.hasUnit() ) {
                        ...
                        if (index >= 0) {
                            ...
                            if ( !u.defaultConfiguration.empty() ) {
                                ...
                                int confIndex = SendDlgItemMessage(
                                     sharedData.dialog.getWindowHandle(), IDC_UNIT_CONFIGURATION, CB_FINDSTRINGEXACT,
                                    -1,
                                    reinterpret_cast<LPARAM>(f) );
                                if (confIndex >= 0)
                                    SendDlgItemMessage(
                                        sharedData.dialog.getWindowHandle(), IDC_UNIT_CONFIGURATION, CB_SETCURSEL,
                                        confIndex,
                                        0);
            ...
            struct HelperCommand : public ICommand {
                ...
                {
                    int index = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_GETCURSEL, 0, 0);
                    if ( sharedData.activeUnit.hasUnit() ) {
                ...
                void addPoint(const VC3 &position)
                {
                    int index = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_UNIT_HELPERS, LB_GETCURSEL, 0, 0);
                    if (index < 0 ||
                        IsDlgButtonChecked(sharedData.dialog.getWindowHandle(), IDC_POINT_INSERT) != BST_CHECKED)
                        return;
            ...
            public:
                ...
                void execute(int id)
                {
                    ...
                    std::vector<std::string> usedStrings = sharedData.terrainUnits.getUnitUsedProperties(
                        sharedData.activeUnit);
            ...
            public:
                ...
                {
                    int index = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_UNIT_SIDE, CB_GETCURSEL, 0, 0);
            ...
            public:
                MouseTracker(SharedData    &sharedData_,
                             Mouse         &mouse_,
                             HelperCommand &helperCommand_,
                             SideCommand   &sideCommand_)
                    :   sharedData(sharedData_),
                    mouse(mouse_),
                    helperCommand(helperCommand_),
            ...
            public:
                ...
                void execute(int id)
                {
                    ...
                    int indexP = SendDlgItemMessage(
                        data.dialog.getWindowHandle(), IDC_UNIT_TYPE_PRIMARY, CB_GETCURSEL, 0, 0);
                    int indexS = SendDlgItemMessage(
                        data.dialog.getWindowHandle(), IDC_UNIT_TYPE_SECONDARY, CB_GETCURSEL, 0, 0);
