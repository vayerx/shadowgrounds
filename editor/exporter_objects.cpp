...
namespace frozenbyte {
    namespace editor {
        ...
        struct ExporterObjectsData {
            ...
            void saveObjects(const std::string &fileName)
            {
                ...
                for (unsigned int i = 0; i < objects.size(); ++i) {
                    ...
                    stream << data.fileName << int(type) << int(data.fallType) << data.height << data.radius
                           << data.fireThrough;
                    ...
                    for (std::map<std::string, std::string>::iterator it = data.metaValues.begin();
                         it != data.metaValues.end();
                         ++it) {
                    ...
                    for (unsigned int j = 0; j < objects[i].instances.size(); ++j) {
                        ...
                        stream
                        << (unsigned int)( objects[i].instances[j].uniqueEditorObjectHandle
                                           & ( ( (UniqueEditorObjectHandle)1 << 32 ) - 1 ) );
                        stream
                        << (unsigned int)(objects[i].instances[j].uniqueEditorObjectHandle
                                          >> (UniqueEditorObjectHandle)32);
                    }
                }
            }
        ...
        int ExporterObjects::addTerrainObject(const std::string &fileName,
                                              CollisionType type,
                                              FallType fallType,
                                              float height,
                                              float radius,
                                              bool fireThrough,
                                              const string &explosionObject,
                                              const string &explosionScript,
                                              const string &explosionProjectile,
                                              const string &explosionEffect,
                                              const vector<string> &explosionSounds,
                                              ...
                                              int physicsType,
                                              float physicsMass,
                                              const std::string &physicsSoundMaterial,
                                              const VC3 &physicsData1,
                                              const VC3 &physicsData2,
                                              const std::string &durabilityType,
        ...
        void ExporterObjects::addObject(int               id,
                                        const VC3        &position,
                                        const VC3        &rotation,
                                        const COL        &color,
                                        float             height,
                                        signed short int *lightIndices,
                                        bool              lightmapped,
                                        bool              inBuilding,
                                        const VC3        &sunDir,
                                        float             sunStrength)
        ...
        void ExporterObjects::save(const ExportOptions &options) const
        {
            ...
            std::string buildingFileName = options.fileName + std::string("\\") + options.id + std::string(
                "_buildings.dhs");
