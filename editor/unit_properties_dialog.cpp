    ...
    namespace editor {
        ...
        struct UnitPropertiesDialog::Data {
            ...
            string getKey(HWND window) const
            {
                ...
                SendDlgItemMessage( window, IDC_UNIT_DATANAME, CB_GETLBTEXT, keyIndex,
                                    reinterpret_cast<LONG>( key.c_str() ) );
            ...
            static BOOL CALLBACK DialogHandler(HWND windowHandle, UINT message,  WPARAM wParam, LPARAM lParam)
            {
                if (message == WM_INITDIALOG) {
                    ...
                    Data *data =
                        reinterpret_cast<UnitPropertiesDialog::Data *>( GetWindowLong(windowHandle, GWL_USERDATA) );
        ...
        UnitPropertiesDialog::UnitPropertiesDialog(UnitProperties         &properties,
                                                   const StringList       &usedStrings,
                                                   const StringProperties &stringProperties)
        ...
        void UnitPropertiesDialog::execute(int id)
        {
            DialogBoxParam( GetModuleHandle(0), MAKEINTRESOURCE(
                                IDD_UNIT_PROPERTIES), 0, UnitPropertiesDialog::Data::DialogHandler,
