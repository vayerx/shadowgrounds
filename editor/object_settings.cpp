...
namespace frozenbyte {
    namespace editor {
...
#ifdef LEGACY_FILES
                if ( !data.explosionObject.empty() && data.explosionObject != "(disappear)" &&
                     !fileExists(data.explosionObject) )
                    data.explosionObject = FileWrapper::resolveModelName("Data\\Models\\Terrain_objects",
        ...
        std::vector<std::string> ObjectSettings::getMetaKeys() const
        {
            ...
            for (ObjectSettingsData::ValueMap::iterator it = data->defaultValues.begin();
                 it != data->defaultValues.end();
                 ++it) {
