...
namespace frozenbyte {
    namespace editor {
        namespace {
            ...
            struct BlendMap {
                ...
                void setPixel(const VC2I &blockIndex, const VC2I &blockPosition, int textureIndex, int weight)
                {
                    ...
                    blocks[finalBlockIndex].setPixel(textureIndex,
                                                     weight,
                                                     blockPosition.x,
                                                     blockPosition.y,
                                                     forceSingleTexture);
                    ...
                    if (blockPosition.x == 0 && blockIndex.x > 0)
                        blocks[finalBlockIndex - 1].setPixel(textureIndex,
                                                             weight,
                                                             BLOCK_SIZE - 1,
                                                             blockPosition.y,
                                                             forceSingleTexture);
                    if (blockPosition.y == 0 && blockIndex.y > 0)
                        blocks[finalBlockIndex - blockAmount.x].setPixel(textureIndex,
                                                                         weight,
                                                                         blockPosition.x,
                                                                         BLOCK_SIZE - 1,
                                                                         forceSingleTexture);
                    ...
                    if (blockPosition.y == 0 && blockIndex.y > 0)
                        if (blockPosition.x == 0 && blockIndex.x > 0)
                            blocks[finalBlockIndex - blockAmount.x - 1].setPixel(textureIndex,
                                                                                 weight,
                                                                                 BLOCK_SIZE - 1,
                                                                                 BLOCK_SIZE - 1,
                                                                                 forceSingleTexture);
                ...
                void addSplat(int textureIndex, const TextureSplat &splat)
                {
                    ...
                    for (int j = 0; j < limitY; ++j) {
                        for (int i = 0; i < limitX; ++i) {
                            ...
                            VC2I blockPosition( positionX - blockIndex.x * (BLOCK_SIZE - 1),
                                                positionY - blockIndex.y * (BLOCK_SIZE - 1) );
        ...
        void TerrainTextures::addTexture(const std::string &fileName)
        {
            ...
            int textureIndex = 0;
            for (TerrainTexturesData::TextureContainer::iterator it = data->textures.begin();
                 it != data->textures.end();
                 ++it) {
