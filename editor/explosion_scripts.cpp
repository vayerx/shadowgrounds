...
namespace frozenbyte {
    namespace editor {
        namespace {
            ...
            template <typename T> struct NameSorter : public binary_function<T, T, bool>{
            ...
            template <typename T> void parse(vector<T> &list, const ParserGroup &group, const string &base)
            {
                list.resize(1);
                list[0].name = "(empty)";
