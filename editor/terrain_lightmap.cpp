                        ...
                        for (int x = xstart; x < xend; ++x) {
                            ...
                            if (updateLight) {
                                ...
                                for (; it != lights.end(); ++it) {
                                        ...
                                        for (int i = sampleStart; i < sampleEnd; ++i) {
                                            ...
                                            for (int j = 0; j < smoothSamples; ++j) {
                                                ...
                                                if (!info.hit || info.range * info.range > rangeSq - bias) {
                                                    ...
                                                    IStorm3D_Material::ATYPE alpha =
                                                        info.object->GetMesh()->GetMaterial()->GetAlphaType();
                                    ...
                                    float fadeFactor2 = 1.f - cosf(3.14f * 0.5f - sqrtf(
                                                                       rangeSq) / lightRange * PI * .5f);
                                        ...
                                        for (int i = sampleStart; i < sampleEnd; ++i) {
                                            ...
                                            if (!info.hit) {
                                                ...
                                                if (alpha != IStorm3D_Material::ATYPE_NONE) {
                                                    ...
                                                    if (quality != 3)
                                                        storm.scene->RayTrace(it->position,
                                                                              sunRayDir,
                                                                              it->range + 1.f,
                                                                              info);
 ...
 */
                        ...
                        if ( !values.empty() )
                            for (int yt = 0; yt < ySize; ++yt) {
                                for (int xt = 0; xt < xSize; ++xt) {
                                    ...
                                    int r =
                                        (3 * c.r
                                         / 4) + (1 * (c1.r + c2.r + c3.r + c4.r + c5.r + c6.r + c7.r + c8.r) / 32);
                                    int g =
                                        (3 * c.g
                                         / 4) + (1 * (c1.g + c2.g + c3.g + c4.g + c5.g + c6.g + c7.g + c8.g) / 32);
                                    int b =
                                        (3 * c.b
                                         / 4) + (1 * (c1.b + c2.b + c3.b + c4.b + c5.b + c6.b + c7.b + c8.b) / 32);
                                    //int r = c.r;
                                    //int g = c.g;
                                    //int b = c.b;
                           ...
                           const VC3       &sunDir)
            {
                ...
                for (int y = start_y; y < end_y; y++) {
                    for (int x = start_x; x < end_x; x++) {
                        int dx = (x - center_x);
                        int dy = (y - center_y);
                        float merge_factor = merge_mul
                                             * ( 1.0f - sqrtf( (float)(dx * dx / fradius_x + dy * dy / fradius_y) ) );
                        ...
                        pixel.light.r = pixel.color.r =
                                            (int)( lightColor.r * merge_factor + pixel.color.r * (1.0f - merge_factor) );
                        pixel.light.g = pixel.color.g =
                                            (int)( lightColor.g * merge_factor + pixel.color.g * (1.0f - merge_factor) );
                        pixel.light.b = pixel.color.b =
                                            (int)( lightColor.b * merge_factor + pixel.color.b * (1.0f - merge_factor) );
                        ...
                        pixel.light.r =
                            (int)( (lightColor.r + sun_light) * merge_factor + old_light_r * (1.0f - merge_factor) );
                        pixel.light.g =
                            (int)( (lightColor.g + sun_light) * merge_factor + old_light_g * (1.0f - merge_factor) );
                        pixel.light.b =
                            (int)( (lightColor.b + sun_light) * merge_factor + old_light_b * (1.0f - merge_factor) );
                    }
                }
            }
        ...
        void TerrainLightMap::setShadow(const VC2       &position,
                                        float            value,
                                        const VC2       &rect,
                                        const LightList &lights,
                                        const VC3       &sunDir)
