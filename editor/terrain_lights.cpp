...
namespace frozenbyte {
    namespace editor {
        ...
        struct TerrainLights::Data {
            ...
            void redrawSpots()
            {
                ...
                for (; it != spots.end(); ++it) {
                    ...
                    if (spot.properties.pointLight && spot.properties.type == SpotProperties::ShadowCaster) {
                        ui::Light light;
                        light.position = spot.position + VC3(
                            0,
                            spot.properties.sourceHeight
                            + spot.properties.heightOffset,
                            0);
                                                ...
                                                VisualizationType     type) const
            {
                ...
                if (properties.type == SpotProperties::ShadowCaster) {
                    ...
                    // Light source
                    IStorm3D_Mesh *lightBlue = createWireframeObject(storm, model.get(), COL(.12f,
                                                                                             .12f,
                                                                                             .5f) * colorMul, "...");
                ...
                // Cone
                if (properties.type != SpotProperties::ShadowCaster) {
                    IStorm3D_Mesh *cone = createWireframeObject(storm, model.get(), COL(.5f,
                                                                                        .5f,
                                                                                        .5f) * colorMul, "cone");
            ...
            void save(filesystem::OutputStream &stream) const
            {
                ...
                for (; it != spots.end(); ++it) {
                    ...
                    stream
                    << (unsigned int)( spot.uniqueEditorObjectHandle & ( ( (UniqueEditorObjectHandle)1 << 32 ) - 1 ) );
        ...
        void TerrainLights::doExport(Exporter &exporter) const
        {
            ...
            for (; it != data->spots.end(); ++it) {
                Spot &spot = *it;
                lights.addSpot(spot.position, spot.yAngle, convertProperties(
                                   spot.properties), spot.properties.pointLight);
            }
