 ...
 */
        ...
        Dialog::Dialog(int resourceId, const Window &parentWindow, ResizeType type)
        {
            ...
            CreateDialogParam( GetModuleHandle(0), MAKEINTRESOURCE(resourceId),
                               parentWindow.getWindowHandle(), DialogData::DialogHandler,
                               reinterpret_cast<LPARAM>( tempData.get() ) );
            data.swap(tempData);
        }
        ...
        Dialog::Dialog(int resourceId, HWND parentWindowHandle, ResizeType type)
        {
            ...
            CreateDialogParam( GetModuleHandle(0), MAKEINTRESOURCE(
                                   resourceId), parentWindowHandle, DialogData::DialogHandler,
                               reinterpret_cast<LPARAM>( tempData.get() ) );
            data.swap(tempData);
        }
        ...
        void Dialog::show()
        {
            if (data->modalDialog)
                DialogBoxParam( GetModuleHandle(0), MAKEINTRESOURCE(
                                    data->resourceId), 0, DialogData::DialogHandler, reinterpret_cast<LPARAM>( data.get() ) );
