...
namespace frozenbyte {
    namespace editor {
        ...
        namespace {
            template <class T> struct LineDeleter {
                Storm &storm;
            ...
            struct SharedData {
                ...
                void updateDialog()
                {
                    ...
                    for (int i = 0; i < textureAmount; ++i) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_HMAP_TEXTURES, LB_ADDSTRING, 0,
            ...
            public:
                    ...
                    heightShape(0)
                {
                    ...
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_MODE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Raise") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_MODE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Lower") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_MODE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Flatten") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_MODE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Smoothen") );
                    SendDlgItemMessage(dialog.getWindowHandle(), IDC_HEIGHT_MODE, CB_SETCURSEL, 0, 0);
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_SHAPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Circle") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_HEIGHT_SHAPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Rectange") );
                ...
                void setHeightmap()
                {
                    ...
                    if ( !heightMap.empty() )
                        storm.terrain->setHeightMap(
                            &heightMap[0],
                            mapSize,
                            realSize,
                            textureDetail,
                            0,
                            EDITOR_COLLISION_HEIGHTMAP_MULTIPLIER,
                            EDITOR_COLLISION_HEIGHTMAP_MULTIPLIER
                            * EDITOR_OBSTACLEMAP_MULTIPLIER);
            ...
            class MouseTracker {
                ...
                {
                    boost::shared_ptr<IStorm3D_Line> line(
                         sharedData.storm.storm->CreateNewLine(), LineDeleter<IStorm3D_Line>(sharedData.storm) );
            ...
            public:
                ...
                void update()
                {
                    if (sharedData.updateMouse) {
                        radius =
                            float( SendDlgItemMessage(sharedData.dialog.getWindowHandle(), IDC_EDIT_RADIUS, TBM_GETPOS,
                                                      0, 0) );
                ...
                void tick()
                {
                    ...
                    if (sharedData.mode == 0) {
                        if ( mouse.isLeftButtonDown() ) {
                            ...
                            int texture = SendDlgItemMessage(
                                sharedData.dialog.getWindowHandle(), IDC_HMAP_TEXTURES, LB_GETCURSEL, 0, 0);
                            if (texture < 0)
                                return;

                            float strength =
                                float( SendDlgItemMessage(sharedData.dialog.getWindowHandle(), IDC_EDIT_STRENGTH,
                                                          TBM_GETPOS,
                                                          0, 0) );
                            strength /= 10.f;
                        ...
                        if ( mouse.isLeftButtonDown() ) {
                            ...
                            if (data.heightMode == 0) {
                                ...
                                float height =
                                    GetDlgItemInt(data.dialog.getWindowHandle(), IDC_HEIGHT, 0, TRUE) / 100.f;
                        ...
                        if ( mouse.isLeftButtonDown() ) {
                            ...
                            float strength = 0.1f
                                             * float( SendDlgItemMessage(sharedData.dialog.getWindowHandle(),
                                                                         IDC_EDIT_STRENGTH,
                                                                         TBM_GETPOS, 0, 0) );
                            ...
                            map.setShadow( VC2(ci.position.x,
                                               ci.position.z), strength, rect, lights,
                                           sharedData.editorState.getSunDirection() );
                            map.apply();
                        }
                    }
                }
        ...
        {
            boost::scoped_ptr<TerrainModeData> tempData( new TerrainModeData(storm,
                                                                             gui.getTerrainDialog(),
                                                                             gui.getMenuDialog(), editorState, gui) );
            data.swap(tempData);
