...
namespace frozenbyte {
    namespace editor {
                ...
                for (unsigned int i = 0; i < units.size(); ++i) {
                    ...
                    for (unsigned int j = 0; j < unit.spawnText.size(); ++j) {
                        ...
                        if (printLineUnconverted)
                            stream << "   " << replaceString(unit.spawnText[j],
                                                             unit.properties.strings,
                                                             unit.stringProperties.defaults) << std::endl;

                        if (convertToItemParams || convertToLightParams
                            || convertToParticleParams)
                        {
                            addParamsAdded = true;
                            stream << "   setPosition " << "s," << unit.position.x << "," << unit.position.y
                                   << std::endl;
                        ...
                        if (convertToDecorParams) {
                            addParamsAdded = true;
                            stream << "   setDecorPosition " << "s," << unit.position.x << "," << unit.position.y
                                   << std::endl;
                        ...
                        if (convertToWaterParams) {
                            addParamsAdded = true;
                            stream << "   setWaterPosition " << "s," << unit.position.x << "," << unit.position.y
                                   << std::endl;
                             ...
                             || (j == unit.spawnText.size() - 1 && !addParamsAdded) )
                        {
                            ...
                            stream << "   setUnitSpawnCoordinates " << "s," << unit.position.x << ","
                                   << unit.position.y << std::endl;
        ...
        void ExporterUnits::addUnit(const std::vector<std::string> &spawnText,
                                    const VC2                      &position,
                                    float                           yRotation,
                                    float                           height,
