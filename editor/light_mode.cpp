        ...
        namespace {
            ...
            void initialize(Dialog &dialog)
            {
                ...
                for (int i = 0; i < ui::SpotProperties::NumLightingModelTypes; i++) {
                    addComboString( dialog, IDC_LIGHT_LIGHTINGMODEL_TYPE,
                                    makeString( (ui::SpotProperties::LightingModelType)i ) );
                }
            ...
            struct Shared {
                ...
                void setProperties(int index)
                {
                    ...
                    colorValue =
                        RGB( unsigned char(p.color.r * 255), unsigned char(p.color.g * 255),
                ...
                TerrainLights::SpotProperties getProperties() const
                {
                    ...
                    result.lightingModelType = (TerrainLights::SpotProperties::LightingModelType) getComboIndex(
                        dialog,
                        IDC_LIGHT_LIGHTINGMODEL_TYPE);
            ...
            public:
                ...
                void execute(int id)
                {
                    ...
                    if (shared.active >= 0) {
                        ...
                        COL ambient = shared.state.getColorMap().getColor(position)
                                      + shared.state.getLightMap().getColor(position);
                ...
                void tick()
                {
                    ...
                    if (active == -1 && shared.active == -1) {
                        ...
                        if (shared.active >= 0) {
                            ...
                            pos = shared.storm.unitAligner.getMovedPosition(pos,
                                                                            *shared.storm.scene->GetCamera(),
                                                                            &updatePos);
