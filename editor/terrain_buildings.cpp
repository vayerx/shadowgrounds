            ...
            struct TerrainBuilding {
                ...
                void hideRoof(bool hide)
                {
                    for (unsigned int i = 0; i < instances.size(); ++i) {
                        boost::scoped_ptr<Iterator<IStorm3D_Model_Object *> > objectIterator(
                             instances[i].model->ITObject->Begin() );
        ...
        struct TerrainBuildingsData {
            ...
            void addTerrain(TerrainBuilding &terrainBuilding, int index)
            {
                ...
                if (!instance.model || !instance.floorModel) {
                    ...
                    instance.model =
                        boost::shared_ptr<IStorm3D_Model>( terrainBuilding.model->GetClone(true, true, true) );
                    instance.floorModel =
                        boost::shared_ptr<IStorm3D_Model>( terrainBuilding.model->GetClone(true, true,
                                                                                           true),
                                                           ModelDeleter(storm.floorScene) );
            ...
            void testCollisions(CollisionData &collisionData, Building &building)
            {
                ...
                for (; it != buildings.end(); ++it) {
                    ...
                    for (unsigned int i = 0; i < terrainBuilding.instances.size(); ++i) {
                        ...
                        if ( collisionVolume.testCollision(instance.model->GetPosition(),
                                                           VC3(0, instance.rotation, 0), collisionData, 0.f) ) {
