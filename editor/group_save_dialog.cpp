...
namespace frozenbyte {
    namespace editor {
            ...
            BOOL CALLBACK DialogHandler(HWND windowHandle, UINT message,  WPARAM wParam, LPARAM lParam)
            {
                ...
                if (message == WM_INITDIALOG) {
                    ...
                    for (int i = 0; i < data->list.getGroupAmount(); ++i) {
                        ...
                        SendDlgItemMessage( windowHandle, IDC_GROUP_GROUP, CB_ADDSTRING, 0,
                    ...
                    if (command == IDOK) {
                        ...
                        if (group >= 0) {
                            for (int i = 0; i < data->list.getSubgroupAmount(group); ++i) {
                                ...
                                SendDlgItemMessage( windowHandle, IDC_GROUP_SUBGROUP, CB_ADDSTRING, 0,
                                                    reinterpret_cast<LPARAM>( str.c_str() ) );
                            }
        ...
        {
            if ( !DialogBoxParam( GetModuleHandle(0), MAKEINTRESOURCE(IDD_GROUPSAVE), 0, DialogHandler,
                                  reinterpret_cast<LPARAM>(&data->strings) ) )
                return false;
