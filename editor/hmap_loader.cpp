        ...
        namespace {
            BOOL CALLBACK DialogHandler(HWND windowHandle, UINT message,  WPARAM wParam, LPARAM lParam)
            {
                ...
                if (message == WM_INITDIALOG) {
                    ...
                    SendDlgItemMessage( windowHandle, IDC_HMAP_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("8-bit integer") );
                    SendDlgItemMessage( windowHandle, IDC_HMAP_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("16-bit integer") );
                    SendDlgItemMessage( windowHandle, IDC_HMAP_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("24-bit integer") );
                    SendDlgItemMessage( windowHandle, IDC_HMAP_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("32-bit float") );
            ...
            {
                HmapFormat format;
                DialogBoxParam( GetModuleHandle(0), MAKEINTRESOURCE(IDD_HMAP), 0, DialogHandler,
