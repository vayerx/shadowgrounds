    ...
    namespace editor {
        ...
        namespace {
            ...
            struct ActiveCollision {
                ...
                {
                    collision =
                        boost::shared_ptr<CollisionModel>( new CollisionModel(collisionData.objectData,
                                                                              COL(1, 0,
                                                                                  0), collisionData.collisionPosition,
                                                                              object.getRotation(), storm) );
                    collision->create();
                    collision->scale();
                }
...
//        tm.model->SetRotation (getRotation(object.rotation));
                ...
                if (storm.lightManager)
                    storm.lightManager->getLighting( getTerrainPosition(object), lights, getRadius(tm,
                                                                                                   object), false, true,
                ...
                for (ModelContainer::iterator it = models.begin(); it != models.end(); ++it) {
                    ...
                    for (unsigned int i = 0; i < tm.objects.size(); ++i) {
                        ...
                        //if(collisionVolume.testCollision(objectPosition, object.rotation, collisionData, 2.f))
                        {
                            ...
                            tm.model->RayTrace(collisionData.rayOrigin,
                                               collisionData.rayDirection,
                                               collisionData.rayLength,
                                               ci,
                                               true);
            ...
            void insertTerrainObject(TerrainModel      &terrainModel,
                                     int                index,
            ...
            void updateObject(TerrainObject &object, float height)
            {
                ...
                if (object == collision.object) {
                    collision.collisionData.collisionPosition.y = storm.getHeight(
                        terrainModel.objects[object.index].position) + height;
            ...
            void nudgeObjects(const VC3   &position,
                              const VC3   &direction,
                              float        radius,
                              const char **filter,
                              int          filterAmount,
            ...
            void writeStream(filesystem::OutputStream &stream) const
            {
                ...
                for (ModelContainer::const_iterator it = models.begin(); it != models.end(); ++it) {
                    ...
                    for (unsigned int i = 0; i < tm.objects.size(); ++i) {
                        ...
                        stream
                        << (unsigned int)( object.uniqueEditorObjectHandle
                                           & ( ( (UniqueEditorObjectHandle)1 << 32 ) - 1 ) );
...
#define HAX_TERRAIN_RENAME
            ...
            void loadHaxoredFilenames()
            {
                ...
                files << "// Missing terrain objects, rename this file as rename_terrainobjects.txt to fix these"
                      << std::endl;
...
//                    appendToExistingModel = true;
                    ...
                    for (int j = 0; j < positionCount; ++j) {
                        ...
                        if (version >= 7) {
                            ...
                            //unsigned int internalHandle = time(NULL);
                            unsigned int internalHandle = _time32(NULL);
                            object->uniqueEditorObjectHandle = UniqueEditorObjectHandleManager::createNewUniqueHandle(
                                internalHandle);
                        }
...
//float radius = m->GetRadius();
                ...
                int id = exporterObjects.addTerrainObject(fileName,
                                                          type,
                                                          fallType,
                                                          objectData.height,
                                                          radius,
                                                          objectData.fireThrough,
                                                          objectData.explosionObject,
                                                          script,
                                                          projectile,
                                                          effect,
                                                          sounds,
                                                          ...
                                                          animation,
                                                          objectData.breakTexture,
                                                          objectData.physicsType,
                                                          mass,
                                                          objectData.physicsMaterial,
                                                          physicsData1,
                                                          physicsData2,
                                                          objectData.durabilityType,
                                                          objectData.metaValues);
                for (unsigned int i = 0; i < tm.objects.size(); ++i) {
                    const Object &object = tm.objects[i];
                    const VC2 &position = object.position;
                    ...
                    ui::PointLights lights;
                    lights.ambient = state.getColorMap().getColor(position) + state.getLightMap().getColor(position)
                                     + object.offset;
                    //if(storm.lightManager)
                    ...
                    exporterObjects.addObject(id,
                                              ...
                                              tm.lightmapped,
                                              inBuilding,
                                              sunDir * object.lightMultiplier,
                                              object.lightMultiplier);
                }
            }

            bool TerrainObjectsData::filterImpl(const std::string &fileName,
                                                TerrainModel      &tm,
                                                const char       **filters,
                                                int                filterAmount,
                                                bool               invert)
...
#define OBJECT_PHYSICS_TYPE_STATIC       1
        ...
        void TerrainObjects::resetTerrain()
        {
            ...
            for (TerrainObjectsData::ModelContainer::iterator it = data->models.begin(); it != data->models.end();
                 ++it) {
                TerrainModel &tm = (*it).second;
                tm.terrainId = -1;
...
//    data->storm.terrain->RegenerateTexturing();
        ...
        TerrainObject TerrainObjects::addObject(const std::string &fileName,
                                                const VC2         &position,
                                                const VC3         &rotation,
                                                float              height)
        {
        ...
        void TerrainObjects::drawCollision(bool drawState, const std::string &fileName)
        {
            ...
            for (unsigned int i = 0; i < tm.objects.size(); ++i) {
                ...
                boost::shared_ptr<CollisionModel> collision( new CollisionModel(objectData, COL(.5,
                                                                                                0,
                                                                                                0), position,
                                                                                object.rotation, data->storm) );
        ...
        TerrainObject TerrainObjects::traceActiveCollision(const VC3 &rayOrigin,
                                                           const VC3 &rayDirection,
                                                           float      rayLength)
        {
            CollisionData collisionData;
        ...
        void TerrainObjects::nudgeObjects(const VC3   &position,
                                          const VC3   &direction,
                                          float        radius,
                                          const char **filter,
                                          int          filterAmount,
        ...
        void TerrainObjects::removeObjects(const VC3   &position,
                                           float        radius,
                                           const char **filter,
                                           int          filterAmount,
                                           bool         invert)
        ...
        void TerrainObjects::copyObjects(const VC3              &position,
                                         float                   radius,
                                         GroupList::ObjectGroup &group,
                                         const char            **filter,
                                         int                     filterAmount,
        ...
        void TerrainObjects::doExport(Exporter &exporter) const
        {
            ...
            for (TerrainObjectsData::ModelContainer::const_iterator it = data->models.begin();
                 it != data->models.end();
                 ++it) {
                const TerrainModel &tm = it->second;
                if ( !tm.objects.empty() ) {
