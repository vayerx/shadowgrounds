...
//---
        ...
        struct TerrainUnitsData {
            ...
            UnitHandle testCollisions(CollisionData &collisionData)
            {
                ...
                for (; it != units.end(); ++it) {
                    ...
                    for (unsigned int i = 0; i < terrainUnit.instances.size(); ++i) {
                        ...
                        if ( collisionVolume.testCollision(position,
                                                           VC3(0, -instance.yRotation, 0), collisionData, 0.f) ) {
            ...
            void setScript(int                      id,
                           ExporterScripts::SubType subType,
                           UnitScripts::ScriptType  scriptType,
...
//---
        ...
        void TerrainUnits::rotateUnit(const UnitHandle &unitHandle, int wheelDelta)
        {
            ...
            data->activeUnit =
                boost::shared_ptr<CollisionModel>( new CollisionModel(objectData,
                                                                      COL(1, 0,
                                                                          0), position,
                                                                      VC3(0, instance.yRotation, 0), data->storm) );
        ...
        void TerrainUnits::setActiveUnit(const UnitHandle &unitHandle)
        {
            ...
            data->activeUnit =
                boost::shared_ptr<CollisionModel>( new CollisionModel(objectData,
                                                                      COL(1, 0,
                                                                          0), position,
                                                                      VC3(0, unitInstance.yRotation, 0), data->storm) );
            data->activeUnit->create();
            data->activeUnit->scale();
        }
        ...
        void TerrainUnits::doExport(Exporter &exporter) const
        {
            ...
            for (int index = 0; it != data->units.end(); ++it) {
                ...
                for (unsigned int j = 0; j < terrainUnit.instances.size(); ++j) {
                    ...
                    data->setScript(id,
                                    ExporterScripts::Spotted,
                                    UnitScripts::Spotted,
                                    scripts,
                                    instance.scripts.spottedScript);
                    data->setScript(id,
                                    ExporterScripts::Alerted,
                                    UnitScripts::Alerted,
                                    scripts,
                                    instance.scripts.alertedScript);
                    data->setScript(id, ExporterScripts::Hit, UnitScripts::Hit, scripts, instance.scripts.hitScript);
                    data->setScript(id,
                                    ExporterScripts::Hitmiss,
                                    UnitScripts::Hitmiss,
                                    scripts,
                                    instance.scripts.hitmissScript);
                    data->setScript(id,
                                    ExporterScripts::Noise,
                                    UnitScripts::Noise,
                                    scripts,
                                    instance.scripts.noiseScript);
                    data->setScript(id,
                                    ExporterScripts::Execute,
                                    UnitScripts::Execute,
                                    scripts,
                                    instance.scripts.executeScript);
                    data->setScript(id,
                                    ExporterScripts::Pointed,
                                    UnitScripts::Pointed,
                                    scripts,
                                    instance.scripts.pointedScript);
                    data->setScript(id,
                                    ExporterScripts::Special,
                                    UnitScripts::Special,
                                    scripts,
                                    instance.scripts.specialScript);
                    ...
                    if (unit.name == "C, scripting/spawnpoint")
                        ...
                        units.setSpawn( convertSceneToMap(instance.position, data->storm.heightmapResolution,
                                                          data->storm.heightmapSize) );
        ...
        filesystem::OutputStream &TerrainUnits::writeStream(filesystem::OutputStream &stream) const
        {
            ...
            for (; it != data->units.end(); ++it) {
                ...
                for (unsigned int i = 0; i < terrainUnit.instances.size(); ++i) {
                    ...
                    stream
                    << (unsigned int)( instance.uniqueEditorObjectHandle & ( ( (UniqueEditorObjectHandle)1 << 32 ) - 1 ) );
