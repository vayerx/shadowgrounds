    ...
    namespace editor {
        namespace {
            ...
            struct SharedData {
                ...
                {
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("No type set") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("No collision") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Cylinder collision") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Box collision") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Mapped collision") );

                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_FALL_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Static") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_FALL_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Tree") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_FALL_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Plant") );
                    ...
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("None") );
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("Scale HP") );
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("Always") );
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("Scriptable") );

                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("No physics") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Static mesh") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Dynamic box") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Dynamic cylinder") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Dynamic capsule") );
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Rack") );
#ifdef PROJECT_CLAW_PROTO
                    SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_ADDSTRING, 0,
                                        reinterpret_cast<LPARAM>("Car") );
#endif

                    for (int i = 0; i < physicsMass.getMassAmount(); ++i) {
                        std::string name = physicsMass.getMassName(i);
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_PHYSICS_MASS, CB_ADDSTRING, 0,
                    ...
                    for (int i = 0; i < soundMaterialParser.getMaterialAmount(); ++i) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_PHYSICS_MATERIAL, CB_ADDSTRING, 0,
                            reinterpret_cast<LPARAM>( name.c_str() ) );
                    }
                    ...
                    for (int i = 0; i < objectDurabilityParser.getDurabilityTypeAmount(); ++i) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_PHYSICS_DURABILITY, CB_ADDSTRING, 0,
                            reinterpret_cast<LPARAM>( name.c_str() ) );
                    }
                    ...
                    for (unsigned int i = 0; i < metaKeys.size(); ++i) {
                        ...
                        SendDlgItemMessage( dialog.getWindowHandle(), IDC_OBJECT_ID, CB_ADDSTRING, 0,
                                            reinterpret_cast<LPARAM>( name.c_str() ) );
                    }
                ...
                void updateModelList()
                {
                    ...
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("(none)") );
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_ADDSTRING, 0,
                        reinterpret_cast<LPARAM>("(disappear)") );
                    ...
                    explosionScripts.reload();
                    {
                        ...
                        for (ScriptList::const_iterator it = scripts.begin(); it != scripts.end(); ++it) {
                            ...
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_EXPLOSION_SCRIPT, CB_ADDSTRING, 0,
                    ...
                    }
                    {
                        ...
                        for (ProjectileList::const_iterator it = projectiles.begin(); it != projectiles.end(); ++it) {
                            const char *str = it->name.c_str();
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_EXPLOSION_PROJECTILE, CB_ADDSTRING, 0,
                                reinterpret_cast<LPARAM>(str) );
                        }
                    ...
                    }
                    {
                        ...
                        for (EffectList::const_iterator it = effects.begin(); it != effects.end(); ++it) {
                            const char *str = it->name.c_str();
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_EXPLOSION_EFFECT, CB_ADDSTRING, 0,
                                reinterpret_cast<LPARAM>(str) );
                        }
                    ...
                    }
                    {
                        ...
                        for (SoundList::const_iterator it = sounds.begin(); it != sounds.end(); ++it) {
                            const char *str = it->name.c_str();
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_EXPLOSION_SOUND, CB_ADDSTRING, 0,
                                reinterpret_cast<LPARAM>(str) );
                        }
                    ...
                    }
                    {
                        ...
                        for (MaterialList::const_iterator it = materials.begin(); it != materials.end(); ++it) {
                            const char *str = it->name.c_str();
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_MATERIAL, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(str) );
                        }
                    ...
                    }
                    {
                        ...
                        for (AnimationList::const_iterator it = animations.begin(); it != animations.end(); ++it) {
                            const char *str = it->name.c_str();
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_ANIMATION, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(str) );
                        }
                    ...
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_SUBGROUP, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>("All") );

                    for (int i = 0; i < fileWrapper.getRootDirAmount(); ++i) {
                        const std::string &dir = fileWrapper.getRootDir(i);
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_OBJECT_GROUP, CB_ADDSTRING, 0,
                        ...
                        if (i == groupIndex)
                            for (int j = 0; j < fileWrapper.getDirAmount(i); ++j) {
                                ...
                                SendDlgItemMessage(
                                     dialog.getWindowHandle(), IDC_OBJECT_SUBGROUP, CB_ADDSTRING, 0,
                                    reinterpret_cast<LPARAM>( dir.c_str() ) );
                                ...
                                if (subGroupIndex == 0 || j == subGroupIndex - 1) {
                                    for (int k = 0; k < fileWrapper.getFileAmount(i, j); ++k) {
                                        ...
                                        SendDlgItemMessage(
                                             dialog.getWindowHandle(), IDC_OBJECT_MODELS, LB_ADDSTRING, 0,
                ...
                void updateGroupList()
                {
                    ...
                    SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP, CB_ADDSTRING, 0,
                    ...
                    // Add groups
                    {
                        for (int i = 0; i < list.getGroupAmount(); ++i) {
                            ...
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_OBJECT_GROUP_GROUP, CB_ADDSTRING, 0,
                                reinterpret_cast<LPARAM>( name.c_str() ) );
                        }
                    }
                    ...
                    // Add object groups
                    {
                        for (int i = groupMin; i < groupMax; ++i) {
                            for (int j = 0; j < list.getObjectGroupAmount(groupGroupIndex, i); ++j) {
                                ...
                                SendDlgItemMessage(
                                     dialog.getWindowHandle(), IDC_OBJECT_GROUP_LIST, LB_ADDSTRING, 0,
                                    reinterpret_cast<LPARAM>( grp.name.c_str() ) );
                            }
                        }
                    }

                    SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_GROUP_GROUP, CB_SETCURSEL, groupGroupIndex, 0);
                    SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP, CB_SETCURSEL, groupSubGroupIndex, 0);
                ...
                void setProperties(const std::string &fileName)
                {
                    ...
                    if ( data.explosionObject.empty() ) {
                        ...
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_SELECTSTRING, 0,
                            reinterpret_cast<LPARAM>( file.c_str() ) );
                    }

                    {
                        ...
                        if ( !script.empty() )
                            SendDlgItemMessage( dialog.getWindowHandle(), IDC_EXPLOSION_SCRIPT, CB_SELECTSTRING, 0,
                                                (LPARAM) script.c_str() );
                        else
                        ...
                        if ( !projectile.empty() )
                            SendDlgItemMessage( dialog.getWindowHandle(), IDC_EXPLOSION_PROJECTILE, CB_SELECTSTRING, 0,
                                                (LPARAM) projectile.c_str() );
                        else
                        ...
                        if ( !effect.empty() )
                            SendDlgItemMessage( dialog.getWindowHandle(), IDC_EXPLOSION_EFFECT, CB_SELECTSTRING, 0,
                                                (LPARAM) effect.c_str() );
                        else
                        ...
                        if ( !sound.empty() )
                            SendDlgItemMessage( dialog.getWindowHandle(), IDC_EXPLOSION_SOUND, CB_SELECTSTRING, 0,
                                                (LPARAM) sound.c_str() );
                        else
                        ...
                        if ( !material.empty() )
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_MATERIAL, CB_SELECTSTRING, 0, (LPARAM) material.c_str() );
                        else
                        ...
                        if ( !animation.empty() )
                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_ANIMATION, CB_SELECTSTRING, 0, (LPARAM) animation.c_str() );
                        else
                    ...
                    SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_SETCURSEL, data.breakTexture, 0);
                    ...
                    if ( !data.physicsMaterial.empty() )
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_PHYSICS_MATERIAL, CB_SELECTSTRING, 0,
                            (LPARAM) data.physicsMaterial.c_str() );
                    else
                        SendDlgItemMessage(dialog.getWindowHandle(), IDC_PHYSICS_MATERIAL, CB_SETCURSEL, 0, 0);

                    if ( !data.durabilityType.empty() )
                        SendDlgItemMessage( dialog.getWindowHandle(), IDC_PHYSICS_DURABILITY, CB_SELECTSTRING, 0,
                                            (LPARAM) data.durabilityType.c_str() );
                    else
...

                    {
                        ...
                        if ( !metaKeys.empty() ) {
                            int selection = SendDlgItemMessage(
                                dialog.getWindowHandle(), IDC_OBJECT_ID, CB_GETCURSEL, 0, 0);
                            if (selection < 0) {
                                SendDlgItemMessage(dialog.getWindowHandle(), IDC_OBJECT_ID, CB_SETCURSEL, 0, 0);
                                selection = 0;
                            }

                            int length = SendDlgItemMessage(
                                dialog.getWindowHandle(), IDC_OBJECT_ID, CB_GETLBTEXTLEN, selection, 0);
                            if (length != CB_ERR) {
                                char *buffer = (char *)alloca(length + 1);
                                buffer[0] = 0;
                                SendDlgItemMessage(dialog.getWindowHandle(), IDC_OBJECT_ID, CB_GETLBTEXT, selection,
                                                   (LPARAM)buffer);
                ...
                void saveProperties(const std::string &fileName)
                {
                    ...
                    if ( canUpdate(IDC_OBJECT_TYPE) )
                        data.type =
                            SendDlgItemMessage(dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_GETCURSEL, 0, 0) - 1;
                    ...
                    if ( canUpdate(IDC_EXPLOSION_OBJECT) ) {
                        int explosionIndex = SendDlgItemMessage(
                            dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_GETCURSEL, 0, 0);
                        if (explosionIndex <= 0) {
                            ...
                            int length = SendDlgItemMessage(
                                dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_GETLBTEXTLEN, explosionIndex, 0);
                            std::string name;
                            name.resize(length + 1);

                            SendDlgItemMessage(
                                 dialog.getWindowHandle(), IDC_EXPLOSION_OBJECT, CB_GETLBTEXT, explosionIndex,
                                reinterpret_cast<LPARAM>( name.c_str() ) );
                            name.resize(length);
                    ...
                    if ( canUpdate(IDC_OBJECT_BREAK_TEX) )
                        data.breakTexture = SendDlgItemMessage(
                            dialog.getWindowHandle(), IDC_OBJECT_BREAK_TEX, CB_GETCURSEL, 0, 0);
                    if ( canUpdate(IDC_PHYSICS_TYPE) )
                        data.physicsType = SendDlgItemMessage(
                            dialog.getWindowHandle(), IDC_PHYSICS_TYPE, CB_GETCURSEL, 0, 0);
                ...
                {
                    int selectionCount = SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_MODELS, LB_GETSELCOUNT, 0, 0);
                    if (selectionCount == 0)
                        return;
                    ...
                    int elements = SendDlgItemMessage(
                         dialog.getWindowHandle(), IDC_OBJECT_MODELS, LB_GETSELITEMS, selectionCount,
                        reinterpret_cast<WPARAM>( selections.get() ) );
                    assert(elements);

                    for (int k = 0; k < elements; ++k) {
                        int length = SendDlgItemMessage(
                            dialog.getWindowHandle(), IDC_OBJECT_MODELS, LB_GETTEXTLEN, selections[k], 0);
                        assert(length);

                        boost::scoped_array<char> oldString(new char[length + 1]);
                        SendDlgItemMessage(
                             dialog.getWindowHandle(), IDC_OBJECT_MODELS, LB_GETTEXT, selections[k],
                            reinterpret_cast<LPARAM>( oldString.get() ) );
                ...
                void setLighting( IStorm3D_Model *model, const COL &selfIllumination = COL() )
                {
                    ...
                    lights.ambient = editorState.getColorMap().getColor(position) + editorState.getLightMap().getColor(
                        position);
                    lights.ambient += selfIllumination;

                    if (storm.lightManager)
                        storm.lightManager->getLighting(model->GetPosition(), lights, ui::getRadius(
                                                            model), false, true, model);
            ...
            public:
                ...
                void execute(int id)
                {
                    ...
                    int subGroupIndex = SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_SUBGROUP, CB_GETCURSEL, 0, 0);
            ...
            public:
                ...
                void execute(int id)
                {
                    int groupGroupIndex = SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_GROUP_GROUP, CB_GETCURSEL, 0, 0);
                    int groupSubGroupIndex = SendDlgItemMessage(
                        dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP, CB_GETCURSEL, 0, 0);
            ...
            public:
                ...
                void execute(int id)
                {
                    ...
                    if (sharedData.noUpdate) {
                        type = SendDlgItemMessage(
                            sharedData.dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_GETCURSEL, 0, 0) - 1;
                        return;
                    }

                    int newType = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_OBJECT_TYPE, CB_GETCURSEL, 0, 0) - 1;
                    if (type == newType)
                        return;
            ...
            public:
                ...
                void rebuild()
                {
                    ...
                    amount = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_OBJECT_AMOUNT, TBM_GETPOS, 0, 0);
                    ...
                    for (unsigned int i = 0; i < selection.size(); ++i) {
                        for (int j = 0; j < amount; ++j) {
                            ...
                            ObjectData &objectDataTmp = sharedData.terrainObjects.getObjectSettings().getSettings(
                                fileName);
                            std::string postfix = objectDataTmp.metaValues["filename_postfix"];
                            if ( !postfix.empty() )
                                loadFileName += postfix;

                            boost::shared_ptr<IStorm3D_Model> model = createEditorModel(*sharedData.storm.storm,
                                                                                        loadFileName);
                ...
                void modeTick()
                {
                    ...
                    if ( mouse.hasLeftClicked() ) {
                        ...
                        if (trace == activeObject) {
                            ...
                            sharedData.terrainObjects.traceActiveCollision(VC3(-10000, 100000, -100000), VC3(0,
                                                                                                             1,
                                                                                                             0),
                                                                           rayLength);
                        } else {
                    ...
                    if ( activeObject.hasObject() ) {
                        ...
                        // Move
                        {
                            VC3 pos = sharedData.storm.unitAligner.getMovedPosition(
                                 VC3(), *sharedData.storm.scene->GetCamera() );
                        ...
                        if (GetKeyState('R') & 0x80) {
                            ...
                            sharedData.terrainObjects.moveObject( activeObject,
                                                                  getDialogItemFloat(sharedData.dialog, IDC_LONKERO) );
                        }
            ...
            public:
                ...
                void modeTick()
                {
                    ...
                    if ( modelGroup.models.empty() ) {
                        ...
                        if (wheelDelta) {
                            if (GetKeyState('X') & 0x80)
                                modelGroup.rotationEul.x  = sharedData.storm.unitAligner.getRotation(
                                    modelGroup.rotationEul.x,
                                    wheelDelta);
                            else if (GetKeyState('Z') & 0x80)
                                modelGroup.rotationEul.z = sharedData.storm.unitAligner.getRotation(
                                    modelGroup.rotationEul.z,
                                    wheelDelta);
                            else
                                modelGroup.rotationEul.y = sharedData.storm.unitAligner.getRotation(
                                    modelGroup.rotationEul.y,
                                    wheelDelta);
                        }
                        ...
                        if (GetKeyState(VK_PRIOR) & 0x80)
                            ...
                            modelGroup.heightOffset = sharedData.storm.unitAligner.getHeight(modelGroup.heightOffset,
                                                                                             -1);
                ...
                void rebuild()
                {
                    ...
                    int selectedIndex = SendDlgItemMessage(
                        sharedData.dialog.getWindowHandle(), IDC_OBJECT_GROUP_LIST, LB_GETCURSEL, 0, 0);
                    ...
                    for (int i = groupMin; i < groupMax; ++i) {
                        ...
                        if (selectedIndex < objectGroupAmount) {
                            const GroupList::ObjectGroup &objectGroup = sharedData.list.getObjectGroup(groupIndex,
                                                                                                       i,
                                                                                                       selectedIndex);
                ...
                void insertObjects(const VC3 &position_clicked)
                {
                    ...
                    for (unsigned int i = 0; i < modelGroup.objectGroup.instances.size(); ++i) {
                        ...
                        TerrainObject terrainObject =
                            objects.addObject(instance.model,
                                              VC2(position.x + position_offset.x,
                                                  position.z + position_offset.z),
                                              modelGroup.getRotation(sharedData.storm,
                                                                     i),
                                              instance.position.y + position_offset.y + modelGroup.heightOffset);
                ...
                void getGroupToolFilter(const char ***filterOut, int *filterAmountOut, bool *invertOut)
                {
                    ...
                    if (dynamic) filter[filterAmount++] = "*DYNAMIC"; if (filtstatic) filter[filterAmount++] =
                            "*STATIC"; if (nocollision) filter[filterAmount++] = "*NOCOLLISION";
#ifdef LEGACY_FILES
                    if (back) filter[filterAmount++] = "back_"; if (ground) filter[filterAmount++] = "floor";
                    if (decal) filter[filterAmount++] = "Decals\\"; if (helper) filter[filterAmount++] = "Helpers\\";
                    if (prop) filter[filterAmount++] = "Props\\";
#else
                    if (back) filter[filterAmount++] = "back_"; if (ground) filter[filterAmount++] = "ground\\";
                    if (decal) filter[filterAmount++] = "decal\\"; if (helper) filter[filterAmount++] = "helper\\";
                    ...
                    if (maingroup)
                        if (sharedData.groupIndex >= 0) {
                            ...
                            if (sharedData.subGroupIndex > 0) {
                                std::string subGroupFiltName = sharedData.fileWrapper.getDir(
                                    sharedData.groupIndex,
                                    sharedData.subGroupIndex
                                    - 1);
                        ...
                        for (int i = 0; i < (int)modelList.size(); i++) {
                            if (filterAmount < maxFilters) {
                                ...
                                if (!spammedTheError) {
                                    ...
                                    MessageBox(
                                        sharedData.dialog.getParentWindowHandle(),
                                        "Too many models selected for filtering. (this message will be shown only once)",
                                        "Oopsie.", MB_OK);
                                }
                                break;
                            }
            ...
            if (md.currentMode != md.changeToMode) {
                ...
                if (md.changeToMode == 3) {
                    ...
                    ShowWindow(GetDlgItem(
                                   data->sharedData.dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP), SW_SHOW);
                    ...
                    EnableWindow(GetDlgItem(
                                     data->sharedData.dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP), FALSE);
                    ...
                    ShowWindow(GetDlgItem(
                                   data->sharedData.dialog.getWindowHandle(), IDC_OBJECT_GROUP_SUBGROUP), SW_HIDE);
