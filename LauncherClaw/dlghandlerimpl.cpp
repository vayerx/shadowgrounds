    ...
    namespace launcher {
                                           ...
                                           const std::string                &select)
        {
            ...
            for (int i = 0; i < (int)items.size(); i++) {
                std::wstring str = ::game::DHLocaleManager::getInstance()->getWideString(
                     ::game::DHLocaleManager::BANK_GUI, items[ i ].c_str() );
            ...
            if ( !select.empty() ) {
                std::wstring str = ::game::DHLocaleManager::getInstance()->getWideString(
                     ::game::DHLocaleManager::BANK_GUI, select.c_str() );
                SendMessageW( hwnd, CB_SELECTSTRING, -1, (LPARAM) str.c_str() );
            }
#else
            for (int i = 0; i < (int)items.size(); i++) {
                localizationComboBox.insert( std::pair< std::string,
                                                        std::string >(game::getLocaleGuiString( items[ i ].c_str() ),
                                                                      items[ i ]) );

                if (SendMessage( hwnd, CB_FINDSTRING, -1,
                                 (LPARAM) (LPCSTR) game::getLocaleGuiString( items[ i ].c_str() ) ) == CB_ERR)
...
#ifdef FB_RU_HAX
            std::wstring uni_text = ::game::DHLocaleManager::getInstance()->getWideString(
