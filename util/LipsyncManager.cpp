...
namespace util {
    ...
    struct ActiveCharacter {
        ...
        void render()
        {
            ...
            if (scene && renderTarget) {
                ...
                if (pictureMaterial) {
                    const Storm3D_SurfaceInfo &info = renderTarget->GetSurfaceInfo();
                    scene->Render2D_Picture( pictureMaterial.get(),
                                             VC2(0, 0), VC2( float(info.width), float(info.height) ) );
                }
    ...
    struct LipsyncManager::Data {
            ...
            numCharacters(2)
        {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                activeCharacters[i].lightIndex = terrain->addLight(activeCharacters[i].lightPosition,
                                                                   20.f,
                                                                   activeCharacters[i].lightColor);
        ...
        void init()
        {
            ...
            filesystem::InputStream lipsyncfile = filesystem::FilePackageManager::getInstance().getFile(
                "Data/Animations/lipsync.txt");
            lipsyncfile >> parser;
    ...
    void LipsyncManager::playSpeech(const std::string                            &character,
                                    const boost::shared_ptr<sfx::AmplitudeArray> &amplitudes,
                                    int                                           time)
    {
