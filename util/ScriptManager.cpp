...
// (but cannot use that, 'cos simpleparser will remove those lines)
                    if (strncmp(l, "%define ", 8) == 0) {
                        ...
                        if (l[8] == ' ') {
                            sp.error(
                                "ScriptManager::loadInternalPreprocessorMacros - Assuming single whitespace seperators, but encountered multiple whitespaces.");
                        } else {
                            int slen = strlen(l);
                            int seppos = slen;
                            ...
                            if (seppos >= slen) {
                                sp.error(
                                    "ScriptManager::loadInternalPreprocessorMacros - Invalid macro definition, parameter expected (macros expanding to empty string not allowed).");
                            } else {
                                if (seppos < 10) {
                                    sp.error(
                                        "ScriptManager::loadInternalPreprocessorMacros - Macro names less than 2 chars not accepted.");
                                } else {
...
//                        sp.error("ScriptManager::loadInternalPreprocessorMacros - Invalid non-#define line encountered.");
                    ...
                    sp.error(
                        "ScriptManager::loadInternalPreprocessorMacros - Invalid non-preprocessor line encountered.");
                }
            }
            ...
            Logger::getInstance()->warning(
                "ScriptManager::loadInternalPreprocessorMacros - Failed to open internal preprocessor macros file.");
            Logger::getInstance()->debug(filename);
        }
    ...
    void ScriptManager::loadScripts(const char *filename_, const char *relativeToFilenamePath, bool replace)
    {
        ...
        if ( game::SimpleOptions::getBool(DH_OPT_B_FORCE_SCRIPT_PREPROCESS) ) {
            Logger::getInstance()->warning(
                "ScriptManager::loadScripts - Preprocessed script is recreated (force preprocess on).");
...
#  ifndef __APPLE__
                            ...
                            if (noppfout != NULL) {
                                ...
                                if (gotwr != 1)
                                    Logger::getInstance()->warning(
                                        "ScriptManager::loadScripts - Failed to write nopp-flagged preprocessed script file.");
                                fclose(noppfout);
                            } else {
                                Logger::getInstance()->warning(
                                    "ScriptManager::loadScripts - Failed to open nopp-flagged preprocessed script file for writing.");
                            }
#  endif
                        } else {
                            Logger::getInstance()->warning(
                                "ScriptManager::loadScripts - Failed to read a file with preprocess off flag.");
                        }
                        noppbuf2[flen] = '\0';
                        delete[] noppbuf2;
        ...
        if (!noPPFile)
            if (ppWarning) {
                Logger::getInstance()->warning(
                    "ScriptManager::loadScripts - Preprocessed script is not up to date, but it will not be recreated (auto preprocess off).");
                Logger::getInstance()->debug(old_filename);
            }
...
#  endif
    ...
    bool ScriptManager::doInternalPreprocess(const char *filename,
                                             char      **bufOut,
                                             int        *datalenOut,
                                             const char *buf_unchanged,
                                             int         datalen_unchanged)
    {
    ...
    void ScriptManager::loadMemoryScripts(const char *filename,
                                          char       *buf_unchanged,
                                          int         datalen_unchanged,
                                          bool        replace)
    {
        ...
        if (!ippSuccess) {
            Logger::getInstance()->error(
                "ScriptManager::loadScripts - Internal script preprocessor failed to preprocess script.");
            Logger::getInstance()->debug(filename);
            return;
        }
        ...
        fileHash->insert( std::pair<int,
                                    std::pair<char *,
                                              unsigned long> >( hc,
                                                                std::pair<char *, unsigned long>(filename_copy,
                                                                                                 timestamp) ) );
                ...
                if (buf[lastpos] != '\0' && !atCommentLine) {
                    ...
                    if (strcmp(cmd, "import") == 0) {
                        if (scrman_importdepth < SCRIPT_MAX_IMPORT_DEPTH) {
                            ...
                            Logger::getInstance()->error(
                                "ScriptManager::loadMemoryScripts - Script import depth over maximum limit.");
                    ...
                    if (strcmp(cmd, "script") == 0) {
                        if (currentScript != NULL)
                            error("Expected endScript, script block not properly ended before next script block.",
                                  lineNumber,
                                  true);
                        ...
                        if (tmp != NULL) {
                            if (replace) {
                                ...
                                {
                                    error(
                                        "Script name hashcode calculation error, same hashcode for different script names.",
                                        lineNumber,
                                        true);
                        ...
                        if (currentScript != NULL) {
                                ...
                                && data[strlen(data) - 1] == '"')
                            {
                                ...
                                if (strcmp(cmd, "return") == 0) {
                                    if (ifDepth >= 16)
                                        error("Nesting depth too great for nested return (if depth greater than 16).",
                                              lineNumber,
                                              true);
                                    else
                                        if (ifDepth >= 2) {
                                            ...
                                            if (!retconvok) {
                                                error("Internal error while trying to convert nested return (bug).",
                                                      lineNumber,
                                                      true);
                                ...
                                if (strcmp(cmd, "if") == 0) {
                                    ...
                                    if (ifDepth >= 2)
                                        if ( (isAlreadyManuallyNested & ifNestMask) == 0 ) {
                                            ...
                                            if (!autonestok) {
                                                error(
                                                    "Internal error while trying to autonest if blocks, at _externCallPush (bug).",
                                                    lineNumber,
                                                    true);
                                ...
                                if (strcmp(cmd, "endif") == 0) {
                                    ...
                                    if (ifDepth >= 2) {
                                        if ( (isAlreadyManuallyNested & ifNestMask) == 0 ) {
                                            if (didABreakLoop) {
                                                error(
                                                    "Bad automatically nested if block because of breakLoop (need to nest manually).",
                                                    lineNumber,
                                                    true);
                                            ...
                                            if (!autonestok) {
                                                error(
                                                    "Internal error while trying to autonest if blocks, at _externCallPop (bug).",
                                                    lineNumber,
                                                    true);
                                    ...
                                    ifNestMask /= 2;
                                    if (ifDepth < 0) {
                                        error("Internal error while trying to autonest if blocks, if depth below zero.",
                                              lineNumber,
                                              true);
                                        assert(
                                            !"Internal error while trying to autonest if blocks, if depth below zero.");
                                    }
                                }
                                if (strcmp(cmd, "breakLoop") == 0) {
                                    ...
                                    if (nestedInsideLoop) {
                                        error(
                                            "Possibly an erronous use of manual _externCallPush/Pop nesting inside loop.",
                                            lineNumber,
                                            false);
                                        assert(
                                            !
                                ...
                                if (strcmp(cmd, "then") == 0) {
                                    commandCountThen++;
                                    if (ifDepth == 0)
                                        error("then encountered outside if block (if expected before then).",
                                              lineNumber,
                                              true);
                                }
                                if (strcmp(cmd, "local") == 0) {
                                    if (selectDepth != 0
                                        || commandCountLoop != 0
                                        || commandCountIf != 0)
                                        error("Local variables must be declared in the beginning of the sub.",
                                              lineNumber,
                                              true);
                                    inSub = true;
                                }
                                if (strcmp(cmd, "sub") == 0) {
                                    commandCountSub++;
                                    if (inSub)
                                        error("sub encountered while inside sub (endSub expected after sub).",
                                              lineNumber,
                                              true);
                                    inSub = true;
                                }
                                if (strcmp(cmd, "endSub") == 0) {
                                    if (!inSub)
                                        error("endSub encountered without sub (sub expected before endSub).",
                                              lineNumber,
                                              true);
                                    commandCountEndSub++;
                                    inSub = false;
                                    ...
                                    if (commandCountThen != commandCountIf || commandCountEndif != commandCountIf) {
                                        if (commandCountEndif < commandCountIf)
                                            error(
                                                "Script parse error, invalid if block encountered (missing endif after if).",
                                                lineNumber,
                                                true);
                                        else if (commandCountEndif > commandCountIf)
                                            error(
                                                "Script parse error, invalid if block encountered (missing if before endif).",
                                                lineNumber,
                                                true);
                                        else
                                            error(
                                                "Script parse error, invalid if block encountered. (missing then inside if block)",
                                                lineNumber,
                                                true);
                                    }
                                    commandCountIf = 0;
                                ...
                                if (strcmp(cmd, "endSelect") == 0) {
                                    if (selectDepth == 0)
                                        error(
                                            "endSelect encountered without select (select expected before endSelect).",
                                            lineNumber,
                                            true);
    ...
    int ScriptManager::reloadChangedScripts()
    {
        ...
        for (ScriptFileHashType::iterator it = fileHash->begin(); it != fileHash->end(); ++it) {
            scriptFiles.push_back( std::pair<std::string, unsigned long>(std::string(it->second.first),
                                                                         it->second.second) );
        }
        ...
        for (int i = 0; i < (int)scriptFiles.size(); i++) {
                ...
                && dhsFilename.substr(dhsFilename.length() - 5, 5) == ".dhps")
            {
                ...
                if ( (unsigned long)FileTimestampChecker::getFileTimestamp( dhsFilename.c_str() ) >
                     scriptFiles[i].second ) {
