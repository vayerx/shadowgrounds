...
namespace util {
    ...
    ObjectDurabilityParser::ObjectDurabilityParser()
    {
        ...
        EditorParser parser;
        filesystem::InputStream durabilitiesfile = filesystem::createInputFileStream(
            "data/misc/object_durabilities.txt");
        ...
        for (int i = 0; i < groups; ++i) {
            ...
            dur.requiredAcceleration = convertFromString<float>(accelString,
                                                                OBJECTDURABILITYPARSER_DEFAULT_REQUIRED_ACCELERATION);

            const std::string &angAccelString = group.getValue("required_angular_acceleration");
            dur.requiredAngularAcceleration = convertFromString<float>(
                angAccelString,
                OBJECTDURABILITYPARSER_DEFAULT_REQUIRED_ANGULAR_ACCELERATION);
    ...
    int ObjectDurabilityParser::getDurabilityTypeIndexByName(const std::string &name) const
    {
        ...
        Logger::getInstance()->warning(
            "ObjectDurabilityParser::getDurabilityTypeIndexByName - Given durability type name name not found.");
