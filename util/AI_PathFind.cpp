                ...
                inline void slideUp(int index, int mapSizeX)
                {
                    ...
                    while (currentIndex * 2 < arrayPosition) {
                        ...
                        if (nodes[array[childIndex]] < nodes[array[currentIndex]]) {
                            int hashA = nodes[array[childIndex  ]].yIndex * mapSizeX
                                        + nodes[array[childIndex  ]].xIndex;
                            int hashB = nodes[array[currentIndex]].yIndex * mapSizeX
                                        + nodes[array[currentIndex]].xIndex;
                ...
                inline void slideDown(int index, int mapSizeX)
                {
                    ...
                    while (currentIndex > 1) {
                        ...
                        if (nodes[array[currentIndex]] < nodes[array[currentIndex / 2]]) {
                            int hashA = nodes[array[currentIndex  ]].yIndex * mapSizeX
                                        + nodes[array[currentIndex  ]].xIndex;
                            int hashB =
                                nodes[array[currentIndex
                                            / 2]].yIndex * mapSizeX + nodes[array[currentIndex / 2]].xIndex;
...
// class PathFind
        ...
        bool PathFind::findRoute(Path      *resultPath,
                                 int        xStart,
                                 int        yStart,
                                 int        xEnd,
                                 int        yEnd,
                                 ...
                                 float      climbPenalty,
                                 const VC3 &startWorldCoords,
                                 const VC3 &endWorldCoords) const
        {
            assert(resultPath);
...
//Logger::getInstance()->debug("TEMP - patfind doors code, restore me!!!");
...

            {
                ...
                if (startId >= 0) {
                    ...
                    if (endId >= 0) {
                        costEstimate += estimateCost(xStartPortalPosition,
                                                     yStartPortalPosition,
                                                     xEndPortalPosition,
                                                     yEndPortalPosition);
            ...
            if ( (startId >= 0) && (endId >= 0) ) {
                ...
                result = findActualRoute(resultPath,
                                         xEndPortalPosition,
                                         yEndPortalPosition,
                                         xEnd,
                                         yEnd,
...
// Returns model (from pathblocks) at given point. 0 if not found
        ...
        bool PathFind::findActualRoute(Path            *resultPath,
                                       int              xStart,
                                       int              yStart,
                                       int              xEnd,
                                       int              yEnd,
                                       int              maxHeightDifference,
                                       float            climbPenalty,
                                       const Pathblock *avoidBlock) const
        {
            //PSDHAX
            //return false;
...
// Clamp to even positions
            ...
            if (xStart < 0 || xStart >= xSize || yStart < 0 || yStart >= ySize || xEnd < 0 || xEnd >= xSize || yEnd <
                0 || yEnd >= ySize) {
            ...
            while (openList.empty() == false) {
                ...
                for (int j = nodes[currentNode].yIndex - 2; j <= nodes[currentNode].yIndex + 2; j += 2) {
                    for (int i = nodes[currentNode].xIndex - 2; i <= nodes[currentNode].xIndex + 2; i += 2) {
                        ...
                        if (isMovable(nodes[currentNode].xIndex, nodes[currentNode].yIndex, i, j,
                                      maxHeightDifference) == false)
                            continue;
                        ...
                        nodes[loopNode].realCost += realCost(nodes[currentNode].xIndex,
                                                             nodes[currentNode].yIndex,
                                                             i,
                                                             j,
                                                             climbPenalty);
