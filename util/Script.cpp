...
#define ARRAY_INDEX_VALUE_NONE -7777777
        ...
        struct ScriptResourceReleaser {
            ...
            ~ScriptResourceReleaser()
            {
                if (globalVariableHash != NULL) {
                    for (VariableHashType::iterator it = globalVariableHash->begin();
                         it != globalVariableHash->end();
                         ++it) {
                    ...
                    globalVariableHash->clear();
                    delete globalVariableHash;
                    globalVariableHash = NULL;
                }
    ...
    void Script::optimizeArrayVariableData(ScriptProcess *sp,
                                           int            ip,
                                           const char    *s,
                                           int            arrIdxStart,
                                           int            arrIdxEnd,
                                           bool           arrIsValueIndex,
                                           bool           arrIsRegIndex,
                                           int           *varHashCode)
    {
...
#  endif
            ...
            if (commandArray[ip] < keywordsAmount) {
                    ...
                    if (multi) {
                        ...
                        if (depth == 0) {
                            if (stringDataArray[ip] != NULL) {
                                ...
                                sp->error(
                                    "Script::run - Internal error, _externalCallReturnMultiple parameter missing.");
                                sp->finished = true;
                            }
                        }
                    ...
                    if (multi)
                        for (int i = 1; i < multipop; i++) {
                            if ( sp->ipStack->isEmpty() ) {
                                sp->error(
                                    "Script::run - Call stack empty, internal error while executing _externCallReturnMultiple.");
                                sp->finished = true;
                            } else {
                                ...
                                if (valueCheck != -1) {
                                    sp->error(
                                        "Script::run - Wrong type of stack data, internal error while executing _externCallReturnMultiple.");
                                    sp->finished = true;
                                }
                            }
                        }
                ...
                case SCRIPT_CMD_PUSHVALUE:
                    ...
                    if (sp->userStackSize == 50) {
                        ...
                        if (!stackSizeWarningLogged) {
                            ...
                            sp->warning(
                                "Script::run - User stack size seems high, possibly indicating an error (further warnings of this type omitted).");
                        }
                    }
                    if (sp->userStackSize > USER_STACK_SIZE_LIMIT) {
                        sp->error(
                            "Script::run - User stack size limit reached (assuming the script is buggy and terminating it for fail safety.)");
                        sp->finished = true;
                    }
                    break;
                        ...
                        if ( sp->ipStack->isEmpty() ) {
                            ...
                            if (commandArray[ip] == SCRIPT_CMD_RETURNMULTIPLE) {
                                ...
                                for (int i = 1; i < multipop; i++) {
                                    if ( sp->ipStack->isEmpty() ) {
                                        sp->error(
                                            "Script::run - Call stack empty, internal error while executing returnMultiple.");
                                        sp->finished = true;
                                    } else {
                                        // WARNING: void * -> int casts
                                        if ( (intptr_t)sp->ipStack->popLast() )
                                        ...
                                        if (valueCheck != -1) {
                                            sp->error(
                                                "Script::run - Wrong type of stack data, internal error while executing returnMultiple.");
                                            sp->finished = true;
                                        }
                                        //sp->leaveLocalScope();
                                    }
                            ...
                            if ( sp->ipStack->isEmpty() ) {
                                ...
                                if (ip == -1) {
                                    sp->error(
                                        "Script::run - Wrong type of stack data, internal error while executing return from sub.");
                                    sp->finished = true;
                                }
                                sp->leaveLocalScope();
                            }
                    ...
                    while (ip < commandAmount) {
                        if (commandArray[ip] == SCRIPT_CMD_SELECT) {
                            ...
                            if (selectDepth > 0) {
                                if (selectDepth == 1)
                                    sp->warning(
                                        "Script::run - Select block end was reached, matched none of the cases and no default.");
                            } else {
                                sp->error("Script::run - Internal error while processing select block.");
                                sp->finished = true;
                                break;
                ...
                case SCRIPT_CMD_EXTERNCALLPOP:
                    if (sp->ifDepth != 0) {
                        ...
                        if ( sp->ipStack->isEmpty() ) {
                            ...
                            if (valueCheck != -1) {
                                sp->error(
                                    "Script::run - Wrong type of stack data, internal error while executing _externCallPop.");
                                sp->finished = true;
                            }
                            // don't do this here. _externCallPush/Pop should not affect scope
                            //sp->leaveLocalScope();
                        ...
                        if (intDataArray[ip].i & SCRIPT_LOCAL_VAR_BITMASK) {
                            ...
                            if ( localVarPos >= 0 && localVarPos < sp->getLocalVarStackSize() ) {
                                if (commandArray[ip] == SCRIPT_CMD_SETVARIABLE)
                                    ...
                                    sp->setLocalVarStackEntryValue(localVarPos,
                                                                   sp->getLocalVarStackEntryValue(
                                                                       localVarPos) + sp->lastValue);
                                    ...
                                    sp->lastValue =
                                        ( ( sp->lastValue == sp->getLocalVarStackEntryValue(localVarPos) ) ? 1 : 0 );
                                else
                                    sp->lastValue = sp->getLocalVarStackEntryValue(localVarPos);
                            } else {
                                sp->error(
                                    "Script::run - setVariable or getVariable or variant, local variable stack size smaller than it should be (internal error).");
                            }
                        } else {
                            if (commandArray[ip] == SCRIPT_CMD_SETVARIABLE)
                        ...
                        if (stringDataArray[ip] != NULL) {
                            ...
                            if (varOptimizeDataArray[ip] != NULL) {
                                ...
                                for (int chi = 1; chi < slen; chi++) {
                                    if (s[chi] == '[') {
                                        if (arrIdxStart != 0)
                                            sp->error(
                                                "Script::run - setVariable or getVariable or variant, variable name / array reference syntax error.");
                                        isArr = true;
                                        arrIdxStart = chi;
                                ...
                                if (slen >= 256 && isArr)
                                    sp->error(
                                        "Script::run - setVariable or getVariable or variant, variable name too long.");

                                if (isArr && arrIdxStart != 0 && arrIdxEnd != 0
                                    && slen < 256)
                                {
                                    this->optimizeArrayVariableData(sp,
                                                                    ip,
                                                                    s,
                                                                    arrIdxStart,
                                                                    arrIdxEnd,
                            ...
                            if ( iter != globalVariableHash->end() ) {
                                if ( (*iter).second.dataType == SCRIPT_DATATYPE_INT ) {
                                    ...
                                    {
                                        sp->error(
                                            "Script::run - setVariable or getVariable or variant, attempt to use non-array (int) variable with array index.");
                                    } else {
                                        if (commandArray[ip] == SCRIPT_CMD_SETVARIABLE)
                                    ...
                                    if (varOptimizeDataArray[ip] != NULL) {
                                        ...
                                        if (arrayIndexValue == ARRAY_INDEX_VALUE_NONE) {
                                            ...
                                            if (arrayIndexVariableHC != 0) {
                                                if (varOptimizeDataArray[ip]->arrayIndexVariableLocalIP != 0) {
                                                    if ( (varOptimizeDataArray[ip]->arrayIndexVariableLocalIP
                                                          & SCRIPT_LOCAL_VAR_BITMASK) != 0 ) {
                                                        int negator =
                                                            (varOptimizeDataArray[ip]->arrayIndexVariableLocalIP
                                                             ^ SCRIPT_LOCAL_VAR_BITMASK);
                                                        int localVarPos = sp->getLocalVarStackSize() - negator;
                                                        if ( localVarPos >= 0 && localVarPos <
                                                             sp->getLocalVarStackSize() ) {
                                                            arrayIndexValue = sp->getLocalVarStackEntryValue(
                                                                localVarPos);
                                                        } else {
                                                            int tmp = sp->ip;
                                                            sp->ip = ip;
                                                            sp->error(
                                                                "Script::run - setVariable or getVariable or variant, internal error (reference to invalid local array index variable).");
                                                            sp->ip = tmp;
                                                        }
                                                    } else {
                                                        arrayIndexValue =
                                                            intDataArray[varOptimizeDataArray[ip]->
                                                                         arrayIndexVariableLocalIP
                                                            ].i;
                                                    }
                                                } else {
                                                    VariableHashType::iterator iter2 = globalVariableHash->find(
                                                        arrayIndexVariableHC);
                                                    if ( iter2 != globalVariableHash->end() ) {
                                                        if ( (*iter2).second.dataType == SCRIPT_DATATYPE_INT ) {
                                                            ...
                                                            sp->ip = ip;
                                                            sp->error(
                                                                "Script::run - setVariable or getVariable or variant, array index variable is not integer type.");
                                                            sp->ip = tmp;
                                                        ...
                                                        sp->ip = ip;
                                                        sp->error(
                                                            "Script::run - setVariable or getVariable or variant, array index variable not found.");
                                                        sp->ip = tmp;
                                                        Logger::getInstance()->debug(stringDataArray[ip]);
                                                    }
                                                }
                                        ...
                                        if (arrayIndexValue == ARRAY_INDEX_VALUE_NONE) {
                                            ...
                                            sp->ip = ip;
                                            sp->error(
                                                "Script::run - setVariable or getVariable or variant, attempt to use array variable without array index.");
                                            sp->ip = tmp;
                                        } else {
                                            if (arrayIndexValue >= 0 && arrayIndexValue < (*iter).second.arraySize) {
                                                if (commandArray[ip] == SCRIPT_CMD_SETVARIABLE)
                                                    ...
                                                    sp->lastValue =
                                                        ( (sp->lastValue ==
                                                           (*iter).second.arrayValue[arrayIndexValue]) ? 1 : 0 );
                                                else
                                                    sp->lastValue = (*iter).second.arrayValue[arrayIndexValue];
                                            } else {
                                                int tmp = sp->ip;
                                                sp->ip = ip;
                                                sp->error(
                                                    "Script::run - setVariable or getVariable or variant, array index out of bounds.");
                                                sp->ip = tmp;
                                                Logger::getInstance()->debug(stringDataArray[ip]);
                                    ...
                                    sp->error(
                                        "Script::run - setVariable or getVariable or variant, attempt to manipulate variable declared non-int, use appropriate specialized command instead (i.e. setStringVariable).");
                                }
                ...
                case SCRIPT_CMD_SETARRAYVARIABLEVALUES:
                    if (intDataArray[ip].i != 0) {
                        //sp->lastValue = 0;
                        sp->error(
                            "Script::run - setArrayVariableValues, shared local variables not supported by command.");
                    } else {
                        if (stringDataArray[ip] != NULL) {
                            int hc = 0;
                            ...
                            if (varOptimizeDataArray[ip] != NULL) {
                                ...
                                for (int chi = 1; chi < slen; chi++) {
                                    ...
                                    if (s[chi] == '[') {
                                        if (arrIdxStart != 0)
                                            sp->error(
                                                "Script::run - setArrayVariableValues, variable name / array reference syntax error.");
                                        isArr = true;
                                        arrIdxStart = chi;
                                        if (s[chi + 1] >= '0' && s[chi + 1] <= '9') {
                                            arrIsValueIndex = true;
                            ...
                            if ( iter != globalVariableHash->end() ) {
                                if ( (*iter).second.dataType == SCRIPT_DATATYPE_ARRAY ) {
                                    if (varOptimizeDataArray[ip] != NULL) {
                                                ...
                                                if (varOptimizeDataArray[ip]->arrayIndexVariableLocalIP != 0) {
                                                    ...
                                                    if ( iter2 != globalVariableHash->end() ) {
                                                        if ( (*iter2).second.dataType == SCRIPT_DATATYPE_INT )
                                                            ...
                                                            sp->error(
                                                                "Script::run - setArrayVariableValues, array index variable is not integer type.");
                                                    } else {
                                                        sp->error(
                                                            "Script::run - setArrayVariableValues, array index variable not found.");
                                                    }
                                                }
                                            } else {
                                                if (varOptimizeDataArray[ip]->arrayIndexRegister != 0)
                                        ...
                                        if (elementListSize >= 0) {
                                            if (nonIndexed && arrayIndexValue + elementListSize < arrsize)
                                                sp->warning(
                                                    "Script::run - setArrayVariableValues, array is longer than amount of given elements, values for the rest of array is not set (if intended, use [0] array index to get rid of this message).");

                                            if (arrayIndexValue >= 0 && arrayIndexValue + elementListSize <= arrsize) {
                                                for (int eidx = 0; eidx < elementListSize; eidx++) {
                                                    (*iter).second.arrayValue[arrayIndexValue
                                                                              + eidx] = elementListValues[eidx];
                                                }
                                            } else {
                                                sp->error(
                                                    "Script::run - setArrayVariableValues, array range (start index / start index + element list size) out of bounds.");
                                                Logger::getInstance()->debug(stringDataArray[ip]);
                                                Logger::getInstance()->debug( int2str(arrayIndexValue) );
                                            }
                                        } else {
                                    ...
                                    sp->error(
                                        "Script::run - setArrayVariableValues, attempt to manipulate non-array variable.");
                                }
                ...
                case SCRIPT_CMD_POPCALLSTACK_INT:
                    if ( !sp->isCallParamStackEmpty() ) {
                        ...
                        sp->error(
                            "Script::process - popCallStack_int, call parameter stack empty (more parameters expected or internal error).");
                        sp->finished = true;
                    }
                    break;
                ...
                case SCRIPT_CMD_POPCALLSTACK_MARKER:
                    if ( !sp->isCallParamStackEmpty() ) {
                        ...
                        if (tmp != 0xF00BABE1) {
                            sp->error(
                                "Script::process - popCallStack_marker, bad magic number in call parameter stack (internal error).");
                            sp->finished = true;
                        }
                    } else {
                ...
                case SCRIPT_CMD_CASES:
                    ...
                    sp->error(
                        "Script::run - Instruction pointer reached invalid cases command (cases parameter was bad?).");
                    sp->finished = true;
                    break;
                 ...
                 || failCount > SCRIPT_FORCED_PAUSE_LIMIT * 50 )
            {
                ...
                if (sp->misbehaveCounter > 100 || noForcedPause) {
                    ...
                    Logger::getInstance()->debug(
                        "Script::run - Script process did not pause within step limit, forcing pause.");
                }
            }
        }
        sp->ip = ip;
        ...
        for (int keyw = 0; keyw < keywordsAmount; keyw++) {
            if (strcmp(cmd, keywords[keyw]) == 0) {
                ...
                if (keyw == SCRIPT_CMD_LABEL) {
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError("Script::addCommand - Label name missing.",
                                                                         true);
                            ...
                            || strncmp(data, "_endloop_", 9) == 0)
                        {
                            ...
                            if (ip != -1) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Label already defined within script.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                    ...
                    if (!dont_convert_paramgetvariable) {
                            ...
                            && strncmp(data, "$_param_", 8) == 0)
                        {
                                ...
                                && paramNum >= 0)
                            {
                                if ( paramNum < (int)script_paramVars.size() ) {
                                    ...
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - More parameters expected.",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                        ...
                        if (!isOk) {
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - _paramGetVariable parameter invalid (internal error).",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                           ...
                           || keyw == SCRIPT_CMD_VARIABLEEQUALSVALUE)
                {
                    if (data == NULL) {
                        ...
                        if (keyw == SCRIPT_CMD_SETVARIABLE)
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - setVariable parameter missing.",
                                true);
                        else if (keyw == SCRIPT_CMD_GETVARIABLE)
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - getVariable parameter missing.",
                                true);
                        else
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - Variable add/decrease command parameter missing.",
                                true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        ...
                        if (ip != -1) {
                            ...
                            if ( iter != globalVariableHash->end() ) {
                                ...
                                if (keyw == SCRIPT_CMD_SETVARIABLE)
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - setVariable, no variable defined with given name.",
                                        true);
                                else if (keyw == SCRIPT_CMD_GETVARIABLE)
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - getVariable, no variable defined with given name.",
                                        true);
                                else
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Variable add/decrease command, no variable defined with given name.",
                                        true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                    ...
                    if (data == NULL) {
                        intDataArray[commandAmount].i = 0;
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - setArrayVariableValues parameter missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        ...
                        if (ip != -1) {
                            ...
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - setArrayVariableValues not supported for shared local variables.",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        } else {
                            ...
                            if ( iter != globalVariableHash->end() ) {
                                if ( (*iter).second.dataType == SCRIPT_DATATYPE_ARRAY ) {
                                    ...
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - setArrayVariableValues, given variable is not of array type.",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                                ...
                                // variable was not properly defined...
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - setArrayVariableValues, no variable defined with given name.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                    ...
                    if ( loopNumBuf->isEmpty() ) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Missing loop before endLoop.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                    ...
                    if ( loopNumBuf->isEmpty() ) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Missing loop before breakLoop.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                    ...
                    if ( loopNumBuf->isEmpty() ) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Missing loop before restartLoop.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError("Script::addCommand - Case number missing.",
                                                                         true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        ...
                        if (str2int_errno() != 0) {
                            ScriptManager::getInstance()->scriptCompileError("Script::addCommand - Number expected.",
                                                                             true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Up To Case number missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Cases number range missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        ...
                        if (!numRangeOk) {
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - Number range expected.",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        } else {
                            ...
                            if (from > upto) {
                                ...
                                from = tmp;
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Number range should be from lower to upper value.",
                                    false);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Goto label name missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                ...
                {
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError("Script::addCommand - Call sub name missing.",
                                                                         true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        ...
                        if ( !dont_process_call_subs && (withParams || hasEntrySub) ) {
                            if ( !this->hasSub(subname) ) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Subs with parameters must be declared before calling.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            } else {
                                if (withParams && !hasEntrySub) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Sub does not take in any parameters (but call has parameters).",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                            ...
                            if (hasEntrySub) {
                                    ...
                                    && dataSubNameCutPosEnd != -1)
                                {
                                    ...
                                    ptmp =
                                        ptmp.substr( dataSubNameCutPos + 1, dataSubNameCutPosEnd
                                                     - (dataSubNameCutPos + 1) );

                                    bool parseOk = parse_script_params(ptmp, script_paramVars);
                                    if (!parseOk) {
                                        ScriptManager::getInstance()->scriptCompileError(
                                            "Script::addCommand - Call parameter parse error.",
                                            true);
                                        Logger::getInstance()->debug(cmd);
                                        Logger::getInstance()->debug(data);
                                    }
                                ...
                                for (int i = 0; i < (int)script_paramVars.size(); i++) {
                                    if (script_paramVars[i] != "_param_used") {
                                        ScriptManager::getInstance()->scriptCompileError(
                                            "Script::addCommand - Too many call parameters.",
                                            true);
                                        Logger::getInstance()->debug(cmd);
                                        Logger::getInstance()->debug(data);
                                        break;
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - subExists parameter sub name missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    }
                           ...
                           || keyw == SCRIPT_CMD_POPCALLSTACK_INT)
                {
                    if (data != NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - _pushCallStack_int/_popCallStack_int, no parameter expected.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    }
                           ...
                           || keyw == SCRIPT_CMD_POPCALLSTACK_MARKER)
                {
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - _pushCallStack_marker/_popCallStack_marker expected integer parameter.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                    ...
                    if (data == NULL) {
                        ...
                        for (int j = 0; j < datalen; j++) {
                            ...
                            if (data[j] == ',') {
                                if (containsParenthesis == -1) {
                                    if (containsComma == -1) {
                                        ...
                                        ScriptManager::getInstance()->scriptCompileError(
                                            "Script::addCommand - Sub parameter format bad (expected type,subname(parameters...).",
                                            true);
                                        Logger::getInstance()->debug(cmd);
                                        Logger::getInstance()->debug(data);
                                    }
                            ...
                            if (data[j] == '(' && data[j + 1] != ')') {
                                if (containsParenthesis != -1) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Sub parameter format bad (multiple starting parenthesis).",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                                ...
                                && j > 0 && data[j - 1] != '(')
                            {
                                if (containsParenthesis == -1) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Sub parameter format bad (starting parenthesis missing before ending parenthesis).",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                                if (containsParenthesisEnd != -1) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Sub parameter format bad (multiple ending parenthesis).",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                            ...
                            if (parseParamFromHere) {
                                            ...
                                            || tmpbuf[k] == '\0')
                                        {
                                            ...
                                            if (k < tmpsize) {
                                                     ...
                                                     && !tmp_name2.empty() )
                                                {
                                                    if (tmp_type2 != "int") {
                                                        ScriptManager::getInstance()->scriptCompileError(
                                                            "Script::addCommand - Sub parameter type unsupported (only int type supported).",
                                                            true);
                                                        Logger::getInstance()->debug(cmd);
                                                        Logger::getInstance()->debug(data);
                                                    }
                                                    params.push_back( std::pair<std::string, std::string>(tmp_type2,
                                                                                                          tmp_name2) );
                                                    wasOk = true;
                                                }
                                            }
                                ...
                                if (!wasOk) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - Sub parameter list missing type definition or name.",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                        ...
                        if (containsParenthesis != -1)
                            if (containsParenthesisEnd == -1) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Sub parameter format bad (missing ending parenthesis).",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            || containsParenthesis != -1)
                        {
                            ...
                            if (containsComma == 0) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Sub parameter format bad (return datatype missing).",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                        ...
                        if (ip != -1) {
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - Sub already defined within script.",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                        ...
                        if (createEnterAndLeaveSubs) {
                            ...
                            if (strcmp(rettypename, "int") == 0) {
                                ...
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - Unsupported sub return value type (only void or int supported).",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - local variable name missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        if (strncmp(data, "int,", 4) == 0) {
                            ...
                            if ( globalIter != globalVariableHash->end() ) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - local variable shadows global variable with same name.",
                                    false);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            if ( shaiter != variableIPHash->end() ) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - local variable shadows sharedLocal variable with same name.",
                                    false);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            while (localSeek > 0) {
                                if (commandArray[localSeek] == SCRIPT_CMD_LOCAL) {
                                    ...
                                    if (hc == intDataArray[localSeek].i) {
                                        ScriptManager::getInstance()->scriptCompileError(
                                            "Script::addCommand - duplicate local variable name.",
                                            true);
                                        Logger::getInstance()->debug(cmd);
                                        Logger::getInstance()->debug(data);
                                        break;
                            ...
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - local variable type bad (only int type supported).",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - sharedLocal variable name missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        if (strncmp(data, "int,", 4) == 0) {
                            ...
                            if ( globalIter != globalVariableHash->end() ) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - sharedLocal variable shadows global variable with same name.",
                                    false);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - sharedLocal variable type bad (only int type supported).",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                           ...
                           || keyw == SCRIPT_CMD_PERMANENTGLOBAL)
                {
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - global or permanent global variable name missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                                ...
                                if ( tmp[tmpi] == ']'
                                         ...
                                         (tmp[tmpi + 1] == ']' && tmp[tmpi + 2] == ',') )
                                    {
                                        ...
                                        if (arraySize > 0) {
                                            ...
                                            ScriptManager::getInstance()->scriptCompileError(
                                                "Script::addCommand - array variable size must be a positive integer",
                                                true);
                                        }
                                        break;
                                    }
                        ...
                        if (createDatatype != SCRIPT_DATATYPE_NONE) {
                            ...
                            if (didExist && existIsPermanent != createPermanent) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - global or permanent global variable already declared with different persistance.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            if (didExist && existType != createDatatype) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - global or permanent global variable already declared with different type.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            if (!didExist) {
                                ...
                                if (strcmp( (*iter).second.name, varname ) != 0) {
                                    ScriptManager::getInstance()->scriptCompileError(
                                        "Script::addCommand - global variable hash failure.",
                                        true);
                                    Logger::getInstance()->debug(cmd);
                                    Logger::getInstance()->debug(data);
                                }
                            ...
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - global or permanent global variable type bad.",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                    ...
                    if (data == NULL) {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - externInclude parameter missing.",
                            true);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    } else {
                        if (strlen(data) < 256) {
                                ...
                                if (splitbuf[splitp] == ':') {
                                    ...
                                    if (s == this)
                                        ...
                                        if (script_currently_parsing_sub == p2) {
                                            ...
                                            Logger::getInstance()->error(
                                                "Script::addCommand - externInclude attempting to include itself (not allowed as it would result in infinite recursion).");
                                            Logger::getInstance()->debug(cmd);
                                            Logger::getInstance()->debug(data);
                                            selfInc = true;
                                            s = NULL;
                                    ...
                                    if (s != NULL) {
                                        if ( s->hasSub(p2) ) {
                                            ...
                                            if (includedSubIsEmpty) {
                                                ...
                                                for (; subip < s->commandAmount; subip++) {
                                                    ...
                                                    if (s->commandArray[subip] < keywordsAmount) {
                                                        ...
                                                        } else if (s->commandArray[subip] ==
                                                                   SCRIPT_CMD_RETURNMULTIPLE) {
                                                            //char numbuf[16];
                                                            //strcpy(numbuf, int2str(s->intDataArray[subip].i))
                                                            //addCommand("_externCallReturnMultiple", numbuf);
                                                            assert(!dont_add_externcallpushpop);
                                                            addCommand("_externCallReturnMultiple",
                                                                       s->stringDataArray[subip]);
                                                        } else {
                                                            addCommand(keywords[s->commandArray[subip]],
                                                                       s->stringDataArray[subip]);
                                                        }
                                                    } else {
                                                        ...
                                                        if (s->processorDatatypes[pkey] == SCRIPT_DATATYPE_INT) {
                                                        ...
                                                        } else if (s->processorDatatypes[pkey] ==
                                                            ...
                                                            addCommand(s->processorKeywords[pkey],
                                                                       s->stringDataArray[subip]);
                                                        }
                                                    }
                                                }
                                            ...
                                            ScriptManager::getInstance()->scriptCompileError(
                                                "Script::addCommand - externInclude failed, given script has no given sub.",
                                                true);
                                            Logger::getInstance()->debug(cmd);
                                            Logger::getInstance()->debug(data);
                                        }
                                        ...
                                        if (!selfInc) {
                                            ScriptManager::getInstance()->scriptCompileError(
                                                "Script::addCommand - externInclude failed, given script not loaded.",
                                                true);
                                            Logger::getInstance()->debug(cmd);
                                            Logger::getInstance()->debug(data);
                                        }
                            ...
                            if (!paramOk) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - externCall parameter format bad.",
                                    true);
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                            ...
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::addCommand - externCall parameter too long.",
                                true);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
        ...
        for (int i = 0; i < processorKeywordsAmount; i++) {
            if (strcmp(cmd, processorKeywords[i]) == 0) {
                ...
                if (processorDatatypes[i] == SCRIPT_DATATYPE_NONE) {
                    ...
                    {
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::addCommand - Parameter data ignored as no data expected.",
                            false);
                        Logger::getInstance()->debug(cmd);
                        Logger::getInstance()->debug(data);
                    }
                ...
                if (processorDatatypes[i] == SCRIPT_DATATYPE_FLOAT) {
                    ...
                    if (data != NULL) {
                        ...
                        if ( (data[0] < '0' || data[0] > '9') && data[0] != '-' ) {
                            ScriptManager::getInstance()->scriptCompileError("Script::addCommand - Float expected.",
                                                                             false);
                            Logger::getInstance()->debug(cmd);
                            Logger::getInstance()->debug(data);
                        }
                ...
                if (processorDatatypes[i] == SCRIPT_DATATYPE_STRING) {
                    ...
                    if (data != NULL) {
                        ...
                        if (slen > 0 && script_doublequotes_warning) {
                            ...
                            if (strchkpos == slen) {
                                ScriptManager::getInstance()->scriptCompileError(
                                    "Script::addCommand - String expected, but parameter seems like integer, possibly an error.",
                                    false);
                                Logger::getInstance()->debug(
                                    "Either use doublequotes to avoid this message, or use float number notation (example: 1.0f) instead of integer number notation (example: 1).");
                                Logger::getInstance()->debug(cmd);
                                Logger::getInstance()->debug(data);
                            }
                        }
    ...
    void Script::optimizeJumps()
    {
        ...
        for (int i = 0; i < commandAmount; i++) {
            if (commandArray[i] == SCRIPT_CMD_SUBEXISTS) {
                ...
                Logger::getInstance()->debug(
                    "Script::optimizeJumps - subExists encountered, assuming next call to be conditional.");
            }
            if (commandArray[i] == SCRIPT_CMD_GOTO) {
                if (stringDataArray[i] == NULL) {
                    scrman_currentline = 0;
                    ScriptManager::getInstance()->scriptCompileError(
                        "Script::optimizeJumps - Goto command has no target label name.",
                        false);
                } else {
                    ...
                    if (badip != -1) {
                        scrman_currentline = 0;
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::optimizeJumps - Goto command target name specifies a sub.",
                            false);
                    ...
                    if (ip == -1) {
                        scrman_currentline = 0;
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::optimizeJumps - Requested label was not found.",
                            false);
                ...
                || commandArray[i] == SCRIPT_CMD_CALLIFEXISTS)
            {
                ...
                if (stringDataArray[i] == NULL) {
                    scrman_currentline = 0;
                    ScriptManager::getInstance()->scriptCompileError(
                        "Script::optimizeJumps - Call command has no target sub name.",
                        false);
                } else {
                    ...
                    if (badip != -1) {
                        scrman_currentline = 0;
                        ScriptManager::getInstance()->scriptCompileError(
                            "Script::optimizeJumps - Call command target name specifies a label.",
                            false);
                        Logger::getInstance()->debug(stringDataArray[i]);
                    }
                    ...
                    if (ip == -1) {
                        if (conditionalCall) {
                            Logger::getInstance()->debug(
                                "Script::optimizeJumps - Requested sub was not found, but this is probably a conditional call.");
                        } else {
                            scrman_currentline = 0;
                            ScriptManager::getInstance()->scriptCompileError(
                                "Script::optimizeJumps - Requested sub was not found.",
                                false);
                            ScriptManager::getInstance()->scriptCompileError(stringDataArray[i], false);
                        }
                        Logger::getInstance()->debug(stringDataArray[i]);
    ...
    char *Script::getDebugDump(ScriptProcess *sp, bool fullDump)
    {
        ...
        if (fullDump) {
            ...
            sprintf(&buf[strlen(buf)], "Process if-depth: " FMT_INTPTR "\r\n", sp->ifDepth);
        }
