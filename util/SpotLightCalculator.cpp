...
namespace util {
    ...
    struct SpotLightCalculator::Data {
        ...
        float getAmount(const VC3 &pos, float angle, const IStorm3D_Terrain &terrain, float rayHeight) const
        {
            ...
            if (!lengthOk[index]) {
                ...
                terrain.rayTrace(position + VC3(0,
                                                rayHeight,
                                                0) + (rayDir * .5f), rayDir, range, cInfo, oInfo, true, true);
    ...
    float SpotLightCalculator::getLightAmount(const VC3 &position, const IStorm3D_Terrain &terrain,
                                              float rayHeight) const
    {
