...
namespace util {
    namespace {
        struct MaterialSorter {
            bool operator () (const SoundMaterialParser::SoundMaterial &a,
                              const SoundMaterialParser::SoundMaterial &b) const
            {
    ...
    SoundMaterialParser::SoundMaterialParser()
    {
        ...
        for (int i = 0; i < groups; ++i) {
            ...
            material.requiredAcceleration = convertFromString<float>(accelString,
                                                                     SOUNDMATERIALPARSER_DEFAULT_REQUIRED_ACCELERATION);

            const std::string &angAccelString = group.getValue("required_angular_acceleration");
            material.requiredAngularAcceleration = convertFromString<float>(
                angAccelString,
                SOUNDMATERIALPARSER_DEFAULT_REQUIRED_ANGULAR_ACCELERATION);

            const std::string &effectForceString = group.getValue("required_effect_force");
            material.requiredEffectForce = convertFromString<float>(effectForceString,
                                                                    SOUNDMATERIALPARSER_DEFAULT_REQUIRED_EFFECT_FORCE);

            const std::string &effectAccelString = group.getValue("required_effect_acceleration");
            material.requiredEffectAcceleration = convertFromString<float>(
                effectAccelString,
                SOUNDMATERIALPARSER_DEFAULT_REQUIRED_EFFECT_ACCELERATION);

            const std::string &effectAngAccelString = group.getValue("required_effect_angular_acceleration");
            material.requiredEffectAngularAcceleration = convertFromString<float>(
                effectAngAccelString,
                SOUNDMATERIALPARSER_DEFAULT_REQUIRED_EFFECT_ANGULAR_ACCELERATION);

            const std::string &effectMaxRateString = group.getValue("effect_max_rate");
            material.effectMaxRate = convertFromString<int>(effectMaxRateString,
                                                            SOUNDMATERIALPARSER_DEFAULT_EFFECT_MAX_RATE);
