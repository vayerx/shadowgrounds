        ...
        if (preproscheck != NULL && preproscheck[0] != '\0') {
            ...
            if (chk != NULL) {
                ...
                Logger::getInstance()->warning(
                    "Preprocessor::preprocess - Preprocessor check failed, preprocessor not will not be executed.");
            }
        } else {
            Logger::getInstance()->warning(
                "Preprocessor::preprocess - Preprocessing without checking not allowed (check option not specified).");
        }
        if (checkPassed) {
