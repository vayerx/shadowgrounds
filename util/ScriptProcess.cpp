    ...
    ScriptProcess::~ScriptProcess()
    {
        if (this->finished)
            if ( !ipStack->isEmpty() )
                Logger::getInstance()->warning(
                    "ScriptProcess - IP stack not empty at delete although process has finished.");
        ...
        if (this->finished)
            if ( !userStack->isEmpty() )
                Logger::getInstance()->warning(
                    "ScriptProcess - User stack not empty at delete although process has finished.");
        ...
        if (this->scopeStackUsed > 0) {
            ...
            if (this->localVarStackUsed >= popToUsed) {
                ...
                Logger::getInstance()->error(
                    "ScriptProcess::leaveLocalScope - Local variable stack used less than it should be.");
