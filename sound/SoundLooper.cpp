        ...
        for (int i = 0; i < SOUNDLOOPER_MAX_SOUNDS; i++) {
            if (playPhase[i] != PLAYPHASE_NONE) {
                ...
                if (playPhase[i] != prevPhase) {
                    ...
                    // TODO: sound volumes!!!
                    if (playPhase[i] == PLAYPHASE_START) {
                        if (ambient[i]) startHandle[i] = soundMixer->playAmbientSound(startSound[i],
                                                                                      true,
                                                                                      range[i],
                                                                                      priority[i]);
                        else startHandle[i] = soundMixer->playSoundEffect(startSound[i], true, range[i], priority[i]);
                        if (startHandle[i] != -1) {
                            soundMixer->setSoundPosition(startHandle[i],
                                                         positionX[i],
                                                         positionY[i],
                                                         positionZ[i],
                                                         0,
                        ...
                        if (ambient[i]) loopHandle[i] = soundMixer->playAmbientSound(loopSound[i],
                                                                                     true,
                                                                                     range[i],
                                                                                     priority[i]);
                        else loopHandle[i] = soundMixer->playSoundEffect(loopSound[i], true, range[i], priority[i]);
                        if (loopHandle[i] != -1) {
                            soundMixer->setSoundPosition(loopHandle[i],
                                                         positionX[i],
                                                         positionY[i],
                                                         positionZ[i],
                                                         0,
                        ...
                        if (ambient[i]) endHandle[i] = soundMixer->playAmbientSound(endSound[i],
                                                                                    true,
                                                                                    range[i],
                                                                                    priority[i]);
                        else endHandle[i] = soundMixer->playSoundEffect(endSound[i], true, range[i], priority[i]);
                        if (endHandle[i] != -1) {
                            soundMixer->setSoundPosition(endHandle[i],
                                                         positionX[i],
                                                         positionY[i],
                                                         positionZ[i],
                                                         0,
