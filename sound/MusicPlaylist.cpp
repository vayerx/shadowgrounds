...
namespace sfx {
    ...
    void MusicPlaylist::play()
    {
        ...
        if (Timer::getTime() > musicplaylist_last_playing_track_msg_time
            + MUSICPLAYLIST_ALLOWED_TRACK_CHANGE_TIME_LIMIT) {
    ...
    void MusicPlaylist::run()
    {
        if (playing)
                 ...
                 && mixer->hasMusicEnded() )
            {
                ...
                if (Timer::getTime() > musicplaylist_last_track_changing_msg_time
                    + MUSICPLAYLIST_ALLOWED_TRACK_CHANGE_TIME_LIMIT) {
                    ...
                    if (!musicplaylist_last_track_changing_msg_multi) {
                        Logger::getInstance()->warning(
                            "MusicPlaylist::run - Multiple track changes within time limit, possibly due to missing music track file.");
