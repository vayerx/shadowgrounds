        ...
        if (result) {
            ...
            if (hwchans && data->useEax) {
...

                {
                    EditorParser parser;
#ifdef LEGACY_FILES
                    filesystem::InputStream env_file = filesystem::FilePackageManager::getInstance().getFile(
                        "Data/Ambient/environment.txt");
#else
                    filesystem::InputStream env_file = filesystem::FilePackageManager::getInstance().getFile(
                        "data/audio/environment/environment.txt");
#endif
                    env_file >> parser;
...

                {
                    EditorParser parser;
#ifdef LEGACY_FILES
                    filesystem::InputStream alias_file = filesystem::FilePackageManager::getInstance().getFile(
                        "Data/Ambient/environment_aliases.txt");
#else
                    filesystem::InputStream alias_file = filesystem::FilePackageManager::getInstance().getFile(
                        "data/audio/environment/environment_aliases.txt");
#endif
                    alias_file >> parser;
